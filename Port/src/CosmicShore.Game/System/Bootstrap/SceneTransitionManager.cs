// Ported verbatim from Assets/_Scripts/System/Bootstrap/SceneTransitionManager.cs
...
// (UniTask → Task; op.ToUniTask(ct) → op.WaitAsync(ct); UniTask.Delay(TimeSpan,
// DelayType.UnscaledDeltaTime, ct) → GameTask.Delay(seconds, unscaledTime: true, ct);
// UniTask.WaitUntil(pred, ct) → GameTask.WaitUntil(pred, ct); UniTask.Yield(ct) →
// GameTask.Yield(ct)); Unity.Netcode → CosmicShore.Engine.Networking;
// UnityEngine.SceneManagement → CosmicShore.Engine.SceneManagement;
// UnityEngine → CosmicShore.Engine.
//
// LIVE: the whole fade/transition core on the engine CanvasGroup — IsTransitioning,
// OnSceneLoadComplete, LoadSceneAsync (fade-out → engine scene load → settle delay →
// event → fade-in, incl. the sync-fallback catch), LoadNetworkSceneAsync (server /
// client / no-NM branches against the NetworkManager.SceneManager placeholder),
// FadeToBlack / FadeFromBlack / SetFadeImmediate, FadeAsync's unscaled-time lerp,
...
//   Docs/THREADING.md + Engine/Tasks/GameSynchronizationContext.cs); carried as
//   commented source with the boundary it guarded.
...
namespace CosmicShore.Core
{
    ...
    public class SceneTransitionManager : MonoBehaviour
    {
        ...
        /// </summary>
        public async Task LoadSceneAsync(string sceneName, bool fadeOut = true, bool fadeIn = true)
        {
            if (_isTransitioning)
            {
            ...
            try
            {
                ...
                {
                    // (Port: op.ToUniTask(cancellationToken: ct) → op.WaitAsync(ct).)
                    await op.WaitAsync(ct);
                }

                // Let the new scene's Awake/Start complete.
                if (_postLoadSettleDelay > 0f)
                {
                    await GameTask.Delay(_postLoadSettleDelay, unscaledTime: true, cancellationToken: ct);
                }
        ...
        /// </summary>
        public async Task LoadNetworkSceneAsync(string sceneName)
        {
            if (_isTransitioning)
            {
                Debug.LogWarning($"[SceneTransition] Already transitioning. Ignoring request for '{sceneName}'.");
            ...
            try
            {
                ...
                else
                {
                    ...
                    // (Port: .ToUniTask(cancellationToken: ct) → .WaitAsync(ct).)
                    await SceneManager.LoadSceneAsync(sceneName).WaitAsync(ct);
                }
        ...
        public void SetFadeImmediate(float alpha)
        {
            ...
            // PORT Deviation (threading 2026-07-08, MainThreadDispatcher.IsOnMainThread — part of the
            // retired `.AsMainThread()` boundary; GameSynchronizationContext owns affinity
            // structurally, so an off-thread arrival here is impossible by construction):
        ...
        async Task FadeAsync(float from, float to, CancellationToken ct)
        {
            if (_fadeCanvasGroup == null) return;
