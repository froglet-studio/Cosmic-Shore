// Ported verbatim from Assets/_Scripts/System/FriendsServiceFacade.cs
// (party-system arc 2026-07-08). Mechanical substitutions (README):
// Cysharp.Threading.Tasks → System.Threading.Tasks (UniTask → Task);
// Unity.Services.Friends / .Exceptions / .Models / .Notifications →
// CosmicShore.Engine.Services.Friends (one flat placeholder namespace);
// UnityEngine → CosmicShore.Engine; the retired `.AsMainThread()` boundary
// maps to a plain await (GameSynchronizationContext owns affinity — see
// Engine/Tasks/GameTask.cs).
//
// FULLY LIVE against the engine Friends placeholder surface (grown per the
// ISession precedent): initialization + event wiring, the whole relationship
// API (requests/accept/decline/cancel/remove/block/unblock), presence,
// refresh, IsFriend/IsBlocked queries, SDK→SOAP sync incl. the
// StripUgsDiscriminator display-name rule and AvailabilityToInt mapping.
// FriendsService.Instance is null until the real SDK binds — InitializeAsync's
// own catch degrades that to a warning, exactly upstream's failure path.
