// Ported verbatim from _Scripts/System/CloudData/Providers/UGSCloudSaveProvider.cs
// (CloudSave arc 2026-07-09). Mechanical substitutions (README): UnityEngine →
// CosmicShore.Engine; Unity.Services.Authentication / Unity.Services.CloudSave /
// Unity.Services.Core → CosmicShore.Engine.Services (the CloudSaveSdk placeholder —
// CloudSaveService.Instance.Data.Player + Item.Value.GetAs<T>() run verbatim against
// the in-memory LocalCloudSaveService); Newtonsoft.Json → System.Text.Json with the
// shared CloudSaveJson.Options (IncludeFields — the models are field-based; the
// JsonUtility-drops-Dictionary hazard the upstream comment guards against does not
// exist in System.Text.Json, but the fallback branch is kept verbatim for
// legacy-string payload parity). `await MainThreadDispatcher.SwitchToMainThreadAsync()`
// maps to nothing — the retired `.AsMainThread()` convention: GameSynchronizationContext
// owns affinity structurally (see Engine/Tasks/GameTask.cs).
//
// One carried deviation: the ToastNotificationAPI failure toast (UI shell) — the
// static toast API is unported; the once-per-episode LogError + OnSaveFailed event
// (the observability contract AnalyticsServiceFacade subscribes to) are live.
using System;
...
namespace CosmicShore.Core
{
    ...
    public class UGSCloudSaveProvider : ICloudSaveProvider
    {
            ...
            try
            {
                ...
                if (result.TryGetValue(key, out var item))
                {
                    ...
                    catch
                    {
                        ...
                        // (Port: Newtonsoft → System.Text.Json + CloudSaveJson.Options per header.)
                        var json = item.Value.GetAs<string>();
                        if (!string.IsNullOrEmpty(json))
                            return System.Text.Json.JsonSerializer.Deserialize<T>(json, CloudSaveJson.Options);
                    }
                }
            }
            catch (Exception e)
        ...
        public async Task<bool> SaveAsync<T>(string key, T data, CancellationToken ct = default) where T : class
        {
            ...
            {
                _failedKeys.Add(key);
                // (Port: `await MainThreadDispatcher.SwitchToMainThreadAsync();` maps to
                // nothing — GameSynchronizationContext owns affinity structurally.)
                Debug.LogError($"[UGSCloudSaveProvider] Save '{key}' failed after {RetryBackoffMs.Length + 1} attempts: {last?.Message}");
                // PORT Deviation (UI shell 2026-07-09, ToastNotificationAPI — restore when the toast API ports):
