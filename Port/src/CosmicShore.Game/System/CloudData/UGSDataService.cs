// Ported verbatim from Assets/_Scripts/System/CloudData/UGSDataService.cs (CloudSave arc
// 2026-07-09 — replaces the Deviation-#14 type-preserving shell). Mechanical substitutions
// (README): UnityEngine → CosmicShore.Engine; Reflex.Attributes → CosmicShore.Engine.Injection.
// FULLY live against the engine CloudSaveSdk placeholder: all 11 repositories load through
// UGSCloudSaveProvider → CloudSaveService.Instance (LocalCloudSaveService default), the
// auth-driven initialization runs off the real OnSignedIn SOAP event, and SyncHangarToVessels
// unlocks SO_Vessel assets from the cloud hangar record. Restoring this un-carries the
// PlayerDataService / GameSetting deviation-#14 inject sites (next iteration).
using System;
