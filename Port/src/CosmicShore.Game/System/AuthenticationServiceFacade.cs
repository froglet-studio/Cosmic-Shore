// Ported verbatim from Assets/_Scripts/System/AuthenticationServiceFacade.cs
// (bootstrap arc 2026-07-08). Mechanical substitutions (README):
// Unity.Services.Core / Unity.Services.Authentication → CosmicShore.Engine.Services.
// The `#if UNITY_EDITOR` MPPM profile block compiles out verbatim (UNITY_EDITOR is
// never defined in the port); its Unity.Multiplayer.Playmode using sits inside the
// guard upstream too.
//
// FULLY LIVE against the grown engine auth shim: the startup guard + coalesced
// initialization, anonymous sign-in with the cached-token silent path, the
// TrySignInCachedAsync three-branch flow, SignOut (+ token clear), the auth event
// wiring (SignedIn / SignInFailed / SignedOut / Expired), the single-writer
// AuthenticationData state + SOAP raises with the double-raise dedup, and the
// provider stubs. The shim's local sign-in mirrors the real SDK's observable
// contract; the real wire arrives at the services phase.
