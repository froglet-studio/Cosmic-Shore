// Ported verbatim from Assets/_Scripts/System/ApplicationStateMachine.cs
// (bootstrap arc 2026-07-08). Mechanical substitutions (README):
// UnityEngine → CosmicShore.Engine. FULLY LIVE — pure C# single-writer phase
// tracker over the ScriptableApplicationState SOAP family: the table-driven
// transition graph, the special states (ShuttingDown always allowed; Paused
// from any non-terminal state with restore-to-previous; Disconnected from any
// active state), the SOAP auto-wiring (OnSessionStarted → InGame,
// OnMiniGameEnd → GameOver, OnAppPaused/OnAppQuitting statics, OnNetworkLost
// → Disconnected), and the PreviousState + OnStateChanged publication.
