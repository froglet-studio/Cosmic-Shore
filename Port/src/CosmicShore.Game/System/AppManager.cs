// Ported verbatim from Assets/_Scripts/System/AppManager.cs
...
// (UniTaskVoid → Task + .Forget(); UniTask.Yield(PlayerLoopTiming.*, ct) →
// GameTask.Yield(ct); UniTask.Delay(TimeSpan, DelayType.UnscaledDeltaTime, ct) →
// GameTask.Delay(seconds, unscaledTime: true, ct)); Reflex.{Attributes,Core,Enums} →
// CosmicShore.Engine.Injection; UnityEngine → CosmicShore.Engine;
// UnityEngine.SceneManagement → CosmicShore.Engine.SceneManagement; the
// `Debug = UnityEngine.Debug` / `Resolution = Reflex.Enums.Resolution` aliases map
// to their engine counterparts. The `#if UNITY_2023_1_OR_NEWER` forks compile the
// #else branches against the engine's FindObjectOfType alias.
//
// FULLY LIVE — the whole DI root: platform config from BootstrapConfigSO
// (targetFrameRate / vSyncCount / sleepTimeout), the bootstrapped-once guard,
// TryResolveManagersEarly + the DontDestroyOnLoad persistence stamp,
// InstallBindings (11 RegisterAsset values, 15 RegisterManagerSingleton lazy
// factories with deferred scene search, the pure-C# service quartet + analytics +
// tournament + the 9 party-ring factories), ConfigureGameData,
// StartAuthentication / StartNetworkMonitor (incl. NetworkDiagnostics.Initialize),
// the RunBootstrapAsync splash-hold → OnBootstrapComplete → Authenticating →
// Authentication-scene handoff (via SceneTransitionManager when present), Shutdown,
// and the two RuntimeInitializeOnLoadMethod statics (the attribute is a data-only
// marker in the port — harnesses call them directly, nothing dispatches domain
// reloads).
...
namespace CosmicShore.Core
{
    ...
    public class AppManager : MonoBehaviour, IInstaller
    {
        ...
        async Task RunBootstrapAsync(CancellationToken ct)
        {
            ...
            try
            {
                ...
                // (Port: UniTask.Yield(PlayerLoopTiming.PreUpdate/PostLateUpdate, ct) → GameTask.Yield(ct).)
                ...
                if (remaining > 0f)
                {
                    ...
                    await GameTask.Delay(remaining, unscaledTime: true, cancellationToken: ct);
                }
