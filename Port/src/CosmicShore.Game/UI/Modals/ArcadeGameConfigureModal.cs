// Ported from Assets/_Scripts/UI/Modals/ArcadeGameConfigureModal.cs (Arc F 2b-iii(b))
// — verbatim with the standard substitutions (UnityEngine → CosmicShore.Engine,
// TMPro / UnityEngine.UI → CosmicShore.Engine.UI, Obvious.Soap → CosmicShore.Engine.Soap,
// Reflex.Attributes → CosmicShore.Engine.Injection, Unity.Netcode →
// CosmicShore.Engine.Networking, UnityEngine.Video → CosmicShore.Engine's VideoPlayer/
// VideoClip stubs, UnityEngine.Serialization → CosmicShore.Engine). No behavior
// deviations: the sync-manager path, chip lifecycle, ready-up flow, and launch-authority
// logic all ride the live ports (ArcadeConfigSyncManager, Player, GameDataSO,
// HostConnectionDataSO, PlayerDataService, ModalWindowManager).
using System;
