// Ported from Assets/_Scripts/UI/ScreenSwitcher.cs (Arc F) — verbatim with the standard
// substitutions (UnityEngine → CosmicShore.Engine; UnityEngine.UI / UnityEngine.EventSystems
// → CosmicShore.Engine.UI; UnityEngine.InputSystem → CosmicShore.Engine.InputSystem;
// Reflex.Attributes → CosmicShore.Engine.Injection). Deviations (marked inline): the
// HangarScreen/LeaderboardsMenu scene references + their LoadView calls — those screens
// port with their own arcs; every other behavior (screen slide, modal stack, PlayerPrefs
// return-state, gamepad trigger nav, IScreen lifecycle, freestyle input handoff) is live.
// The [RuntimeInitializeOnLoadMethod] ClearReturnState is data-only per port convention —
// hosts invoke RunOnStart at startup (the menushell does, via the SetPrivateField idiom).
...
namespace CosmicShore.UI
{
    public class ScreenSwitcher : MonoBehaviour, IDragHandler, IEndDragHandler
    {
        ...
        // PORT Deviation (Arc F, HangarScreen/LeaderboardsMenu — restore when those screens port):
        ...
        private void NavigateTo(int ScreenIndex, bool animate = true)
        {
            ...
            switch (screenId)
            {
                case MenuScreens.HANGAR:
                    ...
                    // PORT Deviation (Arc F, HangarScreen — restore when it ports):
                    // if (HangarMenu)
                    //     HangarMenu.LoadView();
                    break;
                case MenuScreens.PORT:
                    // PORT Deviation (Arc F, LeaderboardsMenu — restore when it ports):
                    // if (LeaderboardMenu)
