// Ported verbatim from _Scripts/Utility/DataPersistence/DataAccessor.cs (painting-toy
// drift-sync 2026-07-09). Mechanical substitutions (README): UnityEngine → CosmicShore.Engine;
// Newtonsoft.Json → System.Text.Json (JsonConvert.SerializeObject/DeserializeObject →
// JsonSerializer.Serialize/Deserialize with s_jsonOptions; Formatting.None is the default;
// ReferenceLoopHandling.Ignore → ReferenceHandler.IgnoreCycles; IncludeFields because the
// persisted data classes are Unity-style field-based — the CloudSaveSdk precedent).
// The `new BinaryFormatter()` is dead upstream (its Serialize/Deserialize calls are commented)
// AND BinaryFormatter is removed from .NET 9+; carried as a comment so no behavior is lost.
...
namespace CosmicShore.Utility
{
    ...
    static class DataAccessor
    {
        ...
        public static void Save<T>(string fileName, T data) where T : new ()
        {
            ...
            //BinaryFormatter converter = new BinaryFormatter();  // (Port: BinaryFormatter removed in .NET 9+; dead upstream.)
            //converter.Serialize(dataStream, data);

            dataStream.Write(Encoding.ASCII.GetBytes(JsonSerializer.Serialize(data, s_jsonOptions)));
        ...
        public static T Load<T>(string fileName) where T : new()
        {
            ...
            if (File.Exists(FilePath))
            {
                ...
                try
                {
                    ...
                    Data = (T)JsonSerializer.Deserialize(json, typeof(T), s_jsonOptions);
