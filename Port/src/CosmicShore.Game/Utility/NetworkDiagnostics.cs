// Ported verbatim from Assets/_Scripts/Utility/NetworkDiagnostics.cs (party-system arc
// 2026-07-08). Substitutions: UnityEngine -> CosmicShore.Engine (Application.internetReachability
...
namespace CosmicShore.Utility
{
    public static class NetworkDiagnostics
    {
        ...
        public static string ClassifyException(Exception e)
        {
            ...
            // PORT: the engine's SessionException placeholder carries a structured Error —
            // classify it directly, mirroring the upstream string branch below (which stays
            // verbatim for when the real SDK type returns).
            if (inner is CosmicShore.Engine.Networking.SessionException se)
            {
                switch (se.Error)
                {
                    case CosmicShore.Engine.Networking.SessionError.SessionNotFound:
                    case CosmicShore.Engine.Networking.SessionError.SessionDeleted:
                    case CosmicShore.Engine.Networking.SessionError.NotInLobby:
                        return "SessionGone";
                    case CosmicShore.Engine.Networking.SessionError.RateLimitExceeded:
                        return "RateLimit";
                    default:
                        return "Transient";
                }
