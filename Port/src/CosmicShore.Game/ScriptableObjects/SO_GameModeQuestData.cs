// Ported from Assets/_Scripts/ScriptableObjects/SO_GameModeQuestData.cs (Arc F 2b-iii(b))
// — verbatim; UnityEngine → CosmicShore.Engine. Pure data SO: the quest chain that gates
// game-mode + intensity unlocks. Consumed by GameModeProgressionService (still a shell —
// its GetQuestForMode returns null until the real progression port) and by
// ArcadeGameConfigureModal.HandleLockedIntensitySelected (locked-intensity goal toasts).
