// Ported verbatim from _Scripts/Controller/Toys/PaintingRunner.cs (painting-toy drift-sync 2026-07-09). Mechanical substitutions per README.
// Substitutions: using Cysharp.Threading.Tasks; → using System.Threading.Tasks; + using CosmicShore.Engine.Tasks;. using UnityEngine; → using CosmicShore.Engine;.
//   async UniTaskVoid → async Task (call sites already carry .Forget()); UniTask.Yield(PlayerLoopTiming.Update, ct) → GameTask.Yield(ct);
//   UniTask.Delay(<ms>, ignoreTimeScale: true, ct) → GameTask.Delay(<seconds>, unscaledTime: true, ct) (no int-ms unscaled overload — 100→0.1f, 900→0.9f);
//   UnityEngine.Object → CosmicShore.Engine.Object.
using System;
...
namespace CosmicShore.Gameplay
{
    ...
    public class PaintingRunner : MonoBehaviour
    {
        ...
        /// </summary>
        async Task RestorePrismsAsync(CancellationToken ct)
        {
            ...
            for (int tries = 0; tries < 100 && controller == null; tries++)
            {
                ...
                if (controller == null)
                    await GameTask.Delay(0.1f, unscaledTime: true, cancellationToken: ct);
            }
        ...
        IVesselStatus ResolveVessel()
        {
            ...
            if (vs == null || (vs is CosmicShore.Engine.Object o && !o)) return null;
            return vs;
        }

        // ── Async transitions (continuity law: nothing pops in or out) ──────

        async Task BloomIn(CancellationToken ct)
        {
            float elapsed = 0f;
        ...
        async Task CelebrateAsync(CancellationToken ct)
        {
            ...
            await GameTask.Delay(0.9f, unscaledTime: true, cancellationToken: ct);
