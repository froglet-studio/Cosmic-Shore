// Ported verbatim from Assets/_Scripts/Controller/Party/Services/PartySessionService.cs
...
// (UniTask → Task; UniTask.Delay(ms) → GameTask.Delay(ms));
// Unity.Services.Multiplayer → CosmicShore.Engine.Networking (ISession / IHostSession /
// PlayerProperty / VisibilityPropertyOptions / SessionException placeholders);
// UnityEngine → CosmicShore.Engine; the retired `.AsMainThread()` boundary maps to a plain
// await (GameSynchronizationContext owns affinity structurally — see Engine/Tasks/GameTask.cs).
//
// LIVE: ActiveSession/CreatedAtUnscaledTime state (backed by GameDataSO.ActiveSession),
// the PlayerLeaving relay wire/unwire chokepoints, both retry loops with their catch
// classification (host-conflict / rate-limit / transient), IsHostConflictException,
// IsTransientSessionException (engine SessionException), BuildLocalPlayerProperties
...
namespace CosmicShore.Gameplay
{
    ...
    public sealed class PartySessionService : IPartySessionService
    {
        ...
        /// <c>MultiplayerService.Instance</c> in the constructor — this
