// Ported verbatim from Assets/_Scripts/Controller/Party/Services/LobbyPropertyWriter.cs
// (party-system arc 2026-07-08). Substitutions per README: UniTask -> Task, and the retired
// `.AsMainThread()` boundary maps to a plain await (GameSynchronizationContext owns affinity
// structurally — see Engine/Tasks/GameTask.cs). ISession.RefreshAsync/SaveCurrentPlayerDataAsync
// live on the engine placeholder.
// ─────────────────────────────────────────────────────────────────────────────
        ...
        public async Task SaveWithRetryAsync(ISession lobby, int maxRetries = 3, int baseDelayMs = 2000)
        {
            for (int attempt = 0; attempt <= maxRetries; attempt++)
            {
                     ...
                     e.Message.Contains("Index was out of range")))
                {
                    ...
                    await GameTask.Delay(baseDelayMs);
