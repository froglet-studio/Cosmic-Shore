// Ported verbatim from Assets/_Scripts/Controller/Party/HostConnectionService.cs
// (party-system arc 2026-07-08). Mechanical substitutions (README):
// UnityEngine → CosmicShore.Engine; UnityEngine.SceneManagement → CosmicShore.Engine.SceneManagement;
// Obvious.Soap → CosmicShore.Engine.Soap; Reflex.Attributes → CosmicShore.Engine.Injection;
// Unity.Netcode + Unity.Services.Multiplayer → CosmicShore.Engine.Networking (NetworkManager +
// ISession / IReadOnlyPlayer / PlayerProperty / SessionException placeholders);
// Unity.Services.Authentication → CosmicShore.Engine.Services (AuthenticationService shim,
// fully-qualified at the two call sites as upstream); Cysharp.Threading.Tasks →
// System.Threading.Tasks + CosmicShore.Engine.Tasks (UniTask → Task; UniTaskVoid → Task +
// .Forget(); UniTask.CompletedTask → Task.CompletedTask; UniTask.Delay → GameTask.Delay;
// UniTaskCompletionSource → TaskCompletionSource; tcs.Task.AttachExternalCancellation(token) →
// tcs.Task.WaitAsync(token)); the retired `.AsMainThread()` boundary maps to a plain await
// (GameSynchronizationContext owns affinity structurally — see Engine/Tasks/GameTask.cs).
//
// LIVE: singleton/DontDestroyOnLoad lifecycle, SceneManager.sceneLoaded wiring, auth
// OnSignedIn init trigger, EnsureInitializedAsync (profile wait + identity sync + presence
// lobby join + post-join state + solo-Relay creation), the full refresh cycle
// (RefreshAsync: lobby refresh, converge, online-player diff, incoming-invite scan,
// acceptance-signal scan + republish, joined-party presence scan, member reconcile,
// party-state publish, the complete error-classification matrix incl. reconnect), invite
// send/cancel/accept/decline (SendInviteAsync, CancelInviteAsync, AcceptInviteAsync's
// three-phase accept, DeclineInviteAsync), invite expiry + clear paths, member reconcile
// (ReconcilePartyMembersNow + RefreshPartyMembersAsync + HandleDefiniteSessionGoneAsync),
// EnsurePartySessionAsync (mutex + NM shutdown + session create + state transitions),
// LeavePartySessionAsync, ClearPartySessionRef, all property publishing
// (PublishJoinedParty / ClearJoinedParty / RepublishLocalIdentity / PublishPartyState),
...
namespace CosmicShore.Gameplay
{
    ...
    public class HostConnectionService : MonoBehaviour, IPartyStateQuery
    {
        ...
        /// </summary>
        public Task CancelInviteAsync(string targetPlayerId)
...
#pragma warning restore CS1998
        ...
        private async Task ReconcilePartyMembersNowAsync()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < RECONCILE_MAX_ATTEMPTS; i++)
                {
                    ...
                    if (i < RECONCILE_MAX_ATTEMPTS - 1)
                        await GameTask.Delay(RECONCILE_RETRY_DELAY_MS);
                }
            }
            finally
            {
        ...
        /// does this).
        /// </summary>
        public async Task EnsurePartySessionAsync()
        {
        ...
        /// </para>
        /// </summary>
        public async Task LeavePartySessionAsync()
        {
            try
            {
                if (_partySessionService.ActiveSession != null)
        ...
        private async Task RefreshPartyMembersAsync(bool bypassGraceGate = false)
        {
        ...
        /// </para>
        /// </summary>
        private async Task HandleDefiniteSessionGoneAsync()
        {
        ...
        private async Task PublishJoinedPartyAsync(string partySessionId)
        {
        ...
        /// </summary>
        public async Task ClearJoinedPartyAsync()
        {
            var lobby = _lobbyService.ActiveLobby;
            if (lobby == null) return;
            await _propertyWriter.WriteAsync(
        ...
        private async Task RepublishLocalIdentityAsync()
        {
            var lobby = _lobbyService.ActiveLobby;
            if (lobby == null) return;
            await _propertyWriter.WriteAsync(
        ...
        private async Task PublishPresenceImmediateAsync()
        {
            if (_lobbyService.ActiveLobby == null) return;
        ...
        private void SyncLocalIdentity()
        {
            ...
            if (string.IsNullOrEmpty(connectionData.LocalDisplayName))
            {
                try
                {
                    var ugsName = CosmicShore.Engine.Services.AuthenticationService.Instance?.PlayerName;
        ...
        private string ResolveLocalPlayerId()
        {
            ...
            var auth = CosmicShore.Engine.Services.AuthenticationService.Instance;
