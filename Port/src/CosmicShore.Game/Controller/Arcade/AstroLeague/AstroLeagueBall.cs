// Ported from Assets/_Scripts/Controller/Arcade/AstroLeague/AstroLeagueBall.cs
// (AstroLeague arc). Mechanical substitutions (README) + GameTask mappings. The physics
// core — server rigidbody sim (engine E18 ballistic integration), momentum-conserving
// vessel strikes, anti-clip ejection, spherical-boundary containment, per-tick
// PrismSpatialIndex prism resolution, dead-reckoned client replication — is verbatim.
// Deviation surfaces (all presentation): the icosphere mesh swap (mesh arc), the
// ParticleSystem aura/burst rig (presentation arc), and haptics (presentation arc).
// The engine's collision solver never fires OnCollisionEnter/Stay and OnTriggerStay
// (TriggerPass = Enter/Exit only), so vessel contacts arrive through the verbatim
// OnTriggerEnter path — the same route the original uses for the trigger-only ships
// (Serpent/Sparrow); the collision-path methods stay verbatim-shaped for the physics
// phase to reactivate.
using System;
...
namespace CosmicShore.Gameplay
{
    ...
    public class AstroLeagueBall : NetworkBehaviour
    {
        ...
        void Awake()
        {
            ...
            // fast ball tunnels past tiny box colliders). Excluding the TrailBlocks layer on the ball's own
        ...
        void SetupVisuals()
        {
            ...
            trail.shadowCastingMode = CosmicShore.Engine.Rendering.ShadowCastingMode.Off;
            trail.receiveShadows = false;
            trail.generateLightingData = false;

            // PORT Deviation (presentation arc, restore when ParticleSystem ports):
        ...
        static void MakeTransparent(Material mat)
        {
            ...
            mat.SetInt("_SrcBlend", (int)CosmicShore.Engine.Rendering.BlendMode.SrcAlpha);
            mat.SetInt("_DstBlend", (int)CosmicShore.Engine.Rendering.BlendMode.OneMinusSrcAlpha);
            mat.SetInt("_ZWrite", 0);
            mat.EnableKeyword("_ALPHABLEND_ON");
            mat.renderQueue = 3000;
        }

        // PORT Deviation (presentation arc, restore when ParticleSystem ports): the whole
        // CreateParticles builder (aura orbit rig + impact burst rig, ~65 lines of
        // ParticleSystem module configuration) — pure per-peer visuals, no gameplay reads.
        ...
        // PORT Deviation (physics arc, reactivates when the engine gains contact
        // resolution): the engine's solver never dispatches OnCollisionEnter/Stay, so
        // the hull-collider strike path — including the Collision-typed parameters —
        // is carried as commented source. Vessel contacts flow through the verbatim
        // trigger path below (HandleVesselTrigger), the same route the original uses
        // for the trigger-only ships; wall bounces are the boundary-sphere radial
        // reflect (ContainWithinBoundary → HandleWallBounce), which is collider-free.
        //
        ...
        /// NOTE (physics arc): the engine's TriggerPass dispatches Enter/Exit only — Stay re-arms
        /// with the full physics phase. Enter-only is self-consistent headless: the elastic bounce
        /// separates the pair, and a fresh approach re-fires Enter.
        /// </summary>
        ...
        void Detonate_ClientRpc(Vector3 position)
        {
            ...
            // PORT Deviation (presentation arc, restore when NiceVibrations haptics port): HapticController.PlayHaptic(HapticType.MineCollision);
        }
        ...
        void WallBounce_ClientRpc(Vector3 position, Vector3 normal, float intensity)
        {
            ...
            // PORT Deviation (presentation arc, restore when NiceVibrations haptics port): HapticController.PlayHaptic(HapticType.ShipCollision);
        }
