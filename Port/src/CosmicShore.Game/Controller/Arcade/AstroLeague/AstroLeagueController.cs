// Ported verbatim from Assets/_Scripts/Controller/Arcade/AstroLeague/AstroLeagueController.cs
// (AstroLeague arc). Mechanical substitutions (README) + GameTask mappings
// (`async UniTaskVoid`/`async UniTask` → `async Task` + `.Forget()`; `UniTask.Delay(
// TimeSpan.FromSeconds(s), ignoreTimeScale: true, ct)` → `GameTask.Delay(s,
// unscaledTime: true, ct)`). RPCs are local-invoke until the transport phase.
using System;
...
namespace CosmicShore.Gameplay
{
    ...
    public class AstroLeagueController : MultiplayerDomainGamesController
    {
        ...
        async Task CelebrateThenKickoffAsync(Domains scoringDomain)
        {
            ...
            try
            {
                await GameTask.Delay(settings.celebrationSeconds,
                    unscaledTime: true, cancellationToken: token);
        ...
        async Task RunKickoffAsync(CancellationToken token, bool returnToOvertime)
        {
            ...
            await GameTask.Delay(settings.kickoffFreezeSeconds,
                unscaledTime: true, cancellationToken: token);
        ...
        async Task EnterOvertimeKickoffAsync()
        {
            if (matchCts == null) return;
        ...
        async Task FinishMatchAsync(Domains winner)
        {
            if (phase == MatchPhase.Finished) return;
            ...
            if (matchCts != null)
            {
                try
                {
                    await GameTask.Delay(settings.winnerBannerSeconds,
                        unscaledTime: true, cancellationToken: matchCts.Token);
                }
        ...
        async Task RunCelebrationSlowMoAsync()
        {
            if (matchCts == null) return;
