// Ported verbatim from Assets/_Scripts/Controller/Arcade/AstroLeague/AstroLeagueArena.cs
// (AstroLeague arc). Mechanical substitutions (README) + `UnityEngine.Rendering.*` →
// `Rendering.*` (CosmicShore.Engine.Rendering). One deviation surface: the editor-only
// OnDrawGizmos body (engine has no Gizmos) — everything gameplay-bearing (boundary
// handoff to the ball, goal-portal ring geometry, midfield ring, anticipation flare)
// is verbatim over the engine's data-only LineRenderer/Material stubs.
...
namespace CosmicShore.Gameplay
{
    ...
    public class AstroLeagueArena : MonoBehaviour
    {
        ...
        void OnDrawGizmos()
        {
            // PORT Deviation (editor arc, restore when Gizmos ports): the editor-only wire
            // preview (arena box + goal spheres) — no runtime behavior.
