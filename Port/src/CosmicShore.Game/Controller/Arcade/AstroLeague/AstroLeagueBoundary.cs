// Ported verbatim from Assets/_Scripts/Controller/Arcade/AstroLeague/AstroLeagueBoundary.cs
// (AstroLeague arc). Mechanical substitutions only (README table) + fully-qualified
// `UnityEngine.Rendering.IndexFormat` → `CosmicShore.Engine.Rendering.IndexFormat`
// (same mapping as IcosphereMeshGenerator). Pure math + mesh building over the engine's
// data-only Mesh — no deviation surfaces. Engine need: static Vector3.Normalize(Vector3)
// (used by BuildTangent; integrator adds alongside the existing instance Normalize()).
    ...
    public sealed class AstroLeagueBoundary
    {
        ...
        Mesh FinalizeMesh(List<Vector3> meshVerts, List<int> tris, List<Vector3> normals)
        {
            ...
            mesh.indexFormat = meshVerts.Count > 65000
                ? CosmicShore.Engine.Rendering.IndexFormat.UInt32
                : CosmicShore.Engine.Rendering.IndexFormat.UInt16;
