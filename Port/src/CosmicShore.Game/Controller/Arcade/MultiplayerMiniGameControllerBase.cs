// Ported verbatim from Assets/_Scripts/Controller/Arcade/MultiplayerMiniGameControllerBase.cs
// (controller-chain arc). Mechanical substitutions (README) + GameTask mappings
// (`async UniTaskVoid` → `async Task` + `.Forget()`; `UniTask.Delay(ms, DelayType.
// UnscaledDeltaTime)` → `GameTask.Delay(ms/1000f, unscaledTime: true)`). ClientRpcs
// carry local-invoke semantics until the transport phase (Player/RoundStats precedent).
// Two deviation surfaces, both scene-infrastructure: SceneTransitionManager (fade
// overlay) and the Netcode scene reload — everything that runs within one scene
// (turn/round state machine, replay AI despawns, config sync) is verbatim.
using System;
...
// PORT Deviation (scene arc, restore when Netcode scene management ports): using UnityEngine.SceneManagement;
...
namespace CosmicShore.Gameplay
{
    ...
    {
        // PORT Deviation (scene arc, restore when SceneTransitionManager ports): [Inject] private SceneTransitionManager _sceneTransitionManager;
        ...
        /// </summary>
        async Task InitializeAfterDelay()
        {
            try
            {
                ...
                await GameTask.Delay(InitDelayMs / 1000f, unscaledTime: true, cancellationToken: destroyCancellationToken);
        ...
        /// </summary>
        private async Task ExecuteSceneReloadReplay()
        {
            try
            {
                ...
                await GameTask.Delay(500 / 1000f, unscaledTime: true, cancellationToken: destroyCancellationToken);
                ...
                // PORT Deviation (scene arc, restore when Netcode scene management ports):
                ...
                // }
                _ = nm; // the reload itself is the deviation; everything above ran verbatim
            }
            finally
            {
        ...
        private void PrepareForSceneReload_ClientRpc()
        {
            _isResetting = false;
            gameData.IsReplayReload = true;
            // PORT Deviation (scene arc, restore when SceneTransitionManager ports): _sceneTransitionManager?.SetFadeImmediate(1f);
        }

        private void FadeFromBlackOnReplay()
        {
            gameData.OnClientReady.OnRaised -= FadeFromBlackOnReplay;
            // PORT Deviation (scene arc, restore when SceneTransitionManager ports): _sceneTransitionManager?.FadeFromBlack().Forget();
        }
        ...
        async Task ResetServerRoundAfterDelay()
        {
            await GameTask.Delay(100, cancellationToken: destroyCancellationToken);
            SetupNewRound();
        }
