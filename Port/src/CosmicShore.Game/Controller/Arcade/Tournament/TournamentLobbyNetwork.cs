// Ported verbatim from Assets/_Scripts/Controller/Arcade/Tournament/TournamentLobbyNetwork.cs
// (Tournament arc). Mechanical substitutions (README): Unity.Netcode → CosmicShore.Engine.Networking,
// UnityEngine → CosmicShore.Engine. The universal-RPC surface ([Rpc(SendTo.Server)] / RpcParams)
// is a new engine addition; RPCs carry local-invoke semantics until the transport phase
// (Player/RoundStats precedent — the default RpcParams sender is client 0, the host).
// NetworkManager.ServerTime is harness-assigned until the session/transport phase provides a
// synchronized clock (see Engine.Networking.NetworkTime).
