// Ported verbatim from Assets/_Scripts/Controller/Arcade/Tournament/TournamentController.cs
// (Tournament arc). Mechanical substitutions (README): Unity.Netcode →
// CosmicShore.Engine.Networking, UnityEngine → CosmicShore.Engine,
// UnityEngine.SceneManagement → CosmicShore.Engine.SceneManagement (engine addition —
// the sceneLoaded surface; harnesses announce loads via SceneManager.NotifySceneLoaded
// until real scene management lands), Random → CosmicShore.Engine.Random. The standings
// fold, the race-to-N summary decision (IsShuffleComplete), the random draw, and the
// phase lifecycle are all verbatim.
