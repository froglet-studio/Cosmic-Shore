// Ported from Assets/_Scripts/Controller/Arcade/Tournament/TournamentSceneView.cs
// (Tournament arc). Mechanical substitutions (README): TMPro + UnityEngine.UI →
// CosmicShore.Engine.UI (the TMPro shim + the new Button shim share the namespace),
// Unity.Netcode → CosmicShore.Engine.Networking, UnityEngine → CosmicShore.Engine.
// The phase-driven panel flow (Start/ShowActive/ShowSummaryPanel/button handlers),
// standings text, roster ordering, and countdown math are verbatim. Deviation surfaces
// (all presentation, marked per line):
//   • DOTween pulses / typewriter reveal (DG.Tweening) — Pulse/AnimateRankText bodies no-op.
//   • Round/summary card population — TournamentRoundCard / TournamentSummaryPlayerCard
//     (Image/CanvasGroup/DOTween-heavy prefab views) are unported; PopulateRoundCards /
//     PopulateSummaryCards bodies restore with them.
//   • ScrollRect auto-scroll + LayoutRebuilder/Canvas (UI layout arc).
//   • SO_AIProfileList avatars (unported) — human profile icons resolve normally.
//   • NetworkManager.SpawnManager / LocalClient reads (Netcode session arc).
using System;
...
// PORT Deviation (UI shell): using UnityEngine.UI; folds into CosmicShore.Engine.UI above (Button shim).
...
namespace CosmicShore.Gameplay
{
    ...
    public class TournamentSceneView : MonoBehaviour
    {
        ...
        // PORT Deviation (UI shell, restore when TournamentRoundCard ports): [SerializeField] TournamentRoundCard roundCardPrefab;
        [SerializeField] Transform historyContent;
        // PORT Deviation (UI shell, restore when ScrollRect ports): [SerializeField] ScrollRect historyScrollRect;
        ...
        // PORT Deviation (UI shell, restore when TournamentSummaryPlayerCard ports): [SerializeField] TournamentSummaryPlayerCard summaryCardPrefab;
        ...
        {
            // PORT Deviation (UI shell, restore when TournamentRoundCard ports): the card
            // instantiation body — preview card, chronological per-round cards with running
            // per-domain totals (see the original). The data it renders (tournamentData.History,
            // PointsForPlace, BuildActiveRoster/OrderRoster/ResolveAvatar) is all live here.
        }
        ...
        {
            // PORT Deviation (UI shell, restore when ScrollRect + LayoutRebuilder port): the
            // deferred ScrollToBottomRoutine coroutine (layout rebuild + verticalNormalizedPosition = 0).
        }
        ...
        {
            // PORT Deviation (UI shell, restore with the DOTween arc + TMP mesh surface):
            // the typewriter reveal (maxVisibleCharacters tween) + OutBack scale pop. The
            // final text is already fully set by the caller, so headless output is complete.
        }
        ...
        {
            // PORT Deviation (UI shell, restore when TournamentSummaryPlayerCard ports): the card
            // instantiation body (ordered roster → card.Setup + staggered PlayEntrance). The data it
            // renders (SummaryRoster/OrderRoster/StandingPoints/ResolveAvatar) is all live here.
        }
        ...
        List<TournamentPlayerSnapshot> BuildActiveRoster()
        {
            ...
            // PORT Deviation (scene arc, restore when NetworkManager.SpawnManager ports):
        ...
        Domains GetLocalDomain()
        {
            ...
            // PORT Deviation (scene arc, restore when NetworkManager.LocalClient ports):
            // var nm = NetworkManager.Singleton;
        ...
        {
            // PORT Deviation (UI shell, restore with the DOTween arc): DOKill + unit scale +
            // DOPunchScale tick pulse.
        }
