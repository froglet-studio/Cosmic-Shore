// Ported verbatim from Assets/_Scripts/Controller/Arcade/HexRaceController.cs
...
// UnscaledDeltaTime[, ct])` → `GameTask.Delay(ms/1000f, unscaledTime: true[, ct])`).
// RPCs are local-invoke until the transport phase.
...
namespace CosmicShore.Gameplay
{
    public class HexRaceController : MultiplayerDomainGamesController
    {
        ...
        /// initial sync and the ClientRpc was sent before this client spawned.
        /// </summary>
        private async Task WaitForTrackSeed(CancellationToken ct)
        {
            try
            {
                for (int i = 0; i < 50; i++)
        ...
        /// </summary>
        private async Task SpawnTrackEarly()
        {
            // Small delay to ensure all clients have joined and intensity is synced
            await GameTask.Delay(1500 / 1000f, unscaledTime: true, cancellationToken: destroyCancellationToken);
