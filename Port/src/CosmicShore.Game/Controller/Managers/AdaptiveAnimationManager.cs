// Ported from Assets/_Scripts/Controller/Managers/AdaptiveAnimationManager.cs
// PORT managed-array conversion: using Unity.Collections; dropped — the
// NativeArray<TAnimationData> job-input buffer below is a plain managed array
// (precedent: the PrismSpatialIndex / BlockDensityGrid managed-array ports; the
// port has no Jobs/Burst, subclasses run their former job structs sequentially).
...
namespace CosmicShore.Gameplay
{
    ...
    {
        // PORT managed-array conversion: former IJobParallelFor batch size — kept for the
        // verbatim constant set; the subclasses' sequential Execute loops don't consume it.
        ...
        protected TAnimationData[] animationData; // PORT managed-array conversion: NativeArray<TAnimationData>
        ...
        {
            animationData = new TAnimationData[capacity]; // PORT managed-array conversion: new NativeArray<TAnimationData>(capacity, Allocator.Persistent)
        }
        ...
        protected virtual void EnsureCapacity()
        {
            if (activeAnimators.Count > animationData.Length)
            {
                ...
                var newArray = new TAnimationData[newSize]; // PORT managed-array conversion: new NativeArray<TAnimationData>(newSize, Allocator.Persistent)

                if (animationData != null) // PORT managed-array conversion: animationData.IsCreated
                {
                    if (animationData.Length > 0)
                    {
                        System.Array.Copy(animationData, newArray, animationData.Length); // PORT managed-array conversion: NativeArray<TAnimationData>.Copy(animationData, newArray, animationData.Length)
                    }
                    // PORT managed-array conversion: animationData.Dispose() — the old buffer is released to the GC.
                }
        ...
        protected virtual void CleanupResources()
        {
            ...
            // PORT managed-array conversion: if (animationData.IsCreated) animationData.Dispose();
            animationData = null;
