// Ported verbatim from Assets/_Scripts/Controller/Multiplayer/ClientPlayerVesselInitializer.cs
// (vessel-initializer / menu-swap arc). Mechanical substitutions (README) + GameTask mappings
// (`async UniTaskVoid` → `async Task` + `.Forget()`; `UniTask.Delay(ms, DelayType.UnscaledDeltaTime)`
// → `GameTask.Delay(ms/1000f, unscaledTime: true)`). Server/Client RPCs carry local-invoke
// semantics until the transport phase (Player/RoundStats precedent) — on the host the client-side
// paths (pending-pair queue, roster pull) are skipped by the verbatim IsServer early-return.
using System;
...
namespace CosmicShore.Gameplay
{
    ...
    public class ClientPlayerVesselInitializer : NetworkBehaviour
    {
        ...
        /// </summary>
        async Task RosterPullRetryLoop(CancellationToken ct)
        {
            ...
            for (int attempt = 0; attempt < maxAttempts && !_localPairResolved; attempt++)
            {
                ...
                try
                {
                    await GameTask.Delay(intervalMs / 1000f, unscaledTime: true, cancellationToken: ct);
                }
