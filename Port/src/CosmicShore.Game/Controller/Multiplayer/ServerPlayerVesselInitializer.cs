// Ported verbatim from Assets/_Scripts/Controller/Multiplayer/ServerPlayerVesselInitializer.cs
// (vessel-initializer / menu-swap arc). Mechanical substitutions (README) + GameTask mappings
// (`async UniTaskVoid` → `async Task` + `.Forget()`; `async UniTask` → `async Task`;
// `UniTask.Delay(ms, DelayType.UnscaledDeltaTime)` → `GameTask.Delay(ms/1000f, unscaledTime: true)`).
// ClientRpcs carry local-invoke semantics until the transport phase (Player/RoundStats precedent);
// with no remote clients connected the Notify* fan-outs iterate an empty ConnectedClientsList.
...
namespace CosmicShore.Gameplay
{
    ...
    public class ServerPlayerVesselInitializer : MonoBehaviour
    {
        ...
        async Task HandlePlayerNetworkSpawnedAsync(ulong ownerClientId, CancellationToken ct)
        {
            ...
            await GameTask.Delay(preSpawnDelayMs / 1000f, unscaledTime: true, cancellationToken: ct);
            ...
            if (!IsReadyToSpawn(player))
            {
                ...
                {
                    await GameTask.Delay(retryIntervalMs / 1000f, unscaledTime: true, cancellationToken: ct);
        ...
        /// </summary>
        protected virtual async Task OnPlayerReadyToSpawnAsync(Player player, CancellationToken ct)
        {
            ...
            await GameTask.Delay(postSpawnDelayMs / 1000f, unscaledTime: true, cancellationToken: ct);
