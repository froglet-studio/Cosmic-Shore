// Ported verbatim from Assets/_Scripts/Controller/Multiplayer/ServerPlayerVesselInitializerWithAI.cs
// (initializer-remainder arc 2026-07-07). Mechanical substitutions only (README):
// `Reflex.Attributes` + `Reflex.Injectors` → `CosmicShore.Engine.Injection`;
// `Unity.Netcode` → `CosmicShore.Engine.Networking`; `UnityEngine` → `CosmicShore.Engine`.
// No async/UniTask in this file. AI spawn/despawn uses the engine NetworkObject's
// Spawn(destroyWithScene)/Despawn(destroy) — same destroyWithScene=false semantics as upstream.
