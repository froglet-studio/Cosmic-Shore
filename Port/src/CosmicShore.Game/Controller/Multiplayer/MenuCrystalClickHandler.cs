// Ported verbatim from Assets/_Scripts/Controller/Multiplayer/MenuCrystalClickHandler.cs
// (initializer-remainder arc 2026-07-07). Mechanical substitutions (README) + GameTask
// mappings (`async UniTaskVoid` → `async Task` + `.Forget()`; `async UniTask` → `async Task`;
// `UniTask.Delay(ms, ignoreTimeScale: true, ct)` → `GameTask.Delay(ms / 1000f,
// unscaledTime: true, cancellationToken: ct)`; `UniTask.Yield(ct)` → `GameTask.Yield(ct)`).
// STATE LOGIC is live: the menu↔freestyle transition state machine (_isInFreestyle /
// _isTransitioning / CTS lifecycle), autopilot toggles, input pause/unpause +
// lockInputDuringEnterTransition sequencing, the IsMultiplayerSession Time.timeScale guard,
// the SOAP transition bracket raises, and the transition-duration gate (the _isTransitioning
// window). Presentation deviations (marked per line):
//   • CanvasGroup fade machinery — the two CanvasGroup[] fields, fadeDuration,
//     _savedMenuAlphas, the Start() initial-hide, the fade arm of each UniTask.WhenAll
//     (the duration arm runs alone; with the defaults the 2s camera window outlasts the
//     0.5s fade, so the gate timing is preserved), and the whole UI Fade region — restore
//     when the UI-shell/CanvasGroup arc ports.
//   • MainMenuCameraController per-mode duration (field + CurrentTransitionDuration
//     branch; the serialized fallback duration is the live path) — restore when the
//     camera arc ports.
...
namespace CosmicShore.Gameplay
{
    ...
    public class MenuCrystalClickHandler : MonoBehaviour
    {
        ...
        async Task TransitionToFreestyle()
        {
