// Ported verbatim from Assets/_Scripts/Tests/EditMode/MenuFreestyleToggleTests.cs
// (initializer-remainder arc). Mechanical substitutions (README): UnityEngine →
// CosmicShore.Engine; the `#if UNITY_EDITOR` guard is dropped (this suite always
// runs headless). The MainMenuController region is carried as a commented deviation
// (subject unported); the "no NetworkManager" test explicitly nulls
// NetworkManager.Singleton to reproduce the edit-mode precondition upstream gets
// for free (the port's Singleton is a settable static shared across the test run).
...
namespace CosmicShore.Tests
{
    ...
    public class MenuFreestyleToggleTests
    {
        ...
        public void MenuCrystalClickHandler_IsMultiplayerSession_ReturnsFalse_WhenNoNetworkManager()
        {
            ...
            // PORT: the port's NetworkManager.Singleton is a settable static shared across the
            // test run, so establish the upstream edit-mode precondition explicitly.
