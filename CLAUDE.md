...
| `ModePreview/` | `Docs/` | The arcade card's live arena **diorama** and the in-menu **Test Flight** that replace the preview video: why a preview can't be playable inside a thumbnail (input ownership is exclusive), why a Test Flight SWAPS the menu world instead of standing a second ecology beside the Lattice cell, the private-layer + distance + lifetime rules that keep the diorama camera off the frame budget, and the ONE idempotent exit every route funnels through. **Read before adding a preview for a mode, or before touching `ArcadeGameConfigureModal`'s preview window.** |
