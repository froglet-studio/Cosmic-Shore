...
| `PERFORMANCE_OPTIMIZATION.md` | `Docs/` | **The one perf doc to read first — short, current, and a PLAN, not a log.** Where the game stands, what was done since day 1 (a dated table), what is left as a ranked lever list, and **how to measure without fooling yourself**. The headline: **the prism renderer is solved** (instanced path 12.9× cheaper than legacy; 8,436 prisms in 23 draw calls) and the cost has moved to **everything built AROUND prisms** — ~14 ms of the main thread idle-waiting on culling/render jobs (best lead: ~40,000 enabled flora-spindle renderers against 4,599 visible), plant growth `Instantiate`ing GameObjects, ~17 ms of unattributed `UpdateScene`, and 154.5 KB/frame of gameplay GC. **bleeding-edge made Garland the boot world** (Lattice is now opt-in), so the plan starts by re-baselining a fixed five-scenario set on the merged tree. §3.5 lists the hypotheses **measured dead** (prism draw calls, the prototype re-key, chunk locality, opaque conversions, render-path GC) — do not redo them. §4 is the method: **attribute with the Profiler first** (it names the caller), A/B only between arms in the **same state** (a growing world breaks every A/B), send text not screenshots. Every raw capture and root cause from 2026-07-08 → 2026-09-22 is frozen in **`Docs/archive/PERFORMANCE_LOG_2026.md`**, whose section numbers (`§0.8`, `§0.11.6`, `Task 10`) code comments still cite. |
| `MEMORY_AUDIT.md` | `Docs/` | The other half of the perf picture — what the game HOLDS (lever L7 in `PERFORMANCE_OPTIMIZATION.md`). §1 carries the first runtime readings (**Editor only**, so the Managed/Unity rows include the Editor's own heap — a Development-build reading is still owed). The asset footprint was measured **on `78a95259f`, not on bleeding-edge** — re-measure before quoting a figure — and its finding is **import settings, not size**: four music tracks totalling 182 MB are all `loadType: 0` (DecompressOnLoad, fully resident PCM) at near-lossless Vorbis `quality: 1`, `loadInBackground: 0`, and `3D: 1` **on music**; 200 of 214 clips project-wide are DecompressOnLoad. Also: texture streaming is OFF in all five quality tiers with a 512 MB budget already authored, 171 MB of menu preview `.mp4`s whose references may all be dead, and no Addressables package. **Measure it with a path-safe walk, never `find | xargs du`** — many filenames contain spaces, and word-splitting them hides the entire audio finding. |
...
| `BENCHMARK_TOOL.md` | `_Scripts/Utility/PerformanceBenchmark/` | Performance Benchmark tool guide (tabs, score/hints, sweep, Load Time Insights, customization) plus the **DiagnosticsHUD console commands** (`fps uncap`, `diag <label> <s>`, `renderers`, `renderers hide/show`, `prismpath`, `prisms`, `freeze on/off`, `ab "<A>" "<B>" [s] [rounds]`, `prof [label] [frames] [root=…] [sort=…]`) and troubleshooting. **An A/B is only valid between two arms taken in the SAME state** — `freeze on` holds ecology production in every cell (`Cell.DiagnosticProductionHold`: production gating only, the same class as Frenzy, removes nothing, released on scene change) and `ab` interleaves counterbalanced rounds and prints one paired delta with its standard error. **`prof` is the Profiler Hierarchy as JSON** (Editor only): it records N frames, then writes the tree AVERAGED over every captured frame (an absent sample counts as 0), self time summed by name, allocators ranked by SELF GC (the Profiler's GC column is inclusive, so ranking it names `PlayerLoop`), thread busy vs wait, and a typical + spike frame picked by `PlayerLoop` time — never by whole-frame time, which in the Editor picks a repaint |
