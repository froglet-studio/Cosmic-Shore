...
| `ToySystem/` | `Docs/` | Freestyle **Toy** system (the new `Toy` fundamental): `ARCHITECTURE.md` — world-space interactive stations the local vessel flies into (no score, no end condition), placed near the Cell membrane in Menu_Main. Three toys: Vessel Changer (reuses `RequestSwap`), Domain Changer (reuses `RequestSetDomain_ServerRpc`), and the revived "fly by numbers" Painting toy (`PaintingToy` + `FreestylePaintingDirector` driving `ShapeDrawingManager`). `ToyboxSO` registry + deferred unlock-state hook; `ToyboxController` self-wires (Resources/default fallback); `Tools > Cosmic Shore > Setup Freestyle Toybox` authors assets + wires the scene. |
...
| Freestyle toys | `Toy` (base world-trigger), `VesselChangerToy`/`DomainChangerToy`/`PaintingToy`, `ToyboxController` (placement near membrane), `FreestylePaintingDirector` (revives the deleted shape-drawing glue), `ToyboxSO`/`ToyDefinitionSO` (registry + unlock state), `ToyboxSetupTool` (editor) | `_Scripts/Controller/Toys/`, `_Scripts/ScriptableObjects/Toys/`, `_Scripts/Editor/` |
