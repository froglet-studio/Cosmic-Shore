...
| `THEATER.md` | `Docs/` | **The match recorder** — save a full game and fly a camera around it afterwards, from any angle, as many times as you like. Records **outcomes, never inputs**: Halo 3 replayed controller input against a deterministic simulation and this one has none (flight integrates in `Update`, the life spawners roll on each peer's own `Random`, `Mathf.Sin` is not bit-identical across Mono and IL2CPP), so recording where things WERE needs none of that to be true — and none of it can break a recording later. It is cheap because **mass is conserved**: the object class that outnumbers everything else has no continuous state to sample, only births and deaths. **P0 is shipped and is vessel-only** (9 records, 8 plays back). Three things in it generalise. **A puppet is HARVESTED from the prefab asset, never instantiated** — which sidesteps the B16 stray-`NetworkObject` trap by construction rather than by remembering a guard, and costs a ghost its jets, tail and morphs. **P0 snaps to any timestamp perfectly and that will not last**: a vessel track is sampled STATE, so a seek is a binary search, while prisms arrive in P1 as EVENTS and a backward seek there becomes rebuild-and-replay-forward off a 9 KB liveness snapshot. And **the audio answer is yes**, because CLAUDE.md's own rule that every sound is an `EventReference` played through `AudioSystem` or a `StudioEventEmitter` collapses 50 call sites into **seven methods and one component** — with snapping backward being the EASY case for audio, since nothing expects sound to be continuous across a time jump. **Read before adding a second gameplay camera, before assuming a recording can replay inputs, or before changing what `ToyModelBuilder` does with a non-positive radius** (zero now means native scale and native pivot). |
