...
- **Lit** — *mass standing inside a force volume is LIT, in the colour of whoever owns that force.*
  Added at the prompter's request, and it earns the weight by REPLACING five bespoke highlights
  rather than adding a sixth: the Dolphin's Echo Sight (a **pending** blast), the Sparrow's
  proximity fuze (an **armed** warhead), the Scarab's cavitation plate (a **resolving** sweep),
  every own-domain explosion passthrough (a blast that **arrived** and spared it) and every
  skimmer's field (**continuous**). They differ only in WHEN the force lands, which is a property
  of the producer; what they share is the volume, the owner and the light. It composes rather than
