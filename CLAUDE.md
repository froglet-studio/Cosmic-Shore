...
- **Audio**: **FMOD** (`Assets/Plugins/FMOD`) — every gameplay/menu SFX is an `EventReference` wired on `AudioSystem`, played as a one-shot through `FMODOneShotVolumeHelper` so it respects the SFX slider, with music still on the legacy Unity `AudioSource` path. A Wwise folder is also present but **no first-party script uses it** — do not add one. **Every gameplay one-shot passes a per-category burst gate** (`GameplaySFXBurstLimiter` + `GameplaySFXPolicySO`): a frame that produces N of the same event must never start N copies of the same FMOD event, because identical one-shots started together sum **coherently** (+20·log₁₀(N) dB — ten crystals is +20 dB) and comb-filter into harsh phasing. Attenuation alone cannot fix that; the fix is **spacing voices apart in time** (`minRetriggerSeconds`), with a voice budget and burst coalescing that replays suppressed events spaced out, quieter, at their centroid so a big burst still *sounds* big. See `Docs/AUDIO.md`.
...
| `AUDIO.md` | `Docs/` | Gameplay SFX + the **burst policy**: why N identical one-shots in one frame is two different problems (FMOD voices, and coherent summation that attenuation cannot fix), the three levers (`minRetriggerSeconds` decoherence / `maxVoicesPerWindow` budget / `maxPendingVoices`+`burstMagnitudeGain` magnitude-preserving coalescing), the Dolphin-blast reference numbers, the four layers that make the gate un-authorable to skip, the 2D-vs-3D call-site rule, and how to add a category. **Read before adding a `PlayGameplaySFX` call site or a `GameplaySFXCategory`.** |
