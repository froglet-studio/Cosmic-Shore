...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  | Sparrow | 4/4 named, **4/4 upgrades** (Time re-scoped 2026-08: indefinite boost, base roll, Elemental Ward. **Mass L5 = Shielded Prisms again** — it briefly moved to Space 5 in 2026-08 round 4 and was returned by design sign-off on 2026-08-13, settling the split: **MASS owns the SUBSTANCE of what you fire** (turret prism stretch, in-flight round growth, armour) and **SPACE owns its REACH** (range, and pierce on both fire modes). In-flight growth is ONE curve — `ElementalScaling.RoundGrowthFactorForLevel` — reaching all THREE things this vessel fires, each with its own authored pair and its own SHAPE (`RoundGrowthRamp`): bullets and turret prisms grow across the whole flight at 3×/6×, the skyburst missile at 20× in the first FIFTH of its flight, then held. **The skyburst is also the ONE documented exception to "growth is a hit volume, not a size"** (see the Projectile charge shell row): it is the only round whose MODEL was authored far SMALLER than the hit volume it already had — ~1.7 u long inside an 8.5 u-RADIUS sphere — so it has no growing hit volume for a shell to draw, and growing the model walks it INTO its own reach instead of out past it. Its collider is untouched. The exception is carried by `Projectile.flightGrowthTarget`, which is empty on every other round in the game; do not reach for it to make an ordinary round bigger (`SPARROW_SKYBURST_BAY.md`)) | 4/4 | ✅ | ✅ | ⚠ no switcher on its HUD |
