...
| Vessel tail + jets | `VesselTail` / `VesselJet` (markers, each carrying `widthScale`), `VesselTailAndJets` (one per vessel: domain tint over every `TrailRenderer`), `VesselFXWidth` (applies the per-vessel width once at Awake), prefabs `Components/VesselTail.prefab` + `Components/Jet/VesselJet.prefab`, audit via FrogletTools > Vessels > Audit Vessel Tails and Jets. **A tail is tuned for OTHERS and sits behind the pilot's own camera; a jet is tuned for its own pilot but visible to everyone; both wear the vessel's domain like the trail does** — `Docs/VESSEL_TAIL_AND_JETS.md` | `_Scripts/Controller/Vessel/`, `_Prefabs/Spacevessels/Components/` |
