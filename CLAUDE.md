...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  | Scarab | complete | 4/4 | ✅ | ✅ | ⚠ no switcher on its HUD — and note `Scarab.prefab` wires **`SparrowHUDVariant`**, not `ScarabHUDVariant`; the two prefabs share ability-icon fileIDs, so `ScarabHUDVariant` may be orphaned. Verified from assets 2026-08-25 (this row was missing from the table entirely) |
  ...
  **All four vessels with a row now wear the ABILITY LOCKUP** (`Docs/ABILITY_LOCKUP.md`) — the totem
  ...
  vessel inherits it by binding a row. Audit with **FrogletTools > Vessels > Audit Ability
  Lockups**, which checks the shared style and — the thing one shared style cannot absorb —
  whether each vessel's own icon size still fits the card after kerning.
