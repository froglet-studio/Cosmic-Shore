...
| `DAILY_CHALLENGE.md` | `Docs/` | **The daily challenge** — one curated objective per UTC day, the same one for every player. The rule that makes it work: **the definition is DERIVED, only the progress is STORED** — today's challenge is `hash(UTC date) % pool` over an authored `DailyChallengeCatalogSO`, so it resolves on a cold launch, offline, and identically on every platform with no server round trip, while UGS Cloud Save carries only what genuinely differs per player (best value, completed, attempts). The hash is hand-written FNV-1a rather than `System.Random`, which is deterministic only *within one runtime's implementation* — not a promise two platforms can hold each other to. **Rollover is a date comparison, never a timer** (a scheduled midnight callback is wrong every time the device slept through it or the clock moved). The challenge is an EXISTING arcade mode launched through the EXISTING arcade modal — never a second launch pipeline — DRESSED to state the challenge rather than the mode (`ArcadeGameConfigureModal.OpenForDailyChallenge` + `ApplyDailyChallengePresentation`): the BRIEFING is the challenge's objective and the objective BOX is hidden (it pairs a win condition with a live counter, and there is nothing to count until the run starts), **Add AI is gone** (the seats are the card's minimum, so there is none to take), the preview is **live but look-only**, and intensity + DOMAIN are pinned and dimmed. Two details there are load-bearing — the preview's own AI score must not tick the challenge's counter (progress the player has not made against an objective they have not started), and **a pinned domain other than Jade must be REQUESTED, not just shown selected**, because the tiles reflect `Player.NetDomain` and highlighting one without the server round trip is the exact "UI claims a domain the server never got" case `HandleDomainSelected` refuses to create (Jade needs no request — the menu already resets every player to it). Every override is undressed again when the next ordinary card opens, because the panel is a shared scene object. Its objective is read off the **local player's own** round stats, never a domain sum, because the run seats the card's minimum and the rest are bots. Three authoring traps it records: **a target above the mode's own end condition is unreachable** (Crystal Capture ends at 20 crystals per domain, so "collect 30" there never completes); **a metric must be credited PER PLAYER** (Nucleus Rush credits a domain's *representative* and is deliberately out of the pool); and **reordering the pool re-rolls which date draws which mode**. Authored ONLY through **FrogletTools > Game Modes > Daily Challenge**, which exists for the VALIDATION rather than the fields: three of the four ways to author an unplayable challenge are invisible in a plain field list. Its layout is **master/detail behind three tabs** — a one-row-per-entry list carrying only what you scan for (on/off, what it asks, a status pill) with fields for the SELECTED entry alone, because N entries × 9 fields is a page and a half of scrolling to compare two numbers 400px apart, and Pool / Preview / Testing are different questions that get harder when stacked into one scroll. **A daily run is a SMALLER version of the mode** — `EndConditionOverridesSO` grew a run-scoped override (`SetRunOverride`/`ClearRunOverride`, consulted by every per-turn target accessor, static because it is set in the MENU and read in the GAME scene, reset at `RuntimeInitializeOnLoadMethod` because a leaked one silently shortens the next ordinary match), so Crystal Capture races to 12 instead of 20. The race target is deliberately ABOVE the objective, because **the objective is personal while an end condition is a domain SUM** — leave them equal and a teammate bot can end the run on somebody else's crystal. `CanOverrideTurnTarget` says which modes it reaches; **Astro League is not one** (its controller owns its goal target), so it is out of the pool rather than shipped as a full-length match with a clock on it. **Played ONCE a day, and the attempt is spent at LAUNCH** — the one ordering that survives an alt-F4 halfway through a bad run, which is why `RecordResult` folds in the result and never touches the counter. Test mode (forced entry, a 'day' of N minutes, no attempt limit, a scaled clock) is inert outside the editor and dev builds AND fails a non-development build outright — a runtime gate makes a stray flag harmless, the build guard makes it loud. Supersedes the inert PlayFab-era `DailyChallengeSystem` / `DailyChallengeModal` — do not wire both. |
