...
| `THIRD_PARTY_REGISTER.md` | `Docs/` | **Every third-party component in `Assets/` and `Packages/manifest.json`**, with the one column that decides urgency — **does it reach a player build** — derived from Unity's actual inclusion rules rather than from folder names: an `Editor/` folder never ships, an asmdef's `includePlatforms` decides, code with no asmdef lands in `Assembly-CSharp` and ships, and **any folder named `Resources/` is packed whole whether or not anything references it** (the rule that produces defects nobody notices — it produced two here). Records vendor, version where discoverable, and whether a licence document is actually in the tree, plus **8 entitlement questions** addressed to the Asset Store account holder. **The register proves PRESENCE; only a purchase record proves ENTITLEMENT** — no licence status is asserted without a document behind it. Two traps it records: NiceVibrations' two "licence files" are a **Rust crate list** and a **haptic-sample CC notice**, neither of which is the product EULA; and provenance for the unattributed folders is **not recoverable here** because the clone is shallow, so `git log --diff-filter=A` reports the graft boundary for all of them. **Read before adding ANY third-party asset, or before answering "are we allowed to ship this".** |
