...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  | Sparrow | 4/4 named, **4/4 upgrades** (Time re-scoped 2026-08: indefinite boost, base roll, Elemental Ward. **Mass L5 = Shielded Prisms again** — it briefly moved to Space 5 in 2026-08 round 4 and was returned by design sign-off on 2026-08-13, settling the split: **MASS owns the SUBSTANCE of what you fire** (turret prism stretch, in-flight round growth, armour) and **SPACE owns its REACH** (range, and pierce on both fire modes). In-flight growth is ONE curve — `ElementalScaling.RoundGrowthFactorForLevel` — reaching all THREE things this vessel fires, each with its own authored pair and its own SHAPE: bullets and turret prisms swell across the whole flight (3×/6×), the skyburst missile reaches 20× in the first FIFTH of its flight and holds, and it grows its MODEL rather than its hit sphere so its reach is untouched (`SPARROW_SKYBURST_BAY.md`)) | 4/4 | ✅ | ✅ | ⚠ no switcher on its HUD |
