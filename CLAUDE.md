...
| `VESSEL_TAIL_AND_JETS.md` | `Docs/` | The **tail** and **jet** contract: the platform has THREE things streaming off a vessel and they are not interchangeable — the **trail** (conserved prism mass), the **tail** (one long beacon streak on the vessel ROOT, for OTHER players to find you) and the **jets** (short plumes on the model's ENGINE nodes, for THIS pilot to read their own thrust). Both new layers are standard, expected parts of every vessel and both take the vessel's **domain** exactly as the trail does. Documents the marker components (`VesselTail`/`VesselJet`), the one vessel-level owner (`VesselTailAndJets`), why jets are drawn on their own pilot's screen only (and the per-jet `visibleToOtherPilots` exception for the Serpent), why the bind sits in `VesselController.Initialize` **ungated** with the pilot flag as its argument, why `ChangePlayer` must re-run it, the Dolphin's measured mount table, and the fleet status. **Read before adding, moving, or recolouring any vessel FX — and before using the word "trail" for something that is not prisms.** |
...
| Vessel tail + jets | `VesselTail` (beacon marker), `VesselJet` (engine-plume marker, `visibleToOtherPilots`), `VesselTailAndJets` (one per vessel: domain tint over every `TrailRenderer` + viewer visibility), prefabs `Components/VesselTail.prefab` + `Components/Jet/VesselJet.prefab`, audit via FrogletTools > Vessels > Audit Vessel Tails and Jets. **A tail is seen by OTHERS, a jet only by its own pilot, and both wear the vessel's domain like the trail does** — `Docs/VESSEL_TAIL_AND_JETS.md` | `_Scripts/Controller/Vessel/`, `_Prefabs/Spacevessels/Components/` |
