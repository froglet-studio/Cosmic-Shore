...
| `HapticsSystem/` | `Docs/` | Audio-matched haptics: `ARCHITECTURE.md` — transient envelopes measured from each SFX's source waveform + a continuous bed following live FMOD bus metering (RMS + spectral centroid), arbitrated onto the single Lofelt haptic channel. Config in `AudioHapticsConfigSO`; editor baker under Tools > Cosmic Shore > Audio Haptics. |
