...
- **Trusting an authored `ScrollRect` Content height, or growing it by an increment.** A scrolling list that has never been scrolled to its end tells you nothing about whether it can be: a card below the reachable range is clipped by the viewport's `Mask`, which cuts the drawing off **and**, being an `ICanvasRaycastFilter`, eats the PRESS — while the scroll stops at the authored height, so a drag springs back. *Half a card, a scroll that snaps back and a dead button are ONE cause.* The arcade grid shipped exactly this the day it grew from 12 cards to 13 (`Docs`/`_Scripts/Controller/Arcade/SWITCHBACK.md`): its Content was authored at 1104 against three rows whose lowest edge sits at −1835.78, so it was already 731 short **before** a row was added, and an increment sized to the new row therefore landed short too. Measure with `RectTransformUtility.CalculateRelativeRectTransformBounds` — and measure **to a fixed point**, because a Content whose layout group has `ChildForceExpandHeight` turns height into spacing: the surplus is shared out among its children, pushing the content down and demanding more height again, so one measure-and-set converges on nothing (1104 → 1835 → 2199 → 2442 …). Switch force-expand off on a scrolling Content — its job is to be as tall as its contents, not to distribute an authored height — then iterate. Two details that are not polish: the needed height is `max(bounds.size.y, -bounds.min.y)` on a TOP-pivoted rect, and the fit must run **after** the cards are activated (the bounds call skips inactive objects). General rule: **adding a card is adding a ROW, and a row is only reachable if the scroll content was measured after it** — and because every failure here is silent, `ArcadeExploreView.ReportUnreachableCards` names any mode with no slot or past the content's height rather than leaving it looking unshipped
