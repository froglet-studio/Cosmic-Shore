...
| `OFFLINE_MODE.md` | `Docs/` | **DESIGN, not implemented.** Why the game cannot be played without a network: nothing in the project ever calls `NetworkManager.StartHost()` — the host only comes up as a side effect of the UGS SDK creating a Relay-backed session, and `AuthenticationSceneController.LoadMainMenuNetworkedAsync` then waits on it **with no timeout**, so an offline launch bricks at the boot splash. Carries the diagnosis, the local-host fallback design (offline = a plain `127.0.0.1` host, so the whole Netcode spawn chain and every AI-backfilled minigame runs unchanged), the full impact list (cloud data, identity, party/friends/store UI, per-mode playability), and the separate live bug that `OnNetworkFound` is never subscribed so `Disconnected` never recovers. **Read before any offline / single-player-fallback work.** |
