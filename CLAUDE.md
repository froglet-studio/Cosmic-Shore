...
| `REAR_VIEW.md` | `Docs/` | **The look-back camera** — while **C** (or **LB+RB together**) is HELD, the gameplay camera moves to the MIRROR of its own follow offset (the same distance AHEAD of the vessel that it normally sits behind it) and keeps looking at the ship; release and it is forward again. It retires the picture-in-picture rear view (`Pip` / `PipUI` / `PipCamera.prefab`). Five things it records generalise. (1) **It is OPT-IN PER VESSEL, not a platform law** (`CameraSettingsSO.enableRearView`; **Manta and Scarab** only) — the corridor, the speed tunnel and the vision band are laws because they answer questions EVERY vessel raises (can I see my ship, how fast am I going, where is that pilot), while a look-back answers one only some hulls are shaped to ask: it reads completely differently at the Urchin's 6.67-unit follow distance and at the Serpent's 250. The flag lives on the per-vessel SETTINGS, never on `CustomCameraController`, because that component is ONE rig shared by every vessel (a child of `CameraManager`, not of the hull), so a field there would be a property of the CAMERA rather than the SHIP and would survive a swap onto a hull that never asked for it. (2) **HELD, never toggled, and the hold EXPIRES rather than waiting to be cancelled** — `SetHeld` stamps `Time.frameCount` and the driver requires the hold to be from THIS frame, so all five of `InputController.Update`'s early returns (and any added later) release it for free; a driver that waited to be TOLD would sit mirrored through a pause, a tab-out, or the frame a vessel is despawned. *A held level also cannot desynchronise: there is no remembered state to disagree with the button.* (3) **There is deliberately NO second camera**: a second live `Camera` falls outside four systems at once — the speed tunnel resolves `CameraManager`'s ACTIVE controller and would keep narrowing the camera the player is no longer looking through, `ApplyCameraGraphicsSettings` pushes the player's FOV and AA onto its three managed cameras and no others, `SetBackgroundColor` is applied per camera, and `Camera.main` returns the first ENABLED camera tagged MainCamera. Reusing the one rig inherits all four, the argument `BeginWindowedPlayerCamera` already records for the mode preview — and only the PLAYER rig is flipped (identity against `GetCloseCamera`), since the death and end/replay cameras are `CustomCameraController`s too. (4) **The mirror is applied at the POINT OF USE, never written into the offset** (`CustomCameraController.EffectiveOffset`; z only — mirroring the Sparrow's authored `(0, 10, -50)` whole would put its rear camera UNDER the ship). Four systems legitimately write `_followOffset` while a pilot flies — the zoom-out abilities, adaptive zoom, the skimmer's camera-scaling prism effect, and a vessel swap re-applying its own `CameraSettingsSO` — so a written mirror means **the first of them to fire silently drops the pilot out of the rear view** with nothing to explain it. (5) **Retiring a panel means checking what OVERRODE its default**: `Pip.prefab`'s root ships INACTIVE, but `CORE/GameCanvas.prefab` and `Panels/MiniGameHUD.prefab` each carried an `m_IsActive: 1` override on their nested instance, so the panel was on by default and was only ever switched off by a vessel announcing it was NOT the local pilot's — removing the grant alone would have left a dead panel showing a stale render texture in every mode. `Pip.cs` is KEPT and never granted, because its `Awake` default-off is now the only thing standing `PipCamera.prefab` (which ships active and enabled) down on the eight hulls that carry one. The gesture is the sibling of `OverviewGesture`, polled ONLY by `InputController` (the one pump already gated on local human pilot and on both pause gates) and deliberately not an `InputEvents` member — it drives a camera, not a vessel. **Read before adding a second gameplay camera, touching `CustomCameraController`'s follow offset, giving another hull the rear view, or binding anything new in `VesselController.Initialize`.** |
