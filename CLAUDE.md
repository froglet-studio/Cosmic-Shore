...
| **Gibbon** | 13 | Brachiation — zero-gravity swinging on two arms (LT/RT), Spider-Man-style traversal with no throttle and no gravity; code name **Spider** (`Spider.prefab`, `SwingingVesselTransformer`). Rebuilt 2026-09-09 around a closed-form winch model proven offline; see `_Scripts/Controller/Vessel/R_VesselActions/GIBBON.md`. Elemental map: two arms bound, two open slots |
...
| `MinigameCanopyRun` | `CanopyRun (45)` | `CanopyRunController` |
...
`CanopyRun(45)` is the **Gibbon-only brachiation race** — Skim Race's domain crystal race (controller
subclassed verbatim, scoring rule reused, first DOMAIN to the crystal target, golf-timed) on a
**canopy**: the SAME per-intensity waypoint line grows super-shielded prism BOUGHS alternately
left/right every 180u at ±80u, and a prism HOOP around every crystal spot, so the vessel's
left-arm/right-arm rhythm IS the racing line and the hoop is the fling target (the latched arm's
reticle shows where the hull lands 0.6 s after letting go). Two findings generalise: (1) **a mode
that lays geometry AROUND a crystal must own the crystal's anchors** — the donor CrystalManager's
per-intensity sets differ from the track's waypoints by 46-81u (and by 14 anchors vs 28 waypoints
at intensity 3), which a ribbon never noticed and a hoop cannot survive, so the generator
re-authors the anchors ONTO the waypoints and shrinks the spawn jitter under the hoop radius,
both asserted; (2) **a scene clone must author every NEW field explicitly** — Unity fills an absent
field from the class initializer with no log, so a guid swap alone ships a tuned number as a
default. 244-630 prisms per intensity, all super-shielded. See
`_Scripts/Controller/Arcade/CANOPYRUN.md`; the vessel: `R_VesselActions/GIBBON.md`.
...
└── MultiplayerMiniGameControllerBase (abstract, NetworkBehaviour)
    ...
    └── MultiplayerDomainGamesController
        ...
        └── CanopyRunController                 — Gibbon-only brachiation race: SkimRaceController verbatim on a canopy of boughs + hoops laid on Skim Race's waypoints
```
...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  | Gibbon | 2/4 named (the two ARMS, bound to LT/RT), 0/4 upgrades; Space/Time open design slots | 0/4 authored icons (the nested `SquirrelHUDVariant` carries the row; no `GibbonHUDVariant`) | — | — | ✅ LT / RT chips derive from the map's real inputs |
