...
| Prism WAKE (§4.7.3) | A fast-moving **CARRIER** drags a **travelling ripple** through the mass around its recent path. `PrismWake` (publisher + residency) + `PrismWake.hlsl` (`PrismWakeDeform`, VERTEX) + `PrismWakeSource` + **`IPrismWakeCarrier`** + `PrismWakeConfigSO` (`Resources/PrismWakeConfig`). The second member of the high-poly morph family the cradle founded, and the first built with the `/prism-morph` skill. **It is granted to TWO THINGS, and that is the finding worth carrying.** It shipped on EVERY vessel — cheap, correct, replicated, un-omittable by wiring, inside budget — and was pulled after one playtest on the verdict *"in this state this is an awesome effect, but it will be overused as a wake on every vessel"*: **an effect strong enough to be an EVENT stops being one the moment it is continuous**, and no measurement would have said so. It now belongs to the Sparrow's **skyburst missile** (`Projectile.leavesWake`, authored true on that one prefab exactly as its tail is — a bullet is a streak already and 54 are alive at once) and the Scarab's **ball** (unconditional in `AstroLeagueBall.Awake`, so the forged and nucleus-seeded balls are covered without either spawn path knowing). There is a quantitative half too: `maxResidentPrisms` is **SHARED and split evenly**, so a per-vessel grant does not add wakes, it DIVIDES the one that mattered — past a headcount every wake is the authored 24-triangle prism again and the feature silently switches itself off everywhere at once. Adding a third carrier is a design call, not a wiring one; `PrismWakeTests` holds the fleet grant out. **A carrier is a CAPABILITY, never a type**: the source asks `IPrismWakeCarrier` for this frame's velocity and LIVE radius, because the two carriers share nothing (one is pooled with a TELEPORTING mover, the other a replicated interpolated rigidbody) and because both change size in flight — a skyburst swells 20×, a forged ball is sized after spawn, and a bigger thing earns a bigger wake. The transform-delta fallback is wrong for both in a way that does not look wrong: it cannot tell a flight step from a POOL REPOSITION, and on a peer it reads a smoothed estimate of a velocity the ball already knows (`n_Velocity`). The frame is a **CYLINDER about the path**, not a sphere about the carrier. The map is a dimensionless **STRAIN** (`r → r(1+E)`), not an offset along `r̂` — so the axis is a FIXED POINT, there is no singularity where `r̂` is undefined, and **NO FOLD is one number with no geometry in it** (`A < 1/(1+max|t·K'(t)|) = 0.529`, clamped to 0.45; note exponent 1 is exactly where that slope is worst, so the shipped look spends margin the proof HAS). Its normal carries a **SHEAR** term the cradle's spherical map has no analogue for — the part that exists only because the wave TRAVELS — which is also the harness's negative control. **It splices BEFORE the cradle**, because the cradle closes mass onto a hull RESTING on it and a wake applied after would re-open the hole; nothing on screen would report the two being swapped, so both edges are asserted. That splice broke three sibling wirers that walked past ONE hard-coded node name and produced `Tools/Shaders/prism_vertex_chain.py`, the STRUCTURAL definition of a vertex morph — *a name list needs one edit per wirer per morph forever, which is a defect amplifier rather than a fix.* The speed window is ABSOLUTE and is now authored from MEASURED carrier speeds (6 → 60 u/s; 6 is the ball's own `ballRestSpeed`) — its first value, 150, sat above the top speed of the hull the mode actually flew, so the window never opened and **that reads on screen exactly like an effect that is too weak**. `PrismWake.ReportState` on `CSLogChannel.PrismRuntime` exists for that reason and names which of the five ways it can look idle it is in. Proven by `Tools/Shaders/verify_prism_wake.py`; **not run in the editor**. `Docs/PRISM_ANIMATION.md §4.7.3` | `_Scripts/Utility/`, `_Scripts/ScriptableObjects/`, `_Scripts/Controller/Projectiles/`, `_Scripts/Controller/Arcade/AstroLeague/`, `_Graphics/Materials/Graphs/` |
