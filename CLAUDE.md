...
- **A file-mode Custom Function node's HLSL signature must declare EVERY INPUT before either `out`.** The node does not GENERATE the function — it CALLS the one in the `.hlsl`, and it builds that call as **all its input slots and then all its output slots**. Break it and the graph does not compile, which means **every material drawn with it renders with no material**, and nothing outside the editor says so: a verifier that compiles the `.hlsl` sees a perfectly good function, because the call it would have to read lives in the generated shader and in no file on disk. It is inviting to break because HLSL itself permits an `in` parameter after an `out` one, so appending a new input as the highest slot id and declaring it last leaves every existing slot id and every existing edge untouched and reads as free — that exact edit put `ErosionThreshold` where `out float Alpha` was declared and turned **every prism in the game** magenta (2026-09-17, `Docs/PRISM_ANIMATION.md` §4.7). **The SLOT IDS are a separate question and they are free**: TextMesh Pro ships file-mode nodes whose ids interleave (`Composite` is in [0, 3] / out [2]) and compile, so a gate asserting the id rule flags 40 shipping nodes — the node keys on the slot TYPE, not the id. Gate: `python3 Tools/Build/check_shadergraph_custom_function_signatures.py` (pure Python, 56 graphs, `--self-test` negative-controlled on the `(in, out, out, in)` shape that caused it). General rule: **a gate that compiles a shared HLSL file proves nothing about the call sites the engine generates for it.**
