...
| **Serpent** | 7 | Playable vessel with dedicated HUD. Holds the fleet's only **scope** — LT opens a magnified eyepiece beside the flight view, shaped like the CHARGE petal (the element that owns its shot), zoomed by the trigger's own depth and NOTHING else, while the pilot's own camera never moves — and a **sniper shot** (RT while scoped) that is the fleet's **second force able to break a SUPER-SHIELD**, after the Rhino's energised blade — same sanctioned `DeactivateShields` → `Damage(devastate: true)` sequence. See `_Scripts/Controller/Vessel/R_VesselActions/SERPENT_SNIPER_SCOPE.md` |
| **Termite** | 8 | Planned |
| **Falcon** | 9 | Planned |
| **Shrike** | 10 | Planned |
