...
| `PRISM_ANIMATION.md` | `Docs/` | **The clock-material law (LOCKED)**: no prism may need multiframe CPU updates to animate — animation = pool-pull + one initial-conditions stamp + GPU runs the course off the shader clock + one scheduled end-state swap; colliders and gameplay state go FINAL at the start. Full audit of every prism update path + migration tracker. **Read before touching any prism visual, animation, or state transition.** |
