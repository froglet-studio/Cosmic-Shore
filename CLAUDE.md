...
| `ShuffleSystem/` | `Docs/` | Shuffle mode (**planned**, `GameModes.Shuffle = 37`): `ARCHITECTURE.md` — sibling of Tournament. A **randomized, variable-length "race to 6"** across the domain minigames: per-**domain** omni-crystal placement scoring (1st=2 / 2nd=1 / 3rd=0) credited to the real crystal wallet, host-authoritative random (mode, intensity∈[1..X]) draw, a loading-splash summary between games. Reuses Tournament's `Single`-load + network-free-standings pattern wholesale (the doc's §10 is the authoritative delta). |
