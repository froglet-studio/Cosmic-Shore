...
| `AUDIO.md` | `Docs/` | Gameplay SFX + the **burst policy**: why N identical one-shots in one frame is two different problems (FMOD voices, and coherent summation that attenuation cannot fix), the three levers (`minRetriggerSeconds` decoherence / `maxVoicesPerWindow` budget / `maxPendingVoices` magnitude-preserving coalescing), the four layers that make the gate un-authorable to skip, the 2D-vs-3D call-site rule, and how to add a category. **Read before adding a `PlayGameplaySFX` call site or a `GameplaySFXCategory`.** |
