...
| `SQUIRREL_CRYSTAL_MORPH.md` | `_Scripts/Controller/Vessel/R_VesselActions/` | The Squirrel's omni-crystal morph — the crystal does not shatter, it BECOMES the eight shielded prisms of the boost ring the same hit lays. **Read before adding a per-vessel omni-crystal retirement, before touching `SpawnableRings`/`BoostRingBuilder`, or before splicing anything into `ShepardGraph`.** Records the 1:1 census that makes it possible (the cage's 40 triangular + 24 pentagonal faces = 64 = 8 octahedra x 8 faces), the two geometry traps it is written around (find a face STRUCTURALLY, never by coplanarity; a panel must BECOME its face, not sit inside it), and why the ring's prisms are laid at once while only their PHOTONS wait. |
...
**A vessel may RETIRE a collected omni crystal itself, instead of the shared husk spray.** `VesselImpactorDataContainerSO.OmniCrystalRetirement` (a `VesselOmniCrystalRetirementSO`) is part of the vessel package: `OmniCrystalImpactor` asks the collecting vessel whether it carries one and skips `Crystal.Explode` when it does, and the animation itself runs from `VesselImpactor.ExecuteOmniCrystalImpact`, which the owner already routes through `NetworkVesselImpactor` and back out to EVERY peer — so a bespoke retirement reaches the same machines the husk did, with **no new networking**. It is a single slot, not an array, because a crystal is retired ONCE: two retirements would both claim the crystal's body and draw over each other; extra flourishes belong in `vesselCrystalEffects` alongside it. A vessel that authors nothing keeps the husk, so the fleet migrates one hull at a time. The **Squirrel** is the first: its crystal MORPHS into the eight shielded prisms of the boost ring the same hit lays. Three things carry over to the next one. (1) **A hand-off is seamless only if the animation draws the ORIGINAL's own renderers** — mesh, shared materials and MaterialPropertyBlock copied off the live crystal, so frame 0 IS the crystal (tint, shell bands and all); a rebuilt look-alike pops on the one frame that must be free. (2) **It must end on the REAL mass, read from what the builder actually laid** (`BoostRingBuilder.RingLaid` hands over the prisms), never re-derived from the same authored numbers — two derivations drift, a thing cannot drift from itself. (3) **The mass is laid at once and only its PHOTONS wait** (`Prism.SetVisualStandIn`, which touches nothing but rendering): the ring is skimmable from frame 0 while the morph is still in flight, which is the clock-material law's own gameplay-final-at-start division applied to a hand-off. Full record + the two geometry traps: `_Scripts/Controller/Vessel/R_VesselActions/SQUIRREL_CRYSTAL_MORPH.md`.
