...
| Home hub (Mission / Toy Box / Arena / Arcade) | `MenuHubButton` (names a modal TYPE + its availability state), `ToyboxModal` + `ToyboxCard` + `ToyOptionCard` (the app-shell Toy Box, driving the LIVE toys), `ToyPortraitLibrary` (the encyclopedia's baked emblem portraits), `IToyShellSurface` / `ToyShellOption` / `ToyShellRegistry` (a toy's flat face) — `Docs/HomeHub/ARCHITECTURE.md` | `_Scripts/UI/Elements/`, `_Scripts/UI/Modals/`, `_Scripts/Controller/Toys/` |
