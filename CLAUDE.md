...
| `REWARD_SYSTEM.md` | `Docs/` | **Everything the game hands the player, and the one door it comes through.** Producers describe WHAT was earned as a `RewardGrant`; `RewardService` owns the wallet write, the failure boundary, dedupe and the announcement — `PlayerDataService` stays the sole writer of `ProfileEconomy`. Records: the payout table's move out of a serialized field on a UI component (duplicated across **nine scenes**, with five more surfaces on a retired key paying out of the C# initializer instead) into `Resources/RewardTable`; why once-ever dedupe reuses the existing `UnlockedRewardIds` list and so needs **no new cloud schema** (and is therefore the door skins and toys come through); why the end-game panel must catch up on a grant raised before it existed (`Scoreboard` awards while building its cards and activates its panel afterwards, so a child listener is structurally unable to hear the raise — hence a grant SEQUENCE rather than a consumed flag, so two displays can catch up independently); and why the displays are placed by a **tool** rather than authored into prefab YAML (the scoreboard's wiring is per-scene and `GameCanvas.prefab` is stale against it — `playerCardContainer` unset, ten keys the script no longer declares). §6 is the standing list of **designed paths that grant nothing**, three of which consume a claim while paying out zero. §7 is the one-balance rule: `CatalogManager.GetCrystalBalance` reads the disabled PlayFab shelf and returns 0 forever. **Read before adding any earn path, any reward display, or any second crystal balance.** |
