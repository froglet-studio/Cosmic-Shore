...
| `WORLD_AND_COPY.md` | `Docs/` | **The world, the vocabulary, and every piece of player-facing copy.** The HyperSea is a SEA and the product is **four places in it** — **the Shore** (the freestyle sandbox and its toys; deliberately not a game, never scored), **the Arcade** (short party games for up to four, selling laughs not fairness, comeback assistance ON and said out loud), **the Arena** (balanced multi-vessel competition, comeback forced OFF, bots labelled, nothing hidden) and **Voyages** (shared-galaxy PvEvP, not in Early Access, and copy must say so every time it appears). Vessels are the constant you take to all four, never a fifth destination. **The frame makes the networking honest**: one instance at a time with events broadcast between them is not a compromise to explain away, it is how a sea works — nobody shares a room with an ocean, they share the water and ships carry news. Tagline **"Leave a wake."** (not invented — `SO_Class_Squirrel` already ends "leaving a wake of creation and destruction"); *a casual game for hardcore gamers* stays the INTERNAL thesis and **"platform" is the investor word**, both barred from the capsule. Carries the premise, the four destination write-ups, the Steam short description / About / EA Q&A / tag order, the trailer's nine beats (beat 3, fauna grazing a player's trail, is the one second no competitor can film), the in-game naming table (**"prism" is an engineering word players never see** — they see trails and wakes), the voice guide, and **§8 the fiction rules**, the standing guard against copy promising what the build does not do. Also drafts the **twelve missing captain voices** (Grizzly / Termite / Urchin × four elements) in the established voice, ready to paste. **Read before writing ANY store, trailer, menu, mode or captain text** — this file wins over any other copy in the repo, including `Docs/MarketAnalysis/MARKET_GAP_PLAN_2026.md` §4, which now points here. |
| `MarketAnalysis/` | `Docs/` | **The September 2026 market-gap plan and its evidence.** `MARKET_GAP_PLAN_2026.md` answers *what does our market want, what are we missing, how do we close the gap* against the repo at `e800630d`: the buyer is the **hardcore restarter** (Trackmania / Distance shape), the two gaps that outrank every other are a repeatable solo loop (fast restart, fixed target, a record that survives) and a Steam page that exists, and the plan is **Time Attack on the four authored Skim Race circuits in a fauna-free `Barren`-derived cell** as the forever loop, a Coming Soon page by Oct 15 2026, a demo by Nov 15, Steam-first identity, a freed fleet, rebinding, competitive integrity (authored comeback rates + a lobby toggle re-applied after every `SyncFromArcadeGame`, submit asserting comeback 0 **and** fauna-buff 0), and EA moved to **Mar 16 2027** so the game gets its one Next Fest (Feb 22 to Mar 1 2027, verified against Valve's pages) at the launch that earns most. It reverses four recorded decisions and says so in each row (Next Fest for 1.0, no Steamworks SDK, the crystal vessel economy, one weekly attempt per day). The tagline verdict: keep *a casual game for hardcore gamers* as the internal compass and investor line, never on the capsule. §11 lists the twelve research/audit claims that were **wrong on the code** and corrected before they reached the plan (comeback magnitude is divided by ten; a Skim Race "seed of the day" varies nothing because the shipped `SegmentSpawner` has one segment and one spawnable; comeback-off certifies one of two power layers because `DomainFaunaBuffSystem` is the other; the arcade grid is NOT locked on a fresh profile). `RESEARCH_DIGEST.md`, `AUDIT_DIGEST.md` and `SOURCES.md` (602 URLs) are the generated appendices. **Read before touching the EA date, the Steam page, the price, Time Attack, the weekly challenge's attempt count, or any leaderboard's submit path.** Nothing in it has been played: no Unity session ran while it was written. |
