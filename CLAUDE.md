...
| `SHEPARD_TONE.md` | `Docs/` | **The mass crystal's endless inward collapse** — four nested shells on `ShepardGraph`, each contracting through its own window of a shared 3 s period and handing its exact (radius, alpha) state to its inner neighbour, so every partial is finite and the ensemble is endless. Records why **alpha blending was undermining the illusion** (no per-shell edge to track, no depth ordering, and a 34% silhouette sawtooth that exposes the loop because the 5%-alpha outer shell is invisible) and the **screen-door dither** that replaced it (shipped 2026-08-12): a distance-to-owner cellular fill with an **octahedral** unit shape, anchored to the crystal's own object-space DIRECTION — glued to the mesh, scale-invariant (so shards accrete rather than reshuffle as a shell solidifies), and per-shell seeded from each shell's own `[Start, Stop]` so four concentric shells can't punch their holes along the same rays. It is the **documented successor to the rejected prism `SHATTER3D`**: distance-to-owner level sets are closed surfaces that cannot lie flat against a facet. Materials are opaque + `_ALPHATEST_ON` (the depth ordering IS half the point). **Read before touching `ShepardGraph`, any `*MassCrystalMaterial`, or the `CrystalMass`/`ActiveCrystalMass` prefabs** — every failure mode here is silent (a reverted graph still looks like "a crystal"; a transparent material gets the stipple with none of the depth; a disabled `_ALPHATEST_ON` compiles the Alpha output away entirely and the shells stop thinning). Gates: `ShepardToneDitherTests` + FrogletTools > Ecology > Prism Animation > **Validate Shepard Tone Dither**. |
...
- **Transparency on this project is a SCREEN DOOR, not blending.** Prisms (`PrismOcclusionFade`, the corridor + debris erosion + cloak — `Docs/PRISM_ANIMATION.md` §4.7) and the mass crystal (`ShepardToneDither`, `Docs/SHEPARD_TONE.md`) both fade by dropping coverage into `AlphaClipThreshold` while staying OPAQUE with `_ALPHATEST_ON`. It is one visual language, and it buys what blending cannot: every surviving fragment keeps full colour and a hard edge, and `ZWrite` gives real depth ordering between fading surfaces. Before adding a new blended material, check whether the thing you are fading wants coverage instead — and if it does, note that the two families deliberately anchor differently (the corridor is SCREEN-anchored because occlusion is a view effect; the crystal and the debris erosion are OBJECT/UV-anchored so the dissolve is glued to the thing dissolving)
