...
| Swordfish flagship | `SwordfishFauna` (the apex of the freestyle worlds: a `LightFauna` predator that, inside its hunt windows, also CHARGES opposing pilots — Cruise → Pursue → Telegraph → Lunge → Recover, the worm colony's grammar on a body built for it; the bill's three danger needles are its ONLY damage, through the ordinary danger-prism contact chain) + `SwordfishStrikeDataSO` (the strike, plus one profile per ELEMENT so Charge/Mass/Space/Time read as four animals from one prefab) + `SwordfishChargeDriver` (animator bools + two empty FMOD slots). **The model is a DRILL** — its trunk+bill bone rolls a full turn per second under fixed fins, read off the curves — so its prisms are three-fold flutes and on-axis needles, every one placed from its PART's measured geometry, under a bone mount scaled 1/armature, non-overlapping by assertion. The artist's FBX is split into eight re-centred bone-parts (`SwordFish_A_Parts.fbx`) so it withers extremity-first and a shot-off fin dissolves alone. Everything is authored by `Tools/Build/author_swordfish_fauna.py` (`--check` in CI); the prefab's references into the nested FBX are pinned ids, verified/repaired by **FrogletTools > Ecology > Swordfish Flagship**. Record: `Docs/ECOSYSTEM.md §42` | `_Scripts/Controller/Environment/FloraAndFauna/`, `_Prefabs/FloraAndFauna/`, `_Models/Fauna/`, `Tools/Build/` |
