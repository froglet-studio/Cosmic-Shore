...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  **A vessel may also carry NON-ELEMENTAL cards, and they go to the LEFT of those four.** An ability
  ...
  keyed on `CoreAbility` and drawn by the lockup as an ability plate with **no element flower above
  it**, in `VesselHUDView.CoreAbilityDisplayOrder`, one card pitch apart. The separation is the whole
  ...
  an element reaching level 5 and a core ability has no element. The **Squirrel's drift** is the
  first and only one today (`CoreAbility.Drift`); every other vessel binds none and its row is
  byte-for-byte unchanged. **Its control chip comes from the BINDING** (`CoreAbilityBinding.input`),
