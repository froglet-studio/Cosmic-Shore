...
| `VESSEL_JET_FX.md` | `Docs/` | **The two-layer vessel jet FX law**: every vessel draws a long **beacon ribbon** (so OTHER players can find it) and short **engine plumes** (pilot feedback), and BOTH wear the vessel's domain through the one `VesselTrailCustomization` pass. Documents the measured Squirrel reference (the only vessel that had either, and the only prefab in the project carrying the tint component), the per-model engine-mount tables, the name+structural mount resolver and why mounts cannot be hand-authored onto bones (the vessel FBX metas ship an EMPTY `internalIDToNameTable`), the authored-FX detection that keeps the Squirrel's hand-tuned jets from being doubled, and the substring-token trap that once deleted every RIGHT-side engine in the fleet (`"right"` contains `"rig"`). **Read before touching any vessel engine FX, `VesselTrailCustomization`, or the trail-tint half of `ShipHelper.SetShipProperties`.** |
