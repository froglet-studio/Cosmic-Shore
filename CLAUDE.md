...
| `HapticsSystem/` | `Docs/` | Audio-matched haptics: `ARCHITECTURE.md` — transient envelopes measured from each SFX's source waveform + an (off-by-default) continuous bed following live FMOD bus metering, arbitrated onto the single Lofelt haptic channel. **Feel doctrine (playtest-locked): haptics are sparse signals — two hero feels (Squirrel skim pulse-train = reward, prism punish thud = mistake); nearly every other category defaults to gain 0.** Config in `AudioHapticsConfigSO`; editor baker under Tools > Cosmic Shore > Audio Haptics. |
...
| Haptics | `AudioHapticsOrchestrator` (single Lofelt channel owner: waveform-measured transients + metered continuous bed), `FmodSfxBusMeter` (bus RMS + FFT centroid), `AudioHapticsConfigSO` + `AudioHapticsBakedDefaults` (per-category envelopes), `HapticPatternBuilder` / `HapticWaveformAnalyzer` / `HapticEnvelopeFollower` / `HapticTransientArbiter` (pure, tested), `HapticController` (gameplay facade). See `Docs/HapticsSystem/ARCHITECTURE.md` | `_Scripts/System/Audio/`, `_Scripts/Utility/Audio/`, `_Scripts/Controller/IO/` |
