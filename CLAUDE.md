...
| Prism WAKE (§4.7.3) | A vessel travelling fast enough drags a **travelling ripple** through the mass around its recent path — most visibly the RAILS of the ribbon it is laying, which run either side of it (the map is a STRAIN, so the axis is a fixed point and mass lying exactly ALONG the path barely moves; the reach must cover a vessel's rail separation or the ribbon sits outside the wake, which is what the first playtest found — along with an engage speed authored ABOVE the Squirrel's top speed, so the window never opened at all; **currently OVERTUNED** while the look is confirmed, and `PrismWake.ReportState` on `CSLogChannel.PrismRuntime` now says which of the five ways it can look idle it is in). `PrismWake` (publisher + residency) + `PrismWake.hlsl` (`PrismWakeDeform`, VERTEX) + `PrismWakeSource` + `PrismWakeConfigSO` (`Resources/PrismWakeConfig`). The second member of the high-poly morph family the cradle founded, and the first built with the `/prism-morph` skill. **It is NOT local-pilot-gated and that is deliberate**: a wake is what OTHER pilots see you leaving behind you (the vessel tail's own argument), and `n_Speed`/`n_Course` replicate, so a remote replica's wake runs on its owner's numbers — `VesselController.Initialize` ensures the source on every vessel, outside the `IsLocalPilot` block the corridor/tunnel/vision band share. The frame is a **CYLINDER about the path**, not a sphere about the hull, because the trail is laid ON that line: measured from the ship's centre the ribbon would sit at one distance and ripple as a whole; measured from the axis it ripples ALONG ITS LENGTH, which is what reads as a wake. The map is a dimensionless **STRAIN** (`r → r(1+E)`), not an offset along `r̂` — so the axis is a FIXED POINT, there is no singularity where `r̂` is undefined, and **NO FOLD is one number with no geometry in it** (`A < 1/(1+max|t·K'(t)|) = 0.529`, clamped to 0.45; a retune of the reach, the wavelength or the hull radius cannot invalidate it). Its normal carries a **SHEAR** term the cradle's spherical map has no analogue for — the part that exists only because the wave TRAVELS — which is also the harness's negative control. **It splices BEFORE the cradle**, because the cradle closes mass onto a hull RESTING on it and a wake applied after would re-open the hole; nothing on screen would report the two being swapped, so both edges are asserted. That splice broke three sibling wirers that walked past ONE hard-coded node name and produced `Tools/Shaders/prism_vertex_chain.py`, the STRUCTURAL definition of a vertex morph — *a name list needs one edit per wirer per morph forever, which is a defect amplifier rather than a fix.* Proven by `Tools/Shaders/verify_prism_wake.py`; **not run in the editor**. `Docs/PRISM_ANIMATION.md §4.7.3` | `_Scripts/Utility/`, `_Scripts/ScriptableObjects/`, `_Scripts/Controller/Vessel/`, `_Graphics/Materials/Graphs/` |
