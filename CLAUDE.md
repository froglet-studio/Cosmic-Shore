...
| `BENCHMARK_TOOL.md` | `_Scripts/Utility/PerformanceBenchmark/` | Performance Benchmark tool guide (tabs, score/hints, sweep, Load Time Insights, customization) plus the **DiagnosticsHUD console commands** (`fps uncap`, `diag <label> <s>`, `renderers`, `renderers hide/show`, `prismpath`, `prisms`, `freeze on/off`, `ab "<A>" "<B>" [s] [rounds]`) and troubleshooting. **An A/B is only valid between two arms taken in the SAME state** — `freeze on` holds ecology production in every cell (`Cell.DiagnosticProductionHold`: production gating only, the same class as Frenzy, removes nothing, released on scene change) and `ab` interleaves counterbalanced rounds and prints one paired delta with its standard error |
