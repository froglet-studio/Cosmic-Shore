...
| `ModePreview/` | `Docs/` | The arcade card's preview **window** — an idle scale model of the mode's real arena while you browse, and the mode itself **playing in that same frame** once you click in. Records why a preview is a FOCUS interaction rather than a full-screen launch (input ownership is exclusive), why the arena is a **satellite cell** (`Cell.InitializeSatellite`) rather than a `RequestCellSwap` (the menu world must survive untouched — at the cost of a second cell's colliders), why it flies the player's OWN vessel (the occlusion corridor and speed tunnel are single-writer globals bound to the one local pilot), and the two ordering rules a satellite depends on: bind its own runtime data while still INACTIVE, and never raise the full-screen load veil. **Read before adding a preview for a mode, before touching `ArcadeGameConfigureModal`'s preview window, or before standing up a second Cell anywhere.** |
