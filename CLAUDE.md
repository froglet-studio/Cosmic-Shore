...
| `EDITOR_TOOL_LEDGER.md` | `Docs/` | Every `[MenuItem]` editor tool: **standing** (re-runnable) vs **one-shot** (must be run, committed, then deleted), which one-shots still owe a run + push, and the copy-pasteable discharge blocks. A session cannot run Unity, so a tool it writes is inert until the prompter clicks it and commits the asset diff — this is where that obligation is recorded instead of scrolling away in a PR body. Enforced by `/ship` §2.5 (`.claude/skills/ship/tool-discharge-check.sh`). **Read before writing any new editor tool** — prefer `/asset-surgery` (author the asset programmatically) over incurring a new pending human step. |
...
| `PRISM_ANIMATION.md` | `Docs/` | **The clock-material law (LOCKED, STRICT — no legacy fallback)**: no prism may need multiframe CPU updates to animate — animation = pool-pull + one initial-conditions stamp + GPU runs the course off the shader clock + one scheduled end-state swap; colliders and gameplay state go FINAL at the start. There is NO CPU animation tier to fall back to: an unwired graph fails LOUD (`PrismClockDiagnostics`) and the visual snaps until the §4.4 wiring lands (in-editor checklist: `Docs/PRISM_CLOCK_WIRING_CHECKLIST.md`; validator: Tools > Cosmic Shore > Prism Animation). Full audit of every prism update path + migration tracker. **Read before touching any prism visual, animation, or state transition.** |
