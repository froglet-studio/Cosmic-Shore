...
| `HEADLONG.md` | `_Scripts/Controller/Arcade/` | Headlong technical reference (Rhino-only circuit race). **Read before touching `GateRaceController`, `HeadlongCircuit`, or the Rhino's ramp boost** — the circuit's every corner is cut against the Rhino's FLAT-OUT turn radius (the tightest circle it can fly without dropping the boost, ~410 u), so retuning `maxBoostMultiplier` or `RotationThrottleScaler` moves every corner on the course with it. Also the record of the gate-race platform extraction, and of the lap bug it found in the AI target provider. |
