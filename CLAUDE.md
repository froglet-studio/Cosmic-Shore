...
|---|---|---|
| `MinigameDuelForTheCell` | `DuelForTheCell (8)` | `SinglePlayerDuelForTheCellController` |
...
one layer down*. **The Serpent is deliberately not on the card**: it has no anti-vessel verb
...
│   ├── SinglePlayerDuelForTheCellController — vessel swap on turn end
...
| `PRISM_ANIMATION.md` | `Docs/` | **The clock-material law (LOCKED, STRICT — no legacy fallback)**: no prism may need multiframe CPU updates to animate — animation = pool-pull + one initial-conditions stamp + GPU runs the course off the shader clock + one scheduled end-state swap; colliders and gameplay state go FINAL at the start. There is NO CPU animation tier to fall back to: an unwired graph fails LOUD (`PrismClockDiagnostics`) and the visual snaps until the §4.4 wiring lands (in-editor checklist: `Docs/PRISM_CLOCK_WIRING_CHECKLIST.md`; validator: FrogletTools > Ecology > Prism Animation). Full audit of every prism update path+ migration tracker. §4.7 documents the ONE sanctioned shape for view-dependent prism visuals — a GLOBAL uniform published once per frame, never a per-prism write — and states the camera↔vessel occlusion corridor as a **PLATFORM LAW** with the four layers that make it un-authorable to skip. Its four citizens are the corridor (coverage), the Dolphin's Echo Sight (colour, §4.7.1), the Urchin's cradle (VERTICES — a smooth radial drape onto the hull's live position, §4.7.2, which also records why a MESH SWAP is a state change rather than an animation and therefore legal under the law) and the SHOCKWAVE FRONT (VERTICES again — a thin spherical shell of rippled prisms travelling out from the Sparrow's HEAVY skyburst to the exact radius its warhead will go off in, §4.7.3, which is deliberately half of a PAIR with that round's LIT fuze sphere — the lit half says WHERE, statically, in colour; the front says HOW FAR, kinetically, in vertices — and which records why the effect was pulled off every vessel, and then off the Scarab's ball, over three playtests), which together show the shape holds for any channel of the surface description. **Read before touching any prism visual, animation, or state transition** — and use the **`/prism-morph` skill** for any effect that DEFORMS prism geometry to live gameplay data (the high-poly-swap + GPU-vertex-map family the cradle founded): it carries the four-part recipe, the residency/ownership contracts, the analytic-normal rules and the proof harness so the next member does not re-pay the cradle's two rejected rounds. |
...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  | Serpent | 3/4 named, **2/4 upgrades** (the 2026-09-16 scope + rifle re-cut: Charge=Sniper Shot on RT / **Pierce** (armour, not a count), Space=Scope on LT / **Deep Focus**, Time=Boost Duration, Mass still an open design slot). Its RIGHT TRIGGER is **contextual** — scoped it fires the rifle, unscoped it still cloaks, and each action asks `SniperScopeActionExecutor.IsScoped` rather than learning the other's wiring. See `_Scripts/Controller/Vessel/R_VesselActions/SERPENT_SNIPER_SCOPE.md` | 0/4 | — | — | n/a — no icons bound, so the lockup renders four LOCKED cards; since 2026-09-16 the Charge card's cooldown veil DOES draw on one (the veil sizes itself on the ability plate, which a locked card has), and the scope additionally draws its own generated reticle + recharge ring inside its eyepiece |
...
| Prism SHOCKWAVE FRONT (§4.7.3) | A **detonating warhead** throws its own wavefront outward through the mass around it: a **thin spherical shell** of rippled prisms, born at the shell's own half-thickness and travelling to the exact radius that blast reaches, **once**, as the blast expands. `PrismWake` (publisher + residency) + `PrismWake.hlsl` (`PrismWakeDeform`, VERTEX) + `PrismWakeSource` + **`IPrismWakeCarrier`** + `PrismWakeConfigSO` (`Resources/PrismWakeConfig`). The second member of the high-poly morph family the cradle founded, and the first built with the `/prism-morph` skill. **It is granted to the heavy skyburst's 10-POINT WARHEAD BLAST and to nothing else** (`AOEExplosion.TryGetShockwave`; the source is added at the one site that knows which blast is the warhead, `ProjectileDetonatorSO`) — **and it is honest precisely BECAUSE that blast touches no prism mass**: `affectsPrisms: 0`, its whole payload aimed at pilots and creatures, so the prisms ripple as the shockwave crosses them and are still standing afterwards, which is exactly what happened to them. A blast that DESTROYED what it rippled would be saying the same thing twice. **It is half of a PAIR, and the pair is the point — separated in TIME as well as in channel**: on the way IN the round publishes its armed fuze volume as a LIT SPHERE (`Docs/LIT.md`, `Projectile.PublishFuzeLit`) saying WHERE it will go off, statically, in the shooter's domain colour; WHEN it goes off the front says HOW FAR, kinetically, in vertices — a promise and its payoff on the same mass. **FOUR playtests, and THREE carriers were each individually good and wrong for one reason.** It shipped on EVERY vessel — cheap, correct, replicated, un-omittable by wiring, inside budget — and was pulled on the verdict *"in this state this is an awesome effect, but it will be overused as a wake on every vessel"*: **an effect strong enough to be an EVENT stops being one the moment it is continuous**, and no measurement would have said so. The **Scarab's ball went for the same reason one step down** (in play for a WHOLE MATCH). Then the **missile IN FLIGHT went on a playtest that APPROVED it** — *"I could see the pulses as a **wake** for the travelling heavy prism"* — and the word is the verdict: a front trailing a travelling object is a wake, a wake is a texture, and the round is in the air for seconds. *The question is never "does this look good on the thing", it is "is the thing an EVENT".* There is a quantitative half: `maxResidentPrisms` is **SHARED and split evenly**, so a per-carrier grant does not add fronts, it DIVIDES the one that mattered — past a headcount every front is the authored 24-triangle prism again and the feature silently switches itself off everywhere at once. **The discriminator is the WARHEAD and it needed NO authored field**: the Sparrow's base and heavy rockets are ONE prefab and ONE pool told apart by a per-shot `ProjectilePayload`, so a prefab bool (the previous cut's `leavesWake`) could not tell them apart at all — while `Projectile.WarheadBlastRadiusMultiplier` is already 0 on the base rocket AND 0 on every non-skyburst prefab, which is the same no-op argument `PublishFuzeLit` makes for itself. *When two variants of one thing share a prefab and a pool, a prefab field is not a discriminator — find the gameplay quantity that already differs.* **A carrier is a CAPABILITY, never a type**, and it answers a **REACH** and a **PROGRESS**, both asked EVERY frame. There is deliberately **no speed window and no fallback**: a blast's criterion was never a speed, and a measured fallback would have to invent the one number that must not be invented — a radius the blast does not have. **The front consequently has NO CLOCK of its own, which is what the carrier move bought**: a blast ALREADY has a travelling wavefront (`colliderRadius × MaxScale × ease` — the number its own damage pass uses), so the position is READ rather than integrated and `pulsesPerSecond` / `PrismWake.PulseRate` are **retired outright** — *a rate authored beside a clock the carrier already owns is a second answer to one question, and the two drift, so the ripple would arrive before or after the blast causing it.* The carrier reports a PROGRESS and never a radius, so it cannot break the no-fold proof (an expanding blast honestly starts at radius 0, where a front would straddle its own centre). **The FIRST FRAME is silent by construction and the invisible-swap contract now rests on it**: `ExplodeAsync` awaits its delay, so the blast exists for one frame at progress 0 where `FrontEnvelope(0)` is exactly zero — the frame the residency pass hands every prism in the volume the dense mesh — so what held statistically on the travelling carrier now holds structurally. The engage ease is **authored 0** (the envelope's C1 zero at birth IS the engage; a second one only attenuates a 0.15 s event), and the round's documented **moving-centre imprecision is retired** because a detonation does not move. ⚠ The honest cost is TEMPO: the sweep is **0.15 s**, ~nine frames at 60 FPS, once per heavy rocket — and that length is **PINNED at ≤0.1658 s by the weapon's own capture guarantee**, not merely a number wanting a playtest (see the sixth playtest below), so the readability budget is the envelope's SHAPE rather than its duration. The frame follows the FORCE, so it is a **SPHERE about the blast** (`_PrismWakeAxis` is deleted; a sphere has no axis, and the Jacobian therefore has no shear term), and the two non-spherical blast shapes (`AOEConicExplosion`, `AOECylindricalExplosion`) **refuse in CODE** rather than relying on nobody granting them a source. The map is `f(r) = r + σ·A·w·(1−s²)²·sin(2πQs)`, `s = (r−c)/σ` — **the amplitude is ABSOLUTE, not a strain**, bounded by `σ·A` everywhere, because the cylinder's dimensionless strain was right for a 30 u reach and would have moved a 95 u reach's outer mass 40 u. `sin(2πQs)` is the **short bandwidth that is the whole design** (Q = 1: one wavelet arriving and passing, where the cylinder carried a train filling its support), and `(1−s²)²` has a DOUBLE root at both faces so P and P′ vanish there whatever Q is — a whole Q buys the second derivative too. **NO FOLD is ONE condition DERIVED from the bandwidth** (`max|P′| = 2πQ` exactly, so `A < 1/(2πQ)`; `b > 0` then follows with no second bound because the publisher births the front at `c = σ` so the shell never straddles the centre — that birth radius is half the proof, not a taste decision). The residency **QUERY** is the reach and the **RANKING** is the travelling shell (see the fifth playtest below) — swept at `reach + σ + margin`, because a prism the front has not reached yet must already carry the dense mesh when it arrives, and the σ is the finding: **a margin written as an ABSOLUTE distance, covering an overshoot that is a FRACTION of the reach, only agrees by coincidence**, and this one agreed by 0.2 of a unit at the first cut and broke by 19 the first time the shell was thickened, which would have swapped prisms to the dense mesh with their own vertices already displaced. The strength envelope is zero in value AND slope at both ends — which is why **`PrismWake.Publish` must NOT drop a slot at zero strength**: on this carrier that zero is the FIRST frame, so dropping it would release every override and re-acquire them mid-sweep, which is exactly the pop the contract forbids. **It splices BEFORE the cradle**, because the cradle closes mass onto a hull RESTING on it and a front applied after would re-open the hole; nothing on screen would report the two being swapped, so both edges are asserted. That splice broke three sibling wirers that walked past ONE hard-coded node name and produced `Tools/Shaders/prism_vertex_chain.py`, the STRUCTURAL definition of a vertex morph — *a name list needs one edit per wirer per morph forever, which is a defect amplifier rather than a fix.* `PrismWake.ReportState` on `CSLogChannel.PrismRuntime` names which of the ways it can look idle it is in, because a window that never opens reads on screen exactly like an effect that is too weak (the first cut's engage speed of 150 sat above the top speed of the hull the mode actually flew — and the SPHERICAL cut's first playtest, *"i couldn't see it at all"*, was the same sentence again: the heavy skyburst is the TURRET-STANCE shot, `armWarhead: 0` on the wing variant, so a rocket fired while flying publishes **no front at all**, which is the discriminator working and the effect's whole discoverability problem). **Overtuned to the structural ceiling** on that report rather than by taste: amplitude and σ both sit on their own bounds (0.143 = 90% of the no-fold `1/(2πQ)`; 0.45 of the reach, since a front born at σ needs `reach − σ` of travel), giving a **5.45 u** peak ripple in a **42.8 u** shell at a resting-Mass reach of 95.2 u — 1.98× the first cut, and as far as the proof allows. **The thick shell is the bigger half of that win and it is not about amplitude**: at 0.45 of the reach nearly every prism inside the blast volume is in the shell at once, so what reads is a neighbourhood of mass moving together rather than a band passing through it. The residency budget was **RE-SPENT, not raised**, twice — the GPU pays in triangles, so 128 prisms at subdivision 10 (153,600) is *fewer* than 96 at 12 (165,888), and the sixth playtest's 160 at 9 (155,520) is a quarter more prisms MOVING for 1.25% more triangles, because what could not be seen is prisms in motion and residency decides how many move smoothly; the count now carries its own ceiling and the subdivision its own floor, since the triangle bound alone would admit a thousand prisms at subdivision 2, i.e. the authored prism with none of the smoothness the family exists for. Proven by `Tools/Shaders/verify_prism_wake.py` — ten properties over the shipped HLSL, the normal by CONVERGENCE RATE and a negative control that PLATEAUS — plus a Roslyn stub compile of the interface and both overrides with a signature-drift control. One gate defect was found and fixed on the way, and it generalises: **a ruling comment that NAMES the call it forbids trips an `Assert.IsFalse(file.Contains(...))` ban**, so the suite's bans read `CodeOnly(file)` — *a textual gate that forbids a token must not be able to read the comment documenting the ban.* **FIFTH playtest: *"i could not detect the shockwave at all"*, and the defect was the RESIDENCY RANKING rather than the tuning** — the preceding playtest approved the identical map, amplitude, shell and budget on the travelling round, so the only variable was which object carries it. `ReconcileResidency` truncated the swept volume NEAREST-TO-THE-CENTRE, and a front lives in a moving SHELL: the whole budget went to exactly the prisms the front leaves behind in its first two frames, so what was on screen was a two-frame pucker at the detonation point and then a 95-unit sweep through prisms that were all still 24 triangles. Its own comment said so and read as a justification (*"nearest the blast — which is where every front spends its early life"*). Measured: 128 residents only reach the sweep's outer life if fewer than ~200 prisms lie inside the 161.8 u query volume — true in open space, false in every arena this weapon is fired in, which is also why the travelling carrier looked fine (its reach was the round's own small hit radius, so nearest-to-centre ≈ nearest-to-shell). Fixed by ranking on `abs(|p − U| − c)` so the resident set is an ANNULUS that TRACKS the shell outward, with the key precomputed per candidate rather than measured inside the comparator. **The general rule is about the FAMILY: a residency selector is a claim about where a morph's field LIVES** — the cradle's decays with distance so nearest-to-source is right there, a shell's does not, and the two are identical in code, so `PrismCradle.CompareByDistance` now carries the note saying which it is. Its corollary is why this survived a playtest: **when the support radius is much smaller than the volume the budget truncates, the two selectors agree** — a wrong selector can pass on one carrier and fail totally on the next with nothing else changed. ⚠ The 0.15 s TEMPO was deliberately NOT fixed in the same commit as the ranking (the Serpent scope's rule: two fixes in one commit cannot be told apart by the next report). **SIXTH playtest: *"i see it now. it is very fast and very subtle"* — the LENGTH cannot be spent, so the SHAPE was.** The blast's duration is pinned: `SparrowMissileFuzeTests.TheWarheadExpandsFastEnoughToCatchOrdinaryFlight` requires it to CONTAIN a target receding at 120 u/s before it stops expanding, which solves to a ceiling of **0.1658 s** against the shipped geometry — and **0.5 s was the ORIGINAL value, rejected for exactly that** (~40 u/s of catchable escape speed, below every vessel's cruise). Buying margin means a bigger warhead (a buff) or a closer fuze (a nerf), so the blast is UNTOUCHED. Amplitude and σ are already ON their ceilings, so the one unspent quantity was how much of the sweep the front spends at the size it already is: the envelope went from a single hump `4·S(u)·S(1−u)` to a **PLATEAU** `S(u/a)·S((1−u)/a)` at `a = 0.3`, which over the real nine-frame sweep holds ≥0.9 for **three** frames instead of one and ≥0.5 for five instead of three (21.4% → **51.7%** of a front's life above 90%, mean strength ×**1.36**) with the **peak unchanged at exactly 1** — which is what leaves the no-fold proof untouched, since the bound is stated against a strength of 1. Both ends stay exactly C1-zero (birth IS the invisible-swap guarantee), the rise fraction is bounded below by the suite's own slope gate (`a = 0.25` clears it by 4%) and above by degenerating back into a crest, and the plateau is now ASSERTED (≥40% above 90%) with the monotonicity check relaxed to non-strict — *strict was what the old shape happened to satisfy, not what the property is; what must never happen is a DIP.* Residency was re-spent a second time, **160 × 9** for 1.25% more triangles than 128 × 10, so a quarter more prisms MOVE. **General rule: when an effect's LENGTH is owned by a gameplay guarantee, the shape is the only budget left** — ask which of *how big*, *how long* and *how much of the time* is actually unspent before reaching for the one a weapon depends on. **Not run in the editor**. `Docs/PRISM_ANIMATION.md §4.7.3`, `§4.7.3a`, `§4.7.3b` | `_Scripts/Utility/`, `_Scripts/ScriptableObjects/`, `_Scripts/Controller/Projectiles/`, `_Graphics/Materials/Graphs/` |
