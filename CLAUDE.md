...
- **Fleet status** (audit it yourself: **FrogletTools > Vessels > Audit Vessel Ability Rows**, which
  ...
  **A vessel may also carry NON-ELEMENTAL cards, and they go to the LEFT of those four.** An ability
  ...
  an element reaching level 5 and a core ability has no element. The **Squirrel's skimming** is the
  first and only one today (`CoreAbility.Skim`, carrying `boostFill` as its gauge); every other
  vessel binds none and its row is byte-for-byte unchanged. `Docs/ABILITY_LOCKUP.md` §
  "Non-elemental cards".
