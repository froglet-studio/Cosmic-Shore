...
**Its companion is `ImpactorBase.RunEffectIsolated` — for a slot that is FILLED but THROWS.** An exception inside an effect's `Execute` is reported once per (effect, impactor type) with its stack, and the rest of the contact's list still runs. Same doctrine, opposite failure: a hole vs. a thrower. The Urchin forced it — its spike container is `[Embed, Steal, ChainFire]` in a load-bearing order, so one throwing effect silently killed both the steal and the cascade while the embed had already visibly landed, and the weapon read as dead with nothing in the console. **Every impactor now routes through both helpers** — `Projectile`, `Vessel`, `Skimmer`, `Explosion` (all four of its dispatch sites, including the per-prism one, which is the densest in the game: a blast resolves hundreds of prisms per frame for its whole sweep), `Mine`, `Prism`, `OmniCrystal` and `ElementalCrystal`. Route a new dispatch loop through BOTH helpers. The general rule the sweep records: **a bare dispatch is not one bug's worth of risk, it is that bug multiplied by however many contacts the site resolves per frame** — so the cost of leaving one unguarded is set by the site's fan-out, not by how likely the effect is to throw. When a storm happens anyway, `ErrorStormReporter` (`_Scripts/Utility/Diagnostics/`) names it: editor and dev builds aggregate repeated errors by `BugSignature` and print ONE ranked summary per 5 s window — rate, distinct faults, first stack each — because a per-frame fault buries its own evidence under thousands of identical console entries. It suppresses nothing.
