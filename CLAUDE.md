...
| `PRISM_ANIMATION.md` | `Docs/` | **The clock-material law (LOCKED, STRICT — no legacy fallback)**: no prism may need multiframe CPU updates to animate — animation = pool-pull + one initial-conditions stamp + GPU runs the course off the shader clock + one scheduled end-state swap; colliders and gameplay state go FINAL at the start. There is NO CPU animation tier to fall back to: an unwired graph fails LOUD (`PrismClockDiagnostics`) and the visual snaps until the §4.4 wiring lands (in-editor checklist: `Docs/PRISM_CLOCK_WIRING_CHECKLIST.md`; validator: FrogletTools > Ecology > Prism Animation). Full audit of every prism update path+ migration tracker. §4.6 documents the ONE sanctioned shape for view-dependent prism visuals — a GLOBAL uniform published once per frame, never a per-prism write (the camera↔vessel occlusion corridor is its reference implementation). **Read before touching any prism visual, animation, or state transition.** |
...
| Prism occlusion corridor | `PrismOcclusionCorridor` (2 `Shader.SetGlobalVector` per frame — vessel position + (outerRadius, innerRadius, coreAlpha); the camera end is read on the GPU from `_WorldSpaceCameraPos`), `PrismOcclusionConfigSO` (`Resources/PrismOcclusionConfig`), `PrismOcclusionCorridor.hlsl` (per-fragment capsule test + ordered-Bayer screen door into `SurfaceDescription.AlphaClipThreshold`, wired into BlockGraph). Prisms between the player's camera and their ship dissolve; everything else is untouched and stays in the opaque queue. **Zero per-prism CPU** — the retired `ClearPrisms` trigger capsule + per-prism material swaps are deleted. Target is set by `CameraManager` wherever a follow target is installed. Gate: FrogletTools > Ecology > Prism Animation > Validate Occlusion Corridor. See `Docs/PRISM_ANIMATION.md` §4.6 | `_Scripts/Utility/`, `_Scripts/ScriptableObjects/`, `_Graphics/Materials/Graphs/` |
