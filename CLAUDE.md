...
| `PERFORMANCE_OPTIMIZATION.md` | `Docs/` | Frame-cost optimization log + prioritized backlog: shipped de-spike commits (do-not-regress list), the locked slice + per-frame budget + atomic publish fix pattern, instrumentation inventory (markers, DiagnosticsHUD, telemetry), per-task root-cause analyses with verified file/line refs, standing verification protocol. **Read before any perf work.** §0 is the current handoff; the `§0.x` ladder below it is session order, **not** chronological. **§0.8 carries the only real measurement of the Lattice boot world (2026-09-10): CPU-bound, a healthy typical frame, and a periodic ~1.2 ms spike that is a flora BIRTH** — one birth, not four coinciding, which is what rules out the obvious per-frame-budget fix (§4 Tier 0a). Everything below §1 still predates enabling Burst. **Every number in it is SHA-stamped, and that is load-bearing: §0.8 and the two companion docs were authored on `78a95259f`, an ABANDONED branch, and recovered onto bleeding-edge on 2026-09-14 — so the findings' SHAPE carries over and their absolute milliseconds do not.** A measurement is a statement about the tree it was taken on; re-take before quoting one. §3's marker inventory drifts (48/18 → 53/19 between those two trees) and is reproducible only with a **multiline-aware** walk — a single-line `grep -c "ProfilerMarker("` finds **4 of 53**, because declarations are target-typed `new("…")`. |
| `PERFORMANCE_CAPTURE_RECIPES.md` | `Docs/` | The four captures that unblock the backlog, written to be run on Windows: **A** Menu_Main boot world (the Lattice cell at ~63,360 colliders, sampled at 1/4/8 min), **B** BenchmarkStressTest (the regression baseline; also the headless `-csmbench` variant), **C** one live mode, **D** load time via `LoadInsights`. Carries the pre-flight that makes a capture trustworthy — **Burst ON, Deep Profile OFF**, and the tell that Burst is actually on (a job row reading `ExecuteJobFunction.Invoke() [Invoke]` is executing managed at ~20×). **Take Capture A first and read the `Bound` row before anything else**: if the boot world is GPU-bound at 8 minutes, all of §4 Tier 1 is the wrong lever. Its Capture A2 section is kept as METHOD because Capture A alone produced a **wrong** conclusion — *an inference from a partly expanded profiler tree is a guess about the rows you did not open* (`ScriptRunDelayedDynamicFrameRate`, the coroutine root, was sitting there collapsed and held the entire finding). |
| `MEMORY_AUDIT.md` | `Docs/` | The other half of the perf picture — what the game HOLDS. Runtime memory is capture-gated and has **never** been recorded (DiagnosticsHUD already exposes it, F7 → F6). The asset footprint was measured **on `78a95259f`, not on bleeding-edge** — re-measure before quoting a figure — and its finding is **import settings, not size**: four music tracks totalling 182 MB are all `loadType: 0` (DecompressOnLoad, fully resident PCM) at near-lossless Vorbis `quality: 1`, `loadInBackground: 0`, and `3D: 1` **on music**; 200 of 214 clips project-wide are DecompressOnLoad. Also: texture streaming is OFF in all five quality tiers with a 512 MB budget already authored, 171 MB of menu preview `.mp4`s whose references may all be dead, and no Addressables package. **Measure it with a path-safe walk, never `find | xargs du`** — many filenames contain spaces, and word-splitting them hides the entire audio finding. |
