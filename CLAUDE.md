...
| **Rhino** | 3 | Feature-complete playable vessel. A prism its energy sword destroys is **CUT, not burst** — split along the plane the blade swept through it, the halves parting, opening and dissolving from the cut face on the shared high-poly prism, off one clock stamp per half with the explosion as the fallback on any refusal (`Docs/PRISM_ANIMATION.md §4.10`, `R_VesselActions/RHINO_ENERGY_SWORD.md` § "The slice") |
| **Urchin** | 4 | Playable vessel — chain-reaction spikes + prismscape rider + a projected rail (see `_Scripts/Controller/Vessel/R_VesselActions/URCHIN_CHAIN_SPIKES.md`, `URCHIN_TRAIL_RIDER.md`, `URCHIN_TRACK_PROJECTOR.md`) — and the fleet's only hull that flies BACKWARDS: free flight reads the same signed stick the grind always did, so rest parks and pulling back reverses at the authored 65 u/s, its jets swing end-for-end and its wake extends out past the nose (`URCHIN_REVERSE_FLIGHT.md`). While it RIDES, the mass around the hull **DRAPES onto it** — every vertex within the drape reach of the hull's surface slides along its own radius toward that surface, closing over what the ship would have clipped through and rising to meet what it has not reached (the cradle — `PrismCradle` + `PrismCradle.hlsl`, the third §4.7 global-uniform citizen; a per-prism material write would have been a clock-material-law violation, so the vessel position is published ONCE and every prism reads it). **Its first two cuts moved the prism's own 24 triangles and were both rejected on look** — a rigid per-triangle motion reads as facets HINGING, and no tuning fixes that, because *a deformation is only as smooth as the surface it moves and 24 triangles is not a surface*. So the handful of prisms near a riding hull are swapped to `HighPolyPrismMesh` (the identical solid at ~3k triangles, SHARED so they still batch) through `Prism.SetRenderMeshOverride` — **a STATE CHANGE, final at the instant it is applied exactly like a shield engaging, not an animation**, budgeted at 24 prisms and performed where the drape provably cannot have moved anything, so it is invisible. The normal is the map's ANALYTIC inverse-transpose and is proven by CONVERGENCE RATE rather than by a tolerance (halving the test patch quarters the error; a wrong derivative converges to a constant, which a tolerance cannot tell apart); `Docs/PRISM_ANIMATION.md §4.7.2`). Elemental map complete; **HUD prefab not yet authored**. Its modes are `Hijack(46)`, the rail heist, and `Skein(51)`, the cable race |
...
| **Serpent** | 7 | Playable vessel with dedicated HUD. Holds the fleet's only **scope** — LT opens a magnified eyepiece beside the flight view, shaped like the CHARGE petal (the element that owns its shot), zoomed by the trigger's own depth and NOTHING else, while the pilot's own camera never moves, with a reticle at the shot's true angular size in BOTH pictures (growing in the eyepiece as the zoom narrows it, staying small over the flight view, and up only while the scope is) — each ring ringed by four FIXED-size posts, because at the 0.5° half-angle the weapon shipped with a true-size ring is 6–24 px in a 540 px window and 8 px over the flight view, i.e. correct and unreadable at the same time: **the posts LOCATE and the ring MEASURES**, and the fix for a mark too small to see is never to draw it bigger than it is — the angle was then raised to **10°** by authored instruction and the report came back **identical**, which is how the size experiment EARNED its answer: nothing that makes a mark bigger can make it appear, so **size was never the cause** and the cone came back down to **1.5°** (78.5 u at 3,000 u — a rifle, where 0.5° is a needle and 10° a 529 u shotgun) with **the dial to move still the weapon's angle, never the cap**. What shipped in its place is a **PROBE**: the same mark drawn a second time out of plain `UnityEngine.UI.Image` quads, which share no geometry code with the generated rings and only the canvas, the parent and the draw order with them — so ONE playtest separates *the generated geometry never reaches the screen* from *nothing parented under the eyepiece does*, where four rounds of reading source could not. **When a component renders and another does not, reach for the one that provably renders.** **The mark is now on screen and tracks the zoom** — and the report cannot say WHICH of the two changes that shipped together did it (the probe, or the 512 px rect the generated rings were given in the same commit), so the probe stays until one observation with it switched off settles that, and the eyepiece draws its reticle twice in the meantime: **when two fixes ship in one commit and either could be the one that worked, the report cannot tell them apart — budget a second observation and keep the cheaper-to-undo one switchable.** Its **sniper shot** (RT while scoped) **PIERCES BY DEFAULT** — the budget is the authored `pierceCount` at every tier and the asset authors **0** = everything the cone contains, where an un-upgraded round used to destroy exactly ONE prism on a twelve-second cooldown — and Charge 5 **Pierce**, whose whole content had thereby become the base behaviour, is re-cut onto **ARMOUR**: below it a super-shielded prism is not a TARGET at all so the round flies through it, at it the sanctioned `DeactivateShields` → `Damage(devastate: true)` sequence runs and the Serpent is the fleet's **second force able to break a SUPER-SHIELD**, after the Rhino's energised blade. The gate is in `IsValidTarget` rather than in the kill loop, and that placement is the whole of why it works — `Prism.Damage` HARD-IGNORES super-shielded mass, so a round that treated armour as a target would stop on it, end its tracer there and destroy nothing. **"Pierce" now names a CAPABILITY rather than a count: how many prisms the round goes through is the weapon's, what it can go through is the element's.** See `_Scripts/Controller/Vessel/R_VesselActions/SERPENT_SNIPER_SCOPE.md` |
| **Termite** | 8 | Planned |
| **Falcon** | 9 | Planned |
| **Shrike** | 10 | Planned |
| **Butterfly** | 13 | The meditative surface painter — the **two-thumb** complement to the Serpent, and the fleet's slowest hull by design (55 u/s cruise, 45°/s turn, a 207-unit camera — 70% further than it shipped, and its `farClipPlane` is the fleet's 12000, not the 1000 its setup tool left at the C# initializer). It lays a **wide slab of piano keys** rather than a ribbon, and it is the vessel that MAKES the 2D prismscapes the Urchin rides. It carries no gun. **The right trigger is a MODE SWITCH** (2026-09-26 re-cut, which retired the wing-energy spread and the two always-on invisible wing skimmers): **Mass mode** paints a WIDE wake — the key widens to **5x at Mass 0, 20x at Mass 15** (`SpreadWingsActionSO.massModeWidth` → the new general `VesselPrismController.WidthMultiplier`, stated through `AdmitTargetScale` so the pool's per-axis clamp cannot cut it), and at L5 'Gilded Wake' its keys come out SHIELDED (`ForceShielded`, gated on the replicated unlock bit); **Dust mode** narrows the wake and switches on ONE dust capsule hanging BELOW the hull (`ButterflyDustSkimmer.prefab` + `ButterflyDustField`, which gates collider AND impactor together and draws the capsule as falling motes only while live). **Space is the capsule's LENGTH** (`elongateYOnly`). On own-domain mass the dust grows keys along a random axis, makes them dangerous or shields them (L5 'Diamond Dust': rare super-shields on plain mass); on opposing mass it destroys, shrinks or steals — each prism's outcome a deterministic HASH of that prism, never `Random`, because a skimmer contact is observed on every peer (`SkimmerScaleDustPrismEffectSO`). It kills opposing lifeform hearts (no speed gate — the slowest hull in the fleet could otherwise never kill anything not rooted), **refreshes** ally lifeforms (`SkimmerNourishLifeformByCrystalEffectSO` → `Nourish`, a food-web event, latched 5 s per lifeform), and debuffs opposing pilots with a **Charge-scaled bite** (`biteScale` on `VesselElementalDebuffBySkimmerEffectSO`). An OMNI crystal detonates a large non-destructive bloom (`AOEButterflyBloom.prefab`) that kills opposing lifeform hearts. Because a skimmer's scaling is read on every peer and **element levels do not replicate**, it added **`R_VesselActionHandler.NetElementLevels`** (owner-write ushort, a 4-bit nibble per element) and **`ElementalFloat.EvaluateReplicated`** — the first fix available for that whole bug class; only the Butterfly reads it today. **Time** is the **Fold**: hold LT, the vessel stops, a ghost reaches out along the heading you arrived on, release and you are there. (The second, SPHERICAL branch that used to address a point in the cell with the sticks is **retired** — the ability did one thing inside a membrane and a different thing outside one, and Time 5 was a no-op in the branch a player spends nearly all their time in. One reach everywhere; pitch and yaw are dead for the duration, so a fold cannot be aimed once begun and EARNING THE LINE before the press is the whole decision.) **Every fold leaves a PAIR OF GATES standing** (`FoldGate`) — one where you left, one where you arrived — and any vessel of the Butterfly's DOMAIN threads either and is at the other, repeatedly, until that Butterfly folds again and the new pair replaces it. It is the second domain-coloured switch outside the toybox after `ScarabSwitch`, one notch further out: there the colour names the domain the switch BELONGS to, here it names **who may thread it** — a claim about the gate, never about the pilot, since a gate cannot change anyone's domain. Removal is only ever the placer folding again or their vessel being destroyed; there is no lifespan and there must never be one. Both ends are laid from positions every peer already agrees on — the hull's replicated pose at the commit, and its replicated pose once the arrival bloom lands — because the tempting alternative (derive the destination from the HOLD, which every peer does accumulate) is correct on the owner and tens of units out everywhere else: **a quantity every peer can compute is not a quantity every peer computes the same**. Each machine tests only the vessels it OWNS and the owner writes its own pose, so a transit needs no new networking; and because `SetPose` bumps `VesselTransformer.TeleportCount`, a gate **cannot thread a race ring** — the rule Waystation needed for the Fold, inherited with nothing added. It forced one platform fix: `VesselController.SetPose` sent a ClientRpc unconditionally and **only a server may send one**, so every client-owned teleport (this ability, a gate, the Wanderway's return) did nothing but log on a party guest; it is now three branches with the server path byte-identical and an owner→server route added. Its hull is PROCEDURAL (the Scarab's second, four wings on their own hinges, four real element morphs). Its platform contributions are a **skimmer→lifeform-heart dispatch arm** (`SkimmerLifeformCrystalEffectSO` — `SkimmerImpactor` returned outright on `crystal.IsEmbedded`, so no skimmer in the fleet could reach a heart; every other vessel leaves the list empty, so it is a no-op fleet-wide), the **Strike-class skimmer drain** `author_combat_debuff_magnitudes.py` already reported as missing, and two opt-in flags on `SkimmerDamagePrismEffectSO` (`opposingDomainOnly`, `requiresUpgradeElement`) that default to today's behaviour and which the re-cut no longer uses. Its dust assets are authored by `Tools/Build/author_butterfly_dust.py` (`--check`). Its mode is **`Waystation(58)`**, the migration race cut against the Fold's one degree of freedom. See `_Scripts/Controller/Vessel/R_VesselActions/BUTTERFLY.md` and `BUTTERFLY_FOLD.md` |
| **Sparrow** | 11 | Shooter — arcade space combat with guns and missiles. Missiles are stocked by DESTROYING PRISMS WITH GUNFIRE (not by crystals, since 2026-09), with the CHARGE ability card's gauge filling toward the next one and resetting each time one is earned (the icon ladder says how many the bay HOLDS, the gauge how close the next is — a tank of several cannot say both on one bar). **ONE BAY, TWO ROCKETS, told apart by the STANCE** (2026-09): a **BASE** rocket on the wing costs half the tank's old price (**25 prisms**, so the bay holds four), flies 120 u/s and carries *nothing* — no proximity fuze, no shockwave, no prism cairn — so it has to be AIMED; a **HEAVY** rocket fired from the turret stance costs **50 prisms**, flies **twice as fast**, lays a small prism every 30 u of its flight, and is the only one carrying the fuze that trips on opposing vessels and living fauna hearts plus the second blast that debuffs pilots and jousts creatures without touching mass. The discriminator is `IsTranslationRestricted` and **not measured speed**, because a fire press is replayed on every peer and the stance is the server-replicated one of the two; it is resolved ONCE at the press and carried through the bay's launch delay, since the ammo is spent there. The variant is a per-shot **`ProjectilePayload`** handed to `Gun.FireGun` beside the existing `spareOwnDomain`/`flightGrowthFactor` snapshots — never a second prefab, which would make every later skyburst edit land twice — with the fuze and the warhead collapsing to zero TOGETHER (they are one weapon) and the cairn filtered by the new platform predicate **`AOEExplosion.CreatesMass`** rather than by a position in an `aoePrefabs` list. **BOTH rockets pay the 20-point blast, because that tier belongs to the DESTRUCTIVE blast rather than to the cairn it was authored on** — the sphere `AOEExplosion.prefab`, which both variants spawn and which carried NO vessel effects at all (two stale keys for fields its impactor script no longer declares); the heavy's advantage is exactly what it reads as, the wider 10-point outer ring. Astro League spawns the same prefab and is unaffected, because it initializes it anonymously and the reporter declines a blast nobody fired. In **Wildlife Liberation** the blast's creature joust is still behind the stance, so a pilot must stop to hunt with rockets (gunfire on body prisms is unchanged). **THE GUN IS THE RELOAD** (2026-09-22): the tank fills from DIRECT GUNFIRE only — a rocket's own blast funding the next rocket closes the *shoot to reload, reload to rocket* loop on itself — and from mass of ANY domain, because what is rewarded is trigger time on mass rather than territorial damage and the colour gate dried the reload up in exactly the arenas whose mass wears the pilot's own. The weapon identity rides the destroyed channel as **`PrismStats.DestroyedByGunfire`** (stamped by `Prism.Damage(…, byGunfire:)` the way `byCreature` already is, set by `ProjectileDamagePrismEffectSO`, which is the Sparrow's two guns and nothing else), because a listener that sees a death cannot see what caused it. It also found a **fleet-wide double subscription**: `R_VesselActionHandler` subscribed to the input channels with a bare `+=` from three paths (`Initialize`, `ChangePlayer`, every un-pause), so one trigger pull sent the press RPC twice and fired two rockets — **invisible on every HELD ability and obvious on exactly one kind, the consumer that SPENDS**; latched, and a `-=`-before-`+=` would not have fixed an already-doubled handler. Its guns **sweep the segment they crossed for vessel contact** rather than trusting the PhysX trigger, which at 0.04 s per sample is blind to a 375 u/s round (`R_VesselActions/SPARROW_SKYBURST_BAY.md`) |
...
`Tools/Build/arcade_mode_lib.py`, the eleven registrations, the gates), **the `/cardart` skill for its
card's background** (rendered offline from the mode's own intensity-2 arena, never hand-painted), **the `/arenagame` skill on
top of it for ANY card that seats SEVERAL hulls** (the Arena roster, the fleet facts table, the
per-hull `StartingElements` handicap, the balance model) and **the `/vessel` skill for ANY vessel-class work** — new vessels, abilities/executors,
...
**A PILOT'S DOMAIN PICK WIDENS THE MATCH, up to the mode's own limit.** The active set is the contiguous slice `ActiveDomains[0..DC-1]` and `DC` is clamped to the PLAYER COUNT (`ArcadeGameConfigureModal.ComputeMaxDomainCount`), so a 1-human + 1-AI lobby has `DC 2` and an active set of `{Jade, Ruby}` — and a pilot who picked **Gold** was silently reassigned off it **at spawn** by `NormalizeUnassignedHumans`, after the launch UI had accepted the pick and shown it. Jade is always `slice[0]`, so they landed on Jade — **whose authored palette is teal-and-blue** (`TrailHighlightColor` (0.05, 0.75, 0.71) over a (0.00, 0.39, 0.75) core), which is why it was reported as *"I selected gold and got the blue domain"*. **No code path can put a human on `Domains.Blue`** — `RequestSetDomain_ServerRpc` rejects the sentinel, `GetBalancedDomain`'s error path returns `ActiveDomains[0]`, and `NetDomain` initializes to Jade — so a player who *looks* like they are on Blue is on **Jade**, and the question to ask is which pick failed to survive. `SpawnAIs` now raises `RequestedDomainCount` to cover every domain a human actually picked, before anything derives from it. Two things make that the right shape: the same principle was **already written for AI placements** ("placing on Gold in a two-domain lobby is the host widening the match, and re-balancing it away silently is exactly the 'cannot add to gold' playtest defect") and simply never applied to humans; and raising the COUNT rather than widening one local list is what keeps every consumer agreeing, since `ScoringRuleSO` sums over the same prefix — widen only the spawn set and a Gold pilot is spawned Gold and then **never scored**. It is capped at **`GameDataSO.MaxDomainsForGame`**, published by `SyncFromArcadeGame` from `SO_ArcadeGame.MaxDomainsAllowed` (previously read ONLY by the launch modal, so nothing server-side could tell *the host chose two domains* — a preference — from *this mode has exactly two goals* — a rule; Astro League and Brood Rush pin it to 2). General rule: **when one authority accepts an input and a later one silently overrides it, the promise the first made is the one that is broken** — `RequestSetDomain_ServerRpc` deliberately accepts any playable domain because "the domain count is a property of how the MATCH is scored, not a gate on which colour a player may fly", and that is only true if the pick survives spawn.
...
| `THIRD_PARTY_REGISTER.md` | `Docs/` | **Every third-party SDK in `Assets/` AND `Packages/`** — not just the big folders. The authority is **`packages-lock.json`, which resolves 104 packages against `manifest.json`'s 77**, so **27 arrive transitively and appear in no hand-maintained list** (NUnit, Mono.Cecil, OpenImageIO among them). Records vendor, version, whether a licence document is ACTUALLY in the tree, and whether the SDK reaches a player build — derived from Unity's real inclusion rules rather than folder names: an `Editor/` folder never ships, an asmdef's `includePlatforms` decides, code with no asmdef lands in `Assembly-CSharp` and ships, and **any folder named `Resources/` is packed whole whether or not anything references it** (the rule that produces defects nobody notices — it produced two). **The register proves PRESENCE; only a purchase record proves ENTITLEMENT**, so no licence status is asserted without a document behind it. §0 is the human's worklist. Its top row **was** a verified gap and is now closed: FMOD's EULA clause 3 requires an in-game credit naming *FMOD* and *Firelight Technologies Pty Ltd.* on **all tiers**, the string appeared **nowhere**, and one **credits screen** (§7) now discharges that plus every MIT notice, the OFL/Apache font notices and EmojiOne — data in `Assets/Resources/CreditsManifest.asset`, rows generated by `CreditsModal`, kept there by a release-build guard AND `Tools/Build/check_credits_manifest.py`. Three corrections it forced are the reusable part: **an SDK can bundle other SDKs** (`com.unity.bindings.openimageio` is licensed Unity Companion/UPDL while the oiio inside it is Modified BSD, and it carries five further projects — so a licence guessed from an upstream project's name can be wrong about the wrapper AND miss its cargo); **a `Resources/` folder makes "ships if used" meaningless** (Roboto, EmojiOne and Liberation Sans all sit in one, and Liberation Sans — TMP's default font — was missing from §7 entirely); and **an asset the build does not contain must NOT be credited**, which is why the CC-BY placeholder models are deliberately absent and their trigger lives in `VESSEL_CONSTRUCTION_FOLLOWUP.md` §1a instead. Four traps it records: **a README can be the entitlement evidence** (SOAP's ends *"Thanks for purchasing"* and ships no EULA, while being the project's primary architecture); **a file that looks like a licence may not be one** (NiceVibrations' two are a Rust crate list and a haptic-sample CC notice, not the product EULA); **an SDK can bundle other SDKs** (FMOD carries Google Resonance Audio; PlayFab's editor extensions carried five Microsoft Identity assemblies before both were deleted); and provenance for the unattributed folders is **not recoverable here** because the clone is shallow, so `git log --diff-filter=A` reports the graft boundary for all of them. **Read before adding ANY third-party asset or package, before answering "are we allowed to ship this", and before assuming a `com.unity.*` package is Unity's own code.** |
| `THIRD_PARTY_DECISIONS.md` | `Docs/` | **The DECISION layer over the register** — what the studio has decided to do about each third-party item, who owns it, and which prompt executes it. §2 is the only list on which inaction costs money: **FMOD's licence TIER is now the SOLE row that can require a purchase** — the Shift seat came off the list on 12 Sep 2026 when that pack was replaced first-party and removed; everything else labelled *no purchase* resolves to a text file or a prompt. The rest is the measurement behind each *replace* call, and the measurements are what make the calls cheap: **NiceVibrations is ONE file and FIVE API symbols** (every haptic clip is already first-party data built in `HapticController.EnsureClips()`, so the plugin contributes a player and no content — and on PC/Steam `Gamepad.SetMotorSpeeds` replaces that player outright); **Shift and PrimitivePlus had ZERO first-party code references** between them, so both were art plus one trivial component, while **44 of PrimitivePlus's 44 meshes shipped for the 4 that were used** — which is why the call was replace rather than chase a receipt, and both were removed 12 Sep 2026 (register §2.1 carries the measurements, including the Cone's before/after bounds, pivot and axis); and **`Effects Library` is 8.2 MB of which exactly one prefab is live**, its four `FE_*` render-pipeline assets — which look like the project's URP configuration — having zero references. It also carries a finding the register's first pass missed: **the four NiceVibrations `.wav` clips wired into shipped UI sit under a notice whose header claims CC-BY-NC (non-commercial)**, while its source list is almost all CC0 and names freesound titles rather than shipped filenames, so no clip maps to a term — the fix is the FMOD migration CLAUDE.md already mandates, not a licence argument. **Read before buying any Asset Store seat, before deleting a vendored SDK, and before reorganising `Assets/` — §5 records why the folder pass is sequenced LAST.** |
| `LAUNCH_BLOCKER_INDEX.md` | `Docs/` | **What is in `Assets/` that should not ship** — 19 candidates, each with a measured inbound-reference check and a `remove`/`keep`/`salvage-first`/`needs-a-human` verdict. **Nothing is deleted on the strength of this document**; it carries the same salvage-before-delete gate as `VESSEL_CONSTRUCTION_FOLLOWUP.md` (*"referenced by nothing" means nothing is USING it, not that it contains nothing*). The method is the guid-ownership one CLAUDE.md already mandates — unique owner asserted per `.meta`, never `grep -rl` piped to `head -1` — and it reproduced that hazard live: **`RhinoModel.fbx` has 0 asset references and 2 `.meta` `externalObjects` remaps** pointing into it. **Its two stated blind spots are the load-bearing part: a guid check cannot see `Resources.Load` BY NAME, and it cannot see a C# type reference.** `TMP Settings.asset` measures **zero** references and every text component in the game needs it; `PlayFabSDK` measured zero while **20 first-party files** compiled against it (the SDK is gone — the blind spot it demonstrated is not) — so a "0 references" result inside a `Resources/` folder, or on anything with code behind it, means nothing. **Read before deleting ANY folder or asset, and before trusting a roster checker's pass as proof that an asset is unreferenced** (`check_gamelist_scenes.py` validates `SO_GameList` rosters only — 6 of the 20 "inert" arcade cards are still referenced from vessel class SOs). **§E is the headline and the one to read first: what ACTUALLY ships, measured.** The rest of the index answers *is this folder referenced?*; §E answers *does this reach a build*, walking guids transitively from Unity's real roots (enabled build scenes + every non-Editor `Resources/` asset + preloaded assets) — **2,889 of 7,128 assets reachable, 655 MB unreached** (re-measured 12 Sep 2026 after the third-party sweep and the PlayFab retirement; the doc carries the A/B), against ~17 MB of folder-by-folder candidates, so **a folder-scoped reference check answers the wrong question by roughly forty to one**. Rooting all `Resources/` is what CLOSES the load-by-name blind spot rather than dodging it (the `--self-test` is negative-controlled on exactly that: drop the Resources root class and `TMP Settings.asset` goes unreachable). Its single largest find is **E1 — 360.6 MB in 109 files, 26% of `Assets/`, zero reachable, no licence, readme or vendor**, flagged as a provenance question rather than a cleanup. **Quote no number from it without its three stated limits**: code compiles regardless of references (so an unreached `.cs` is a dead-code question, not a size one), native plugins ship by platform importer settings and not by guid, and **a RETIRED serialized key still greps as a live edge** — 40 `SO_ArcadeGame` assets carry a `PreviewClip:` the type no longer declares, which pulled 110 MB of video into "ships" (E2), and the general rule there is that the check must be *which type owns the asset*, never *does this identifier exist* (`SO_VesselAbility.PreviewClip` is live on 24 assets). Measured by `Tools/Build/measure_build_reachability.py`, a READER. |
| `LIT.md` | `Docs/` | **The LIT fundamental** — mass inside a force volume is lit in its owner's domain colour. The THREE producers and what moment each one speaks at (plus the skim field that was built as a fourth and CUT — the two questions to ask of the next producer: who is the sentence FOR, and is it on ALL the time), the one three-shape predicate (`LitVolume.Contains`, which `BlastVolume` now delegates to), the cost (zero per-prism CPU, zero colliders), why continuity of existence is structural rather than per-producer, why the viewer's own aim stays a separate bit-identical channel, and why **nothing reads it to decide an outcome yet** (a light's size is owner-local and tick-gated, so peers agree on a boundary prism only to within a tick). § "The temporary shield it replaced" is the one to read before touching any explosion path: the own-domain 2-second shield was a stand-in for a visual and carried a food-web blackout, a targeting-grid churn and a per-prism SFX with it. **Read before adding a highlight to any ability, before consuming lit state in gameplay, or before changing what an explosion does to mass it spares.** |
| `ELEMENTAL_ECONOMY.md` | `Docs/` | **A vessel debuff MOVES elements; it does not decay them.** Every vessel-on-vessel elemental debuff is now a permanent TRANSFER out of the victim's base level, landing in exactly one of three places: **stolen** by the attacker (contact verbs — the Squirrel's joust, the Rhino's sword), **ejected** as free-for-all crystals launched from the victim's hull at the impact velocity (every ranged verb), or **burned** — destroyed, by a **hostile danger prism and nothing else**, which is the economy's only sink. Lifeform reproduction and spawning are the only source, so a match is pilots trading one pool of material. **The unit is the PETAL** — one integer level, one step of the HUD flower, and one crystal at world scale 1, which is an arithmetic identity three files have to keep (`PetalNormalized` x `PetalCrystalWorldScale` x `levelPerUnitScale`) and which `check_elemental_economy.py` asserts. Two rules make it conserve and both live on the VICTIM so no call site can get them wrong: **nothing partial ever leaves** (a hit accrues; a petal comes off only when the accrual reaches a whole one — which is why a 90-rounds-per-second gun mints one crystal per magazine rather than a speck per bullet, and why the flower's step and the crystal are the same event), and **you cannot take what is not there** (clamped to what the victim holds above level 0, so an attacker can never be handed a petal that did not exist). Consequence: the deficit band [-5, 0) is now reachable by TRANSIENTS ONLY, the exact mirror of the overcharge rule. **⚠ THE POT STARTS EMPTY, IN EVERY MODE (§2.1), and that is the design rather than a gap to fill.** Nothing seeds a pilot's levels at spawn — every arcade scene's comeback profile authors `Initial*: 0` (21 of 22 share `AstroLeagueComebackProfile`) and no arcade card authors `SO_ArcadeGame.StartingElements`, which is the ARENA per-hull handicap. The only base-level GAIN in the game is collecting an **ELEMENTAL** crystal through `SkimmerAdjustElementLevelByCrystalEffect` — the single live asset of its type, carried by `Resources/ElementalCrystalSet` (so every lifeform heart, and every ejected petal, pays back on pickup) and by the Wanderway toy; `VesselIncrementLevelByCrystalEffect` and `VesselAdjustLevelByCrystalEffect` are referenced by NOBODY. An **OMNI** crystal grants no levels at all, and on the Sparrow it grants an 8-second ward against every debuff source — so in a mode whose only pickups are omni crystals, the one thing a crystal does to this economy is make you immune to it. Consequently the economy is structurally inert in the seven modes with no lifeform in their spawn profile (Breakwater, Cleave, Hijack, Regatta, Skein, SkimRace, Joust) and inert everywhere else until somebody hunts — which is what produced the playtest report *"I played a dogfight with the Sparrow and saw no crystals leave either of our vessels"*: both pilots sat at 0, so every hit's `AccrueElementalLoss` returned 0 petals and the ejector was never reached, with all five Sparrow reporters correctly wired the whole time. General rule: **a conserved economy with no starting stake is indistinguishable from a broken one** — every piece can be individually correct and the whole thing still does nothing a player can see, because the clamp that makes it conserve is also what makes it silent; a mode meant to have a fight over elements needs an on-ramp its own VERB reaches, not one that merely exists in the arena. **Read before touching any elemental debuff, `ResourceSystem`'s levels, or `CombatHitDrain`.** **A VESSEL MAY NOT MOVE AN OPPOSING VESSEL, NOR TAKE ITS CONTROLS (§9), and that is the other half of this economy's design.** Taking a rival's MOTION — spinning their heading, shoving them sideways — or their CONTROLS — muting their stick, shrinking their skimmer, freezing their trail spawner — is the one hit a pilot cannot answer with flying, and it was reported as simply not fun to receive; taking their PETALS is a loss they can chase down and win back, because the crystals land in the world. **EIGHT effects broke it and were removed in Sep 2026, in two tiers.** MOTION: the Sparrow's guns and rocket (`VesselSpinBySparrowFullAutoProjectileEffect`, and the spin half of what is now `VesselDetonateSkyBurstProjectileEffectSO` — renamed because the SAME effect owns the round's direct-hit detonation, so the asset could not simply be unwired without making a centre-punch, the dearest hit in the game at 30 points, pass through a pilot without going off), the Urchin's spikes and the Rhino's sword (which laterally shoved as well as spun); `VesselTransformer.SpinShip` went with them, those three effects being its only callers. CONTROL: the Sparrow's guns shrank a **Rhino's** blade to 0.7x for 3 s (`VesselChangeSkimmerSizeBySparrowFullAutoProjectileEffect`), the Rhino's sword muted a victim's `RightStickAction` for 5 s (`VesselDamageBySkimmerEffect`) and froze their trail spawner for 10 s (`VesselPrismSpawnerCooldownBySkimmerEffect`), and a Rhino or Squirrel crystal blast muted it for 3 s (`VesselChangeSpeedByExplosionEffect` — misnamed; it changed no speed); `ShieldSkimmerScaleConfigSO.ApplyMaxSizeDebuff` went with them. Three findings from the control tier generalise. **A retired serialized key is indistinguishable from live wiring in the YAML** — the spawner-freeze's ONLY reference was a `shipSkimmerEffectsSO`/`vesselSkimmerEffectsSO` override on `Rhino.prefab`, and all three of `SkimmerImpactor`'s inline effect lists are COMMENTED OUT (it reads a container instead), so Unity's un-prunable modification made an effect that had **never run once** look wired for as long as anybody read the prefab: ask which CONTAINER holds an effect, never which prefab mentions it. **A shared config asset makes one hit hit everybody** — `ApplyMaxSizeDebuff` mutated the `ShieldSkimmerScaleConfigSO` it was called on and ONE asset drives every Rhino, so shooting one pilot shrank every Rhino's blade in the match (the twin of the `GrowSkimmerActionSO` hazard `ElementalAbilitySystem/BACKLOG.md` 5.11a retired — that row could only delete one of the two, because the live one wrote runtime state rather than a serialized field, which made it *safer* and not *safe*). And **a container can be emptied to nothing, which is a real change to state rather than a neutral outcome**: `SlowExplosionImpactorDataContainer` held exactly that one effect, so the **Rhino's sword crystal burst, the Rhino's vessel crystal blast and the Squirrel's vessel crystal blast now have no vessel-facing effect at all** — correct under the law, and a hole whose sanctioned filling is a Debuff-class combat-hit plus drain priced through `author_combat_debuff_magnitudes.py`, not a rewiring. `Tools/Build/check_vessel_on_vessel_motion.py` (`--self-test`, each tier proven on its own pre-fix tree at 6 findings) fails the build if any effect in a victim-facing family (`Vessel Projectile` / `Skimmer` / `Explosion Effects`) reaches a motion, attitude or control API again — because the capability is one `[SerializeField]` away from returning and nothing about its return fails to compile. **What is deliberately NOT removed: a vessel deflecting off MASS it flew into** (`Vessel Prism Effects`' `GentleSpinShip`/`ModifyVelocity` bounce, and the Rhino blade's bounce off a prism it cut) — that is the FLIGHT MODEL rather than a weapon, and a rival's trail deflecting you is you hitting their wall, which a pilot answers by flying around it — **and the DANGER PRISM's input mute** (`SparrowDebuffByRhinoDangerPrismEffectSO`), because a danger prism is a hazard STANDING IN THE WORLD, the risk/reward surface the trail economy is built on, answered by not flying into it: **the line the law draws is between the ARENA and a weapon somebody AIMED at you**, which is also why the danger prism is the economy's only burn sink. |
...
| `PALETTE.md` | `Docs/` | The domain colour set (`SO_ColorSet`): which asset is live, what `_DarkColor`/`_BrightColor` actually are (prism **base face** vs **fresnel rim** — "Outside/Inside" is a misnomer), the **linear-HDR colour-space rule** (Rec.709/CIELAB apply directly; scaling a pair changes brightness but NOT contrast), the measured shielded-tier contract (ΔL\* 29.34 across all domains), and the **danger tier** — which has no colour fields of its own: it composes the domain's *shielded* base face with the shared domain-independent `EnvironmentColors.Danger` rim. **The invariant that outranks every per-tier contract (§4.0): in every tier, on every domain, the rim is brighter than the base** — it held on nine of twelve tier×domain pairs by accident rather than by rule, and each of the three violations was separately rationalised as a local trade-off before being recognised as one defect (danger was inverted on all three domains at ΔL\* −3.8; it is now +9.30). Two warm-hue traps are recorded there and are the reason those violations were invisible: **absolute `C*` is not comparable across hues** (equal chroma leaves a warm hue's blue channel starved, so judge by screen saturation *after clipping*, §4.1), and **authoring a peak channel silently overshoots `L*` at a warm hue** — set lightness against the other tiers first, then solve the channels. §2.2 covers the **crystal tier**: crystal colour signals WHO MAY COLLECT (element is shape, never colour), a free pickup wears the lime CTA, and in every crystal shader the composition is `lerp(dull, bright, (1−N·V)⁴)` — so at that fresnel power **`DarkCTA` paints ~93% of the crystal and `BrightCTA` is a 2.5% hairline**; tune the body, not the rim. **§2.5 is the caveat that costs you if you miss it: that is a statement about a CRYSTAL** — anything that does not compose the pair (a prism, a UI chip) painted from `DarkCTA` alone renders the shipped (0.28, 0.50, 0.08) as a dark olive rather than lime, so read `SO_ColorSet.GetCtaSignalColor()` (the sibling of `GetDomainSignalColor`, normalised the same way, alpha 0 when the palette authors no CTA). **§2.6 is the same trap a THIRD time and the one that generalises it: a colour authored FOR A SHADER is not a colour a UI slot may read.** `EnvironmentColors.Danger` is HDR at 1.498 **with alpha 0**, so a raw read both clips and is fully transparent — and a transparent tint is indistinguishable from a tint that never ran. Read `SO_ColorSet.GetDangerSignalColor()`, the third `*SignalColor` sibling. The rule for all three: the shader composes the pair, tolerates HDR and never looks at alpha; a HUD does none of those, so add an accessor rather than reading the field. It also records the finding that outranks §3 in practice: **gameplay bloom is CLAMPED at 0.5** (URP's default is 65472), so bloom saturates at max channel 0.5 and 56 of the 86 authored colours bloom identically — brightness above the clamp is a **dead dial**, and §3's "channels above 1.0 bloom" is false as shipped. Within the clamp, extra bloom is bought with bright **area**, not intensity. **§4.3** generalises §4.1 past prism tiers and adds the composition law: **two saturated OPPOSITE hues cannot be blended, only separated** — any lerp between them spends its range in a third hue that belongs to neither (and on an additive surface it also SUMS with what is behind it), so a two-colour effect reading as one muddy colour wants a threshold, not a new pair; plus the two judging rules that caught it — **ACES desaturates anything bright**, so "desaturated" is a post-tonemap measurement and the inspector swatch lies, and **judge a candidate at the size it will be judged**, because a downsampled contact sheet averages two colours into the third one. **Read before editing any `*BlockColor` or `*CTA` field, changing which field feeds a prism or crystal material, authoring a two-colour VFX, or trying to make anything glow harder.** |
| `ABILITY_LOCKUP.md` | `Docs/` | **The ability lockup (TOTEM)** — the one system that fuses a vessel's ability icon with the element indicator that upgrades it: **TWO BORDERLESS TRAPEZOIDS** meeting at their wide edges across a small gap (element flower in the upper, ability icon in the lower), the card (not the icon) carrying the upgrade as a bloom behind both plates plus a lift in their fill, the control chip below. **The gap is the divider and the silhouette is the frame** — both the hairline and the rim are retired, because a line and an outline were drawing a boundary the shape states on its own; two trapezoids facing wide-edge to wide-edge read as one waisted object where two borderless rectangles read as a list. **The two plates are the SAME HEIGHT and the two marks are the SAME SIZE** (a true mirror about the gap): unequal plates stack into a lopsided hexagon that reads as a coffin, and against a mirrored plate a smaller flower reads as an under-filled plate rather than as hierarchy — the two marks are different KINDS of thing in fixed columns, so size was never what told them apart (`PlateImbalance` ≤25%, `0.75 ≤ flower/icon ≤ 1.0`). A **graded band rides the two SLOPED sides only** — solid the whole length of each slant, then WRAPPING around both corners onto the horizontals where it grades to nothing (a grade that died on the slant left the corner unlit, which reads as unfinished), with **antialiasing baked into the geometry** as a zero-alpha feather either side, because a canvas gives a generated diagonal none. It never closes, so it is not the rim returning; the plate's core stays transparent behind it. Generated plates cost ~200 verts/card and are the TUNABLE form — the finalization path is baking them to PNGs (`Docs/ABILITY_LOCKUP.md` § Finalizing), where `Mask` reads the same sprite's alpha for the clips. The plates are GENERATED (`TrapezoidGraphic`), never sprited: a trapezoid has no 9-slice, so a sprited one would freeze the slant into the art and need one asset per direction — generated, the slant is a single float both plates read *mirrored*, and `bloomSprite` is the only authored asset left. The flower is the SHIPPED radial one, unchanged and drawn smaller than the icon; **no authored rect moves** (the card is a sibling behind the icon, the upper cell is added above), so a vessel adopts the style by gaining ONE component and no art. Fleet-wide and **structural** — ensured in `VesselHUDController.Initialize`, audited by FrogletTools > Vessels > Audit Ability Lockups, asserted by `AbilityLockupStyleTests`. **The lockup owns the ROW**: position, pitch, cell size, host scale and icon size all come from one style asset and are written onto every vessel, so a prefab cannot make the row diverge and no icon is ever re-authored to match (each icon's scale is DERIVED as `iconBoxSize / its authored size`). It retired eight things it superseded — the upgrade corner badge, the upgrade icon tint, the decagon button plate, the per-vessel RING gauges, the circular press glow, the Squirrel's undriven heat halo, the hairline divider, and the rim itself (with the `LockupPlate`/`LockupPlateRim` art). **The row is ALWAYS four cards**: a slot the vessel has no ability for yet renders LOCKED (quieter plate, hairline mark, no gauge track — never a padlock, because the ability is not locked to the *player*, it does not exist yet), which is what puts the Rhino on the fleet's UI instead of leaving it on the old one until design lands. **A locked card DOES draw the cooldown veil**, and that is the one exception in the whole style: locked is a fact about the ART, while a slot being DRIVEN is an ability that exists — the veil never needed an icon anyway, since it sizes itself on the ability PLATE. The Serpent proved it, binding 0/4 icons and reporting "no cooldown indication" for three rounds of playtest against a completely correct driver, so the rule is **an indicator that refuses to draw because its decoration is missing is indistinguishable from an indicator nobody is driving**; `SetUpgraded` is deliberately NOT changed by the same argument read the other way, because on a locked card there is nothing to light. **The fleet's gauge is a LINEAR fill that takes over the ability plate, never a ring** — a vessel binds its existing meter `Image` as `AbilityIconBinding.gauge` and keeps writing `fillAmount` on the very same object, so no drive site changes; the lockup only re-homes and re-forms it, MASKING it to the trapezoid rather than reshaping it (a `Filled` Image is a rectangle and would overhang a tapering plate; mirroring its value onto a `TrapezoidGraphic` instead would need a per-frame poll of somebody else's field). That re-homing is load-bearing, because a meter is regularly authored under a DIFFERENT ability's button than the ability it reports on (the Squirrel's boost fill under its skimming button, the Scarab's ball energy under its throttle button) — which is also why `Build` adopts EVERY gauge before retiring ANY chrome. **The fleet's COOLDOWN is a CLOCKWISE radial veil swept over the ability plate, OVER the icon** (`VesselHUDView.SetAbilityCooldown(element, remaining01)`; clockwise needs `fillClockwise = FALSE`, because the veil depletes — the flag names the direction the wedge is drawn and the moving edge is its far end) — radial where the gauge is linear and in front where the gauge is behind, so one card can carry both without the two reading as one meter running backwards. It is a VALUE the vessel pushes, not an `Image` it binds (a cooldown has no per-vessel artwork worth preserving), the overlay is built LAZILY so an ability that never recharges costs nothing, and it is the one piece of the lockup that lives OUTSIDE the card — it must darken the icon, and the icon is a later sibling than the card. It retired the Squirrel's bespoke reload animation, which was four channels (sink-and-rise, breathing pulse, radial fill on the icon, slam flash) saying one thing on one hull. **The lockup DRAWS the control chip from ONE fleet-wide glyph set** (`ControlGlyphSetSO`, `Resources/ControlGlyphSet`) — a vessel authors no glyphs at all. Each card derives its own artwork: ability → its map entry's `InputEvents` → `InputHintBindingMap.BindingFor` → physical control → sprite (pad) or label (keyboard), so a wrong label is structurally impossible. A passive ability draws nothing, and a pad button with no keyboard equivalent draws nothing there — **blank is the honest state**, since a pad glyph shown to a keyboard player is the misinformation this replaced. **One sprite serves both pad families, and that is a deliberate REGRESSION from what shipped, not a continuation of it.** Measured at the merge base: every HUD that authored glyph art authored FAMILY-SPECIFIC art (Sparrow and Scarab each carried PS `L2/o/square/triangle` against Xbox `A/B/R1/R2`), which is what a device switcher is for. Two things make the simplification acceptable anyway: the SHOULDER pair cannot be authored per-family today at all — the project has `PS/L1` and `XBOX/R1` and no `XBOX/L1` or `PS/R1`, so the mixed sourcing in the asset is the only art that exists — and the FACE buttons, where both families do exist, are used by one vessel (the Sparrow's `A`/`B`). The stated cost is therefore narrow and real: **a PlayStation player sees Xbox `A`/`B` letters on the Sparrow.** Closing it is a `psGlyph` field on `ControlGlyphSetSO.Glyph` plus a family argument to `For()` — recorded as a follow-up, not done here. Device DETECTION stays in `InputDeviceIconSetSwitcher`, which `VesselHUDController` now ENSURES like the lockup. **`NormaliseHudRoot` makes the HUD root the SCREEN**, because the row anchors to `(1,0)` of it and the roots disagreed — Rhino/Scarab/Sparrow stretch to the canvas while Dolphin and `VesselHUDPrefab` (Squirrel/Serpent/Manta) point-anchor at the CENTRE at 100×100, so the margins meant two different things and the vessels disagreed about where the row sits. *A container's rect is part of the contract for everything anchored inside it — standardising the contents while leaving the frame per-vessel standardises nothing.* **A control chip lands on its own card's `ControlChip` socket at zero offset**, so an (LT)/(RT) label moves with the totem instead of floating near it at a per-vessel `attachOffset`. Two earlier attempts are worth not repeating: re-anchoring the SWITCHER's authored hints onto the socket needed the lockup to supply their size (pad glyphs measure 50×50 against PC text at 106×22, so one 24px socket gave them different CLEARANCES and the label appeared to move when you switched pad → keyboard → pad), and re-homing those hints INTO the socket as children broke device switching outright — *a component that shows and hides things by toggling a parent owns that parenting.* Both were fixing the wrong layer: the card now DRAWS the chip and there are no authored hints to place. **`InputDeviceIconSetSwitcher` was consequently demoted to a pure DETECTOR** (627 lines → 138) — its three glyph roots, per-set hint visuals, placement pass, `SetHintActive` and `BindHintsToAbilities` are deleted, because a second set of glyphs competes with the chip no matter who toggles it, and on the three `VesselHUDPrefab` variants the switcher's own reference was the ONLY thing sparing those roots from the retire sweep (while `ApplySet` re-activated them on every device change, so merely switching them off could never have held). That deletion surfaced three defects the authored display had masked: **`OnSetChanged` was declared, subscribed and NEVER RAISED**, so a chip could never follow a device change — *an event nobody raises looks identical to an event nobody needs*; **the keyboard set was unreachable on every vessel**, since `KeyboardSet()` fell back to Xbox whenever `keyboardTextRoot` was null and no vessel wires one — *a fallback that protected an authored display keeps firing after the display stops being authored*; and **`padGlyphHeld`/`heldColor` were authored and read by nothing**, the held-state swap having regressed when chips replaced hints (the chip now takes it off the card's existing press path, so the card lights the ability and the chip lights the button — one press, and a one-shot `PlayPressFlash` is explicitly not a hold). General trap: *two things that look interchangeable at one size are only interchangeable at that size.* The lockup also retires **root-level content that no component on the HUD still REFERENCES** — by then everything it owns has moved into the row, so what is left is either driven or dead, and which one is a reference question rather than a guess. That clears the device-glyph roots on EVERY vessel — the stranded ones on Sparrow/Dolphin/Scarab (standalone prefabs with no switcher, so their glyphs were never lit, never device-switched and never placed) and the switcher-driven ones on Squirrel/Serpent/Manta alike, since the card draws the chip and a second glyph set is a competing display either way. Three guards: only children that DRAW; reference sources limited to the HUD ROOT (a component inside a leftover branch would spare its own branch); and a reference from `highlights` does not count, because that is the retired press glow and the Rhino's entry points inside its old boost container — *a reference from something the lockup retired is not evidence anything still uses it.* **A vessel that binds NO ability icon skips the reference guard entirely and has its whole root cleared** (Rhino/Manta, and the Serpent until it bound its Time icon on 2026-09-25 — binding a vessel's FIRST icon flips it back onto the reference guard, so any root branch a HUD-root component still references reappears; the Serpent's stale `shieldIcon` had to be nulled for that reason): the guard asks "is this driven?" as a proxy for "is this still wanted?", and on a vessel with four LOCKED cards the honest answer to both can be YES — the Rhino's status cluster is genuinely driven (six readouts the view writes) *and* is the old UI, anchored in the bottom-right corner the row now occupies. Accepted cost, by design call: the Rhino stops showing its debuff timer, slowed count, skimmer-size ring, laser and crystal indicators until they are re-homed; the view still writes to them, so a branch is switched OFF rather than unwired, and the rule reverses itself the day that vessel binds its first icon. General trap it records: **a HUD prefab asset does not tell you what a vessel WIRES** — `Rhino.prefab`'s instance adds three root branches `RhinoHUDVariant.prefab` has never heard of and populates six fields the asset leaves empty, which is why this doc previously (and wrongly) claimed the sweep cleared the Rhino's `BoostContainer`. Chrome is retired POSITIONALLY (a direct child of the host that is not the icon and not the card), with ONE exception: a UGUI **touch target** is made invisible rather than disabled, because an absent graphic does not raycast and disabling it silently deletes the on-screen ability control on every touch device. **Read before touching the four-icon ability row, `ElementalBarsView` placement, any vessel's HUD gauge or upgrade signal, or the control-hint placement.** |
| `CONNECTING_PANEL.md` | `Docs/` | **The load screen every game scene shows while its arena builds** — and the three different KINDS of answer it gives to "is this doing anything?": words (the phase and its raw counts), a number (one monotonic bar), and the thing itself (a live window onto the arena being built). The bar and the text read the same counters; the preview reads none, which is why a stalled load looks different from a merely long one. **`ArenaLoadProgress` is MONOTONIC by construction** because the signals under it genuinely go backwards — `LayProgress` reads 1 while IDLE and drops to 0 when a batch starts, a second build re-queues the counters, and `GrowRemainingCount` climbs before it falls — and a bar that ran backwards would turn "this is slow" into "this is broken". Three details are each a test: the idle CREEP's ceiling is a function of the furthest phase ENTERED, never of the current value (inferring it relies on a lerp never quite reaching its target — a float accident standing in for a decision, and the day it lands the dwell creeps to 60% before a prism is laid); the grow denominator is the PEAK count seen this load (the first reading as total shows 0% forever while prisms are still being queued); and it finishes at exactly 1 (a bar that vanishes at 0.9 reads as abandoned). The slider is forced into a READOUT — non-interactable, no transition, no navigation, **handle removed**, because a handle is the affordance that says drag me — and wears two 9-sliced white capsules from `Tools/Build/author_loading_bar_sprites.py` (`--check`), tinted at runtime; the 9-slice is load-bearing, since a capsule stretched without a border turns its caps into ellipses. The preview renders into a **RenderTexture, never the screen** (so no depth ordering against the panel's own camera or the gameplay camera), creates its camera at runtime when none is wired, excludes the UI layer (or it draws the panel inside its own window, one frame stale), and comes down with the panel on every exit including a cancelled load. **Its FRAMING is the bug `ModePreviewArena.FramingRadius` already recorded**, hit again: `Cell.MembraneRadius` returns 0 until the membrane has SPAWNED, so a camera placed against it parks at a fallback distance with the arena's size unknown and shows the skybox — the radius falls back membrane → `ExpectedNucleusWorldRadius × 3` → 1200 and is re-read every tick, and the CLIP PLANES are derived from the shot rather than copied from the template camera (whose far plane is sized for a backdrop a few units away and clips the whole arena). It renders **on demand** with a disabled camera stepped by hand, because an enabled one takes a full extra pass over ~50k prisms every frame of the heaviest load in the game — and the bigger cost was the GAMEPLAY camera doing exactly that behind an OPAQUE panel, so it is now MUTED for the duration (`suppressGameplayCamera`: culling mask 0 + post off, never `enabled = false`, because `Camera.main` returns the first ENABLED camera tagged MainCamera and disabling it nulls that for the whole project). That pays for the preview rendering at the surface's REAL pixel size and with the game's own post/AA/shadows. Note **a preview stepped from `LateUpdate` can never be smoother than the frame rate** — at 10 FPS a 20 Hz preview advances in 100 ms steps, which is what "the video is stuck" looks like, and the only fixes are the build tempo and the muted camera. Its FRAMING then hit the sibling of the same bug from the other side: **a cell's reported radius is its BOUNDARY, not the extent of what is in it** — at the arcade card's 1.95x a sphere of radius r subtends 27.1 degrees against a 30 degree half-FOV, so the membrane fills the frame BY CONSTRUCTION and the mass being laid inside it is a speck; the two cards want different shots because the card shows a WORLD (frame it from outside) and this shows a BUILD (sit in the room it is happening in: 0.805r back, 45 degree FOV). Zooming was only half of it — `r` was still the membrane, and Scurry's arena is a small fraction of it, so the extent is now **MEASURED FROM THE BUILD** (`PrismTrailBuilder.TryGetLaidBounds`, a running world AABB of everything laid since the hold began; the cell's size is only the fallback for the dwell and for an authored `EnvironmentPrefab` that is instantiated rather than laid). It costs nothing on the hot path (pose read off the lay plan into a LOCAL AABB, flushed through the parent once per 256-prism batch — all EIGHT corners, since a rotated parent maps a box's corners to a box neither original corner sits on), only ever GROWS and is eased (raw tracking jitters every batch; shrinking would dolly IN while the world got bigger), and is reset with the hold. The camera is also clamped INSIDE the membrane (`insideCellMargin` 0.9, measured from the CELL centre) — outside it you are looking at the arena through its own boundary shell. **Serialized values beat field initializers**: the prefab carries `framingFactor`/`liftFactor`/`fieldOfView`, so changing the C# defaults alone changes nothing on screen. The **frame rate** during the build is neither of those — it is the load gate's own tempo, a **250 ms lay slice** plus **512 creation completions per frame**, both justified by the same premise (*the screen is covered, so no frame is worth protecting*) that **a panel showing the build makes false**. A watched hold therefore states its own slices (`LoadGateLayBudgetOverrideMs` 25 ms, the new `LoadGateCreationBudgetMsOverride` 18 ms); both are **work-conserving**, so the trade is a somewhat longer load for a frame rate the view can be read at, and the creation dial is stated in MILLISECONDS rather than as a count for the reason `LayBudgetedAsync` already gives about laying — per-prism cost varies per machine, so only a time budget holds a frame budget. Both belong to the HOLD (cleared by `SetLoadGateHolding(false)`, so an aborted hold cannot leak its tempo into the next load); `EnvironmentLoadVeil` reached the same shape from the other direction at 80 ms. It also carries the **pilot roster**: one chip per HUMAN pilot (avatar over domain colour, greyed while that machine is still building, lit with a slow glow when done) and a hold until every human reports. **"Loaded" is PER-MACHINE and needs a report** — every peer builds the arena independently off its own clock, so it rides the platform's owner-detects/server-records round trip (`Player.ReportArenaReady` -> `NetArenaReady`, the fourth sibling of `ReportFaunaKill_ServerRpc`), is **reset per scene** (a stale true releases the next match's panel before a prism is laid), is **true by construction for an AI** (no machine to wait for — which is also why AI are absent from the row: chips that can never change read as players who never arrive), and is reported by `MiniGameHUD`'s **no-panel** branch too, because a peer's panel is waiting on that answer. The wait is capped at 45 s and releases LOUD. The row is **structural** (ensured at `Awake` like the `CanvasGroup`) and adopts its own container/template/pieces by name, so a prefab carrying only the art still lights up. |
| `WEEKLY_CHALLENGE.md` | `Docs/` | **The weekly challenge** — one curated objective per UTC week, the same one for every player. The rule that makes it work: **the definition is DERIVED, only the progress is STORED** — this week's challenge is `hash(UTC Monday) % pool` over an authored `WeeklyChallengeCatalogSO`, so it resolves on a cold launch, offline, and identically on every platform with no server round trip, while UGS Cloud Save carries only what genuinely differs per player (best value, completed, attempts). The hash is hand-written FNV-1a rather than `System.Random`, which is deterministic only *within one runtime's implementation* — not a promise two platforms can hold each other to. **The week starts on the UTC MONDAY** (ISO-8601; a boundary that is a matter of taste makes two neighbours draw different challenges for one day in seven), and the trap it guards is that **`DayOfWeek` numbers Sunday 0**, so the naive subtraction puts Sunday at the start of the FOLLOWING week. **A SECOND trap of that class shipped, in the FORMATTER rather than the boundary, and was found from outside this system** (the analytics wave cohort, which reuses this week key): `ToString("yyyy-MM-dd")` renders through **`CurrentCulture`, whose CALENDAR is not always Gregorian** — and Unity takes CurrentCulture from the device locale, so the same instant keyed as `2026-09-07` on en-US came out `1448-03-25` on ar-SA, `2569-09-07` on th-TH and `1405-06-16` on fa-IR. Those players drew a different challenge from everyone around them and never rolled over in step. All three period formatters now pass `CultureInfo.InvariantCulture`. General rule: **a date rendered without an explicit culture is a date in whatever calendar the device chose** — any KEY, id or filename built from a `DateTime` needs InvariantCulture, not just a displayed one, and the defect is invisible on every dev machine in a Gregorian locale. **Rollover is a period-key comparison, never a timer** (a scheduled callback is wrong every time the device slept through it or the clock moved). The challenge is an EXISTING arcade mode launched through the EXISTING arcade modal — never a second launch pipeline. **Two separate decisions that are easy to conflate: the ASK is fixed, the MODE is not altered.** The launch panel PINS intensity, domain and seat count and removes Add AI (plus: briefing = the objective, objective box hidden, preview look-only) because the challenge is one fixed ask with one attempt, not a lobby — while the RUN itself plays at **that mode's own end conditions**, untouched. A run-scoped `EndConditionOverridesSO.SetRunOverride` that made a challenge a *smaller* version of its mode is RETIRED and should not be re-derived; the panel's locks are NOT, and the natural reading of "the run is an ordinary match" — that the card should therefore be configurable — is the mistake to avoid. What survives from the retired mechanism is the authoring rule underneath: **an objective above what a match of that mode can produce is unreachable by construction** (Crystal Capture ends at 20, so "collect 30" never completes), and it should sit meaningfully BELOW that target because **a metric must be credited PER PLAYER** while an end condition is a domain SUM (which is also why Nucleus Rush, crediting a domain's *representative*, is out of the pool). Reordering the pool re-rolls which week draws which mode. Authored ONLY through **FrogletTools > Game Modes > Weekly Challenge**, which exists for the VALIDATION rather than the fields; its layout is **master/detail behind three tabs**, because N entries × 9 fields is a page and a half of scrolling to compare two numbers 400px apart. **The CHALLENGE is weekly, the ATTEMPT is daily — one run per UTC day, spent at LAUNCH** (the one ordering that survives an alt-F4 halfway through a bad run); the day boundary is a key comparison like the week's, a completed challenge is still playable tomorrow, and only a FASTER completion submits (`BestTimeMs`). Test mode (forced entry, a 'week' of N minutes, no attempt limit, a scaled clock) is inert outside the editor and dev builds AND fails a non-development build outright — a runtime gate makes a stray flag harmless, the build guard makes it loud. **The LEADERBOARD is "who completed this week's objective fastest"** — every challenge is *reach N of something*, so the only thing left to rank is the TIME, lowest first (`WeeklyChallengeLeaderboardService`, `WeeklyChallengeRanking`, `WeeklyChallengeLeaderboardPanel`). **Only a COMPLETION earns an entry** — a player who never reached the target has no time, not a slow one, and a sentinel would either rank non-finishers above finishers or bury the real times, leaving the end-of-week reward tiers computed off a list that is mostly not a ranking; `FinishAttempt` is the single site that can submit. It is **ONE leaderboard reset weekly by UGS, never one per week** (the SDK cannot create leaderboards, so a per-week id needs a server job minting them forever), and its **archive on reset** is what the separate reward system reads — rewards are deliberately not implemented here. **Three settings live on the UGS SERVICE and no code can enforce them** — the client SDK has no reset or delete call at all, so they are set in the dashboard, the UGS CLI or the Leaderboards Admin API and nowhere else: Sort Order **ascending** (the score is a time), update strategy **keep best**, and a **recurring weekly reset with archiving on**, aligned to the same UTC Monday. **TWO of the three fail silently and both are checked at runtime.** The sort order is the first — a wrongly-sorted board has real rows, real names and real times, simply upside down with the slowest run at rank 1 — so it is checked instead (UGS returns rows in rank order, so a correct board is non-decreasing) and screams once per session. **The RESET is the second, and it is the worse one**: the score is a time and the sort is ascending, so an old fast run outranks every new one **forever** and a board that stops resetting freezes on the week it stopped — which is how it was reported (*"not getting cleaned after the week reset"*). The net under it is that **every submitted score is stamped with the PERIOD it was run for** (`WeeklyChallengeRanking.PeriodKey`, metadata `"w"`) and every read keeps only the current one, so last week's rows go away on the client whether or not the service-side reset fired. Five decisions travel with the stamp: it is the **DRAW** key, deliberately not `WeeklyChallenge.PeriodKey` — that is the RECORD key and carries `attemptResetToken`, which re-issues the SAME challenge to hand an attempt back, so stamping it would orphan every score already submitted that week and fire the not-resetting error on a healthy board; a row that **does not say** which week it is from counts as NOT this week (unknown has to fall on the stale side, since an unstamped entry is exactly what a non-resetting board is full of — the stated cost being that every pre-stamp entry disappears from the panel); an unknown **CURRENT** period keeps everything, the opposite call for the opposite reason, since emptying a board over a missing catalog is worse than showing it; ranks are renumbered **only when something was dropped**, so a healthy board keeps the true world ranks UGS gave it and a filtered one does not read as 1st, 4th, 812th (the Friends scope's own argument); and the submit's metadata **stopped being optional** — it used to be null when the player had no avatar, and an unstamped row is one this game then hides from itself. **It is a net, not a fix**: a read can hide stale rows, never remove them, so `FetchPageAsync` pages past other periods for at most 5 requests (a healthy board pages **once**, asking for exactly the rows the caller wanted — byte-for-byte the pre-filter behaviour) and past that the panel reads short, while `WarnIfBoardNotResetting` asks for the real fix once per session. The **KEEP BEST** trap is the second reason the reset is mandatory: "best" spans the whole board's life, so once it is missing a player whose stale time beats their new one keeps the stale row and gets no entry for this week at all, which no client-side rule can undo. A code-side `BIG - time` encoding was rejected: it makes every raw score in the dashboard, every export and the reward archive unreadable to save one dashboard setting. Two implementation notes worth carrying: **`WeeklyChallengeRanking` is a project type, not the UGS entry** (a view taking `Unity.Services.Leaderboards.Models.LeaderboardEntry` drags the SDK into the UI layer, and the project already has TWO types called `LeaderboardEntry`); and **mm:ss.cc converts the whole value to centiseconds in ONE step and ROUNDS** — the obvious whole-seconds-then-`floor((s - whole) * 100)` prints 47.3 as `0:47.29`, because the double nearest 47.3 is a hair below it and the subtraction keeps the error. **No avatar travels with an entry** (UGS holds an id, a name, a rank and a score, not a profile), so the panel does not reference `SO_ProfileIconList` and rows keep the template's art; real faces need a second lookup and are a follow-up. The inert PlayFab-era `DailyChallengeSystem` / `DailyChallengeModal` / `DailyChallengeLeaderboardView` cluster deliberately KEEPS its old name: renaming a dead feature to match a live one is how two systems come to look like one. Do not wire both. It also now owns its OWN `DailyChallenge` struct (mode + intensity, nothing else) — the rename took the shared one and the dead system stopped compiling, which is the useful half of the discovery: the two features were sharing a value type, so "the legacy cluster is separate" was true of its names and not of its data. **Renaming a feature is a compile-time test of whether it was ever actually separate from the one it superseded** — what the compiler names is where they were still joined. |
| `OFFLINE_MODE.md` | `Docs/` | **CORE IMPLEMENTED (2026-08-26)** — offline is a plain `127.0.0.1` local host started by `OfflineModeService` when UGS auth/Relay is unreachable, with `LocalCloudDataCache` giving every cloud repository a last-known-good disk snapshot (name, vessel unlocks, episodes, progression survive offline); `GameDataSO.IsOfflineSession` stands matchmaking + party creation down. §§0–3 keep the original diagnosis; §6 records what shipped; §7 the online-only UI gating (`OfflineUIGate` + service-level guards on invites/leaderboards/purchases) and the in-place **reconnect** (`ReconnectService`/`ReconnectButton` re-runs the boot chain with no app restart — and why it loads the Authentication scene rather than re-loading Bootstrap, whose ~15 persistent roots have no duplicate guards). **Read before any offline / single-player-fallback work.** |
| `OFFLINE_MODE_SUMMARY.md` | `Docs/` | One-page version of the above: the problem (no `StartHost` anywhere + an unbounded Relay wait at boot) and the fix, for anyone who does not need the full record. |
...
| `PRISM_ANIMATION.md` | `Docs/` | **The clock-material law (LOCKED, STRICT — no legacy fallback)**: no prism may need multiframe CPU updates to animate — animation = pool-pull + one initial-conditions stamp + GPU runs the course off the shader clock + one scheduled end-state swap; colliders and gameplay state go FINAL at the start. There is NO CPU animation tier to fall back to: an unwired graph fails LOUD (`PrismClockDiagnostics`) and the visual snaps until the §4.4 wiring lands (in-editor checklist: `Docs/PRISM_CLOCK_WIRING_CHECKLIST.md`; validator: FrogletTools > Ecology > Prism Animation). Full audit of every prism update path+ migration tracker. §4.7 documents the ONE sanctioned shape for view-dependent prism visuals — a GLOBAL uniform published once per frame, never a per-prism write — and states the camera↔vessel occlusion corridor as a **PLATFORM LAW** with the four layers that make it un-authorable to skip. Its three citizens are the corridor (coverage), the Dolphin's Echo Sight (colour, §4.7.1) and the Urchin's cradle (VERTICES — a smooth radial drape onto the hull's live position, §4.7.2, which also records why a MESH SWAP is a state change rather than an animation and therefore legal under the law), which together show the shape holds for any channel of the surface description. §4.10 is the high-poly prism's second consumer and the first that is a STAMP rather than a global — the Rhino sword's SLICE, which maps each half's far skin onto the cut by a central projection from a point inside the half (exact for a convex solid: a bijection onto the true cut face) and dissolves each half from its cut face; **when every input of a deformation is known at the instant it starts, it is a stamp, whatever mesh it needs**. **Read before touching any prism visual, animation, or state transition** — and use the **`/prism-morph` skill** for any effect that DEFORMS prism geometry to live gameplay data (the high-poly-swap + GPU-vertex-map family the cradle founded): it carries the four-part recipe, the residency/ownership contracts, the analytic-normal rules and the proof harness so the next member does not re-pay the cradle's two rejected rounds. |
| `SPEED_TUNNEL.md` | `Docs/` | **The speed-tunnel PLATFORM LAW**: every vessel's camera FOV narrows and Panini relaxes as a function of its own measured speed — a quasi dolly zoom, sold entirely through optics with no camera-distance change. The mapping is **absolute and fleet-wide** (the same speed on any vessel looks the same); there is deliberately no per-vessel window, scalar, or normalization. Documents the four layers that make it un-authorable, the one sanctioned suppression (manual replay camera), the single tuning asset, and where every vessel lands in the shared window. **There is now also exactly ONE sanctioned way an ability may MAGNIFY the view** (§2.1, 2026-09-16): `VesselSpeedTunnel.SetHomeFieldOfViewOverride` moves the value the law narrows FROM, rather than the ability writing `Camera.fieldOfView` — a direct write is overwritten every frame while the tunnel is engaged AND is CAPTURED as the home the next time it engages, which bakes the zoom in permanently and never gives the player their setting back. The override keeps one writer (speed still narrows, from the scoped base), never leaks into `RestoreFov` (a release always hands back the PLAYER's value), engages the law by itself (a scope must hold at a standstill, where the speed term is zero), and is keyed on the owning Transform so a vessel swap cannot strand it. Its one caller is the Serpent's scope. **The bar for a second is that the ability must still be worth shipping WITHOUT the zoom** — the Dolphin's Echo Sight was, and its surface was reverted. **Read before touching vessel speed, the gameplay camera's FOV, or the Panini override.** |
| `VESSEL_VISION.md` | `Docs/` | **The vessel-vision-band PLATFORM LAW**: every vessel is progressively re-shaded into a flat, cel-banded silhouette in its own **domain** colour as a function of its distance from the camera drawing it — nothing up close, full mark across an arena, gone again at extreme range, **both edges graded** (the Halo Infinite player-outline read). The mapping is **absolute and fleet-wide**; there is deliberately no per-vessel window and no "is this me" test — the near cutoff is what excludes your own hull, and there is consequently **no suppression hold** (a replay camera marks everything, which is what a broadcast view wants). Documents the four layers that make it un-authorable, why the tint's ALPHA is a marker rather than an opacity, the MPB-restore rule it forced on every sibling effect, and the single tuning asset. **Read before touching `VesselGraph.shadergraph`, `VesselHelper.SetShipProperties`, or any system that writes a vessel renderer's MaterialPropertyBlock.** |
| `REAR_VIEW.md` | `Docs/` | **The look-back camera** — the gameplay camera flips to the MIRROR of its own follow offset (the same distance AHEAD of the vessel that it normally sits behind it) and keeps looking at the ship, on **C** or **LB+RB together**. It retires the picture-in-picture rear view (`Pip` / `PipUI` / `PipCamera.prefab`). Three things it records generalise. (1) **There is deliberately NO second camera**: a second live `Camera` falls outside four systems at once — the speed tunnel resolves `CameraManager`'s ACTIVE controller and would keep narrowing the camera the player is no longer looking through, `ApplyCameraGraphicsSettings` pushes the player's FOV and AA onto its three managed cameras and no others, `SetBackgroundColor` is applied per camera, and `Camera.main` returns the first ENABLED camera tagged MainCamera, so two live gameplay cameras is a coin toss for everything that asks. Reusing the one rig inherits all four, which is the argument `BeginWindowedPlayerCamera` already records for the mode preview. (2) **The mirror is applied at the POINT OF USE, never written into the offset** (`CustomCameraController.EffectiveOffset`; z only — mirroring the Sparrow's authored `(0, 10, -50)` whole would put its rear camera UNDER the ship). Four systems legitimately write `_followOffset` while a pilot flies — the zoom-out abilities, adaptive zoom, the skimmer's camera-scaling prism effect, and a vessel swap re-applying its own `CameraSettingsSO` — so a written mirror means **the first of them to fire silently drops the pilot out of the rear view** with nothing to explain it. (3) **Retiring a panel means checking what OVERRODE its default**: `Pip.prefab`'s root ships INACTIVE, but `CORE/GameCanvas.prefab` and `Panels/MiniGameHUD.prefab` each carried an `m_IsActive: 1` override on their nested instance, so the panel was on by default and was only ever switched off by a vessel announcing it was NOT the local pilot's — removing the grant alone would have left a dead panel showing a stale render texture in every mode. `Pip.cs` is KEPT and never granted, because its `Awake` default-off is now the only thing standing `PipCamera.prefab` (which ships active and enabled) down on the eight hulls that carry one. The gesture is the sibling of `OverviewGesture`, polled ONLY by `InputController` (the one pump already gated on local human pilot and on both pause gates) and deliberately not an `InputEvents` member — it drives a camera, not a vessel. **It had a SIBLING for three weeks and it was RETIRED, which is the part to read before adding a third vantage.** `VesselFirstPersonView` (the Serpent's scope) was built to the same shape — same static + `LateUpdate` driver, same `GetCloseCamera` identity test, same bind sites, same apply-at-the-point-of-use rule — and differed in two ways: **first person BEAT rear view** in `EffectiveOffset` rather than composing with it (the z-mirror of a cockpit offset is another point inside the same hull, so 'look behind from the cockpit' is not a vantage the mirror can express — two vantages that re-pose one camera must be ORDERED, never blended), and it also changed the FIELD OF VIEW, which the rear view never touches. **The vantage was right and the MAGNIFICATION on it was not**: it read as *nauseating* on its first playtest, because **a magnified view is a lever on every motion that reaches it** — a 22° scope multiplies the pilot's own turn, the hull's roll, the camera's settle and the speed tunnel's own narrowing by exactly the ~4× it multiplies the target, and the pilot cannot tell which half of what they see they caused. The scope's magnified picture moved into a WINDOW of its own, the flight camera went back to doing one thing, and `CustomCameraController` no longer carries `FirstPerson`/`FirstPersonOffset` at all (an unreferenced camera vantage bound at two `IsLocalPilot` sites is the 'eventually mistaken for a live feature' trap). `VesselSpeedTunnel.SetHomeFieldOfViewOverride` is KEPT with no caller, and that asymmetry is deliberate: it is a **guard** (the one sanctioned way to magnify the gameplay camera) rather than a feature, so deleting it would re-open the direct `Camera.fieldOfView` write the law exists to prevent. **§3.1.1 is the carve-out and the only one**: a camera that renders ONLY into a `RenderTexture`, is left DISABLED and stepped by hand, and is never tagged MainCamera falls outside all four of those systems by construction — that is the `ConnectingArenaPreview` shape, and the Serpent scope's window (`ScopePipView`, drawn into a `RawImage` SUBCLASS shaped like the CHARGE petal) is its second user. Being outside the speed tunnel is ALSO what makes that scope's zoom a pure function of the trigger. It is a genuine EXTRA render of the world (the panel preview stands the gameplay camera down; this one cannot), so it is paid for with a small square target, no shadows, no AA, a capped refresh and a lifetime of exactly as long as the ability is held. **Passing those four tests is NOT sufficient, and that is the part that has cost four playtests**: a bare `AddComponent<Camera>` comes up with URP's DEFAULTS — no post-processing, no volume layer mask, SDR — and this world is authored almost entirely HDR-emissive against the gameplay volume's tonemapper, so an un-adopted camera renders a flat, near-black version of it. Adopt through **`OffscreenCameraSetup`** (`AdoptGameCameraFraming` for what it sees, `AdoptGameCameraImage` for how it draws, **post-processing ON by default because it is not a quality setting here**, clip planes deliberately derived per window rather than borrowed). That helper exists because `ModePreviewArena`, `ConnectingArenaPreview`, `ToyPreviewCamera` and the Serpent's scope each rediscovered and separately wrote down the same finding, and the general rule is **a picture that renders WRONG and a picture that does not render at all are the same report** — the first three framed a bright subject and read as merely low quality, while the fourth framed open space, where the whole picture IS the skybox and the volume, and was reported as the window being GONE. A window whose subject can legitimately be empty also needs an opaque BACKING and a full-strength rim, so "showing nothing" and "not there" do not look the same. **Read before adding a second gameplay camera, touching `CustomCameraController`'s follow offset, or binding anything new in `VesselController.Initialize`.** |
| `VESSEL_CONSTRUCTION.md` | `Docs/` | **How the fleet is actually built, and the four ways that goes wrong SILENTLY.** The fleet is built TWO incompatible ways — a **skinned family** (one armature, one skinned hull, four element shapes: Manta/Falcon/Shrike/Termite, Sparrow, Scarab, Serpent, Squirrel) and a **part-per-mesh family** (a static mesh per part, no armature, no shapes, therefore no hull morph and no puppetry: Urchin 13, Rhino 7, Grizzly 8 — **the Dolphin left this family on 2026-08-26**, the only vessel that ever has, and now draws one skinned hull with four real morphs) — so "where do I mount a jet / find a part / read a size" has a different answer per hull. Records four measured discrepancy classes: (1) **exactly one `.meta` OWNS a guid** — an FBX's `.meta` can carry an `externalObjects` material remap into ANOTHER FBX, so `grep -rl | head -1` returns a plausible false positive (it put two passes of Rhino jets on a placeholder hull a fifth the ship's height); resolve with `grep -c "^guid: $g"` per candidate and cross-check against something the prefab authored; (2) **a nested prefab instance is reachable TWO ways** — `m_TransformParent` in its modification block ALWAYS, plus an entry in the parent Transform's `m_Children` **iff that parent is a plain (non-stripped) Transform**; the Squirrel's jets are the exception that structurally cannot carry one, and generalising from it is what produced eight unreachable Rhino jets; (3) **model vestiges** — and "referenced by nothing" is measured against PREFABS, so it misses the case that matters: an `AnimatorController` references CLIPS by a model's guid, and `urchan_shapekey_with_animations.fbx` (7 clips → 9 vessels) and `SparrowModel4.fbx` (the Missile Launch states) have zero prefab references and are **load-bearing** (§7.1). Six genuinely-dead models plus a duplicate Manta controller were retired in 2026-08; `RhinoModel.fbx` is HELD as the only copy of the Rhino's pre-subdivision wings. **The finding that outranks the rest: a LABELLED shape is not a SHAPE.** `rhino_shapekey_with_animations` and `urchan_shapekey_with_animations` carry four element blend shapes that move **one vertex by zero** — verified across all 93 FBX blobs in the project's whole history, so the Rhino has never had a morph anywhere — and the morph audit now measures **magnitude relative to the mesh's own diagonal** (real shapes 2.46–17.94%, these 0.0000%) so a swap reports "LABELLED BUT INERT" instead of a green. `dolphin_shapekey_with_animations` carries a REAL morph AND is the shipped hull **in the same place, no offset**, plus six exhaust nozzles `Dolphin_Test` scales to 0.00095 units and never draws. **A rig swap is THREE jobs and the tool only did one** — move the geometry, remap the MATERIALS, make the ANIMATION rig-safe — and passing the first is no evidence about the other two, both of which only fail on screen: the Dolphin swap passed every asset check (bones, colliders, morph, reachability, zero dangling refs) and flew broken. Its `.meta` had `externalObjects: {}`, so it wore its Blender materials and the accent that should carry the DOMAIN colour rendered emissive orange at 10x; and submesh ORDER is NOT a convention (Sparrow Domain at 1, Manta family at 2, this rig's body at 1), so `VesselCustomization._domainMaterialSlot`'s default of 1 painted the team colour over the whole body — use `_domainReplacesMaterials`, which resolves by material IDENTITY. Separately, `VesselAnimation` had grown rest-relative ROTATION for rigs and never the POSITION half, so an absolute `localPosition` write — fine on root-parented art — flung each of six engine bones 1.7u along a different axis, because a bone's position is relative to its PARENT and Blender rests every bone at `(0, boneLength, 0)`. **Two further things a rig can break that every per-part check is blind to** (§4.6.1/§4.6.2): a rig can DELETE a parent/child relationship the old art gave for free — the Dolphin's wings and engines used to hang off the animated Chassis and inherited its `(25·pitch, 25·yaw, 25·roll)` turn, while the rig puts them on a sibling branch of `fuse`, and since their own pitch input is `Brake(throttle)` (zero unless braking) they went dead on that axis; put such a term back by COMPOSING quaternions, never by adding Euler angles (6.43° apart on an ordinary stick pose; composed it reproduces the legacy hierarchy to one ULP). Its corollary is the expensive one: **a missing composition term reads exactly like a wrong sign**, so three passes of per-axis `-1` scalers were tried first and every one was wrong on playtest — `RiptideAnimation` now carries none. And **a reference frame is a value, not a place in the hierarchy**: the drift's Course-aligned frame was a `DriftHandle` Transform parented under the vessel, so the hull's own aiming carried it between one frame's write and the next read and re-pointing only its FORWARD axis left 24.6° of accumulated twist on parts commanded to hold still (`LookRotation` and a rotation-minimizing re-aim fail the same way — the property only holds if nothing else writes the frame). It is now a value rebuilt every frame — the hull re-aimed onto Course by the shortest arc (`FromToRotation(forward, Course) * rotation`) — which cannot accumulate, keeps the hull's up (so the parts read as a dolphin flying straight down Course, up toward the camera), and injects zero roll; freezing the entry orientation instead is stable but holds where the ship WAS. Its corollary was REVERSED by measurement (§4.6.5): **the drift cage holds ONE frame — the Course-aligned one — for positions AND orientations**, because the legacy DriftHandle re-parent did both, and holding only orientation in the course frame while positions rode the aiming hull swept the cage sideways as the pilot aimed. `CaptureRestPositions`/`MovePartFromRest` now anchor through capture-time vessel-frame constants carried by that one explicit frame — the earlier home-parent/current-parent split is deleted with the re-parenting it served. **A scale-free quantity is only scale-free if the value and its BASIS are measured in the same space** — the Dolphin's drift clearance was re-based on `PrismOcclusionCorridor.MeasureCircumscribedRadius` to escape a disputed `Lcl Scaling 100`, and that helper measures a skinned hull through its ROOT BONE, which is exactly where the disputed factor lives, so the offsets came back ~100x and threw the parts off the ship (§4.6.4). Prefer a basis derived from the thing you are about to modify — and when the ambiguity can be MEASURED AWAY, do that instead: §4.6.5 pins this rig's import model against the colliders the swap itself preserved (a conjugation by 0.01·mirror-X; bones keep lossyScale 100 while their WORLD poses land on the hull, residual 3e-5), which makes plain world units safe and turns every drift offset into a measurement — the proven 2.2-unit wing lunge the fraction scheme's own clamp could not express, the legacy CHASSIS CHAIN reproduced in vessel space (flight 17: the wings and six boosters ORBIT the chassis pivot and swing about bleeding-edge's own shared seats — all six engine cases authored at (0, .147, −2.047) and lerped to (0, .15, −1.7) every frame, both wings (0, 0, .1) → (0, 0, 0) — so every rig vertex lands where the legacy vertex resting at the same point would, proven to 1e-15, and at zero input it reduces to exactly the +0.347 forward booster drag flight 16 had measured plus the 0.1 aft wing drag it missed. Sixteen flights tuned the SEAT: flight 16 landed it on the reference to the digit and the playtest still read worse, because composing a parent's rotation into a child's ORIENTATION reproduces how the parent turns the child and nothing about where it CARRIES it — a hierarchy is a MOTION, not a term, and the seat was the wrong dial; the roll mirror flights 8–15 carried is an opt-in, off, because the construction it was asked against was wrong for its own reasons), the drift cage holding the COURSE frame for positions as well as orientations (the old DriftHandle re-parent did both; holding only orientation reads as a wing translating sideways while claiming to fly straight), a capture-time rest anchor (a live read through an animated parent applies the chassis term twice on legacy art), and a FromToRotation antipode guard (aiming fully backwards whips the unguarded frame at 2× the wobble rate). **A magnitude is only comparable to numbers from the SAME tool**: `mesh.bounds` on a SKINNED mesh is the culling volume in ROOT-BONE space, not the raw vertex AABB, so the in-editor audit and the offline mirror report the same Dolphin shapes 1.283x apart — identical to four digits across all four, i.e. a denominator difference, not a discrepancy (the same root-bone trap the occlusion corridor records). Neither is wrong; do not reconcile them. A rig's REST POSE is not the shipped pose: the Rhino's fuselage is a rigid +1.5545 z (all 29 lathe rings) but its wings sit 1.38× wider, and the Urchin rig is a uniform **2.105× scale** — each rig instance is therefore placed at a FITTED transform so every collider and FX mount keeps its world position and nothing is re-measured. Manta/Falcon/Shrike/Termite each draw **two coincident skinned hulls** from two files (same export at 100x different unit scale) — measured, deliberately NOT yet ruled on. **(4) geometry the importer DISCARDS** — `"... is self-intersecting and has been discarded"` is a report of MISSING GEOMETRY, not console noise: twelve 13-gons on the Dolphin rig each carried a ZERO-LENGTH edge (two adjacent corners indexing different vertices at the identical position) and Unity dropped all thirteen sides of each, 0.263% of the hull. **Measure the ACTION a warning names, not the ratio** — "one face out of 11,113, 0.009%" is true and answers the wrong question. Repaired by dropping the redundant CORNER (UV-lossless; both corners carry the same UV, and which normal survives is measured against the polygon's own Newell normal), scoped to exactly what the importer discards — the same mesh's twelve ZERO-AREA triangles are kept by Unity and deliberately left alone. The reusable technique is **prove a rebuild rule against the SHIPPED artifact before relying on it**: `Edges` had to be rebuilt rather than patched, and emitting the first occurrence of each undirected vertex pair reproduced the shipped array exactly — length, order and values — which is what made the rebuild safe (§4.7). `Tools/Build/repair_fbx_selfintersecting_polygons.py --check`. **Read before wiring a vessel prefab, swapping or deleting a vessel model, or placing anything on a hull by measurement.** Tools: `FrogletTools ▸ Vessels ▸ Swap Vessel Rig` (the writer), `Audit Vessel Construction` (guid ownership + nested-instance reachability + duplicate hull draws), and the offline mirrors `Tools/Build/measure_vessel_models.py` / `measure_vessel_prefabs.py`. Follow-up order of work + the salvage-before-delete gate: `Docs/VESSEL_CONSTRUCTION_FOLLOWUP.md`. |
| `VESSEL_TAIL_AND_JETS.md` | `Docs/` | The **tail** and **jet** contract: the platform has THREE things streaming off a vessel and they are not interchangeable — the **trail** (conserved prism mass), the **tail** (one long beacon on the vessel ROOT, placed **behind that vessel's own camera**, tuned for OTHER players to find you) and the **jets** (short plumes on the model's ENGINE nodes, tuned for THIS pilot to read their own thrust — but **not hidden from anyone**; both layers draw on every machine). Both are standard, expected parts of every vessel and both take the vessel's **domain** exactly as the trail does. **A TAIL is defined by its PURPOSE, not its owner** — the Sparrow's **skyburst missile** carries one (§4.2), the first non-vessel to: it crosses ~360 u in three seconds and a hit is worth 10-30 points, so everyone in the arena has a reason to see it coming and know whose it is. It nests the SAME shared prefab (never a copy) while its two missile-specific numbers are MEASURED per flight off the geometry the hit sphere is already fitted to — the mount is the model's rear face, the width is `0.4 x the round's own body diameter` — because the round swells 14x-38x with MASS and a TrailRenderer's width is world-space. Two things a pooled round needs and a vessel does not: `Clear()` at launch (world-space points, so a reissue otherwise draws one straight ribbon from the last detonation to this launch bay) and a **detach-and-fade at retirement** (the round is switched off 0.025 s after it detonates, which would blink several hundred units of live ribbon out in one frame — continuity of existence applies to a tail exactly as it applies to a prism). A **jet** stays vessel-only: it is a readout of an engine. Documents the marker components (`VesselTail`/`VesselJet`), the one vessel-level owner (`VesselTailAndJets`), why **width is per-vessel** (`widthScale` = `\|followOffset.z\| / 20`; a TrailRenderer's width is world-space and ignores transform scale, and the fleet spans a 37x camera range), the tail rule (`z = -1.05 x \|followOffset.z\|`), the per-vessel mount table with how every number was MEASURED, and why five rig-swap vessels have tails but no jets. **Read before adding, moving, resizing or recolouring any vessel FX — and before using the word "trail" for something that is not prisms.** |
...
| `PartySystem/` | `Docs/` | Party (Relay) layer: `ARCHITECTURE.md` (locked design, investigation Q&A, error-handling matrix, exit criteria), `REFACTOR.md` (active backlog + deferred items + per-commit protocol), `BUGS.md`, `TESTS.md`, `TODOS.md`. EAGER per-user Relay session is the locked design. **`SPECTATOR.md`** — direct **Join** and **Spectate** off the friends panel's online row, both riding a `partySession` presence property and the ordinary client-join transition. **A spectator is a Netcode client with NO Player object** (approval payload `cosmicshore.spectator.v1` → `CreatePlayerObject = false`), so nothing that reads `gameData.Players` ever sees it; the two ready gates subtract it via `SpectatorSession.CountHumanClients`; `SpectatorController` owns the veil fade, the follow/dolly cameras, the overlay and every exit (close, Escape/Start, the watched match's `OnMiniGameEnd`). **Read before touching connection approval, the ready gates, or anything that counts `ConnectedClientsIds` as pilots.** |
...
| `MaelstromSystem/` | `Docs/` | Maelstrom mode (`GameModes.Maelstrom = 36`): `ARCHITECTURE.md` — session-level meta drawing a random mode + intensity per round from an authored SIXTEEN-mode pool via sequential `Single` loads, the cumulative **intensity ladder** over that pool (§1.1 — which modes each intensity unlocks and why, ordered by pick-up-and-play difficulty), the admission criteria and why Astro League and Brood Rush fail them; network-free standings folded from the synced `GameDataSO.Results` by the persistent `MaelstromController`; host-only Continue→hub→Summary end-game flow (summary-vs-hub keyed off the authoritative `IsShuffleComplete`, race-to-6); **§1.3-§1.6 the hub rework** — the draw moved to hub entry and replicated as a `MaelstromRoundTicket` on `Player`, the one-deadline ready-up (30s, snapped to 3s when everyone is ready), the retirement of the never-placed `MaelstromLobbyNetwork`, the three screens (hub / stats / summary), the `Cell.RequestCellSwap` preview, and the AI roster dealt once with its DOMAIN; `MaelstromDataSO` data + file index. |
| `ArcadeLaunch/` | `Docs/` | The arcade card's **one launch panel** — the configure-then-pick-a-vessel pair of screens collapsed into a single panel per card, because every arcade mode locks to one hull so the second screen had nothing left to ask. `ARCHITECTURE.md` records: the two panels and why there are exactly two (`MinigameLaunchPanel` vs `MaelstromLaunchPanel` — a clip, no controls block and a pool list in their place, each following from *the meta-mode draws OTHER modes*); the split that keeps it safe — **the panel owns its widgets, the modal owns the decisions**, so config validation, the network commit, ready-up and launch stay in one place and a panel never writes `ArcadeGameConfigSO`; **why every control resolves through ONE accessor** (a per-control fallback would arm a live satellite arena into a leftover Screen-1 frame nobody can see); **the commit moving to card-open** now that no Confirm button remains (silently — the sting acknowledges a press, and there is none); the controls block — **authored rows resolved per MODE from ONE asset** (`ModeControlsLibrarySO`, `Resources/ModeControlsLibrary`, shipped EMPTY so a card shows its vessel's designated abilities and nothing else; the always-on stick primer was retired) then **ability rows DERIVED** end-to-end the way the ability lockup is, so a wrong label is structurally impossible and a passive honestly draws no chip and no recharge; **the icon animates the way it animates in the GAME** — the lockup's own three beats (press flash → the clockwise depleting recharge veil → the ready flash) with every colour read from `Resources/AbilityLockupStyle`, so the preview cannot drift from the HUD, and with the two details that look like bugs if you get them wrong carried over verbatim (`fillClockwise = false` IS clockwise for a veil that depletes; the overlays are siblings drawn after the icon, never children, or they inherit the scale the row animates); **kicking an AI is LOWERING THE PLAYER COUNT** (the bots spawn in the game scene from `RequestedAIBackfillCount`, so the count is the only representation that cannot go out of step with what spawns) and **ready lights are a COUNT, not an identity** (the sync layer replicates how many confirmed, not which); the preview following the intensity row via `ModePreviewDefinitionSO.PreviewCellsByIntensity` — **rebuilt only when the arena would actually differ**, since standing a satellite costs a cell build and a networked hull swap, with Rampage excluded deliberately (identical forest, and a satellite has no `CrystalManager` to draw the crystal count that varies); and the Maelstrom's **cumulative** `MaelstromDataSO.IntensityTiers` ladder, where a tier lists what it ADDS and an empty ladder keeps the legacy every-mode pool so an un-authored asset is never unable to draw. Note **Skim Race IS SkimRace** (`ArcadeGameSkimRace.asset` carries that DisplayName) — a pool naming both is naming one card twice. It also records the **OBJECTIVE BOX** — the metric's icon (from `ModeControlsLibrarySO.ObjectiveIcons`, the one metric→sprite table) plus a counter that only ever counts UP, deliberately with no target — and its twin the **micro toast**, a "+N ⟨icon⟩" popped where the in-game toast feed sits; both fire on ONE session event and both flash the scoring player's **live domain colour**, so they cannot disagree and a re-picked domain recolours the next tick. The beside-the-window preview HUD (title, objective sentence, progress, countdown) is RETIRED in its favour. Holding an ability's physical control **lights that row** in the controls block while the preview has focus. Authoring lives in **FrogletTools > Game Modes > Mode Map**. §3.2 is the card's MEMORY: a card re-opens on what it was last LAUNCHED with (intensity, placed AI, domain count, own domain, own hull - `LaunchPreferenceStore`, one record per mode, written on a launch and never on a ready press, every field re-validated against the card, the unlocks and the party before it is seeded). **Read before touching `ArcadeGameConfigureModal`, adding a launch panel, or changing which modes the Maelstrom draws.** |
| `HomeHub/` | `Docs/` | **The home screen's four ways to play** — Mission, Toy Box, Arena, Arcade — and the app-shell **Toy Box**. `ARCHITECTURE.md` records: a hub entry names a modal **TYPE** and lets `ScreenSwitcher.OpenModal` find it, never a direct reference to the window (the switcher already owns the modal stack, the return-to-modal pref and the close sweeps, so a button that called `ModalWindowIn` itself would be a second authority on a modal's lifecycle); **availability is a STATE, not a missing button** (`MenuHubButton`: `Locked` = "this exists and you cannot open it yet", which is Arena, whose modal behind the lock is real and complete; `Unavailable` = "this is not built", which is Mission — an entry that is simply not drawn tells the player the game has three things in it, and the day it ships they have to re-learn the screen); **Arena is the arcade pointed at a different roster** (`ArcadeExploreView.rosterOverride`, one accessor — a parallel screen would have to re-derive progression locks, favourites, party picks, the daily challenge and the launch modal, and would drift from all five); and the **Toy Box driving the LIVE toys** — every card is an `IToyShellSurface` registered by a real toy at the cell membrane, so "change your domain" in the menu is literally `DomainChangerToySet.Apply` and no table of toy actions exists to fall out of step, with the 2D art resolved from the encyclopedia's own baked emblem portraits (`ToyPortraitLibrary`). It is **two windows**: a grid of toys, and a detail window carrying that toy's **variants** plus two verbs — **Navigate** (go and fly it) and a commit button **the toy captions** (`ToyShellOption.CommitVerb`: SWITCH for a world, SPAWN for a lifeform, START for a run — a caption table in the UI would be a second opinion about what a press does), which is drawn only for a list with something to commit and lit on arrival when a layer offers exactly one thing. **Every variant has a picture** (`BuildPreview` — a painting in miniature, a species with its element's heart, a cell's scale model) and **a Spawn is WATCHED** (`WatchAfterApply` + `WatchRadius`: the window turns onto the creature where it landed, because the thing blooms in from zero and has to state its own size). The Spawn Matrix offers Fauna and Flora flat and keeps its Vessels hangar for the ring. Back and gamepad B pop ONE layer and close only from the top layer. The preview camera's cost is its FAR PLANE (a few toy-distances, not forty — it was drawing the whole cell to show one ring), and the cards draw the arcade's two plate sprites re-authored at 4x with a 9-slice border (`Tools/Build/author_toy_card_sprites.py`) while `Tools/Build/author_toybox_layout.py` lands the type bands and the 400x250 card layout on the branch and `HomeHubWiringWindow.ToyLayout` writes the same numbers from the editor (`Docs/HomeHub/ARCHITECTURE.md` §4.1.3–§4.1.8). **A 9-slice draws against the CANVAS's `referencePixelsPerUnit`, not only the sprite's PPU** — Menu_Main's is 240, so the cards' sliced plates need `pixelsPerUnitMultiplier = 240/100`, read off the canvas by both authoring paths (§4.1.8); and the preview camera must keep the gameplay volume's post-processing, because every lifeform material is HDR-emissive and renders as a white silhouette without the tonemapper. A row also has a PLACE (`ToyShellOption.WorldAnchor` — picking a domain turns the picture onto that domain's switch) and a Spawn is SPENT (the row deselects after the release, so the button re-arms only on a fresh pick). The second verb was removed once and brought back deliberately: the menu never gets its own copy of what a toy does — every row calls the toy's own `ToyShellOption.Apply` — but a player who does not want to fly had no way to change their domain at all, and "go and fly for it" is a tax rather than a design principle. **A row SELECTS and Switch COMMITS, except where the toy says otherwise** (`ToyShellOption.AppliesOnSelect`, set only by `SwapToySetCoordinator`): that is not a UI preference but what these applies COST — a cell swap suctions the world away and grows another behind a veil, where a domain change is instant and undone by picking another row — and it is declared by the toy for the reason `ToyDefinitionSO.Category` is, so a menu can never hold a second opinion about it. Switch is drawn only for a list with something to commit, so the domain changer never shows a button that could never light up. A selected variant can show **what it would give you** (`ToyShellOption.BuildPreview` — the cell selector's own cached scale model, the vessel changer's own mini hull, so the flat preview and the world station cannot drift), built on a private stage far outside every gameplay camera's far clip and framed by MEASURING it; most options build nothing and the window then keeps photographing the toy. Options **expand** rather than act where the toy is already a tree, with the way back out a synthesized row rather than an authored control, and an option that only means something with the player flying (Wanderway, Arkway, Connect the Dots) enters freestyle first — waiting on `OnGameStateTransitionEnd`, never on `IsInFreestyle`, which flips at the START of the transition while input is still paused. Two lifetime traps it answers: **a destroyed toy still passes `!= null`** (the field is typed as the interface, so the null check is a plain reference comparison and reading `ShellAvailable` then throws), and **switching a cell from this window destroys the toy that offered the switch** — now the ordinary path, so the window re-binds off `ToyShellRegistry.OnChanged` by DEFINITION (an asset, the one thing about a torn-down toy that survives), falling back to display name for the code-built default toybox. **TWO BIG BUTTONS sit above the toy grid**, the arcade's shape with the Toy Box's own two (§4.0): **today's activity** (`DailyToyActivity` + `DailyActivityCard`) and **shuffle** (`ToyShuffle` + `ToyboxShuffleCard`). **ONE flag that already existed divides them** — `ToyShellOption.RequiresFreestyle` means *this only means anything with the player at the stick*, which is exactly the difference between an ACTIVITY and a SETTING — so the left button's pool is the `RequiresFreestyle` leaves and the right button's is everything else, neither carries a list of toys, and a toy authored tomorrow joins whichever button its own declaration puts it in. The activity is the weekly challenge's two rules one feature over: **the definition is DERIVED** (`FNV-1a(UTC day key) % candidates` over the LIVE toys' options in registry-then-option order, so it resolves cold, offline and identically everywhere with nothing authored — an authored `Connect the Dots > Rainbow` catalog would be the second authority on what a toy does that this window exists to avoid) and **only the claim is STORED**, in cloud-merged `PlayerDataService.UnlockReward("toy-activity:<day>")` so a reinstall cannot re-earn a day. **Paid for TRYING, not finishing** (a painting has no pass mark, a wander has no end): the press opens the activity's toy with it SELECTED and the reward lands on the commit, through `ToyConfigureModal.BindToPath(surface, path, onApplied)` — an opaque `Action`, so the detail window knows how to apply an option and nothing about days or crystals. The shuffle **plans, then applies, `ToyCategory.World` LAST**, because a cell swap tears the toybox down and unregisters every other surface, so a shuffle that decided as it went would decide against destroyed toys halfway through; it picks by a random DESCENT rather than enumerating leaves (the Spawn Matrix is three layers deep) and prefers a leaf that is not where the player already is, since the cell selector does offer the current world. **Both buttons sit OUTSIDE the scroll view, unlike the arcade's**, so the grid's extent cannot move them — the trap §4.1.8 records, avoided rather than inherited — at the stated cost that they do not scroll away. Authored by `Tools/Build/author_toybox_top_buttons.py` (`--check`, `--self-test`, four layout negative controls) plus the optional `Resources/ToyboxDailyActivityConfig`. Two authoring traps worth more than the feature: **`set_field` is a regex REPLACE, so binding a field Unity has never serialized does NOTHING** — Unity writes only the fields a component had when the scene was last saved, so a field added to a C# class is absent from every saved document and the replace matches nothing, which shipped two perfectly-built buttons bound to a modal that had never heard of them (the authoring now ADDS the key); and **a cloned Button brings its donor's `onClick`**, so `m_OnClick` is cleared and neither card carries a `MenuAudio` — the press sound is played from `ToyboxModal` on the code path, where it cannot sit in front of the card's own runtime listener and eat it. §6 records the arcade strip that shipped with it and the two things that look legacy and are not (the PC/DC steppers are live; `UsesLaunchPanels` survives as the two-instance gate for the Maelstrom window's panel-less copy). **§3.7 is the card's GENRE PETAL** — one element in every arcade/arena card's bottom-left corner saying what KIND of game it is: a race is TIME, making mass is MASS, destroying it is SPACE, working other pilots over is CHARGE. Keyed on the **MODE, with the METRIC as the FALLBACK** (`ModeGenre.TryElementsFor`), and the fallback is `ObjectiveIconSetSO`'s own rule one surface over — a mode's genre is usually predictable from what it is scored on, so a new mode picking an existing metric gets a badge for free. **It is not the SAME fact, and the two places they part company are the reason the table exists**: Tollway and Scarab Scramble are both `Goals` and are not the same kind of game (one is a ball race, the other acquisition — a toll is paid in mass and raises a 255-prism monument, so the arena ends the match built out of the scoring), and Scurry is `Crystals`, which reads as a race and is a gather. **A genre can be TWO**: Brood Rush lays claim mass inside the nucleus and tears the other side's out, so its card says MASS *and* SPACE, drawn as a second petal ABOVE the first — stacked rather than beside, because nearly every card has one genre and a horizontal pair would move the primary on all of them. It shares **FavoriteIcon's vertical centre**, so the petal and the star read as one pair in opposite lower corners, and at 2.5x the fleet's original size that costs three stated overlaps: 11.37 px below the card rect (bounded — the 313x208 `Background` plate reaches 11.48 px below it, so the petal lands 0.11 px inside the plate and draws over the `Border` frame as a badge clipped to the corner), the party-pick chip row, and — on the one two-genre card — the title band. None can eat a press; both Images are `m_RaycastTarget: 0`. Two is the ceiling: a badge needing three is a mode whose genre nobody can state. It is a RULE so it lives in CODE (the `CosmicShore.Data` leaf assembly) rather than an authored table, and `ModeGenreTests` sweeps every metric, pins every row of the mode table, and asks the SHIPPED rosters and preview library exactly what the card asks them — because a mode with no genre fails SILENTLY (the card simply draws no petal). Art is the fleet's own `ElementalBarsConfigSO.GetPetalSprite` tinted the ability lockup's level-5 white, because **element identity is SHAPE** and a colour channel would only compete with the card art. Maelstrom draws none, correctly — it draws OTHER modes, so it has no genre. Its real lesson is that **`GameCard.prefab` is not what runs**: the live cards are 36 scene-local objects in Menu_Main and both `Arcade Screen` prefabs are referenced by nothing, so `Tools/Build/author_game_card_genre_petal.py` (`--check`) writes both petal children onto every live card AND the prefab — `Docs/GAMECANVAS.md` §9's finding, one prefab over. **§3.8 is the card's BACKGROUND**, which should picture that mode at intensity 2 and measured did not: 22 of the 25 live cards shared FOUR legacy images from the retired single-player era (sixteen of them all wearing `GameCardBackground_Rampage.jpg`) and three had none. It is **rendered, not captured**: an arena is data, so `Tools/Build/render_card_backgrounds.py` resolves each live card's intensity-2 arena off its own scene and `Tools/Build/card_art_harness/` COMPILES AND RUNS the shipped generators (RUN tier - the lay list reproduces the documented counts, Swell 14,277 / Switchyard 3,978 / shells 24,966), the shipped course generators (COURSE), or models what only exists at runtime (MODEL: a grown forest's planting as the preview measures it, a court off its controller's settings), then rasterizes it in the live palette with pilots and each mode's signature act as staging; Maelstrom's card is a MONTAGE of the modes it draws. Byte-deterministic, so `--check` fails the moment a generator, course, palette or spawn profile moves a card. A new mode's card is the **`/cardart` skill** - a step of `/arcadegame` and `/arenagame` - and an unknown card makes the renderer raise. `author_card_backgrounds.py` stays the import half (Sprite meta, rewire, `--strict` coverage now 25/25, hand captures opted out by name through `HAND_CAPTURED`). Traps it paid for: a `CapsuleMembrane`'s radius is a FIELD, not its scale (forests first modelled in a one-unit cell), a primitive array serializes as ONE hex blob, and the nucleus is glass. **§3.9 is ARENA SEATING** — six seats, one pilot per hull, a server-arbitrated lobby hull claim, every AI seat a placement, and the D-pad `PilotSwap` into an AI teammate's hull (with the three playtests that shaped it, the last of which was a hull-handover throwing on the Urchin's absent HUD). §3.6 and §4.1.7a are the hub's MEMORY: the arena carousel re-opens on the hull last flown on that card (still to be confirmed), and a toy's window re-descends to the variant last COMMITTED (`ToyPreferenceStore`, keyed by definition name, a path of labels; it selects and never applies, and only from the grid). **Read before adding a home-screen entry, a modal opened from one, or a toy that should be reachable from the menu.** |
...
| `SCURRY.md` | `_Scripts/Controller/Arcade/` | Scurry game mode technical reference |
...
| `CLEAVE.md` | `_Scripts/Controller/Arcade/` | Cleave technical reference (Rhino-only slicing race). **Read before adding an arena to a mode whose intensities are different PLACES, and before trusting any offline budget model** — it records why nothing in the mode may be shielded (an AI can never energize a blade, so hardened mass is mass an all-AI domain cannot score against), the one shared envelope four independent arenas are built to, the measure-by-running harness that replaced an analytic model, and the doubled `E[k^3]` jitter factor that made the old volume ladder inert. |
| `SCARAB_CRYSTAL_MORPH.md` | `_Scripts/Controller/Vessel/R_VesselActions/` | The Scarab's omni-crystal retirement — the crystal CLOSES into the ball it forges instead of shattering, and the platform that makes a per-vessel retirement possible. **Read before touching `Crystal.Explode`, the spent-crystal husk, `ScarabBallForge`, or any vessel's crystal pickup visual.** Records why a convex-HULL projection replaces the Squirrel's exact panel→face census (320 facets against 64 panels — there is no coincidence to exploit, and a hull needs no census), why the retirement rides the forged object's own replication rather than new networking, and why the husk suppression travels on the payload the crystal manager already broadcasts. Also the two platform pieces it needed: `Crystal.CollectPose` (a pickup is serviced by TWO unordered trigger callbacks in one physics step and one of them respawns the crystal synchronously, so the live transform is the crystal's NEXT home in one of two arbitrary orders — and across the wire, nearly always) and `ExplodeParams.SuppressHusk`. Feel is ONE asset (`Resources/CrystalMorphConfig`), never a per-prefab duration — the same rule `Docs/ECOSYSTEM.md §31` records for crystal capture, at the same 0.44 s beat |
| `SERPENT_SNIPER_SCOPE.md` | `_Scripts/Controller/Vessel/R_VesselActions/` | The Serpent's **scope** (Space, LT, analog zoom on the trigger's own depth) and **sniper shot** (Charge, RT while scoped). **Read before adding a camera VANTAGE, before magnifying the view from any ability, or before writing a second force that breaks super-shielded mass.** It records: **the magnification is a SECOND PICTURE, never the flight view** — a round eyepiece drawn beside it while the pilot keeps flying — because the first cut took the gameplay camera into the cockpit and narrowed its FOV and read as *nauseating*, and **a magnified view is a lever on every motion that reaches it** (the pilot's turn, the hull's roll, the camera's settle and the speed tunnel's own narrowing all multiply by the same ~4x as the target); so **the zoom composes with NOTHING but its own trigger** — the two terms removed to get there were each defensible, one a design idea (a stick-driven 'Steady Eye' bleed, whose Space-5 upgrade existed only to switch it off and was re-cut to **Deep Focus**, one more magnification step) and one a genuinely correct composition (the speed tunnel going on narrowing the scoped base), which is why 'compose magnification with nothing' is the rule rather than 'do not compose it with the stick'. That inversion DELETED `VesselFirstPersonView` and `CustomCameraController.FirstPerson`/`FirstPersonOffset`, and left the speed-tunnel home-FOV override (`Docs/SPEED_TUNNEL.md §2.1`) standing with NO caller as a guard rather than a feature — a direct `Camera.fieldOfView` write is still baked in permanently. The window is the `ConnectingArenaPreview` shape (runtime RenderTexture camera, never tagged MainCamera, left disabled and stepped by hand) drawn into a `RawImage` in the top left — round 4 made it a round `MaskableGraphic` with a square target, round 7 put the `RawImage` back, and round 8 re-shaped it into the CHARGE petal by SUBCLASSING that component, for the reason below. It also records the eye MEASURED at 1.05x the vessel's own circumscribing hull radius (so a constant cannot be inside one ship and ahead of another) and the RIGID attach — any lag at all puts the camera inside the hull it is trying to see past, and the usual look vector is ~zero in the cockpit, so it aims along the vessel's OWN forward, which is the same vector the shot is cast along, making the reticle a promise rather than a decoration; the **CONTEXTUAL right trigger** — the shot and the pre-existing cloak are both bound to RT and each asks `IsScoped` rather than learning the other's wiring, with `IsScoped` maintained on EVERY peer because both resolve on every machine and a local-only flag would have run the cloak AND the shot on the same press; the hitscan being an angular **CONE** rather than a ray or a tube (`PrismSpatialIndex.QueryCone`, the tapering sibling of `QuerySegment`) — the query tests a prism's CENTRE and a prism is several units across, so a mathematical line misses almost everything it visually passes through, but **a radius is a WORLD distance while a pilot aims in ANGLE**, so the first cut's 4 u capsule was a blunderbuss at the muzzle and 0.076° (≈7 px in the scope) at its 3,000 u reach; a cone covers a CONSTANT on-screen area at every range, which is what lets the reticle be drawn at the beam's TRUE size, and it is deliberately **NOT a function of zoom** because the zoom is a locally smoothed value while the shot resolves on every peer, so destruction riding it would desync conserved mass; its unordered snapshot must be SORTED along the axis before anything dies; and the sanctioned super-shield teardown reused verbatim from the Rhino's blade. Three more findings came out of its first playtest and each generalises: **a push into a display is only as visible as the display** — the readiness veil was correctly driven the whole time and the Serpent binds 0/4 ability icons, so it had no icon to sit on (answered by a GENERATED scope overlay whose reticle is a MEASUREMENT — the cone's half-angle projected through the camera's LIVE field of view, so it tracks the zoom and the speed tunnel); **a hitscan has no visible output unless one is drawn**, so the shot now leaves a domain-coloured tracer at the cone's own radius plus an impact flare, fading rather than vanishing and drawn on EVERY peer because the press already replicates (it shares ONE colour resolver with the reticle, so the mark you aim with and the mark you leave cannot disagree); and — from its SECOND playtest — **an indicator whose only rendering is its VALUE has no rendering at its extremes, and its extremes are usually the two readings the player actually needs**: the recharge arc was a bare fill, so it drew NOTHING on the frame the shot fired (`Sweep01 = 0`) and nothing once it was ready, which is indistinguishable from having no readout at all — answered with the goal stack's own rule (`Docs/GAME_MODE_TOPBAR.md`, *a progress bar needs a BED*): a dim full ring the fill runs over, and READY drawn as a complete bright ring rather than as the absence of one; and the scope's window is built on that preview shape rather than as a revival of the retired `Pip` prefab, whose dangling border texture draws a navy quad over ~55% of the screen, with its cost (a genuine SECOND render of the world, only while the trigger is held) stated rather than hidden. **From its THIRD playtest came a fleet-wide UI fix**: `AbilityLockupView.SetAbilityCooldown` refused a LOCKED card, so this vessel's recharge — correctly driven from the day the ability shipped — landed nowhere for three rounds, because the Serpent binds 0/4 icons; the veil never needed an icon (it sizes itself on the ability PLATE, which a locked card has), and the rule is **an indicator that refuses to draw because its DECORATION is missing is indistinguishable from an indicator nobody is driving** — locked is a fact about the ART, while a slot being DRIVEN is an ability that exists. `SetUpgraded` is deliberately NOT changed by the same argument read the other way: on a locked card there is nothing to light. **From its FOURTH playtest — "i no longer saw the pip" — came the camera-adoption rule above**, and the timing is the lesson: the window had been drawing with URP's defaults since it first shipped, and it only read as MISSING once the inversion pointed it at open space instead of at the pilot's own lit hull. Nothing about the code path changed; the SUBJECT did — a bright object is unmistakable even rendered wrong, so it reads as low quality and nobody reports it, while a flat render of empty space is a dark circle on a dark screen. Two hardenings rode along, each a failure that would have presented identically: a ZERO hull measurement is no longer latched (`MeasureCircumscribedRadius` skips disabled renderers, so a hull asked before its art is on answers 0 and a latched 0 parks the eye inside the ship for that vessel's whole life — it re-asks while the answer is 0, warns once by name, and floors the eye in FRONT of the hull), and the render target is `Create()`d outright because the surface binds it in the same frame. **Its FIFTH playtest reported the same four words a round after a fix designed to make them impossible, and the answer was to stop guessing**: since round 4 retired the cockpit, the eyepiece is the ability's ONLY visible output, so ONE report — "it is not there" — covers four unrelated faults (never engaged / returned at a gate / ticked but invisible / drew and was covered), and three rounds were spent picking between them from source. `SniperScopeDiagnostics` is the house answer (`MouseFlightDiagnostics`' shape): **a refusal is an unconditional warning naming the gate** and **the happy path is one line on a channel that is off by default** — so enabling `CSLogChannel.SerpentScope` is the whole procedure for separating the first two cases from the last two, and there is deliberately NO rung for "it is being covered", because that is not decidable from source and an unconditional "everything checks out" warning becomes a permanent false positive the day the real defect is fixed (the guidance rides the happy-path line and names **Report On-Screen UI** instead). Two real defects were found reading for it, and the first is fleet-general: **`Canvas.enabled` and `GameObject.SetActive` are not interchangeable ways to hide a generated UI — only one of them lets its graphics rebuild afterwards.** A `Graphic` caches its canvas in `m_Canvas` and `IsActive()` is `base.IsActive() && m_Canvas != null`, so every `SetVerticesDirty` is a silent no-op while that cache is null — and `OnCanvasHierarchyChanged` nulls the cache and THEN tests `IsActive()`, which is false *because it just nulled it*, so it never re-caches in either direction. Hiding by the canvas therefore froze the reticle at its first-frame zoom and the arc at its first-frame sweep (the rim and backing survive on the pending `OnEnable` rebuild, which is what makes it look like a working instrument until you watch it move); `SelfCheck` asserts it through `IsActive()` and never through the `canvas` PROPERTY, whose getter re-caches on read and would heal the thing being tested. The second: **`IVesselStatus.ShipTransform` is `Vessel.Transform` with no guard** and `VesselStatus.Vessel` logs an error and returns null, so it THROWS rather than answering null and takes the whole instrument with it — resolved through a cached `ResolveHull` that falls back to the executor's own root. Everything else was measured clean off the shipped assets (both executors present AND in `ActionExecutorRegistry._executors`, `_shipInstance` wired, `Input: 2` intact, the tracer colour never black), and the registry row is the one to keep: `Get<T>` falls back to `GetComponentInChildren(true)`, so an executor missing from that serialized list would still `Engage` and still zoom while `_registry` — assigned only in `Initialize` — stayed null and took `_shot` and the entire overlay with it. **An ability that half-works is not evidence that its wiring is complete.** Its stated limitation is that the cooldown's element scaling is OWNER-LOCAL — element levels never replicate, so a non-owner enforces only a FLOOR, which can never refuse a shot the owner admitted; the exact fix is to replicate the resolved cooldown the way `NetEchoSightShape` does. **Its SIXTH report was the same four words again, and the pilot supplied the answer the source could not**: *"you were showing the pip fine when you had the vessel view in the pip ... there has been no pip since we have tried to swap those."* The window was on screen for rounds 1-3; round 4 did not re-point it, it REPLACED it — one change swapped the surface (`RawImage` -> a generated circular `MaskableGraphic`, 16:9 -> square, pivot and hierarchy with it) AND re-pointed the camera. **When one change replaces a surface and re-points it, the report cannot tell you which half broke** — "I don't see it" is the same sentence either way — so rounds 5 and 6 each found a REAL defect (URP camera defaults; a `Canvas.enabled` visibility toggle that freezes UGUI mesh rebuilds) and neither changed anything on screen, because neither was about the surface. Its corollary, which the Scarab records from the other direction: **three correct fixes that buy no part of one complaint means the thing being fixed is not the thing that is broken.** Round 7 restores round 3's surface verbatim — the same component, the same 16:9, the same half-screen height, the same 16/220 margins — and keeps only round 4's camera; `ScopeDiscGraphic` is DELETED (nothing else referenced it) so two windows cannot compete for the next report, and **why it never rendered is recorded as unknown rather than as diagnosed** — the geometry, winding, UVs and rect arithmetic all read correct, so the honest entry is that the pilot asked for the window back, not for an explanation. Two things are kept from round 4 because they answer real problems rather than being part of the substitution: the reticle stays INSIDE the panel (the middle of the screen is no longer the scope, so a reticle there would measure a view nobody is looking through) projected against half the panel's HEIGHT, and the opaque backing stays, so a panel pointed at empty space is still visibly a panel. **Round 8 re-shaped that window into the CHARGE petal — one petal of the element flower — and the method is round 7's rule applied to a re-shape**: `ScopePetalImage` SUBCLASSES `RawImage` and overrides ONLY `OnPopulateMesh`, so the texture property, the white-texture fallback, the material handling and the whole rebuild path are inherited and the only thing that changed is the vertex list. **When a component renders and its shape is wrong, subclass it and override the geometry — never write a new one**: a bad emit then reads as a wrong SHAPE, which a pilot can report, rather than as an absence, which they cannot. The outline is a MEASUREMENT of the shipped `charge_petal` sprite's alpha rather than a drawing (`ScopePetalGeometry`), and two facts came OUT of the trace rather than being put in — the two lower edges meet at **72.30°**, the same 72° the five-petal flower is built from, and the shape is symmetric to half a pixel. Three things generalise. **Positions ARE the UVs** when an outline is normalised over its own bounding box, which is what lets a shaped window sample a render target with nothing squashed and nothing cropped — the shape IS the crop — and is why the target went square (the petal's bbox is square to within 0.5%, and `fieldOfView` is vertical either way so the reticle's projection is unchanged). **A shaped window cannot bound its own furniture by its bounding box**: every radius drawn inside it is capped by the petal's measured INRADIUS (0.59× the half-side, set by the two long edges running down to the apex, not the top edge), derived in a static constructor so re-tracing the sprite moves the picture, the frame, the reticle cap and the recharge ring at once. And **a corner sharper than a right angle needs a MITER, not a bisector** — the 72° apex's miter is 1.70, so a bisector-offset feather would be 41% too thin there and the point would read as harder-edged than the rest of the outline. The BACKING is the same component with no texture, which is what makes the frame the petal's own outline rather than a rectangle behind it. One thing was deliberately NOT done: the recharge ring stays a circle INSIDE the petal rather than a band sweeping its perimeter, because a circumscribing ring needs 1.18× the half-side and pushes the instrument off the left of the screen at this window's authored margins, and a perimeter band is a SECOND new generated graphic — this window gets one change at a time. **Round 9 gave it a SECOND reticle, over the FLIGHT view, and the pair is ONE function** — `ReticlePixels(coneHalfAngle, tan(fov/2), halfHeight, floor)` handed each view's own half-height and own field of view, so *both circles are the same measurement through different optics*: the eyepiece's **GROWS ~8x** across the zoom dial because its field of view narrows while the cone does not, and the flight view's **stays ~5 px** because the gameplay camera's field of view is not the scope's dial (it does widen a little under the speed tunnel, which is the same measurement correctly following its own optics). Only the FLOOR differs and it belongs to the CALLER, because the two views want opposite ones — 6 px keeps the eyepiece's ring a ring at the wide end, while the flight view takes 3 px since it is SUPPOSED to be small and a generous floor there would inflate the very number the ring exists to state. **The flight reticle is PROJECTED at the point the shot reaches, never centred**, and that is not belt-and-braces: the camera is on the shot's axis only while it is exactly behind the hull, which the Serpent's authored `(0, 0, -250)` gives in the steady state and its own smoothing takes away through every turn — so a centred circle would mark the CAMERA rather than the round, exactly when the pilot is swinging onto a target. It is stood DOWN when the projection's `z` is non-positive (the rear view), because **`WorldToScreenPoint` behind the camera returns a MIRRORED position — a plausible-looking lie rather than an obvious failure**. Both reticles are built with the instrument and hidden with it, so a mark on screen can only ever mean *I am aiming*; the cone itself is untouched and every round 3–8 number is byte-for-byte unchanged. Its new diagnostic rung (`NoGameCamera`) stands the flight reticle down and leaves the eyepiece working, which is why it is its own reason rather than a refusal of the whole instrument — reporting it as "the scope window is not drawn" would send the next reader to the wrong half. |
...
| `URCHIN_REVERSE_FLIGHT.md` | `_Scripts/Controller/Vessel/R_VesselActions/` | The Urchin's reverse flight — ONE signed throttle mapping on the rail and off it, the magnitude/direction split that keeps `Speed` positive, the jets that swing end-for-end, and the attach seed that was right about forward attaches only. **Read before adding a hull that can fly backwards, before touching `VesselTransformer.ThrottleAxis` / `MoveShipScalar`'s publish, or before assuming `VesselStatus.Course` is `transform.forward`.** Records why `Speed` is a magnitude and `Course` carries the sign, why the jet flip is gated on a CODE capability rather than on the course dot (a drifting hull's course swings past broadside), and the negative control that first passed 1 of 3 because it was picked above the brake's own crossover. |
...
| `ONE_THUMB_MOUSE_CONTROLS.md` | `_Scripts/Controller/IO/` | The desktop mouse + keyboard scheme for the ONE-THUMB vessels (`SingleStickMouseInputStrategy`) — why a single-stick hull gets nothing from the dual-WASD mix, the **two-regime** stick model (`MouseVirtualStick`) — near centre a spring makes **mouse speed mean turn rate** (`deflection = drag px/s × unitsPerPixel / springPerSecond`, exact at any frame rate) and letting go straightens the vessel; out past `holdOuterRadius` the spring is exactly ZERO, so a sustained sweep parks the stick in the **hold annulus** and the vessel keeps turning with the mouse dead still (holding hard over: **228 px** against the pure spring's 917 for 3 s, and unlike the spring it does not grow with the turn). Committing costs TIME as well as distance (`holdEngageSeconds`), because hard over is only ~91 px at a responsive gain, so every ordinary flick saturates the stick and a position-only annulus latched on the first one and locked the vessel into a spin. `MouseFlightWidget` draws the stick as a centred reticle with that annulus as a ring, because the two regimes fly completely differently and nothing else on screen tells them apart. Also the pad-mirrored button map and why the shift keys are kept beside the mouse buttons, and why `XDiff` publishes the NEUTRAL 0.5 rather than full throttle. Carries two defects worth not repeating — a spring that only ran while the mouse was STILL (so any drag pinned the stick and no partial turn existed), and a dead zone applied to the ACCUMULATOR, which is a ratchet that blocked slow aiming movement outright and scaled with frame rate — plus the standing finding that **Invert Y is dead on every one-thumb vessel on gamepad, keyboard and touch**, because `DualStickMix` inverts only `YSum`/`YDiff` and those hulls read neither. §4.0 is the one that generalises past this scheme: **"which device is the player using" must have ONE implementation** — `InputDeviceIconSetSwitcher` detected by last meaningful ACTUATION while `InputController.SelectStrategy` keyed on PRESENCE (`Gamepad.current != null`), so a controller left plugged in gave the player ability chips that correctly named their keyboard and a ship that ignored it, with unplugging the pad as the only fix. Neither component was wrong alone; *the defect was that one question had two answers*, and no test can catch a second detector by calling the first — both now read `InputDeviceActuation` and `InputDeviceUnificationTests` holds it as a source law. **That fix was half a fix, and the other half cost five playtests**: the shared detector counted buttons, keys and clicks but NOT mouse movement, so a desktop player with a pad merely plugged in still could not fly with the mouse — `DetectInitial` hands a connected pad the family, a click won it back until the pad's resting stick crossed 0.25 (**drift qualifies**), movement was not evidence of anything, and every hand-over runs `ResetStrategyState` → `stick = Vector2.zero` so the mouse could not accumulate a deflection even on the frames it owned. Two changes close it: **sustained mouse MOTION actuates** (`MouseMotionActuation`, ≥120 px/s held 0.08 s — a jolt of one or two frames is a bumped desk, not a player), and the stick threshold is **asymmetric** (`StickThresholdFor`: 0.25 normally, **0.6 to take the family from an active mouse player**), because a stick off centre is the one signal here that worn hardware produces on its own. General shape: *unambiguous acts (a press, sustained motion) must outrank ambiguous held states*, and *taking a device back is a separate guarantee from taking it* — fix only the second and the handover is one-way. §4.3 records the roster and the reason it is a LAW: `IsSingleStickControls` is written by whichever transformer `TryGetComponent` resolves, i.e. by COMPONENT ORDER, so Falcon and Shrike carrying a second (inert, disabled) base `VesselTransformer` made them correct-by-luck rather than correct — both are excised and `OneThumbVesselCoverageTests` holds one-transformer-per-vessel. §4.1.1: **Escape is the OVERVIEW gesture** (`OverviewGesture`, Escape or pad Start), and each HUD answers it by invoking its OWN volume/pause button rather than reimplementing the overview, so the key cannot drift from the button; fullscreen moved off Escape to **F11**. A mouse pilot is therefore never handed the cursor mid-flight — the one control they needed it for is on that key, and the cursor is released only when the overview actually opens (both paths pause the input controller). §4.2 records why the scheme reports its own non-engagement (`MouseFlightDiagnostics`): when it declines, `KeyboardInputStrategy` still steers the hull off WASD and still fires every ability off the same keys, so "not engaged" and "broken" are indistinguishable on screen — *when a system's failure mode is that another system quietly covers for it, the system has to say so itself.* §6 is a SEPARATE, older bug it surfaced: **no vessel has ever shown a keyboard control chip**, because `InputHintBindingMap.BindingFor` answers a keyboard query with `KeyLeftShift`/`KeyRightShift` while `Resources/ControlGlyphSet` authors the label on the PAD row (one entry deliberately carries both representations) — now bridged by `InputHintBindingMap.Canonical` — and because the map had no keyboard control for the pad FACE buttons at all, leaving every such ability (the Sparrow's Mass and Time) permanently blank. The action keys are **QWER + Space**, assigned in reverse priority (the two mouse buttons are the highest, so only Space and R are needed), and `KeyboardInputStrategy` was moved onto the same keys rather than left on B/N — `ControlGlyphSetSO` authors ONE `keyboardLabel` per control, so two desktop schemes binding different keys makes the chip confidently wrong for one of them. **Read before touching any `IInputStrategy`, `DualStickMix`, `InputDeviceType`, `InputHintBindingMap`, or `ControlGlyphSetSO`.** |
...
| `SALVO.md` | `_Scripts/Controller/Arcade/` | Salvo technical reference (Sparrow-only demolition race in the Boneyard — Dog Fight's inverse). **Read before touching the Sparrow's missile/ammo economy, the omni-crystal collection channel, or the shared Boneyard cell assets** — this mode's wingman reload (a crystal collected by any pilot reloads the whole domain's missile bays) rides `EventOnCrystalCollected` server-side, and the arena assets are shared with Dog Fight, so a retune there retunes this mode's quarry too. |
| `SKEIN.md` | `_Scripts/Controller/Arcade/` | Skein technical reference (Urchin-only cable race). **Read before touching `SkeinCourse`, `SpawnableSkein`, or the Urchin's ride/launch path** — the arena is authored in the PILOT'S TIMES (a vessel speed change moves every distance in it), `Tools/Build/skein_budget.py` is the offline authority that PROVES it and FAILS the build, and §15 is the worked example of an intensity ladder whose rungs promise different things and are asserted accordingly. |
| `REDLINE.md` | `_Scripts/Controller/Arcade/` | Redline technical reference (Manta-only circuit race). **Read before touching `RedlineCourse`, the Manta's flight numbers, or the autopilot Soar drive on `MantaAnalogTurnBoostExecutor`** — the course is cut against the Manta's authored speed/turn constants and its intensity ladder was tuned on the solver's REACH (`AngularSpread`) rather than its profile, which is the finding to carry. |
| `WRECKING_BALL.md` | `_Scripts/Controller/Arcade/` | Wrecking Ball technical reference (Scarab-only demolition race). **Read before touching `AstroLeagueBall`'s attacker name, `ScarabBallForge.Request`, or an AI Scarab's dash** — this mode made a forged ball score for its pilot (replicated `n_PilotName`) and gave autopilot a committed dash (`ScarabJukeController.TryAutopilotDash`), and it is the worked example of forking a court cell so a forest sits INSIDE the nucleus. |
| `BROADSIDE.md` | `_Scripts/Controller/Arcade/` | Broadside technical reference (the ARENA brawl — seven hulls, one score). **Read before scoring any new vessel-vs-vessel weapon, before adding a `CombatHitClass`, or before assuming a hull can land a scoreable hit** — measured on the shipped containers only FOUR of eight could, and it records the three container wirings that fixed it (the Urchin's `projectileShipEffects` was EMPTY), the new `Strike` verb, the skimmer combat-hit reporter and why its AUTHORITY differs from a projectile's (a skimmer overlap is observed on every peer, so an unconditional raise double-credits), and the latch-window model — **a brawl is more balanceable than a race, because the mode owns the windows**. It also owns the fleet's **elemental drain table**, whose rule is now one constant — **TEN POINTS IS ONE PETAL, on every element the hit touches** (a direct rocket strike drains 3 petals each; ten bullets drain 1). Two things make that shippable and both are the interesting half. **PER ELEMENT, not per total** — "three petals in each element" is a statement about each element, so how WIDE an attack spreads its drain became part of what the attack IS rather than something normalised away (the Manta's two-element bloom now bites the same per element as the Dolphin's four and half as much in total). And **a drain lives in one of exactly two places, which must stay DISJOINT**: the classes whose bite is a pure function of price (Bullet, the three missile tiers) drain straight off the HIT REPORT through **`CombatHitDrain`**, because ONE rocket lands in three ranked classes against one victim and a per-blast asset would stack all three — six petals for a thirty-point event, and a base rocket taking a different total from a heavy one for the same hit; `VesselCombatHitLatch` already pays the best tier once and reports what an upgrade SUPERSEDES, so draining from that seam nets the same way the score credits the difference. The classes whose drain carries design a price cannot express — *which* elements (the Manta's Mass+Space) and whether it mirrors as an ally BUFF (the Squirrel's overtake) — stay authored per weapon: Debuff and Strike. `author_combat_debuff_magnitudes.py` (`--check`) owns both, asserts the two sets are disjoint and that the price list `CombatHitDrain` restates matches `BroadsideScoringRule.asset`, and prices the SUSTAINED pressure off each drain's real rate limiter (an asset cooldown, or the LATCH WINDOW for a reporter-driven class). ⚠ **The Debuff anchor MOVED 4.2× lighter** (−0.5 → −0.12), retiring a gate that pinned it: **The Bends and Undertow are scored entirely on that drain** and want a playtest — their SCORING is untouched by construction, since both pay for the hit landing rather than for its depth. ⚠ **Two of this row's statements have since been superseded by `Docs/ELEMENTAL_ECONOMY.md`, which is now the authority on what a drain DOES:** a drain is no longer a decaying debuff but a permanent TRANSFER (this table still sets its SIZE; `ElementalTransfer.FormFor` decides where the petals go — contact verbs steal, ranged verbs eject as collectable crystals), and *"the Rhino's sword is the one scoring verb with no drain path at all"* is closed — it carries `RhinoSwordStealBySkimmerEffect`, a second asset of the Squirrel's own proven overtake type with the new `requireOvertake` flag off, priced identically because a Strike is one VERB and two hulls landing it must take the same petals. This is still the one place to look before hand-editing any `debuffMagnitude`. |
| `REGATTA.md` | `_Scripts/Controller/Arcade/` | Regatta technical reference (the ARENA race — every playable hull on a rail circuit). **Read before listing several hulls on one card, before touching `SO_ArcadeGame.StartingElements` / `VesselStartingElements`, or before laying a structure meant to be ridden or skimmed by every domain** — it records what each hull can do to shielded mass, the per-hull starting-element capability and its wire path, the rail braid's fairness proof, and the honest residual spread the four fleet-wide levers leave. |
| `UNDERTOW.md` | `_Scripts/Controller/Arcade/` | Undertow technical reference (Scarab-only cavitation duel). **Read before touching the Scarab's cavitation container, or before folding two stats into one domain score** — this mode wired the plate's debuff into the combat-hit path and gave the plate the warhead's creature kill, and it records the honest cost of scoring a second stat through `DomainValue` without a new metric. |
| `HEADLONG.md` | `_Scripts/Controller/Arcade/` | Headlong technical reference (Rhino-only circuit race). **Read before touching `GateRaceController`, `HeadlongCircuit`, or the Rhino's ramp boost** — the circuit's corners are cut against the Rhino's own speed/radius CURVE (`HeadlongCircuitSettings.SpeedAtStick` / `CornerRadiusAtStick` / `FastestSpeedForCorner`, a compile-time copy of the ability's numbers that `RhinoRampGradingTests` holds in step), so retuning `maxBoostMultiplier`, `straightnessGraceBand` or `RotationThrottleScaler` moves every corner on the course with it. Also the record of the gate-race platform extraction, the lap bug it found in the AI target provider, and the measured proof that the mode's first intensity ladder delivered none of what it claimed. |
| `WAYSTATION.md` | `_Scripts/Controller/Arcade/` | Waystation technical reference (Butterfly-only migration race — clusters you weave, folds between them). **Read before containing a generated walk inside a cell, before making one ring of a structure aim at another, before declaring `void Update()` on a `GateRaceController` subclass, or before driving a HELD ability from an autopilot** — it records why a positional clamp breaks the corner it was not aiming at, why a capped re-aim cannot contain anything (and what a geodesic walk does instead), why a coil ring can never be an aiming device, why a corner must be clamped on its CHORD, and the `OnServerTick` seam that exists because a Unity message is a slot a subclass can take without being told. Its course is proved by `Tools/Build/waystation_course.py`, which COMPILES AND RUNS the shipped C# and compares gate for gate. |
...
| `TOLLWAY.md` | `_Scripts/Controller/Arcade/` | Tollway technical reference (Scarab-only ring race). **Read before touching `ScarabSwitch`, `PlaceSwitchActionSO`, or any mode that wants to observe a placed structure being used** — this mode added `ScarabSwitch.OnThreaded` and the `Live` roster, and it is the worked example of a mode whose SCORING SURFACES are placed by players and consumed on use. Also records why an AI's placed structure must go through `PerformShipControllerActionsReplicated` rather than `AIPilot.abilities` (an AI runs server-only, so a local press lays conserved mass on one machine), and the volume-ladder re-derivation for a mode where a SCORE IS A MONUMENT. |
...
| `SCREENSHOT_DIRECTOR.md` | `Docs/` | **Press 0 in flight (or the pad's Select), get a UI-free photograph of your vessel** from a camera angle drawn at random from a library of capture concepts. Zero wiring (`[RuntimeInitializeOnLoadMethod]` + `SingletonPersistent`, the `DisplayGraphicsSettings` shape), so it exists in every scene with nothing to place. Three things it records. **UI-free is STRUCTURAL, not a toggle**: it renders its own camera into a RenderTexture, and a screen-space canvas is composited straight to the display and never reaches one — so the HUD is excluded by construction rather than by a flag that can be left in the wrong state; only WORLD-space UI needs a culling mask. **A capture concept is only RANGES** — there is deliberately no enum of shot types and no per-concept camera code, because every shot is one solve (a spherical offset, an aim point, a lens) and the only structural difference between "over the shoulder" and "static tracking cam" is whether azimuth is measured from the vessel's course or from world north; a new shot type is a row in a list, not a subclass. **And the library is checked for GAPS, not just for length** — a concept is an azimuth range, so a set of them either wraps the subject or leaves holes in it, and the first cut's holes (28-60, 120-155, 205-240, 300-332) were exactly the FRONT and REAR THREE-QUARTERS, which is where anything with a nose and a tail is photographed from when the picture is of the vehicle; four entries closed them (split starboard/port for the reason Sidecar already is) and a test holds the worst remaining gap at ≤5°, measuring only the concepts that actually CHOOSE an angle since a free 0-360 orbit covers everything and proves nothing. So adding a shot type is a question about COVERAGE before it is a question about taste, and the three additions that are not azimuth gaps each fill a different axis: `Underside Pass` (the only view of a hull from below — Top Down's mirror in every sense, same free azimuth, opposite elevation, declining the mark for the same reason), `Dutch Pass` (the only real tilt, whose roll range deliberately does NOT span zero, since a range through level rolls the shot back upright in the middle of its own draw and stops being a dutch — the Sidecar split, one axis over) and `Telephoto Isolation` (the only long lens still inside the vision band's near edge, capped at 150 for the same reason Static Tracking Cam is). And **lead room is measured in FRAMES, never in metres** — `aimLeadSeconds × speed` is a world distance, so the 0.35 s that frames a 110u tracking shot is 105u of lead on a 300 u/s vessel and throws the ship clean out of an 8u chase camera's frame; the solve clamps it to a fraction of the shot's own distance. That one was found by the test suite rather than by reading the code, and it generalises: *a composition parameter expressed in units of the SUBJECT is unbounded relative to the FRAME.* **The TWO-SHOT is one geometric fact rather than a search**: when two vessels are 10-30u apart a capture has an 85% chance of framing BOTH, and the set of points equidistant from A and B is the perpendicular bisector PLANE of the segment joining them — so putting the camera anywhere on that plane and aiming at the midpoint yields equal distance (equal apparent size), a broadside pair at equal depth (the separation is the plane's normal, hence perpendicular to the optical axis) and symmetry about frame centre, all three together and exactly (measured: 8.9e-15 relative equidistance error over 200k random pairs). `elevationDegrees` and `aimLeadSeconds` are IGNORED by that solve and that is structural — the first pushes the camera off the plane, which is the one move that breaks equidistance, and the second slides both ships toward one edge; ignoring them beats authoring zeros, because **a promise you can author your way out of is not a promise** and this one fails silently (the photograph still comes out, just not as claimed). Candidates come from `VesselVisionShading.CollectStampedVessels`, the vision band's own roster — the same argument as reading the corridor's target, since **a platform law maintains the handle because the law depends on it being right** — which also excludes the toy matrices' mini hulls by construction. It meets three platform laws and changes only one — it takes the prism occlusion corridor's `SetSuppressed` hold for the capture frame (the corridor's second sanctioned holder, same argument as the replay camera's), leaves the speed tunnel and the vessel vision band alone, and shapes the shipped concept distances around the vision band instead: every concept sits inside the ~150u near edge where a hull still renders as itself, except one that breaks it deliberately and says so in its name — which is also what capped the 1.5x zoom-out applied to the whole solo library (Static Tracking Cam at 150 rather than its scaled 165, Establishing left alone at 220-520): *a ratio applied to a list of numbers is not a decision until you check what each number was up against.* **Each solo band is now a UNION rather than a window** (Over the Shoulder 12-39, Sidecar 14-51, Oncoming 18-67.5, Low Chase 8-27, Top Down 28-105, Static Tracking Cam 35-150), because that 1.5x scale moved each band's NEAR edge out along with its far one and so deleted the close shots instead of adding to them — a band running from the tight cut's floor to the roomy cut's ceiling gets its variety from the roll rather than from a decision made once at authoring time, and a test holds the vision-band cap by reading the edge off the SHIPPED asset rather than the C# field initializer. **Two things about the file itself are worth stating rather than reading off the word "4K"**: the WIDTH is a function of the window (`captureHeight` 2160 sets the height and the width follows the window's aspect, so 3840x2160 only on 16:9 — deliberate, since it makes a capture frame exactly what is on screen), and **MSAA had to be asked for** — `RenderTexture.GetTemporary`'s `antiAliasing` parameter defaults to **1**, so this shipped rendering every capture with no MSAA while the game beside it ran the URP asset's 4x, and it was not obvious precisely because post-process AA *was* correctly adopted: the shots were anti-aliased, just less than the screen they came from, and stair-stepping on a prism edge reads as "the renderer" rather than as a missing argument. It now requests `QualitySettings.antiAliasing` snapped to a legal 1/2/4/8 and RESOLVES it with a blit, because a multisampled target cannot be `ReadPixels`'d directly. *A default-valued optional parameter is a decision nobody made, and a quality default of "none" fails by looking slightly worse rather than by failing.* **The SHIPS in a capture are MARKED**, by a scoped `VesselVisionShading.BeginCapturePass` that does two things for the one hand-stepped render: it lifts the local-pilot exclusion (a photograph OF that hull is the one case the exclusion's reasoning — *do not clutter a pilot's own cockpit view* — does not cover, and without it the feature is empty since the subject of a solo shot is always the local ship), and it COLLAPSES the band onto one flat THRESHOLD (`markDistance` **100**) — past it a hull is the solid domain-coloured silhouette, inside it the hull renders as itself, with nothing in between and the centre break-up closed at the same point (leave the authored 900 and every marked photograph comes back an outline with a dithered middle). At 100 the mark is a thing the PULLED-BACK shots do and the close ones do not — eleven of the fifteen concepts can never reach it — so **check what each concept's range was up against before moving the number**, the same rule the 1.5x zoom scale records. A concept may also **DECLINE** the mark (`ScreenshotConcept.markVessels`: Top Down, Underside Pass and Static Tracking Cam do), and that is a VETO rather than the per-concept band it lives beside — it leaves the rule statable in one sentence (*past `markDistance`, marked, unless this shot said not to*) where a per-concept band replaced the one sentence with fifteen. Static Tracking Cam is the case it was added for: at 35-150 against a 100u mark the same named shot came back a hull about half the time and a silhouette the other half, which is the very inconsistency the flat threshold was adopted to remove, arriving through the DISTANCE roll instead of through the concept. The band's four control points and two graded edges exist because *a mark that pops on reads as a new object appearing*, and **a photograph is ONE frame**, so there is nothing for a fade to protect against — dropping the edges is free here. An earlier pass rescaled the law's whole three-beat arc onto EACH CONCEPT's own zoom range; it was correct, it kept every shape ratio of the law, and it made "will this ship come back a silhouette?" a question you could only answer by knowing which of eleven concepts the roll landed on: *a rule you cannot state in one sentence is one nobody can aim.* The FAR edges are deliberately NOT moved — they exist so a pilot is not reading coloured dots across half an arena, which a photograph never does. It is **not the suppression hold that law deliberately does not have**: a suppression can switch an aid OFF, which is what makes an aid authorable-away, while this can only ever mark MORE, on a camera nobody is looking through. A capture also **holds the TAILS dark on every ship within `hideTailsWithin` (30) of the LENS** — the long identity streak tuned to be read from across a cell, which at the 8-50u most concepts shoot from is several hull lengths of ribbon crossing the frame in front of the subject. Nothing about the tail changes in play and a shot from further back still gets it; *distance is what separates the two readings, so distance is what the hold gates on*. Three scoping rules: **tails only, never jets** (a jet reads THRUST, which is a close-range read — hiding it removes the thing that works and keeps the thing that does not); the prism **TRAIL is unreachable from here and must never be** (conserved mass is never hidden, and it is not a `TrailRenderer` at all); and it is **scoped to the MARKERS** through `VesselTailAndJets.HideTails`, since a sweep of a vessel's `TrailRenderer`s takes the Rhino's five blade tracers with it. It hides by `Renderer.forceRenderingOff`, never by disabling the component — **a disabled `TrailRenderer` stops RECORDING as well as drawing**, so the ribbon bridges the gap with a straight segment on release, which is a visible artefact left behind by a hold that is supposed to leave nothing. A capture also **prefers a clear line of sight** — it rolls the VANTAGE (never the concept, which would collapse the library onto whichever shot type looks at open space) `clearShotSamples` times and keeps the one with the least prism mass between the lens and the subject, stopping early when one is clear; a preference rather than a rule, so a shot taken deep inside a forest still comes out framed from wherever the mass was thinnest. The occluder test is `PrismSpatialIndex.CountInCone` — **the occlusion corridor's own volume**, a cone rather than a capsule because a prism a metre off the axis at the lens fills the frame while the same prism at the far end barely clips the silhouette — and it **stops one hull radius short of the subject**, which is what lets a ship sit in and among the blocks as freely as it likes: what ruins a shot is a prism BETWEEN the lens and the hull. Captures are named **`<Concept>_<YYYY-MM-DD>_<HH-mm>.png`** — the shot leads (that is what you triage a folder by), the date-time trails and still sorts, and SECONDS are deliberately gone because a filename is read by a person: the precision that made two captures unique is the precision that made every name unreadable. That moves the uniqueness out of the format and into the folder (`ResolveUniquePath` — the first capture of a minute keeps the clean name, a later one takes `_2`, `_3`), since *a screenshot you took and no longer have is worse than one with an ugly name*; `fileNamePrefix` was retired with the seconds, because a constant `CosmicShore_` spends the most valuable characters in the name on the one fact the path already carries. They land in **`<repo>/Recordings`**, resolved per machine off `Application.dataPath`'s parent rather than hardcoded, so one default is right in every clone — and **they are never pushed**, because `/Recordings` is already in `.gitignore` (a screenshot is for the person who took it to triage, not repository content; a test pins the default folder name to that ignore rule so the pair cannot drift apart). Two findings from picking the key: **a key is only "free" against the keys some OTHER system is reading, and an input scheme that consumes raw `Keyboard.current` reads advertises none of them** — the dual-WASD scheme eats `WASD`, `P`, `;`, `L`, `'`, `QWER`, Space and both Shifts, with `P` in particular being right-stick-UP, so capture bound there would have nudged the vessel it was photographing; and **when every ergonomic key is taken, ask whether the new binding actually wants an ergonomic key** — moving the flight binding to free `P` was tried and reverted, because a capture is a deliberate occasional act rather than a flight control, so the number row (unclaimed project-wide) is a shelf you cannot fat-finger mid-manoeuvre and it leaves the flight scheme a photograph depends on untouched. **Read before adding a capture concept (and read the coverage rule first — the question is which angle the library does not have yet), before binding another debug key (F5-F9 and F11 are taken, F12 is Steam's, and the keyboard flight scheme eats `WASD`, `P`, `;`, `L`, `'`, `QWER`, Space and both Shifts), or before standing up any second camera.** |
...
| `GAMECANVAS.md` | `Docs/` | GameCanvas as one source of truth: the two forked prefabs, the 1,733 identical-in-every-scene overrides that masked the prefab, the 16 that differ, the dangling cross-prefab refs, the code fixes that removed per-scene wiring — and **§9, the retirement**: the fork was on **15** scenes, and every one also carried STRUCTURAL edits (the prefab's own `MultiplayerHUD`/`Scoreboard` removed and re-added as scene components, the old end-game subtree deleted, `ConnectingPanel` added), identical across twelve — *the prefab asset was never what ran*, so a donor SCENE defines the shipped canvas. **FrogletTools ▸ Game Modes ▸ GameCanvas Unifier** (three buttons: Fix prefab / Fix scene / Delete the fork; the Prefab Kit links to it) absorbs it into `CORE/GameCanvas` by hierarchy path (script-swapping base→derived in place so fileIDs survive), then enforces the **canvas contract** — 1920x1080, Scale-With-Screen-Size, `AdaptiveCanvasScaler`, smart re-anchored — through the Canvas Upgrader's own passes; re-points each scene with a dry run and drops the overrides a CORE scene no longer needs; and deletes the fork; `Tools/Build/gamecanvas_unification_report.py --check` is the gate and asserts the contract. **Both prefab assets are authored at 800x450 and only scene overrides ever said 1920x1080**, which is how the first re-point handed Skim Race an 800x450 canvas — a re-point onto a CORE not at the contract is now refused. The one real per-mode value (`statsToTrack`) moved to `Resources/GameModeStatsProfile`, so scenes carry zero survivors. Also records that 1,390 of a scene's 1,771 overrides address objects INSIDE nested prefabs through fileIDs the outer YAML never contains. **Read before touching any game-mode scene's canvas.** |
...
| `Analytics/` | `Docs/` | **The analytics contract** — `DATA_ARCHITECTURE.md` is the authority on implemented shape (the classification model, the Cloud Save schemas, the event envelope, and §7.3 PostHog: identity, person properties, erasure, and the legal exposure inventory in §8); `EVENT_SCHEMA.json` is the machine-readable contract that drives BOTH the UGS Event Manager and PostHog config; `POSTHOG_SETUP.md` is the out-of-repo checklist. The rule that shapes everything here: **a fact about a PERSON goes on the person record via `Identify`, never onto every event** — UGS validates each custom parameter against a dashboard schema whose rows are **permanent** and capped at **1,500 per environment**, so a redundant field costs one irrevocable row per event forever. §7.3.3 is the worked example (`invite_wave`, the cohort the paid-EA gate reads) and records three traps that generalise: **an unknown value is OMITTED, never written** (person properties are last-write-wins, so `$set`-ting an empty over a real one is a deletion wearing an update's clothes); **a refresh that only runs at session end misses every session that CRASHES**, which biases exactly the cohort a stability gate measures; and a derived dimension must say what it is a **proxy** for (`invite_wave` is week-of-first-play, not a Steam grant batch — there is no Steam SDK to read one). Note the identity underneath is UGS **anonymous** auth, so a reinstall creates a new person. **Read before adding an event, a parameter, or a person property, or before touching `AnalyticsServiceFacade` / either `IAnalyticsSink`.** |
| `AudioSystem/` | `Docs/` | **The FMOD audit** (`FMOD_AUDIT.md`) and the **audio owner's task list** (`CHARLES_TASKS.md`). Records why the volume sliders did not save (a `Mixer` component wrote to FMOD VCAs that are assigned to no bus, and the cloud snapshot stomped PlayerPrefs unconditionally — now last-writer-wins by `GameSetting.ShouldApplyCloud`), why fresh installs booted silent (level prefs seeded as ints, read as floats), why the music ignored its slider (a looping event fired as a `PlayOneShot` — now an instance `AudioSystem` owns), and the two ways code touching FMOD crashed or spammed (an unguarded `CreateInstance` retried every frame; a `Detach` in `OnDestroy` that re-creates the whole RuntimeManager mid-quit — both now behind `FmodSafe`). **Every FMOD instance the code creates gets its volume from ONE mapping** (`AudioVolumeMath` via `AudioSystem.ResolveSfxInstanceVolume`), per instance today and via the VCAs once the FMOD project routes its buses through them (`AudioSystem.driveFmodVcas`, opt-in, never both). **§1.0 is the one that actually caused the report and outranks the rest**: the Music/SFX/Haptics sliders were authored as FIELD-OF-VIEW sliders (`60..90`, whole numbers, 71), and narrowing a `Slider`'s range CLAMPS and BROADCASTS, so binding the panel fired the prefab's persistent `SetVolume` listener with the clamped 1 and saved full volume over the player's level — see the `Slider` re-range anti-pattern below and `SliderRange`. **Read before creating, attaching or releasing an FMOD instance, touching the audio settings path, or re-ranging any bound UI control.** |
...
| Vessel tail + jets | `VesselTail` / `VesselJet` (markers, each carrying `widthScale`), `VesselTailAndJets` (one per vessel: domain tint over every `TrailRenderer`), `VesselFXWidth` (applies the per-vessel width once at Awake — **to `TrailRenderer`s and nothing else**), `TailGradient` (the ONE composition of a tail's colour gradient, shared by the vessel owner and by `Projectile` so a vessel's tail and a missile's cannot drift), prefabs `Components/VesselTail.prefab` + `Components/Jet/VesselJet.prefab`, audit via FrogletTools > Vessels > Audit Vessel Tails and Jets. **A tail is tuned for OTHERS and sits behind the pilot's own camera; a jet is tuned for its own pilot but visible to everyone; both wear the vessel's domain like the trail does.** A tail is not a vessel's alone — `Projectile` owns one on the Sparrow's skyburst missile (painted, mounted, sized, cleared and released per flight, alongside the spike appearance and charge shell it already owned), and `VesselTail.prefab` was stripped of six dead disabled particle systems to make nesting it into a 20-deep projectile pool affordable. **A JET HAS TWO SIZES AND `widthScale` IS ONLY ONE OF THEM**: `VesselFXWidth` walks `TrailRenderer`s, but `VesselJet.prefab` nests `vfx_Projectile_02` — three `scalingMode: Hierarchy` particle systems beside one `Trail` — so the plumes, i.e. most of what a jet draws, take their size from the **transform chain**, which is exactly the thing a `TrailRenderer` ignores and therefore exactly the thing `widthScale` was written not to be. A jet that authors no `m_LocalScale` renders at `(1,1,1)` × whatever its MOUNT inherited, which is not a decision anybody made: the Urchin's hang on engine nodes carrying a **1.75** scale and shipped at **8.75× the reference girth and 40× its length**, on the smallest hull with the closest camera, while its ribbon was correctly at 0.334 the whole time. The plume's dial is `m_LocalScale` on the jet instance and its target is **`(0.6, 0.6, 0.13) × |followOffset.z| / 20`** — the value BOTH hand-tuned hulls (Dolphin, Squirrel) author, scaled by the same camera ratio the ribbon uses. Sparrow / Rhino / Grizzly / Scarab still author none and the audit names them `UNSIZED`. General rule: **when one object's size is set by two unrelated mechanisms, a dial that reaches one of them reads as a dial that reaches the object.** — `Docs/VESSEL_TAIL_AND_JETS.md` | `_Scripts/Controller/Vessel/`, `_Scripts/Controller/Projectiles/`, `_Prefabs/Spacevessels/Components/` |
| Prism lifecycle | `Prism`, `PrismFactory`, `Trail`, `TrailFollower` | `_Scripts/Controller/Vessel/`, `_Scripts/Controller/Prisms/` |
| Prism occlusion corridor (**PLATFORM LAW**) | Prisms between the player's camera and their ship go see-through so the ship is never hidden — **not a feature a vessel or mode may choose**; it must not be possible to author one in which it is off (the retired `ClearPrisms` was per-vessel opt-in on 3 of 11 vessels and had been silently dead on all three). Four layers make that structural: (1) the fade lives in the prism SHADER GRAPHS themselves (the corridor spliced into `SurfaceDescription.Alpha` on **every graph a live prism can render with** — `PrismOcclusionFade` on BlockGraph, `PrismOcclusionFadeDebris` on ExplodingBlockGraph, two thin wrappers over ONE body), so new prisms inherit it; (2) the target binds in `VesselController.Initialize` under `IPlayer.IsLocalPilot` — the one method every vessel must call on every spawn path — plus `ChangePlayer`, which hands a LIVE vessel to a different player (the Cellular Duel ownership swap) and never reaches `Initialize`, so there is no per-vessel or per-scene wiring to forget; (3) `PrismOcclusionDiagnostics` screams once per material from `Prism.SyncRenderMaterial` if a prism can't fade; (4) `PrismOcclusionCoverageTests` + FrogletTools > Ecology > Prism Animation > **Validate Occlusion Corridor** fail on new content authored outside it. `PrismOcclusionCorridor` publishes just 2 `Shader.SetGlobalVector` per frame (vessel position + (outer, inner, coreAlpha)); the camera end is read on the GPU from `_WorldSpaceCameraPos`. The corridor is a **FRUSTUM** — a circle at the lens (`PrismOcclusionConfigSO.nearRadiusScale`, 0.5 hull radii, published as the file-scope global `_PrismOcclusionNearRadius` so no graph node changed; 2026-09-15 — a pure cone was thinnest exactly where a prism does the most damage, since mass at the lens occludes the whole SCREEN, not just the ship), widening linearly to the sphere that circumscribes the hull and ending ONE HULL RADIUS SHORT of the vessel's plane (`PRISM_OCCLUSION_NOSE_CLEARANCE`, 2026-08-11 — the cone used to run flush to the origin with its gradient still in progress, so a prism was still half-dematerialised when the ship hit it and the impact did not read; the fade now completes with a solid buffer the whole nose sits inside, at the stated cost that mass inside that buffer can occlude the ship at contact range) (no caps, and the base graded on the same shell thickness as the sides so the whole boundary is seamless). **That clearance and its axial grade are both HULL-sized and both eat the same end of a CAMERA-sized corridor, so what they cost is one ratio** — `ρ = cameraDistance / hullRadius`, which the fleet authors across a factor of ~37 (`|followOffset|` 6.72 on the Urchin against 250 on the Serpent). Uncapped they took `1.75/ρ` of the tunnel, so the FULLY-CLEAR corridor was **empty at ρ ≤ 1.75** and 0.384 of the length at the Squirrel's ρ — measured against the shipped function — while the clearance constant's own degenerate-case note claimed the corridor is only lost inside ONE hull radius (an analysis of `tSolid` alone, blind to the grade subtracting a second `(outer − inner)/axisLen` from the same end). On a close-camera hull the corridor was therefore nearly inert, which reads in play as mass — trail, environment, and especially the 24-wedge explosion debris, born wherever the ship destroyed something — **sitting solid in front of the ship while the same mass dissolves for a long-camera hull**. `PRISM_OCCLUSION_MAX_BASE_SHARE` (0.5) caps the pair as a share of the corridor's LENGTH, scaling BOTH by one factor so the grade stays the fraction of the clearance it was derived as, and is a **bit-exact no-op for ρ ≥ 3.5** (proven, negative-controlled, `Tools/Shaders/verify_prism_corridor_base.py`). **The cap was still only half the answer, because the clearance is paid by mass that cannot use it (2026-09-17)**: capped or not, roughly the closest HALF of the tunnel is solid-or-grading on the two closest-camera flying hulls, and explosion debris is born exactly there — at the point of destruction, which in a fight is at or near the hull — so a burst went on occluding the vessel while every fragment was nominally inside the corridor. What the clearance buys is a claim about COLLISION (a prism you are about to HIT must read solid), and **debris has no collider**, so the argument has nothing to buy there while the cost lands where it hurts most. The fix is a second ENTRY POINT, never a second corridor: `PrismOcclusionFadeImpl` is the one body, `PrismOcclusionFade_float` passes `PRISM_OCCLUSION_NOSE_CLEARANCE` (1.0) and `PrismOcclusionFadeDebris_float` passes `PRISM_OCCLUSION_DEBRIS_NOSE_CLEARANCE` (**0.0**) — BlockGraph is untouched, so live (collidable) mass keeps the full clearance and no impact read is lost, while ExplodingBlockGraph dissolves flush to the vessel's plane (clear fraction at the Squirrel's ρ: 0.500 → **0.736**). Stated cost: a burst visibly THINS where it crosses the ship. **Two wrappers rather than a new node input, and that is not a style call** — a file-mode Custom Function node builds its call as ALL INPUTS then ALL OUTPUTS (slot IDs do not decide it), so a parameter declared after an `out` makes the graph fail to compile and every material drawn with it renders UNMATERIALED with nothing in the console naming the file; that is a shipped incident, `Tools/Build/check_shadergraph_custom_function_signatures.py` holds the rule, and both wrappers keep the identical four-in/two-out shape so the debris graph needed only its `m_FunctionName` string changed. **A clearance is bought for a reason, and mass that cannot use that reason should not pay for it** — asserted per ρ by that verifier's T4, which compiles BOTH entry points from the one shipped body with the live clearance as its negative control. **An EROSION rework was shipped against this same report first and is REVERTED** (2026-09-17): the report — *“the dither is applied first and then they are rotated and repositioned”* — was read as a defect in the debris's own body-anchored wipe and answered by re-seeding that wipe per-WEDGE off the object-space tangent, re-normalizing it against the face triangle, refitting the CDF and editing the cube's UV0. Every measurement in it was right and it was **the wrong system**: the report's own next sentence (*inside the dither cone and still getting occluded*) named the CORRIDOR, which is what the debris clearance above fixes. What it produced was 24 independent fronts per prism, each crossing its own tiny triangle in its own hashed direction, play-tested as *“flickering out with overlapping dusty surfaces instead of the clean wipe”*. **The shared UV triangle it called a defect is what makes the wipe legible**: a prism's 24 wedges are read as ONE object coming apart, so the fronts have to be parallel and in step — per-piece identity is not a free improvement on that, it is what destroys it. Two rules: **diagnose WHICH system a report is about before rebuilding one** (a prism carries two independent dithers, the corridor's view effect and the erosion's body effect, and both present as “the dither is wrong on the exploding pieces”), and **a per-instance identity is not automatically an improvement — ask what the player reads as ONE object first.** `Docs/PRISM_ANIMATION.md §4.7`. General rule: **a clearance written in the units of the OBJECT, subtracted from a volume whose extent is a property of the CAMERA, is a different fraction on every vessel** — state it as a share of what it is taken from, or measure it on the closest-camera hull, the minimal volume that can occlude the ship (the old constant-radius capsule was an artefact of the retired `ClearPrisms` CapsuleCollider and massively over-cleared near the camera; tapering makes the cleared region a constant ANGULAR size). It is **ship-sized**: the radii are multiples of the vessel's OWN circumscribing radius, measured hull-only and rotation-invariantly at bind (skimmers and DISABLED renderers excluded; a skinned hull measures its `localBounds` in ROOT-BONE space — the culling bounds that actually render — never `sharedMesh.bounds`, whose bind-pose mesh-space extents overstate an armature-scaled rig by the full armature factor: the Sparrow's rig carries 0.2 in its armature and shipped a ~5× oversized corridor that way, 2026-08-11) — outer edge on that circle, fully-clear core at a quarter of it — so a new vessel of any size is correctly scaled with nothing authored. Per-vessel audit: **FrogletTools > Vessels > Audit Corridor Vessel Radii** runs the exact runtime measurement over every vessel prefab and names each hull's top contributing renderers, so an inflated radius arrives with its offender attached. **Zero per-prism CPU**, no extra draw calls, corridor prisms stay in the OPAQUE queue (screen-door dither into `SurfaceDescription.AlphaClipThreshold`; kernel selected by `PRISM_OCCLUSION_KERNEL` — 4 = SHATTER, **current** (shipped 2026-08-06 at polygon 16.26 px / wall 20 px): the lattice's Voronoi polygons filled between straight lines so the NEGATIVE space is the motif — a cracked lattice of walls, with two independent dials (polygon 8–20 px, wall up to ~1.25× the polygon — the wall window is RELATIVE, not absolute; no CDF needed since `frac` of a hash is uniform); 5 = SHATTER3D, the same proposition lifted into the WORLD as Voronoi polyhedra cut by crack planes — **carried, REJECTED ON LOOK the day it shipped (2026-08-10)**: every fidelity number passed (0.0006 uniform / 0.0031 in-situ via a clang build of the shipped file), but a crack plane lying near-parallel to a viewed surface makes a face-sized plate share one threshold and flash at one alpha — glitchy clipping around the vessel that no flat measurement could see (a candidate must pass the number AND earn its look on real mass at speed; kernel 6 (SHARD3D) is the Lab candidate — Euclidean distance-to-owner fill, level sets are spheres that cannot lie flat against a face; not shipped until it earns its look on real mass at speed); 3 = SHARD, **triangular** flecking, carried; 2 = the same arrangement with round flecks (Worley, now the calibration reference — its CDF-fitted `smoothstep` remap is load-bearing: raw F1 measures 0.140 coverage error, remapped 0.0048); 1 = corridor-relative spiral, an iris anchored to the corridor; 0 = interleaved gradient noise, a dissolve anchored to the screen — only these hold coverage fidelity over a short band; `PRISM_OCCLUSION_MORPH_RATE` slowly evolves the pattern off `_Time.y` at zero CPU — the cellular kernels' cells orbit, the spiral's phase drifts, IGN can't morph because a hash has no continuity to move; and the LAYERED BEAT — surfaces stacked along one camera ray (a prism's own interior through its clipped front face, parallel trail walls) read the same screen-anchored threshold and moiré-beat — is answered by two dials after the depth-parallax domain shear was **rejected on look** (it moved the whole lattice, so at speed it crawled coherently and read as worse flicker than the beat; a fix that moves the pattern globally cannot win against speed): `PRISM_OCCLUSION_SHATTER_DEPTH_PHASE` shifts only each cell's WALL by view depth (lattice still, coverage-neutral) but **ships at 0** — measured, useful decorrelation needs ~50× the rate the speed budget allows, the same conflict — and `PRISM_BACKFACE_POWER` (`PrismBackFaceFade`, spliced after the corridor) sharpens `alpha^power` on away-facing surfaces so the prism's own interior leaves the gradient band while the exterior is still dissolving: the one fix that REMOVES the interference rather than scrambling it, with no temporal cost, at the stated cost of interiors reading as thinner shells mid-fade). **Since 2026-08-10 the dither is ALL prism transparency, not just the corridor's**: `PrismOcclusionFade` engages its threshold for ANY fractional final alpha anywhere, so the exploding-debris fade-out (`PrismExplosionClock`'s Opacity) and the cloak family's authored near-zero alpha ride the same screen door as the corridor, composing in coverage — and every prism material is OPAQUE + `_ALPHATEST_ON` with NO prism in the transparent queue (the seven blending materials were converted, authored `_Alpha`/`_Opacity` preserved as dither coverage; `Tools/Shaders/enable_prism_alpha_clip.py` enforces and converts strays, `PrismOcclusionDiagnostics` faults a transparent prism material at runtime, and the coverage test fails one in CI). **The exploding prism's FADE carries its own dither** — `PrismErosionFade`, anchored to **UV0** so it is never a function of view angle or motion: each face of the debris cube gets ONE jagged erosion front that wipes across it — a HARD edge, seeded off the stamped `_Velocity` so no two prisms peel alike (a dithered fringe shipped briefly and was removed 2026-08-11: a graded debris edge dissolves in the same visual language as the corridor and the two read as one confused surface; the soft half of the motif is the unbroken face and the front's own irregularity). UVs are mesh attributes no vertex animation can move, so the front rides the face through flight AND the shatter spin (the earlier body-position anchoring broke under the spin — fragments migrated across dominant-axis face boundaries as pieces rotated). The wipe completes 15% of the fade early by construction (`PRISM_EROSION_END_MARGIN`) on the 1.5×-extended `PrismExplosion.DefaultDuration` (7.5s), so retirement can never beat it. Spliced between the clock and the corridor node by `Tools/Shaders/wire_prism_explosion_erosion.py` (migrates old wirings in place; CDF fit over the UV square: `fit_prism_erosion_cdf.py`) — the corridor keeps owning occlusion, a view effect by definition, and the two compose in coverage when a fading chunk sits in the cone. **The dither's unit shape obeys the house soft-hard-soft motif**: a circle is soft with a soft gradient either side of it (soft-SOFT-soft), so the 2026-08-06 pass replaced it with two hard-edged candidates and shipped SHATTER after judging both in motion. SHARD changes Worley's METRIC only — Euclidean distance becomes the gauge of an equilateral triangle, `max(q.y, 0.866·|q.x| − 0.5·q.y)` — keeping the lattice, jitter, orbit, 3×3 search and remap while the flecks gain hard straight edges. The gauge is area-normalised (×1.28607) to the circle it replaces, which is BOTH why they are "triangles of the same size" and why one CDF fit (`PRISM_OCCLUSION_CELL_CDF_*`) serves both cellular kernels — retune the area constant and both must be refitted. `PRISM_OCCLUSION_SHARD_ORIENT` turns them (FIXED/FLIP/SPIN). **Choose the look in FrogletTools > Ecology > Prism Animation > Occlusion Dither Lab, not by editing `#define`s** — it drives kernel + scale as shader globals live *in play mode* (the `PRISM_OCCLUSION_LIVE_TUNING` gate, fail-safe to the constants when nothing is published and compiled away entirely at 0), previews through the shipped GPU code rather than a C# copy, runs the real |coverage − alpha| admission rule against the shipped baseline measured in the same pass, and bakes the result back into the constants. `PRISM_OCCLUSION_CELL_SIZE` is a **free** dial inside 4.5–11 px (sweet spot 6–8): the fit is scale-invariant, so the old "re-fit the CDF or the fade degrades 19×" warning was wrong — 19× is the cost of dropping the remap entirely, and what actually bounds the pitch is sampling at both ends (pixel floor below, too few cells per gradient band above), which no re-fit can buy back. Tuning: `PrismOcclusionConfigSO` (`Resources/PrismOcclusionConfig`). `SetSuppressed` is the only sanctioned hold and there are exactly **TWO** holders, both manually-posed vantages that are not the pilot's eye and both taking it for the same stated reason — the manual replay camera (`CameraManager.BeginManualReplayCamera`) and the screenshot director's capture camera (`Docs/SCREENSHOT_DIRECTOR.md`). A third needs the same argument: a camera the pilot is not looking through would cut the camera→ship hole through unrelated mass. Every holder is symmetric (the vessel binding survives), identity-guarded (never lifts a hold it did not place) and lifts unconditionally. See `Docs/PRISM_ANIMATION.md` §4.7 | `_Scripts/Utility/`, `_Scripts/ScriptableObjects/`, `_Scripts/Controller/Vessel/`, `_Graphics/Materials/Graphs/` |
...
| Projectile charge shell | `ProjectileChargeField.shader` + `.hlsl` — the SELF-DRIVEN member of the crackle family, and the reason it exists is a **rule**: **MASS in-flight growth is a HIT VOLUME, not a size.** A round's MODEL is the size it left the muzzle for its whole flight (swelling it drew a small ship firing cannonballs — the exact silliness the growth pass was written to avoid); this see-through additive shell, sized every frame to exactly the swept hit radius, is what the player reads the growth off, so the instrument IS the hit volume rather than a proxy for it — and a pilot can still see the arena through their own enormous round. `Projectile.ApplyFlightGrowth` therefore scales the transform ONLY when that transform draws nothing (`CacheTransformRole`, derived from the prefab's own renderers rather than authored — the Turret Stance's carried collider IS its hit volume and must keep growing; the bullets' tracer is a model and must not). The shell drives itself off `_Time` and its OWN object-to-world matrix — no per-frame CPU write, no MaterialPropertyBlock, one `UnityPerMaterial` cbuffer so every round SRP-batches — because a single Sparrow keeps ~54 rounds in the air at 90 volleys/s and the skimmer's per-renderer property-block driver would be ~54 extra draw calls a frame. **ONE ROUND IS ONE STROKE; THE VOLLEY IS THE SPHERE.** A round draws a SINGLE bolt lying on ONE randomly-oriented great circle — never a crackling ball. The first cut (3 seeds × 5 radiating filaments, a lit centre, a standing rim, re-rolled ~62×/s) had one round painting **73% of its own shell** in a single 0.3 s flight: it assembled the sphere itself, which is the player's job. Great circles because superimposed at random orientations they are a sphere's WIREFRAME, and because one is still a curve of exactly the hit radius, so the instrument survives being toned down. **Raw lit AREA is the wrong metric and that is the general lesson** — the old shell lit *few* pixels *everywhere* and so measured a SMALLER lit fraction than one fat stroke would, while reading as a ball; what separates them is **planarity** (RMS deviation of the lit set from one plane through the centre: 14.1° → **2.8°**) and how much of its own shell one round paints (73.1% → **19.8%**), with the burst's UNION climbing 26.3% → 62.4% → **90.5%** over 0.25/1/3 s of fire. *When a visual is a claim about a DISTRIBUTION over many instances, measure the SHAPE of one and the UNION over many, never the per-instance total.* **A ROUND'S IDENTITY IS AN EXPLICIT PER-INSTANCE SEED**, stamped once per SHOT (`Projectile.StampChargeFieldSeed` → a `MaterialPropertyBlock` → the GPU-instancing buffer), because the shell has NO implicit signal that can tell two rounds apart: consecutive volleys differ in world RADIUS by **0.0183u** (turning that into half a discharge cycle needs a ~159 Hz per-round rate), TIME is identical for every round alive at an instant, and LATERAL POSITION is identical for every round from one muzzle while the ship flies straight. So rounds fired 11ms apart WERE the same round, and the gun's two muzzles were only the most visible instance of it. Three passes derived identity from geometry (radius; a lateral read into the phase; a lateral read spinning the circle), each measured decorrelated, each still read as identical — **a metric can only decorrelate a difference the signal actually carries**, and a corollary worth keeping: **when a periodic effect is keyed on `floor(x)`, a sub-unit offset in `x` changes WHEN, never WHICH**. The seed costs one `SetPropertyBlock` per shot — never per frame — and moves the material from SRP-batched to **GPU-INSTANCED**, the right trade for ~54 identical spheres that must all look different: they still batch, and now they can differ. **The deeper cause was found only by RENDERING**: `Cull Back` draws the front hemisphere alone, so a great circle at a random pole spends most of its length behind the round, and past ~40u most rounds showed no stroke at all and collapsed to a plain dark disc. A round is 15-77px at combat range and the stroke was 2-6px — thinned that far by a well-meant "lit area must not exceed the baseline" guard. The circle is now anchored to the VIEW AXIS (pole near the plane perpendicular to the view, stroke centred toward the camera-facing point) so a round always shows a slash across its visible face. **Judge a candidate at the size it will be judged** (`Docs/PALETTE.md` §4.3) — three rounds of planarity/overlap/brightness measurements all passed while the effect was invisible on screen; `Tools/Shaders/render_projectile_charge_field.py` now rasterizes the shipped HLSL through a real perspective camera at true 1080p density so that cannot happen again, and `verify_projectile_charge_field.py` keeps a `no seed` negative control that reproduces the failure exactly (100% lit-set overlap vs the shipped 1.3%). **A BURST IS THE TUNING SURFACE, NOT A ROUND.** Every per-round metric said the one-stroke design was restrained, and rendering the live stream said it emitted **2.11x the light of the crackling ball it replaced**: a Sparrow keeps **54 shells on screen** and `Blend One One` sums them, so a 4x per-round reduction against a 54x multiplier is not a reduction. It took TWO tone passes to land it — the first reached 0.08x and playtest still called it overtuned — and it ships at **0.04x** the baseline (a 51x cut), bought by deleting the envelope's bright plateau (`_HoldTime` 0.5 -> 0.042) and shortening the stroke (`_ArcSpan` 5 -> 1.45). The harness's test 6 renders the stream, totals the LINEAR emission and fails above **0.06x** — an odd number on purpose, because a ceiling the REJECTED version would pass is not a gate. The second halving is spread across duty, brightness AND length because any single knob that can pay the whole bill also encodes something (a 1.0-radian span hits the budget alone and stops reading as a curve on a sphere). *When N instances of an effect are on screen simultaneously, the tuning surface is the SUM over N — a per-instance metric is structurally blind to it, and N is usually set by a system with nothing to do with the effect (here, the fire rate).* Its corollary: **sparseness became the mechanism, so the duty-cycle FLOOR became a CEILING** — a round shows a stroke 31.3% of the time, and 'most rounds dark most of the time' is how 54 shells stay quiet; continuity of existence only ever required that a round is never FULLY dark, which the rim whisper holds at peak alpha 0.007. Detail: `R_VesselActions/SPARROW_SPRAY_ACCURACY.md` § Round 4, § Round 5 | `_Graphics/Materials/Graphs/`, `_Scripts/Controller/Projectiles/` |
...
| Cell-relative spawn ring | `CellSpawnFormation` (pure math, N players around the cell, all facing it) in two formations: `Symmetric` — spread over a SPHERE (4 tetrahedral, 3 equilateral triangle, 2 antipodal, 5+ Fibonacci), the default; and `EquatorialRing` — evenly spaced on ONE horizontal great circle the way Joust authors its points by hand, for an arena with a meaningful "up" or a pole feature (Cleave: a latitude-hoop cage is densest where the ribs converge, so a tetrahedral spread would hand two of four players a much harder approach). Driven by `ServerPlayerVesselInitializer.arrangeSpawnPointsAroundCell` + `spawnFormation` at `Cell.ExpectedNucleusWorldRadius + spawnDistanceOutsideNucleus`. Opt-in per scene (Symmetric for Crystal Capture, EquatorialRing for Cleave). **A mode may build on the formation rather than just spawn from it**: `ApproachLaneGeometry` strikes one straight APPROACH LANE through each spawn slot, passing the cell centre at an authored standoff (`sin(theta) = offset / ringRadius`) instead of running into it — the crystal manager's `ApproachLanes` placement mode. It was written for Drumfire (removed 2026-09) and KEPT as a platform capability with no consumer today. **Lane ownership is then EMERGENT and nothing assigns it**, which holds only while the scene's `spawnRingRadiusFloor` equals the lane radius and `spawnFormation` equals the lane formation — one number in two places, so the mode that adopts it must assert the pair in its own asset generator rather than trusting a comment. Tests: `CellSpawnFormationTests`, `ApproachLaneGeometryTests` | `_Scripts/Utility/`, `_Scripts/Controller/Multiplayer/`, `_Scripts/Controller/Environment/FlowField/` |
...
| Home hub (Mission / Toy Box / Arena / Arcade) | `MenuHubButton` (names a modal TYPE + its availability state), `ToyboxModal` + `ToyboxCard` (the grid), `ToyConfigureModal` + `ToyVariantCard` + `ToyPreviewCamera` (the detail window: one toy, its variants, Navigate + Switch), `ToyPortraitLibrary` (the encyclopedia's baked emblem portraits), `IToyShellSurface` / `ToyShellOption` / `ToyShellRegistry` (a toy's flat face) — `Docs/HomeHub/ARCHITECTURE.md` | `_Scripts/UI/Elements/`, `_Scripts/UI/Modals/`, `_Scripts/Controller/Toys/` |
| Freestyle toys | `Toy` (base world-trigger; bloom, local-user + freestyle gating, exit-gated re-arm), `MatrixToy` (the one-toy-opens-into-many base: a pass unfolds a matrix of choices out along the outward radial, another folds it away — shared by the cell selector, painting gallery, and vessel changer), `SwapToy` + `SwapToySetCoordinator<T>` (a small set of toys showing "the options you're not on", each flips to your previous option on use — the domain changer), `VesselChangerToy` (one toy opening into a matrix of mini ship models via `VesselModelBuilder`, reuses `RequestSwap` + restores freestyle control after swap), `ElementChargerToy` + `ElementChargerToyDefinitionSO` (one station opening into a row of the four element crystals, charge → time; a pass grants the local vessel whole petals in that element through `ResourceSystem.GrantPetals` — exactly a crystal's grant, owner-local, past 10 it drains back like any overcharge, and it is a freestyle-only SOURCE in the elemental economy, fenced from matches because menu vessels are despawned on the way into a game scene), `DomainChangerToySet` (two SWITCHES in the prism material of the domains you're not — the reserved domain-coloured switch, `ToySwitchSignal.Domain` — `RequestSetDomain_ServerRpc`), `PaintingGalleryToy` + `PaintingToy` + `PaintingRunner` (one toy opening into a matrix of painting stations; multi-stroke multi-domain connect-the-dots: domain gates, pen-up, cone/jack stroke markers in prism material, resumable progress that survives folding the gallery away) + `PaintingDefinitionSO`/`PaintingPresetLibrary`/`PaintingStrokeToolkit` (stroke data + 16 grandiose 3D presets + the curl-field stroke library + Star/Rainbow/Saturn/Taj Mahal generators; runtime flight-continuity stroke ordering via `OrderForFlightContinuity`) + `PaintingProgressStore`/`PaintingPrismStore` (local JSON progress + per-prism drawing state, regrown on return) + `PaintingShareExporter` (self-contained WebGL HTML → NativeShare), `ConveyorToy` + `MicrosceneConveyor` + `Microscene` + `MicroscenePatterns` + `MicroscenePatternsGrand` + `MicroscenePainter` (Wanderway: on/off toggle streaming a speed-scaled field of procedurally-varied microscenes — 48 recipes: the classic forty incl. spine×motif Medley composers, plus the monument-scale grand eight — ahead of the vessel, structurally painted across the full domain triad with capped danger/shield accents; a closed conveyor of a 30k-prism conserved stock built once behind an `EnvironmentLoadVeil` + skimmable crystals + cell-released lifeforms), `CellSelectorToy` + `CellSelectorToyDefinitionSO` (the world picker AND the freestyle reset: a matrix of bare `CellMiniatureBuilder` scale models over `Cell.AvailableConfigs`, sampled from the generator's real output with no prisms spawned; selection routes through `Cell.RequestCellSwap`), `SpawnMatrixToy` + `SpawnMatrixToyDefinitionSO` (the RELEASE bench, three kingdoms deep: kingdom row **Fauna / Flora / Vessels** → that kingdom's species row or the vessel hangar → a species' 4-element variant ROW (a lifeform is its species and its element and nothing else — `Docs/ECOSYSTEM.md §40`; each station wears that element's crystal at its own authored heart size, so the row shows the real size difference between the four before you release any of them) → the release. A lifeform variant spawns a population through the canonical cell spawn paths on a runtime config clone; a **hangar hull releases an AI-piloted vessel of that class in the player's own domain** via `MenuServerPlayerVesselInitializer.RequestSpawnAiCompanion` — the menu's ordinary networked spawn pipeline, host-direct or client-asks-host over `ClientPlayerVesselInitializer.RequestAiCompanion_ServerRpc`, so the bot exists once on the server and replicates to the party. Its roster + mini hulls + domain-preview colour are shared with the vessel changer through `ToyVesselRoster` — the two differ in ONE argument, since the changer excludes the hull you fly and the hangar excludes nothing), `ToyVesselRoster` (the one "which hulls do I offer, and what does one look like?" answer), `ToyMatrixStation` (shared fly-through choice station), `ToyboxController` (places toys near the membrane), `ToyboxSO`/`ToyDefinitionSO` (registry + deferred unlock state), `ToyboxSetupTool` (editor) | `_Scripts/Controller/Toys/`, `_Scripts/ScriptableObjects/Toys/`, `_Scripts/Editor/` |
| Menu screens | `HomeScreen`, `ArcadeScreen`, `StoreScreen`, `HangarScreen`, `LeaderboardsMenu`, `EpisodeScreen` | `_Scripts/UI/Screens/` |
| Credits screen (licence-critical) | `CreditsManifestSO` (+ `Resources/CreditsManifest.asset` — the ONE place a third-party notice is authored), `CreditsModal` (rows GENERATED from the manifest; scroll height MEASURED, never authored), `ScreenSwitcher.ModalWindows.CREDITS` + `EnsureCreditsModal()`, `GameSettingsPanelController.OpenCredits()` + `EnsureCreditsRow()`, `CreditsReleaseGuard` (fails a non-development build with no FMOD line), `Tools/Build/check_credits_manifest.py` (`--check`/`--self-test`). **This is an obligation, not chrome** — FMOD's EULA clause 3 requires an in-game credit naming *FMOD* and *Firelight Technologies Pty Ltd.* on every tier, and the same screen carries every other notice the tree owes (`Docs/THIRD_PARTY_REGISTER.md` §7). Adding the next notice is an ASSET edit. Both the window and the settings row are **ensured in code** and stand down the moment a human authors the real thing. | `_Scripts/ScriptableObjects/`, `_Scripts/UI/Modals/`, `_Scripts/Editor/Build/`, `Assets/Resources/` |
...
| Offline / single-player fallback | `OfflineModeService` (starts the plain `127.0.0.1` host; single writer of `GameDataSO.IsOfflineSession`; persists `OfflinePreferred`), `ReconnectService` (`ReconnectAsync` / `GoOfflineAsync` — both re-run the boot chain via the Authentication scene), `LocalCloudDataCache` (last-known-good disk snapshot under every `CloudDataRepository`, so name/vessel unlocks/episodes/progression survive offline), `OfflineUIGate` + `ReconnectButton` + `OnlineStatusIndicator` + `ConfirmQuestionBar` (the menu surface). **Offline is a LOCAL HOST, not "no netcode"** — host == server == client, so the whole spawn chain, scene management and AI backfill run unchanged. See `Docs/OFFLINE_MODE.md` | `_Scripts/System/`, `_Scripts/System/CloudData/Providers/`, `_Scripts/UI/Elements/` |
...
- **`Instantiate`ing a prefab that carries a `NetworkObject` without spawning it.** Netcode adopts every un-spawned NetworkObject in a loaded scene as an IN-SCENE PLACED object the moment the machine becomes a server, and indexes in-scene objects by `(GlobalObjectIdHash, sceneHandle)` — every instance of one prefab shares that hash, so the SECOND un-spawned instance makes `NetworkSceneManager.PopulateScenePlacedObjects` **throw**, leaving that NetworkManager unable to synchronize any guest for the rest of the session. It fails invisibly on the machine that causes it and surfaces on the OTHER machine as `[Deferred OnSpawn]` warnings and a join timeout. The fauna prefabs shipped exactly this way (a NetworkObject as the `NetworkSynced` opt-in, with the opt-in off), which is why a host that restarted in place — a party leave, a failed-join bounce — could never take another guest, while a cold-booted one always could. Either spawn it or strip it: `NetworkSceneObjectGuard.NeutralizeStray(go, reason)` at the creating site, and `NetworkSceneObjectGuard.Sweep(reason)` immediately before anything that starts a NetworkManager. Audit with FrogletTools > Validation > Audit Stray Network Objects. See `Docs/PartySystem/BUGS.md` B16
...
- **Renaming or removing an ENUM MEMBER and trusting a compile-free check to find the stragglers.** A `dotnet build` over the changed C# with no Unity assemblies — the standing out-of-editor gate — is a **SYNTAX gate and nothing more** for any file whose base type lives in the `Assembly-CSharp` monolith, which is every `MonoBehaviour` and every `ScriptableObject` in this project. Roslyn abandons class-body binding when the base type is unresolved, so a renamed member inside a serialized field's DEFAULT is reported as *nothing*: `[SerializeField] CombatHitClass hitClass = CombatHitClass.Missile;` came back clean over the very file that carried it, and the Editor found it on the next compile. **Stubbing Unity's attributes does not fix it** (measured — only the unresolved BASE type matters), and stubbing the monolith is not maintainable. So a rename is checked TEXTUALLY: run `python3 Tools/Build/check_enum_member_references.py` (~3 s, no Unity), which verifies every `EnumName.Member` reference in `_Scripts` against the members that enum actually declares. It is scoped on purpose — unqualified references only (`Slider.Direction.LeftToRight` and `line.Direction.magnitude` are ordinary code, not defects), `System.Enum`'s own members allowed, two same-named enums skipped rather than guessed — and `--self-test` is its own negative control (it reproduces the escaped bug and the four shapes that must NOT fire), because a gate nobody has watched FAIL is a gate nobody should trust. **General rule: a check that cannot resolve a type cannot see errors ABOUT that type, so what it proves shrinks silently as the code under it gets more Unity-shaped** — say what a gate actually covered, never what it is named after. **And do not conclude "no compiler here" from a missing `dotnet` on `PATH`** — a per-user dotnet-sdk installs in ~40 s with no root (`.claude/skills/asset-surgery` §4), and a stub harness carrying only the API surface your new files touch turns this paragraph's whole blind spot — missing members, drifted overrides, argument mismatches — into a REAL Roslyn type check. Worth the twenty minutes on a new system; the textual fallbacks below are for a small edit inside a large existing file. **Its cheapest failure mode is emptier still: a whole-file `#if UNITY_EDITOR` makes an out-of-editor compile see NOTHING unless the symbol is defined** — 78 of the project's 111 test files open with one (it is the convention), so a syntax check over a test file without `<DefineConstants>UNITY_EDITOR</DefineConstants>` compiles zero lines and reports zero errors, which is indistinguishable from clean. Confirm the compile actually saw the file (a zero unresolved-type count on a file full of `MonoBehaviour`s is the tell) before believing it.
- **Taking "the next free id" in an enum two branches are both extending.** Adding an enum MEMBER is the mirror of renaming one, and it is worse, because C# permits duplicate enum values outright — they are aliases — so the enum itself compiles and *every switch that has to tell the two apart* is what breaks: a switch statement gives **CS0152** ("multiple cases with the label value '46'") and a switch expression gives **CS8510** ("the pattern is unreachable"). Both need class-body binding, so a syntax-only out-of-editor compile is blind to them, and `check_enum_member_references.py` cannot see them either — it asks whether a NAME exists, and both names do. Shipped **twice in one branch**: two parallel game-mode branches each took the next free `GameModes` id, so Tollway landed on 45 against Switchback, was renumbered to 46, and landed on 46 against Hijack — three compile errors (`EndConditionOverridesSO`, `ElementalComebackSystem`, `MiniGameHUD`) that first appeared in the Unity console. The gate is `python3 Tools/Build/check_switch_label_collisions.py` (~2 s, no Unity): it resolves each enum's members to integer values (following `A = B` aliases) and fails when two names sharing a value are BOTH used as a switch label. The second half of that predicate is what makes it usable — `Any = -1`, `Random = 0`, `None = 0`, `All = ~0` are legal aliases nothing switches on, so a blanket duplicate-value ban would be noise; what is reported is the pair a switch actually has to separate. `--self-test` is its negative control (both error shapes fire, both legal-alias shapes do not), and it was additionally run against the real colliding tree before the fix, where it named all three files. **General rule: a merge cannot invent a conflict between two edits that never touched the same line** — both branches appended to different ends of one enum — so the collision an id registry exists to prevent is exactly the one git will hand you silently.
- **Shipping C# you could not compile, and calling the out-of-editor gates a compile.** The three gates above are each written for ONE defect class, and the classes they leave uncovered are not random — they are exactly the ones that survive an unresolvable compile, i.e. the ones decidable on **syntax alone**. Two more of those are gated, and both were written after the errors they catch reached the Unity console from a branch whose every other check was green. `python3 Tools/Build/check_using_directives.py` catches an unqualified first-party type with no `using` that can reach it — **scoped to CHANGED files on purpose**, because it has to guess whether an unqualified name is first-party and project-wide that guess yields 143 false positives (`Key`, `Direction`, `Frame` and `Stats` all also exist in UnityEngine or NUnit). **That scoping silently broke itself once and the failure shape generalises to any tool that picks a diff base from a list:** it treated an EMPTY diff as "this base did not work, try the next one" and fell through to the *local* `bleeding-edge`, which on the day it was found was **661 commits stale** — so a branch sitting exactly at its base widened from 1 file to 411 and reported 15 pre-existing findings belonging to nobody's change. An empty diff is a REAL answer, not a failed one; the first base that RESOLVES now wins, uncommitted work is always unioned in, and the chosen scope is PRINTED on every run (`scope: origin/bleeding-edge...HEAD + uncommitted`) — because a gate that can quietly widen its own scope produces findings indistinguishable from real ones, which is the same disease as a gate that cries wolf. **Fetch before you trust a local base ref, and make every gate say what it actually looked at.** `python3 Tools/Build/check_self_referential_locals.py` catches a local declarator naming itself in its own initializer (`int seed = seed != 0 ? seed : Roll();`); it needs no type resolution at all, so `--all` is clean over the whole tree (0 findings across 1,849 files) and it is safe to run in CI. **CS0165 is the LUCKY half of that second bug** — the compiler objects only because the local happens to be unassigned at that point; give the initializer a well-defined value and C# says nothing at all while the code silently reads the local's default instead of the serialized field it was reaching for. Writing that gate is its own cautionary tale: the first cut produced ~100 findings, **every one false**, in three shapes — an optional parameter default (whose "initializer" runs on into the method body), a keyword in the type position (`else offset = offset / d;` is a re-assignment, not a declaration), and an object-initializer property (`new Foo { options = development }` names *Foo's* property). All three are self-test cases now, beside the real bug as a negative control. **A gate nobody has watched fail is a gate nobody should trust — and a gate that cries wolf a hundred times is one nobody reads.** State plainly what is left: every error needing a symbol table (a member that does not exist, an override whose signature drifted, an argument type mismatch) is **editor-only**, and no amount of scripting moves it. **The nastiest member of that set is OVERLOAD RESOLUTION, because it misnames its own cause:** when no applicable overload of a method is in scope, the compiler reports the nearest candidate it *can* see — which may belong to an API you have never touched — and complains about its arguments. `ConnectedClientsIds.Contains(id)` in a file without `using System.Linq` (`ConnectedClientsIds` is an `IReadOnlyList<ulong>`, whose `Contains` is LINQ's) fails as *"no argument given that corresponds to the required formal parameter 'comparisonType' of `MemoryExtensions.Contains(ReadOnlySpan<char>, ReadOnlySpan<char>, StringComparison)`"* — a **span** API named for a **list** call, pointing at neither the collection nor the absent using. **An error naming an API you did not use means the method you wanted is not in scope: look for a missing `using`, not a wrong argument.** All six gates pass on it (measured, not assumed — `Contains` is a METHOD and `check_using_directives` resolves TYPES), so this class is uncovered by construction and stays editor-only. Two habits blunt it: prefer the receiver's OWN instance method where one exists (`HashSet<T>.Contains`, `List<T>.Contains`, `string.Contains` need no using and no enumerator), and when a roster has to be scanned, index it the way `SpectatorSession.CountHumanClients` and `MultiplayerMiniGameControllerBase.IsClientConnected` do — `var ids = nm.ConnectedClientsIds; for (int i = 0; ...)` — which depends on no using a future tidy-up can remove. **A SEVENTH gate watches a defect that is not a compile error at all** — `python3 Tools/Build/check_fauna_replication_seam.py` (`--self-test`) requires every runtime `Instantiate` of a fauna PREFAB to reach `FaunaNetworkSync.ServerSpawn` in the same method, because an un-spawned `NetworkObject` is adopted by Netcode as an in-scene object keyed on a hash every instance of a prefab shares, so the SECOND one breaks synchronization for every later joiner (`Docs/PartySystem/BUGS.md` B16, then B5). It is here because that shipped **twice**, and the second time through producers that were each individually reasonable: they called the spawner's own entry point and had no reason to suspect the seam lived one level up in a sibling. The seam has since moved to the single `Instantiate` every producer reaches, which closes them by construction — *this gate covers the remaining door*, a producer that calls `Instantiate` itself, and **RUNNING it found three more that reading the diff had not**: `BoidManager` (100–150 `TadPoleFauna`, one of the two prefabs that actually carry a `NetworkObject` — live in the since-retired `MinigameDuelForTheCell`), `LightFaunaManager` (a school of `QuadFish`) and `WormFauna` (five producers, dormant). None of them names a fauna, which is why the gate WIDENS inside a file declaring a `Fauna` subclass: there any `…Prefab` is a creature, and `Instantiate(this, …)` is a colony splitting itself. It stays clean because what a creature class instantiates that is NOT a creature does not end in "prefab" (`Boid.cs`'s `healthPrism` is a negative control). Two costs it states rather than hides: outside a creature file it is still narrow, and it asks about the enclosing METHOD — so a producer that hands its newborn to a funnel one call away is reported even when that funnel is right. That is deliberate, and the answer is to move the `Instantiate` INTO the funnel, which is what `WormFauna.AddSegmentToChain` now does. The runtime backstop for whatever is left is `NetworkSceneObjectGuard.Sweep` at connection approval. **The general shape worth copying: when a rule has to hold at N producers, enforce it at the one call they all reach, and gate the door that leaves.** **An EIGHTH gate covers the one syntax-decidable class the other seven missed** — `python3 Tools/Build/check_duplicate_attributes.py` (`--self-test`) fails on a member carrying the same `AllowMultiple = false` attribute twice (CS0579). It exists because **inserting a serialized field ABOVE an existing one is the edit that produces it**: the replacement targets the existing field's `[SerializeField]` line, which STRANDS that field's own `[Tooltip]` on top of the inserted block, so the new field ends up with two and the old one with none. That shipped in `SniperShotActionSO.cs` and reached the Unity console with all five standing gates green, because CS0579 is an attribute error inside a CLASS BODY and every one of those gates is a syntax gate for a file whose base type lives in the monolith — the blindness this document already describes, met by a defect that is nevertheless decidable without a single type. The scanner's positive control is that file's own broken text, and running it over the pre-fix tree reports the duplicate `[Tooltip]` at the line Unity named **plus a duplicate `[Header]`**, which is the SECOND error the compiler would have given. General rule: **an attribute is part of the member BELOW it, so an insertion point is between a field and its own attributes unless you put it above them** — and the failure is loud in the editor and silent everywhere else. **A NINTH gate takes one bite out of the symbol-table set** — `python3 Tools/Build/check_abstract_member_implementations.py` (`--self-test`) fails when a concrete class does not implement an inherited abstract member (**CS0534**). It walks every first-party base chain textually and asks whether SOME type between the class and the declaring base carries an `override` of that name, which is an EXACT test rather than a heuristic, because an override is the only thing C# accepts. It is here because a new vessel shipped `ButterflyHUDView : VesselHUDView` with no `Initialize()` past every gate then standing AND a Roslyn syntax pass over every changed file — and **that pass is the trap, not the backstop**: when Roslyn cannot resolve a base type (every `MonoBehaviour` and `ScriptableObject` in the monolith) it abandons class-body binding and reports *nothing*, which is indistinguishable from clean. It matches on member NAME only, so it catches CS0534 and deliberately NOT **CS0115** (an override whose signature drifted), and it is silent on abstract or partial classes and wherever a base chain leaves the tree. That narrowness is what makes it usable: 0 findings over 2,103 types today, and reverting the Butterfly fix reproduces the shipped error by file and line. **A DEFAULT INTERFACE MEMBER is the symbol-table class that reads as a missing feature, and `IVesselStatus` has TEN of them.** `Domain`, `PlayerName`, `IsLocalUser`, `Transform`, `InputController`, `InputStatus`, `IsInitializedAsAI`, `AutoPilotEnabled`, `IsImmuneToElementalDebuff` and `ImmuneDebuffSources` are implemented in the INTERFACE BODY, mostly over `Player` — so they exist on `IVesselStatus` and NOT on `VesselStatus`, and the extremely natural `GetComponentInChildren<VesselStatus>().Domain` is `CS1061: 'VesselStatus' does not contain a definition for 'Domain'`, an error that names a property the project plainly has. It is editor-only by construction (it needs the symbol table), it is invisible in review because every neighbouring line reads the same property off an interface-typed field, and the fix is a TYPE rather than a member: resolve the concrete component, do the Unity null check on IT (`== null` on an interface-typed variable is a plain reference compare and misses a destroyed `Object`), then assign to an `IVesselStatus` local and use that. `SniperShotActionExecutor.StripVessels` is the worked example. **And the gate that watched this hull's verb was pinned to one SPELLING of it**: `check_elemental_economy.py` resolved the Serpent's anti-vessel transfer by grepping for `ElementalTransfer.Eject`, so tidying that call into the sanctioned `ApplyAll(ElementalTransferForm.Eject, …)` made the gate report the hull as unable to affect a pilot at all — *a grep-backed capability check must ask which VERB, never which word*, and a refactor that cannot change behaviour can still change a gate's answer.
...
- **A `[Inject]` field that is null on a UI object is not a lost feature — it is a DEAD BUTTON, because a persistent UnityEvent listener that throws eats every runtime listener behind it.** `UnityEvent.Invoke` builds its call list as **persistent-then-runtime** (`InvokableCallList.PrepareInvoke`: `m_ExecutingCalls = m_PersistentCalls + m_RuntimeCalls`) and guards neither, so an exception in an inspector-wired listener aborts the invoke before any `AddListener` handler runs. `MenuAudio.PlayAudio` — which dereferences an `[Inject] AudioSystem` and is the ONE persistent `onClick` listener every arcade `GameCard`'s Button carries — is the shipped instance: `ArcadeExploreView.EnsureGridCapacity` creates a card row at RUNTIME to hold a 13th game mode, Reflex injected nothing in it, `PlayAudio` threw, and `() => SelectGame(game)` never ran. **The card rendered with the right title and art, reported `interactable = true`, passed an `EventSystem` raycast, and opened nothing** — so four separate investigations chased geometry, masks, scroll extents, overlays, the progression lock and the modal, none of which could see a runtime-only difference. It reads as POSITIONAL rather than as belonging to a mode, because only cards in the cloned row are un-injected and only the first of them is active at 13 modes. Two rules come out of it: **(1) inject at the creating site** — a runtime `Instantiate` of ANY object carrying `[Inject]` needs `GameObjectInjector.InjectRecursive(go, container)` right there (the idiom is `[Inject] Container _container` on the creator). **`new GameObject(...)` + `AddComponent` is the same hazard and the word "Instantiate" hides it** — a window or row BUILT at runtime is un-injected exactly as a cloned prefab is, and the `[Inject]` it carries is usually one it inherited from a base class rather than one you can see at the construction site (`ModalWindowManager`'s `AudioSystem` is the live example). **This was the THIRD outing of one bug class, and the first two had already written the fix down**: `ProfileIconSelectView.cs:167-170` (*"Reflex doesn't auto-inject Instantiate()'d prefabs — inject so the button's `[Inject] AudioSystem` resolves (otherwise OnClick NREs on the audio call)"*) and `ProjectilePoolManager.cs:27-33` (*"an un-injected projectile NREs on its null AudioSystem in LaunchProjectile and every shot from that instance is a dud"*) — so a comment written twice for other subsystems was not enough to stop it, which is why the guard below exists instead; and **(2) anything on a persistent listener list must fail SAFE**, because rule 1 has to be remembered once per spawn site forever while rule 2 holds everywhere at once — `MenuAudio` now falls back to `AudioSystem.Instance` and warns once per component, so the next spawn site that forgets loses a SOUND rather than a BUTTON. General shape: **when one component sits on the click path of most of your UI, its failure mode is the whole UI's failure mode**, and "silently does nothing" is the worst one available. The guard is **`Tools/Build/audit_persistent_listener_injection.py`** (`--check`), which resolves every persistent `UnityEvent` listener in every scene and prefab to its target's script and fails on any pair `(class, method)` whose class declares an `[Inject]` field and is not on a reviewed list. It RATCHETS — its **56** pairs are frozen as an explicitly *unreviewed* baseline so the gate passes now and fails on anything NEW; only three are actually reviewed (`MenuAudio.PlayAudio` and the two modals whose close path goes through it, all of which fall back and warn). Two things it deliberately does NOT do: it does not guess a class from `m_TargetAssemblyTypeName` (Menu_Main serialises **190** stale `CosmicShore.App.*` names for a namespace that exists in zero first-party files, and Unity resolves a persistent call from the LIVE target's type, so guessing invented pairs like `MenuAudio.SetActive`, which is not a method on `MenuAudio`), and it does not claim to prove the dereference is unguarded — it reports the SURFACE and makes a human answer one question per pair: *if this object were never injected, does the press still do its job?* **The gate itself was wrong for its whole life and nobody noticed, which is the part worth carrying.** Unity serialises a call as `m_Target` → `m_TargetAssemblyTypeName` → `m_MethodName`, and one regex asked for the method first and the type second: with `re.S` and `.*?` each match ran past its own entry into the NEXT one, so every call was labelled with its successor's type name and — because `finditer` resumes after the match — **alternate calls were skipped entirely**. It read **279 of 992** persistent calls (71.4% invisible) and mislabelled **236** of the 279. It surfaced only as a wrong class NAME, because a type name belonging to another component failed the `in names` test and fell through to a fallback that picked the alphabetically first type in the file — so `ToyConfigureModal.ModalWindowOut` reported as `Layer.ModalWindowOut` (a nested struct) and two `ScreenSwitcher` handlers were **written into the baseline as `ModalStackEntry`**, a nested struct that can never be a `MonoBehaviour`. Three rules generalise past this file: **a regex over serialised YAML must not assume field order** (split the list into entries, then read each entry's own fields — `.*?` across an entry boundary does not fail, it attributes one record's data to another); **count what you parsed against something independent** (`- m_Target: {` and `m_MethodName:` each occur exactly 992 times, so either number would have exposed this on day one — `--check` now asserts that agreement every run); and **a fallback that guesses is a defect amplifier** (the alphabetical pick turned a parse bug into a plausible class name, which is how two wrong rows got written down as fact — resolution is now filename-first, since Unity requires a serialised `MonoBehaviour`'s class to match its file, and every remaining guess is counted and reported rather than presented as an answer). The baseline grew 28 → 56 at that fix: **not one row is new debt**, they are the rows the parser was blind to.
