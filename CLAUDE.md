...
| Projectile charge shell | `ProjectileChargeField.shader` + `.hlsl` — the SELF-DRIVEN member of the crackle family, and the reason it exists is a **rule**: **MASS in-flight growth is a HIT VOLUME, not a size.** A round's MODEL is the size it left the muzzle for its whole flight (swelling it drew a small ship firing cannonballs — the exact silliness the growth pass was written to avoid); this see-through additive shell, sized every frame to exactly the swept hit radius, is what the player reads the growth off, so the instrument IS the hit volume rather than a proxy for it — and a pilot can still see the arena through their own enormous round. `Projectile.ApplyFlightGrowth` therefore scales the transform ONLY when that transform draws nothing (`CacheTransformRole`, derived from the prefab's own renderers rather than authored — the Turret Stance's carried collider IS its hit volume and must keep growing; the bullets' tracer is a model and must not). The shell drives itself off `_Time` and its OWN object-to-world matrix — no per-frame CPU write, no MaterialPropertyBlock, one `UnityPerMaterial` cbuffer so every round SRP-batches — because a single Sparrow keeps ~54 rounds in the air at 90 volleys/s and the skimmer's per-renderer property-block driver would be ~54 extra draw calls a frame. Rounds are decorrelated by their own SIZE (different growth ⇒ different radius ⇒ different phase, drifting continuously so nothing pops); a world-position hash re-rolls every frame and a constant strobes the volley in unison. Keep the shell UNROTATED — the dart's transform is non-uniform (0.75, 0.75, 20) and `Projectile.ChargeFieldLocalScale`'s per-axis divide is only valid axis-aligned. **Because the shell is the instrument, the MODEL's size became a free dial** — halving the dart's cross-section left the hit radius at 0.825 untouched, since a `SphereCollider` takes the LARGEST lossy component and that is the dart's z-stretch. Its palette is **neutral blue + `EnvironmentColors.Danger` verbatim**, and the two are held apart by `_CoreThreshold` rather than blended: **a lerp between two saturated opposite hues spends its range in a third colour** (magenta here, 12.6% of lit pixels before, 7.4% after), so a two-colour effect reading as one muddy colour wants a threshold, not a new pair. And **"desaturated" is a screen measurement**: ACES compresses highlights, so authored pale blues rendered at screen saturation 0.03-0.06 (white) and the shipped pair was chosen by measuring post-tonemap sRGB into the 0.20-0.30 band. The dart also needed its OWN material — `DangerProjectileMaterial` is shared by five prefabs. Detail: `R_VesselActions/SPARROW_SPRAY_ACCURACY.md` § Round 4 | `_Graphics/Materials/Graphs/`, `_Scripts/Controller/Projectiles/` |
