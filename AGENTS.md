...
- **No domain asymmetry.** Fauna spawn in **one color — the cell's controlling color**. Never
  ...
  A mode may redefine what "controls" a cell — Brood Rush makes it the nucleus claim (PeelTheCage
...
Note: A vestigial `_Scripts/Game/` directory exists containing only non-code assets (compute shaders, input action mappings, material files). All C# code has been reorganized into the directories listed above.
...
| `PEEL_THE_CAGE.md` | `_Scripts/Controller/Arcade/` | PeelTheCage / "Peel the Cage" technical reference (Rhino-only cage-breaking race; the layered-orange intensity model, the open-weave generator, the shielded-mass targeting-grid rule, and the record of the removed fauna ladder) |
