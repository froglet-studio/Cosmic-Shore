...
namespace CosmicShore.Game.Arcade
{
    ...
    {
        [Header("Course Settings")] [SerializeField]
        Crystal crystal;
        ...
        [FormerlySerializedAs("resetTrailsEachTurn")] [SerializeField]
        bool resetEnvironmentOnEachTurn = true;
        ...
        [Header("Helix Optional")] [SerializeField]
        SpawnableHelix helix;

        [SerializeField] float helixIntensityScaling = 1.3f;


        int numberOfSegments => scaleSegmentsWithIntensity
            ? baseNumberOfSegments * miniGameData.SelectedIntensity
            : baseNumberOfSegments;

        int straightLineLength => scaleLengthWithIntensity
            ? baseStraightLineLength / miniGameData.SelectedIntensity
            : baseStraightLineLength;

        Vector3 crystalStart => scaleCrystalPositionWithIntensity
            ? crystalStartPosition * miniGameData.SelectedIntensity
            : crystalStartPosition;
        ...
        protected override void StartNewGame()
        {
            ...
            if (helix)
            {
                helix.firstOrderRadius = helix.secondOrderRadius =
                    miniGameData.SelectedIntensity.Value / helixIntensityScaling;
            }
