    ...
    void CreateRings()
    {
        ...
        for (int ring = 1; ring <= ringCount; ring++)
        {
            ...
            for (int block = 0; block < blockCount * ring; block++)
            {
                ...
                CreateRingBlock(block, 0, 0, 0, trails[ring-1], ring, trailBlock, domain); // old value for phase = ring % 2 * .5f
            }
        }
    }

    void CreateRingBlock(int i, float phase, float tilt, float sweep, Trail trail, int ring, TrailBlock trailBlock, Domains domain)
    {
        ...
        CreateBlock(position, position + tilt * ringThickness * transform.forward, "::SpawnableDartBoard::" + Time.time + "::" + i, trail,
            new Vector3(((Mathf.PI / 3f) * ringThickness) - (gap / (2 * ring)), // blockwidth 
                        (ringCount - ring) * ringThickness/3f, // dartboard thickness
                         ringThickness - (gap/5f)), trailBlock, domain); //annulus thickness
    }

    void CreateBlock(Vector3 position, Vector3 lookPosition, string blockId, Trail trail, Vector3 scale, TrailBlock trailBlock, Domains domain)
    {
