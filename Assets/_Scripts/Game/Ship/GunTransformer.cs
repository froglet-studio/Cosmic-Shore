        ...
        void Update()
        {
            ...
            foreach (var child in GetComponentsInChildren<Transform>())
            {
                ...
                var j = i * constant + (Mathf.PI)/2 - Mathf.Atan2(InputStatus.RightNormalizedJoystickPosition.y,
                                                   InputStatus.RightNormalizedJoystickPosition.x);//Get the 90 and orients to the correct point | J is a angle 
                i++;

                child.transform.localPosition = Vector3.Slerp(child.transform.localPosition,radius * new Vector3(Mathf.Sin(j), Mathf.Cos(j), 0),Time.deltaTime*j);//Slerps th child transform to the postion of J
