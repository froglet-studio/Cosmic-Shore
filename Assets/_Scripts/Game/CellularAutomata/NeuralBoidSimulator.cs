...
namespace CosmicShore.Game.CellularAutomata
{
    ...
    public class NeuralBoidSimulator : MonoBehaviour
    {
        ...
        [SerializeField] Prism boidPrefab;
        ...
        readonly List<Prism> _boidVisuals = new();
        int _kernel;
        bool _initialized;
        ...
        void SpawnBoids()
        {
            ...
            for (int i = 0; i < initialBoidCount; i++)
            {
                ...
                if (boidPrefab != null)
                {
                    var visual = Instantiate(boidPrefab, pos, Quaternion.LookRotation(vel));
        ...
        void SyncVisualsFromGPU()
        {
            ...
            for (int i = 0; i < _entities.Length && i < _boidVisuals.Count; i++)
            {
                ...
                boid.transform.position = entity.position;

                if (entity.velocity.sqrMagnitude > 0.0001f)
                    boid.transform.rotation = Quaternion.LookRotation(entity.velocity);
            }
        }
