...
namespace CosmicShore.Game.Projectiles
{
    public class AOEFlowerCreation : AOEBlockCreation
    {
        ...
        public void SeedBlocks(List<Prism> lastTwoBlocks)
        {
            ...
            {
                Prism block1 = CreateBlock(lastTwoBlocks[0].transform.position, lastTwoBlocks[0].transform.forward, lastTwoBlocks[0].transform.up, prism.ownerID + 1 + i, trails[trails.Count - 1]);
                Prism block2 = CreateBlock(lastTwoBlocks[1].transform.position, lastTwoBlocks[1].transform.forward, -lastTwoBlocks[1].transform.up, prism.ownerID + 2 + i, trails[trails.Count - 1]);
        ...
        void CreateBranches(Prism prism, float gap, float angle, int handedness = 1, int depth = 0, Branch branch = Branch.both)
        {
            --depth;
            if (branch == Branch.both) 
            {
                for (int i = -1; i <= 1; i += 2)
                {
                    Prism block = CreateBlock(prism.transform.position, prism.transform.forward, prism.transform.up, prism.ownerID + i, trails[trails.Count - 1]);
            ...
            else
            {
                ...
                Prism block = CreateBlock(prism.transform.position, prism.transform.forward, prism.transform.up, prism.ownerID + angle, trails[trails.Count - 1]);
                Vector3 origin = block.transform.position + (block.transform.right * (blockScale.x / 2 + gap)) * handedness;
