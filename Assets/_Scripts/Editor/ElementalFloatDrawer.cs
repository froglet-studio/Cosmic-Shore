...
public class ElementalFloatDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // Calculate rects
        Rect enabledRect = new Rect(position.x, position.y, position.width, EditorGUIUtility.singleLineHeight);
        Rect valueRect = new Rect(position.x, position.y + 18, position.width, EditorGUIUtility.singleLineHeight);
        Rect minRect = new Rect(position.x, position.y + 36, position.width, EditorGUIUtility.singleLineHeight);
        Rect maxRect = new Rect(position.x, position.y + 54, position.width, EditorGUIUtility.singleLineHeight);
        Rect elementRect = new Rect(position.x, position.y + 72, position.width, EditorGUIUtility.singleLineHeight);
