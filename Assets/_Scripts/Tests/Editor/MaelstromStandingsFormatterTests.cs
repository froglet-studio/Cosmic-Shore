...
{
    /// <summary>
    /// MaelstromStandingsFormatter tests - covers the "(You)" owner tag added beside the local
    ...
    /// </summary>
    [TestFixture]
    public class MaelstromStandingsFormatterTests
    {
        const string YouTag = "<b>(You)</b>";

        MaelstromDataSO _data;
        readonly List<ScriptableEventNoParam> _events = new();

        [SetUp]
        public void SetUp()
        {
            _data = ScriptableObject.CreateInstance<MaelstromDataSO>();
            _data.OnMaelstromStarted   = MakeEvent();
            _data.OnGameResultRecorded  = MakeEvent();
            _data.OnStandingsChanged    = MakeEvent();
            _data.OnMaelstromCompleted = MakeEvent();
        ...
        {
            var final = MaelstromStandingsFormatter.FormatFinal(_data, Domains.Ruby);
        ...
        {
            var running = MaelstromStandingsFormatter.FormatRunning(_data, Domains.Jade);
        ...
        {
            var s = MaelstromStandingsFormatter.FormatConnecting(_data, Domains.Blue);
        ...
        public void FormatConnecting_TagsLocalLead_AndShowsUpNext()
        {
            ...
            var s = MaelstromStandingsFormatter.FormatConnecting(_data, Domains.Jade);
        ...
        public void NoLocalDomain_TagsNothing()
        {
            ...
            Assert.AreEqual(0, CountTags(MaelstromStandingsFormatter.FormatFinal(_data)),
                "Blue sentinel never matches a standings row.");
            Assert.AreEqual(0, CountTags(MaelstromStandingsFormatter.FormatRunning(_data, Domains.Blue)));
        }
    }
}
#endif
