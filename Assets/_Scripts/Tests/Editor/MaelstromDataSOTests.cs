...
{
    /// <summary>
    /// MaelstromDataSO Tests - validates the per-DOMAIN {2,1,0} placement scoring fold that
    /// powers the Maelstrom/Shuffle meta-mode.
    ///
    /// WHY THIS MATTERS:
    ...
    ///     (ScoringRuleSO.ResolvePlacementOrder, passed by MaelstromController); the results-only
    ...
    /// </summary>
    [TestFixture]
    public class MaelstromDataSOTests
    {
        ...
        MaelstromDomainStanding Standing(Domains d) =>
        ...
        {
            // The End Game Conditions tool is the authority: MaelstromController stamps the resolved
        ...
        {
            // Pure data SO (no MaelstromController run) → no tool value resolved → use the field.
