...
namespace CosmicShore.Utility.Tools.DensityPartitionBenchmark
{
    ...
    public static class DensityPartitionBenchmarkAlgorithms
    {
        ...
        /// <summary>
        /// The LITERAL shipped algorithm: 17^3 count grid hard-fixed at +/-500m,
        ...
        /// audit's Sec 7.3 fix): adaptive resolution targeting 75m voxels (clamped
        ...
        //
        //  Was: brute-force scan over ~35^3 candidates x N prisms = O(N x C)
        //  Now: bin once, smooth, argmax. O(N + 3*G^3) with G=64.
        //  ~100x speedup at N=2000.
        //
        //  Conceptually: as G -> inf and kernel -> matched, this converges to the
        ...
        /// <summary>
        /// Separable 3D box filter using a sliding window. O(N^3) per pass x 3 passes,
        /// independent of kernel width - works as well for 3^3 kernels as for 25^3 ones.
        ...
        // algorithms x 36 queries x up to 3 arrays per Search). The resulting
        // GC pauses caused 5-7ms outliers in the 32^3 rows. The pool reuses
        // arrays across queries - typical hot run hits 4 distinct lengths
        // (17^3=4913, 32^3=32768, 64^3=262144, plus an occasional resize).
        // ------------------------------------------------------------------
