...
truth is still a **Development Build run standalone** (the editor inflates frame time ~2-3x).
...
flags spikes (frames over `max(33.3 ms, 1.75 x running mean)`), and writes a report to
...
Editor Play-Mode numbers are inflated (~2-3x) and subsystem costs shift versus a real build. A
...
- **Spike markers** need the Unity Profiler enabled (the window enables it when *Capture spike
  ...
  breakdown).
- **Editor frame time is inflated ~2-3x** - use low-overhead mode (close the Profiler) for a true
