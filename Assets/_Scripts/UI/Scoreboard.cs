...
namespace CosmicShore.UI
{
    ...
    public class Scoreboard : MonoBehaviour
    {
        ...
        {
            // Friction (and any mode that can end with NO winner) declares its result
            // server-side. Blue is the "no team" sentinel, so returning it unchanged is
            // what makes the banner read GAME OVER instead of crowning whoever ranked
            // first on a run nobody won.
