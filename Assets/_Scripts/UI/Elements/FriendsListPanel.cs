...
namespace CosmicShore.UI
{
    ...
    public class FriendsListPanel : MonoBehaviour
    {
        ...
        void PopulateOnlineEntry(OnlineInfoEntry entry, PartyPlayerData player)
        {
            ...
            // The X doubles as a kick when this row is a member of MY party and I'm the
        ...
        // The host clicked the X on a pending row. Retract the outgoing invite - HostConnectionService
        ...
        // The host clicked the X on a row for a member of their own party. Remove that
        ...
        async void OnKickMemberClicked(string playerId)
        {
            ...
            catch (System.Exception e)
            {
                ...
                // Re-render so the optimistically-hidden X recovers if the kick didn't take.
                PopulateOnlineSection();
            }
        }
