...
namespace CosmicShore.Core
{
    ...
    public class WeeklyChallengeLeaderboardService
    {
        ...
        public WeeklyChallengeLeaderboardService(Func<string> leaderboardId, Func<bool> isOffline = null)
        {
        ...
        public async UniTask SubmitCompletionAsync(double seconds, CancellationToken ct = default)
        {
            ...
            try
            {
                await LeaderboardsService.Instance.AddPlayerScoreAsync(id, seconds).AsMainThread();
        ...
        /// <summary>
        /// The top <paramref name="limit"/> rows, fastest first. Empty on any failure — a
        /// leaderboard that cannot be read is a panel with nothing in it, never an exception
        /// crossing into the UI.
        /// </summary>
        public async UniTask<List<WeeklyChallengeRanking>> FetchTopAsync(
            int limit = DefaultPageSize, CancellationToken ct = default)
        {
            var rows = new List<WeeklyChallengeRanking>();

            if (!TryResolveId(out string id)) return rows;
            if (!IsUsable("fetch")) return rows;

            try
            {
                // ── The ONLY place this project touches the UGS Leaderboards read API ──
                var page = await LeaderboardsService.Instance.GetScoresAsync(id).AsMainThread();
                ct.ThrowIfCancellationRequested();
                ...
                if (page?.Results != null)
                {
                    foreach (var entry in page.Results)
                    {
                        ...
                        if (rows.Count >= Mathf.Max(1, limit)) break;

                        rows.Add(new WeeklyChallengeRanking
                        {
