        ...
        async UniTaskVoid MonitorLoopAsync(int intervalSeconds, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (!reachable && _connected)
                {
                    ...
                    Debug.LogWarning($"[NetworkMonitor] Online → Offline (reach={Application.internetReachability}, t={Time.unscaledTime:F1}s)");
                }
                else if (reachable && !_connected)
                {
                    ...
                    Debug.Log($"[NetworkMonitor] Offline → Online (reach={Application.internetReachability}, t={Time.unscaledTime:F1}s)");
                }
