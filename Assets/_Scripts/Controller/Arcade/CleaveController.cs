...
{
    /// <summary>
    /// Cleave - the Rhino-only SLICING race. Domains race to destroy hostile prisms with the
    /// sword; the first to the destruction target wins. Structurally a sibling of
    /// <see cref="RampageController"/> (1 round / 1 turn, HasEndGame=false, server winner
    ...
    /// lives here either.
    ///
    /// The arena is authored, not driven from here, and each intensity is a DIFFERENT PLACE
    /// rather than a bigger version of the last: intensity picks the <c>CellConfigDataSO</c>
    /// (<c>CellTypeChoiceOptions.IntensityWise</c>) and therefore one of four unrelated
    ...
    ///      OUTSIDE the arena and damage on the transit (see ArmCageBreakers). The stations are
    ///      arena-agnostic by construction: a chord between two golden-angle stations crosses the
    ///      middle of the ball, which cuts panes, sheets, shells and ribbons alike.
    ///
    ...
    /// away from bringing the ladder back. See CLEAVE.md and Docs/ECOSYSTEM.md §22.
    /// </summary>
    public class CleaveController : MultiplayerDomainGamesController
    {
        [Header("Scoring")]
        [Tooltip("Drag CleaveScoringRule.asset - the per-mode scoring strategy (winner, scores, results).")]
        ...
        // SliceArenaGeometry rather than here, because the ARENAS are built against them too and
        // two copies of an envelope is how one of them drifts. STRICTLY > 1: AIPilot has no
        // arrive-and-stop behaviour, so any station inside the arena becomes a point the AI
        // orbits from within. Stations live outside; the mass breaks on the transit.
        const float AiStationStandoff = SliceArenaGeometry.AiStationStandoff;
        ...
        // (see CleaveScoringRuleSO.AssignScores) - lower is better, like SkimRace.
        protected override bool UseGolfRules => true;
        protected override bool UseSceneReloadForReplay => true;

        // Cleave handles end-game through OnTurnEndedCustom (server-side winner detection) →
        ...
        void AnnounceMilestone_ClientRpc(int milestone, int domain, int sum, int target)
        {
            ...
            GameToastAPI.Post(
                milestone == MilestoneSecond
                    ? GameToastSituation.CleaveHalfCut
                    : GameToastSituation.CleaveQuarterCut,
        ...
        {
            var d = (Domains)domain;
            GameToastAPI.Post(GameToastSituation.CleaveLeaderChanged, d,
        ...
        void ArmCageBreakers()
        {
            ...
            foreach (var p in gameData.Players)
            {
                ...
                pilot.SetExternalTargetProvider(() =>
                {
                    ...
                    cached = centre + dir * (arena * AiStationStandoff);
                    return cached;
                });
            }
        }
            ...
            int winnerDomain)
        {
            for (int i = 0; i < names.Length; i++)
            {
                ...
                if (stat == null)
                {
                    CSDebug.LogError($"[Cleave] Client could not match RoundStats for '{sName}'. " +
