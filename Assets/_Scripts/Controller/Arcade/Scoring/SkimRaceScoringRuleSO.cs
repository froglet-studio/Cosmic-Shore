...
{
    /// <summary>
    /// SkimRace: golf-timed crystal race. The first active domain whose summed crystals reach the
    /// target wins together; winners score their finish time, losers a sentinel encoding the
    /// team's remaining crystals (golf: lower is better). Target keeps SkimRace's historical
    /// "&gt; 0 ? configured : 39" fallback.
    /// </summary>
    [CreateAssetMenu(menuName = "ScriptableObjects/Scoring Rules/SkimRace", fileName = "SkimRaceScoringRule")]
    public class SkimRaceScoringRuleSO : ScoringRuleSO
    {
