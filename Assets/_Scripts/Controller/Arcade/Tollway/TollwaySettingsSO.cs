...
namespace CosmicShore.Gameplay
{
    ...
    public class TollwaySettingsSO : ScriptableObject
    {
        ...
        [Header("Anchors (the CERTAIN PLACE a ring has to go)")]
        [Tooltip("How close a free plant's heart must be to the AI's nose-line before it presses " +
                 "the switch. This is the AI's own AIMING window, not the admission rule - the " +
                 "vessel owns that (PlaceSwitchActionSO.anchorReach, measured against the segment " +
                 "from the ship to the projected ring centre). Deliberately NOT the same number in " +
                 "two places: too loose only costs a refused press, which is free, and too tight " +
                 "only makes the AI fussier about its approach.")]
        [Min(10f)] public float aiAnchorAimTolerance = 70f;
