...
{
    public class SkimRaceController : MultiplayerDomainGamesController
    {
        [Header("Course")]
        ...
        // SkimRace handles end-game through OnTurnEndedCustom (server-side winner detection) →
        ...
        private async UniTaskVoid WaitForTrackSeed(CancellationToken ct)
        {
            try
            {
                ...
                Debug.LogWarning("[SkimRaceController] Client poll: timed out after 5s waiting for track seed.");
                ReleaseArenaBuildAnnouncement();
            }
        ...
        /// <summary>
        /// Server-side winner detection, mirroring JoustController.OnTurnEndedCustom().
            ...
            int winnerDomain)
        {
            for (int i = 0; i < names.Length; i++)
            {
                ...
                if (stat == null)
                {
                    CSDebug.LogError($"[SkimRace] Client could not match RoundStats for '{sName}'. " +
        ...
        // OnResetForReplayCustom removed - SkimRace uses UseSceneReloadForReplay = true,
