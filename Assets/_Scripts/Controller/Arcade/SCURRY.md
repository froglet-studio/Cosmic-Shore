...
- **Single scene**: `Assets/_Scenes/Multiplayer Scenes/MinigameScurryMultiplayer_Gameplay.unity` — no separate singleplayer scene
- **Single GameMode enum**: `GameModes.Scurry = 35`
- **Always Netcode**: `ScurryController` extends the multiplayer controller hierarchy. Even solo play runs through Netcode (host is always active from Menu_Main)
...
- **Golf-timed scoring** (finish-time change): winning-domain players' `Score` = match finish
  ...
  line). Lower = better, like SkimRace
...
```
gameData.SceneName                = "MinigameScurryMultiplayer_Gameplay"
gameData.GameMode                 = GameModes.Scurry
...
Scene Load Complete
│
├─ ScurryController.OnNetworkSpawn()
...
Unlike SkimRace, Crystal Capture has **no deterministic track generation** — the environment is scene-placed.
...
> results are produced by `ScurryScoringRuleSO` (`IsObjectiveReached` / `ResolveWinner`
...
│   │       └─ ScurryController.OnTurnEndedCustom()  [server only]
...
│               └─ ScurryController.SetupNewRound() override
│                   └─ if (_finalResultsSent) return  — suppresses Ready button
│
├─ ScurryStatsReporter.ReportStats()  [each client, on OnMiniGameEnd]
│   └─ [Winning-domain players] Report finish time + vessel telemetry to UGS
```

**Scoring Rules:**

Ranking is **TEAM-major** (matching SkimRace/Joust, where the team outcome dominates the
...
replicated via the snapshot RPC); losers carry `GolfScoreSentinels.EncodeSkimRaceLoserScore(teamRemaining)`
(the shared time-golf sentinel scheme — the "SkimRace" naming is legacy). Individual crystal
counts still order teammates and fill the scoreboard's secondary line.

### 8. End Game (Scoreboard)

There is no end-game cinematic. When the game ends, `EndGameSequencer` halts the vessels, plays the GameEnd SFX, and raises `OnShowGameEndScreen` — the signal the **`Scoreboard`** (and `LifeForm` ecology cleanup) already listen for. The `Scoreboard` is the sole end-game UI: a `"{DOMAIN} VICTORY"` banner plus one ranked `PlayerScoreCard` per player. Card order, score and the secondary line all come from `ScurryScoringRuleSO` (`Results`) — the single scoring source (crystal totals / difference vs the opposing domain).

The old animated per-player `VICTORY`/`DEFEAT` reveal belonged to the removed cinematic. `ScurryScoringRuleSO.BuildReveal` is retained but currently unconsumed.
...
|---|---|---|
| In-game HUD | `ScurryHUD` (extends `MultiplayerHUD`) | Per-player crystal count cards; subscribes to `OnCrystalsCollectedChanged`; refreshes all cards on turn start |
| Scoreboard | `Scoreboard` (base — per-mode subclass deleted in the scoring refactor; scene-added component, `gameController` wired in inspector) | End-game player ranking; "N Crystals" per card from `ScurryScoringRuleSO.BuildResults`; team-major order (winning domain's players first, then by individual crystals) |
...
| Stats Reporter | `ScurryStatsReporter` | Reports finish time + vessel telemetry to UGS (winning-domain players; `IsFinishTime` gate) |
...
Note: `ScurryController` declares **no NetworkVariables**. All network sync is via ClientRpc arrays.

## Stats & Telemetry

**UGS Stats Reporting** (winner only, via `ScurryStatsReporter`):
...
| Role | File | Location |
|---|---|---|
| Game controller | `ScurryController.cs` | `_Scripts/Controller/Arcade/` |
...
| In-game HUD | `ScurryHUD.cs` | `_Scripts/UI/` |
| Scoreboard | `ScurryScoreboard.cs` | `_Scripts/UI/` |
| End-game scoreboard (shared base) | `Scoreboard.cs` | `_Scripts/UI/` |
| Stats reporter | `ScurryStatsReporter.cs` | `_Scripts/Controller/Arcade/` |
...
3. **Score = finish time / loser sentinel (per-player)**: winning-domain players carry the match time, losers the team-remaining sentinel; the scoreboard's secondary stat reads `CrystalsCollected` directly for individual contribution. The winner banner and end-game attribution use the domain aggregate via `WinnerDomain`. NOTE: UGS `ScurryStats.HighScores` values recorded before this change were crystal counts and shadow real times until cleared server-side (`UGSStatsManager.GetEvaluatedHighScore`).
...
5. **UseSceneReloadForReplay=true**: Same as SkimRace — full network scene reload for clean state. The in-place reset path (`OnResetForReplayCustom`) is retained as a fallback but not used by default.

6. **No comeback system**: Unlike SkimRace (which uses `ElementalComebackSystem`), Crystal Capture has no handicap or catch-up mechanics. It is a straightforward competitive race.

7. **HUD refreshes on turn start**: `ScurryHUD` inherits the base `MultiplayerHUD` refresh on `OnMiniGameTurnStarted` — domain panels (or legacy per-player cards) are initialized from current `RoundStatsList` values, important for replay resets.
...
- **Spawn-ring rollout is a per-mode design call.** `arrangeSpawnPointsAroundCell` is on for
  this scene only. It is wrong for SkimRace (deterministic track, its own start line) and
