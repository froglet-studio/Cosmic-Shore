...
{
    /// <summary>
    /// Data-driven view for the Maelstrom scene. Two panels, picked by <see cref="MaelstromController"/>'s
    /// phase:
    ///
    ...
    ///     scroll of <b>Maelstrom Data Cards</b> (one per round, newest on top), and the button:
    ...
    /// <see cref="MaelstromDataSO"/> (the scene is UI-only, so <c>gameData.Players/Results</c> are cleared).
    /// </summary>
    public class MaelstromSceneView : MonoBehaviour
    {
        ...
        [SerializeField] MaelstromLobbyNetwork lobbyNetwork;
        ...
        [Tooltip("Subtitle - auto-filled \"First domain to N points wins\", where N is the Maelstrom " +
                 "win target from FrogletTools > Game Modes > End Game Conditions (MaelstromDataSO.EffectiveWinTarget).")]
        ...
        [Tooltip("Display countdown used when no MaelstromLobbyNetwork is wired (host auto-starts at 0).")]
        ...
        void Start()
        {
            ...
            bool summary = MaelstromController.Instance != null && MaelstromController.Instance.IsShowingSummary;
            ShowActive(summary);
        }
        ...
        void Update()
        {
            ...
            {
                // Local fallback: auto-start at 0 (the networked path auto-starts inside MaelstromLobbyNetwork).
                _localStarted = true;
                MaelstromController.Instance?.BeginNextRound();
            }
        }
        ...
        // One Maelstrom Data Card per completed round, in CHRONOLOGICAL order (Round 1 at the top,
        ...
        public void OnMainMenuPressed()
        {
            ...
            // locally, and restarts a solo Relay session; MaelstromController clears tournament
