...
namespace CosmicShore.Gameplay
{
    ...
    ///     <see cref="MaelstromController.BeginNextRound"/>.
    ///
    /// Arms and advances ONLY in <see cref="MaelstromPhase.Lobby"/> (the intro lobby OR the
    /// between-round hub). It stays completely idle on the summary screen (phase Summary), which shares
    /// the same scene. <c>MaelstromSceneView</c> reads the public state to drive the UI and calls
    /// <see cref="ToggleLocalReady"/> from the Ready button.
    /// </summary>
    public class MaelstromLobbyNetwork : NetworkBehaviour
    {
        ...
        void Update()
        {
            ...
            if (tc == null || tc.Phase != MaelstromPhase.Lobby) return;
