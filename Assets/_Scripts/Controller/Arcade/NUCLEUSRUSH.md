...
- **Scene**: `Assets/_Scenes/Multiplayer Scenes/MinigameNucleusRush.unity` (single
  unified scene, cloned from Astro League's — no separate singleplayer variant; solo
  play is a party of one + AI backfill)
- **GameMode enum**: `GameModes.NucleusRush = 38` (display name "Brood Rush")
- **Controller**: `NucleusRushController : MultiplayerDomainGamesController`
- **Scoring**: `NucleusRushScoringRuleSO` (`metric = ScoringMetric.Goals`, points not
...
- **Turn monitor**: `NucleusRushWaveTurnMonitor` — resolves the wave target from
...
- **Config**: `_SO_Assets/Games/ArcadeGameNucleusRush.asset` (registered in
...
- **Wave clock alignment**: the cell's spawner starts when its bootstrap crystal
  registers (ready screen), so `NucleusRushController.HandleTurnStarted` (raised on
...
| Change | File |
|---|---|
| `NucleusRush = 38` | `_Scripts/Data/Enums/GameModes.cs` |
...
| Asset | Path |
|---|---|
| Arcade game config | `_SO_Assets/Games/ArcadeGameNucleusRush.asset` (in `GameLists/OrganicRematchGames.asset`) |
| Scoring rule | `_SO_Assets/Scoring Rules/NucleusRushScoringRule.asset` |
