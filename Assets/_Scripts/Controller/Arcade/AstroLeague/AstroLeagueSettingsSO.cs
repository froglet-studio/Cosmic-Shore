...
namespace CosmicShore.Gameplay
{
    ...
    public class AstroLeagueSettingsSO : ScriptableObject
    {
        ...
        [Tooltip("Arcade pop on a vessel strike. The strike is a momentum-conserving ELASTIC bounce " +
                 "off the moving hull (the ball gains up to ~2x the vessel's speed on a head-on hit); " +
                 "this adds an EXTRA launch of (multiplier - 1) x vessel speed along the aim direction. " +
        ...
        [Tooltip("How hard opposing-color prism MASS slows the ball as it plows through (it keeps its " +
                 "direction, only its speed drops). Per eaten prism: speed x= ballMass / (ballMass + " +
                 "this x prismVolume). 0 = no drag (ball never slows); higher = a thick enemy wall " +
        ...
        [Header("Ball - Mesh")]
        [Tooltip("Icosphere subdivision count for the ball mesh (each level x4 the faces: " +
        ...
        [Tooltip("Radius (x the ball's world radius) of the per-tick spatial scan that resolves prism " +
