...
namespace CosmicShore.Gameplay
{
    ...
    public class ElementalComebackSystem : MonoBehaviour
    {
            ...
            GameObject host, GameDataSO gameData, bool useGolfRules = false)
        {
            ...
            if (existing)
            {
                // The search includes INACTIVE objects, so this can be an authored instance the
                // scene deliberately disabled. Hand the reference over either way, but let
                // OnEnable be what subscribes it if it is ever activated: subscribing something
                // that is off would strand its delegates in the GameDataSO's SOAP channels,
                // because OnDisable never fires for a component that is already disabled.
                if (existing.isActiveAndEnabled) existing.Bind(gameData);
                else if (!existing.gameData) existing.gameData = gameData;
                return existing;
            }
