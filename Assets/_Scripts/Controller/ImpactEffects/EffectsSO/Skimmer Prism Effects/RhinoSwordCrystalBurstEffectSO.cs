...
{
    /// <summary>
    /// Rhino energy-sword vs crystal effect (RHINO_ENERGY_SWORD.md). When the sword collects a
    /// crystal it spends ALL stored energy: it spawns an <see cref="AOEExplosion"/> at the crystal
    /// and bursts the blade in all three dimensions — BOTH scaled by the energy at the moment of
    /// the hit (full energy = full-size explosion + full-size burst; less energy scales both down).
    /// The 3D burst + the energy drain are owned by the sword state
    /// (<see cref="IRhinoSwordState.TriggerCrystalBurst"/>); this effect only sizes and spawns the
    /// explosion and kicks off the burst.
    /// </summary>
    [CreateAssetMenu(
    ...
    public sealed class RhinoSwordCrystalBurstEffectSO : SkimmerCrystalEffectSO
    {
        ...
        public override void Execute(SkimmerImpactor impactor, CrystalImpactor impactee)
        {
            ...
            // 3D size burst scaled by that same energy, then consume ALL of it.
