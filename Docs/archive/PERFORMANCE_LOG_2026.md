# Performance Log 2026 (archived) — formerly `PERFORMANCE_OPTIMIZATION.md`

> ## FROZEN ARCHIVE — do not append (2026-09-22)
>
> This is the full performance log from **2026-07-08 to 2026-09-22**, moved here verbatim when
> `Docs/PERFORMANCE_OPTIMIZATION.md` was rewritten as a short state-and-plan document. **Read that
> document first.** This file is kept for two reasons only: it holds the raw evidence (captures,
> tables, root-cause write-ups) behind every claim the new doc makes, and ~20 code comments and
> docs cite its section numbers (`§0.8`, `§0.11.6`, `Task 10`, …), which stay valid here.
>
> - Section numbers below are frozen. New findings go in `Docs/PERFORMANCE_OPTIMIZATION.md`.
> - Two companion docs it links were deleted in the same pass: `PERFORMANCE_CAPTURE_RECIPES.md`
>   (its method now lives in the new doc's "How to measure") and
>   `Assets/_Scripts/Game/Prisms/PRISM_PERFORMANCE_AUDIT.md` (a pre-ECS audit of an architecture
>   that no longer exists). Recover either with `git log --diff-filter=D -- <path>`.
> - Every number here is an **editor** number on the tree named beside it. None is a ship number.
