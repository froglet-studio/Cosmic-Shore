...
| Understand presence vs party | `PresenceSystem/ARCHITECTURE.md` Sec "Why it's separate from the party session" |
| See known issues + their status | `PartySystem/BUGS.md` + `PresenceSystem/BUGS.md` |
| See what we're refactoring next | `PartySystem/REFACTOR.md` Sec "Sequencing" |
| See the next multiplayer TODOs / big-picture roadmap | `MultiplayerArchitecture/ROADMAP.md` |
| Run the manual smoke / stress tests | `PartySystem/TESTS.md` Sec "Smoke gate" |
...
(Sec "Per-refactor commit cadence" + Sec "Per-commit revision protocol").
...
| Marker | Meaning |
|---|---|
| [red] | Open - root cause not understood, or fix not yet attempted |
| [yellow] | Partially mitigated - workaround / symptom-suppression landed; root cause persists or full retest pending |
| [green] | Fixed - root cause addressed; needs verification only |
| [white] | Deferred - known issue, not actively worked |
...
See `PartySystem/ARCHITECTURE.md` Sec "Locked design" for the full set of
locked decisions.
