...
Written 2026-09-09 against `78a95259f` (`origin/Ys-merge-2026-09-07`) — a branch
that was **abandoned**; recovered onto `origin/bleeding-edge` (`1f160508f`) on
2026-09-14. The METHOD carries over unchanged; the numbers it cites from §0.8 do
not. **Re-take Capture A against this tree.**

Two tree-claims in here were re-verified on `1f160508f` and still hold: there is
exactly ONE `BenchmarkStressTest.unity` (Capture B), and `ScarabHullBuilder` does
carry the `GetSharedMaterials` fix. The `ECOSIM_PROBE` claim was **not**
re-verified — check it before Capture A.
