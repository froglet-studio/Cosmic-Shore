...
## <- Validate on return (the things only you can check in-editor)
...
(ECOSYSTEM.md Sec 1 root-causes, Sec 8, Sec 9, Sec 10; the kickoff brief) claimed it was dead
...
the bible had stale statements. Fixed Sec 7 (herbivore description = the tadpole now
Consumes any unshielded non-fauna prism; brittlestar = opposing), Sec 7 intro (only
the two herbivores are wired in; shark built-but-unwired), and Sec 7.2-A (menu has no
...
Requests 1/2/4 from this batch (lower mass-vs-crystal threshold, 10x hunt speed,
...
equilibrium (Sec 0), not a defect.
...
the 3-phase model. Docs: ECOSYSTEM.md Sec 0/Sec 1/Sec 2/Sec 3/Sec 4/Sec 5/Sec 5.1/Sec 9/Sec 10, both kickoff docs.
...
### 1. Reproduction - the population driver (ECOSYSTEM.md Sec 6.1)
...
Resolves two documented v1 approximations in ECOSYSTEM.md Sec 7 using the lineage
...
1. **Real prey-seeking.** The per-species count registry is upgraded to track
   ...
   the fauna analogue of the prism density grid; sanctioned by Sec 7's own "no
...
instantiate (Sec 7 dead `fauna2` note).

### <- Validate on return (Session 12)
...
prism colliders, a few times/sec, x90 fauna) ate ~70 % of the frame budget, and the
...
- The loop: edit config -> `python3 Tools/ecosim/ecosim.py` -> read predicted fps ->
  ...
  See `Tools/ecosim/README.md` and ECOSYSTEM.md Sec 12.
...
### <- Validate on return (Session 13) - and feed the loop
...
make the food web dense AND cheap. Flagged in Sec 12.
...
web still the only down-force. ECOSYSTEM.md Sec 6.2.)
...
4. Gyroid SIZE is perf-capped (~1200 prisms ~ 70 fps); bigger needs the structural
   overlap/prism-cost fix (Sec 12), not just a higher `FrenzyEnter`.
