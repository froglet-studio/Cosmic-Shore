# QA session results

**Nothing here is published until you press Submit.** Add rows across as many days
as you like and submit whenever you want what you have so far to reach engineering;
only new verdicts are published each time. **A published verdict is frozen — a
retest is a NEW session file, never an edit.** Guide: `Docs/QA/README.md`.

## Session

| Field | Value |
|---|---|
| Tester | Andrew |
| Date | 2026-09-12 |
| Branch | bleeding-edge |
| Commit | `d805f3707` |
| Unity version | 6000.4.11f1 |
| Platform(s) | Editor (WindowsEditor) |
| Submitted | yes — 2026-09-12 21:14 · 2 verdict(s), 2 new this run |
## Results

<!-- qa-results-table -->

| ID | Result | Notes |
|---|---|---|
| QA-EDITMODE-TESTS | FAIL | All but "PrismOcclusionCoverageTests" have all of their tests green. There are 1987 green tests, 21 red tests and one blank test. Below are the failed tests. evidence/2026-09-12-andrew-2/QA-EDITMODE-TESTS-QA-EDITMODE-TESTS-Editor.log |
| QA-SPARROW-PROJECTILE-POOL | PASS | No dud sound effects, no null reference exceptions. |

<!-- /qa-results-table -->

---

## Working notes

*(Scratch space. Only the table above is read — write freely here as you go; it
becomes the dev task's detail if an item fails.)*


## Evidence

Files live in `Docs/QA/RESULTS/evidence/2026-09-12-andrew-2/` and are referenced from the Notes
cell of the row they belong to.

## Anything else

*(Feel, tuning opinions, and anything that looked wrong but was not on the list.
These change no item's status, but they are read when the backlog is regenerated
and can become new items.)*
