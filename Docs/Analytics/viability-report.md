...
2. Add `PostHogSink` using the official UPM package. The facade's existing gates (consent,
...
6. **Event budget:** 28 event types at current DAU lands well under 1M/month. The chatty
   ...
   threatens the cap — a one-line predicate in `PostHogSink`, invisible to UGS. Set the
...
bounded by the facade too: `PostHogSink` is swappable for any HTTP-capable vendor in a day
