...
- Implement `PostHogSink`: `distinct_id` = UGS `PlayerId` (rows join across systems);
...
- Per-sink filter hook: predicate in `PostHogSink` to exclude chatty events (`ui_action`,
