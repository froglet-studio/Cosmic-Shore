...
2. **Tunable & variable.** Every knob is SO-config; richness comes from *biome x intensity x
   heritable traits*, not bespoke code per case.
3. **Performant - collider-bounded.** Active physics colliders per cell stay under a
   configurable threshold, maintained *by design* (Sec 4). This is a hard gate, not an afterthought.
...
5. **Toward artificial life.** Each phase advances a concrete life-criterion (Sec 3) so "NASA
...
Mechanics solid (foundation adopted from `bleeding-edge`, Sec 10): the **`PrismSpatialIndex`**
...
wither-to-crystal (Sec 10 item 1), no-domain-asymmetry spawn (item 2), and volume-as-the-spine
...
energy-constrained agent ecology where *survival is selection* - our north-star analog. (Sources Sec 9.)

| Life criterion | Requires | We satisfy it via | Status |
|---|---|---|---|
| Organization / compartmentalization | bounded structured units | Cells (membrane), lifeform prism-bodies, domains | [x] have it |
| Metabolism / energy | matter/energy flux | the **mass economy**: flora produce prism mass, fauna consume opposing mass, crystals = stored energy | [x] have it (consumption = metabolism) |
| Growth | individuals enlarge | flora grow prisms; fauna body scale | [x] have it |
| Response to stimuli | react to environment | aggression-by-phase, prey-seeking, starvation clocks | [x] have it |
| Homeostasis / regulation | self-bounding populations | prey-linked starvation + phase gates; full **Lotka-Volterra** after the predator/herbivore split | [yellow] partial -> **P2** |
| Reproduction | individuals replicate | well-fed fauna breed; spawner becomes a seeder | X -> **P3** |
| Heredity (a "program"/genome) | traits passed to offspring | a small **trait genome** per lifeform, inherited | X -> **P3** |
| Variation / mutation | offspring differ heritably | mutation on inheritance | X -> **P3/P4** |
| Selection | differential survival/reproduction | the energy economy (starvation/predation) **already selects** - becomes *natural* selection once traits are heritable | [yellow] substrate exists -> **P4** |
| **Adaptation / EVOLUTION (the bar)** | open-ended Darwinian evolution -> novelty | reproduction + genome + mutation + selection; then speciation / predator-prey arms races | X -> **P4 (centerpiece)** |
...
count fields x `NominalPrismVolume` (16, the 4x4x1 flora leaf). A mode whose vessel lays
**low-volume prisms** (e.g. Squirrel trail prisms ~ **3.1** volume each, ~1/5 nominal) must author
**explicit `*EnterVolume` / `*ExitVolume`** values - otherwise the x16 derivation sets the ladder
~5x too high, the volume gauge barely moves, and fauna never reach the hunting/frenzy phases. Set
...
(authored in nominal prisms, x16 for the prey-volume check) so herbivores actually seed against the
...
- Lock invariants (`CLAUDE.md`), this plan, the `ecology` skill. [x]
- Adopt the `PrismSpatialIndex` spatial backbone (`QuerySphere`/`TryReserve`). [x] *(merged from `bleeding-edge`)*
- Re-assert the three locked invariants on the foundation (Sec 10): crystal-drop [x], no-asymmetry
  spawn, volume spine. *Life: keeps mass conservation + the credible-alife economy intact.*
- Build the **collider budget + LOD + index-fauna-queries + telemetry** (Sec 4). *Life: enables scale.*
- **`LifeformBootstrap`** + wire ecology into the ~9 bare modes; intensity->biome selection.

**Phase 2 - The food web (Lotka-Volterra)** [x] *(adopted from `bleeding-edge`, Sec 10)*
...
**Phase 3 - Reproduction + the genome** *(reproduction [x] adopted; genome = the evolution substrate, TODO)*
- Well-fed lifeforms **reproduce** (`Fauna.NotifyFed`->`TryReproduce`; `FaunaReproductionRules`);
  the spawner is now a one-time **seeder**. [x] *(adopted from `bleeding-edge`)*
...
- **`ecology` skill** *(created)* - every ecology change starts by restating the invariants
  (Sec CLAUDE.md), confirms intent at design forks, implements surgically, then states its
...
old Sec 10 collider-budget plan is **superseded by PrismSpatialIndex** for the spatial-query half.
...
1. **Mass-conserving crystal drop + wither (continuity) on the fauna death path.** [x] *(done)*
...
2. **No domain asymmetry in spawning.** [x] *(done - adopted onto the merge line)*
...
3. **Volume as the spine.** [x] *(re-asserted on the merge line - retune still owed in-editor)*
   `Cell.LiveVolume` / `GetDomainVolume` now exist: per-domain live volume recomputed from
   live prism state (`Prism.CurrentVolume` x live `Domain`) on a 0.25s cadence, fed by EVERY
   ...
   CellConfig assets auto-derive their volume ladder (count x `NominalPrismVolume` = 16, the
   4x4x1 leaf) via `CellPhaseThresholds.WithDerivedVolumeScale`, so no asset breaks - but the
...
- [x] **Fauna queries ride the index** *(done on the merge line - SPATIAL_INDEX Phase 2)*:
...
- [x] **Proximity collider-LOD** *(shipped on the merge line - in-editor verification owed)*:
...
- [x] **Telemetry** *(shipped)*: `EcosystemPerfProbe` line is now
  `[ECOSIM] prisms= volume= colliders=near/live fauna= phase= fps=` - the Sec 4 budget and the
...
2. **Per-biome retune with the probe** (`[ECOSIM] prisms= volume= colliders=near/live fauna=
   phase= fps=`): volume thresholds per CellConfig (author `*Volume` fields to override the x16
...
4. **`ColliderBudget` per cell** (CellConfigDataSO, target <= ~1,500 - Sec 4): the probe warns when
...
**Phase C - genome & heredity (P3 - the artificial-life centerpiece, Sec 3).**
...
8. **Evidence pipeline:** lineage + trait-distribution telemetry (extend the ECOSIM line or a
   ...
   alive" into "demonstrably adapts" (Sec 3 item 4).

**Phase D - wire ecology into the ~9 bare modes (Sec 5):** cells + spawn profiles per mode,
intensity -> biome/population mapping, powerup crystals feeding each mode's loop. Variety =
biome x intensity x (Phase C) heritable traits - never bespoke per-mode code.
