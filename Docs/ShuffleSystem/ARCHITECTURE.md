...
|---|---|---|---|
| 1 | **Randomized lineup** — a random mode + a random intensity ∈ [1..X] per game (X = chosen intensity; pool = the modes that intensity can DRAW × X "experiences" — `IntensityTiers` is cumulative, so as of 2026-09-10 that is L1 6×1=6 … L4 16×4=64; see `MaelstromSystem/ARCHITECTURE.md` §1.1), with repeat-avoidance, instead of the fixed `GameQueue` | `MaelstromDataSO`, `MaelstromController` | ✅ **shipped** |
