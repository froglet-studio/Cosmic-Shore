...
**The flash wears the scoring player's DOMAIN colour, resolved live at each event.** The modal
reads `gameData.LocalPlayer.Domain` → `SO_ColorSet.GetDomainSignalColor` (through
`GameDataSO.ThemeManagerData.ColorSet`, the path every domain-tinted UI reads — never
snapshotted, per the domain-sync rule) and hands the colour down with the event: the toast wears
it outright, the objective box lerps its icon + counter toward it at the pulse's peak and back
to the authored rest as it decays. Re-pick your domain on the roster and the very next tick
flashes the new colour.
...
| `ModeControlsLibrarySO.ObjectiveIcons` | same asset | The ONE metric→sprite table both objective surfaces read. **Hand-curated, not generated**: the three crystal metrics wear `ElementIcons/OmniCrystal_Active`, jousts `HuntIcon-PLACEHOLDER`, prisms-destroyed `Silhouettes/BigGapBlock`; goals, lifeforms and combat wear the generated white silhouettes in `_Graphics/ObjectiveIcons/` (`Tools/Build/gen_objective_icons.py` regenerates those). Swap any of them in this one table — nothing else reads a metric's art |
