...
Status legend: [red] open * [yellow] partial * [green] done.
...
## Step 4 - [red] Server-ordered results sync (host/client identical ranking)
...
## Step 5 - [red] Per-player time tie-break (owner's rule)
...
## Step 6 - [red] Podium join hardening (`EndGameStatsPanel`)
...
## Step 7 - [red] Docs + parked follow-ups

1. **Docs**: update `ARCHITECTURE.md` Sec 4 (pipeline diagram - clients no longer
   rebuild via `BuildResults`; rows arrive rank-ordered), Sec 5 per-mode table
...
2. **F1 - SOAP `ResetOn` convention**: Soap's `ScriptableVariable` default is
   ...
   (`ApplicationStarts`); codify the rule in `CLAUDE.md` (Sec SOAP) and add an
...
| Replay (scene reload) x2 | `LastCrystalCollectedTime` zeroed (Cleanup); no rank carry-over |
...
- `Docs/ScoringSystem/BUGS.md` **B11** - client-local menu domain writes
  ...
  `RoundStats.Domain`** (also in `CLAUDE.md` Sec Team Domains).
