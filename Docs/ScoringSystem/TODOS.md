...
## Status legend
[red] open * [yellow] in progress * [green] done (verify only) * [white] deferred

---

## TD1 - [red] [recheck tomorrow] Server-sync the LEGACY per-player HUD layout
...
## TD2 - [red] [verify] Joust scene domain-HUD wiring
...
## TD3 - [yellow] [verify + optional cleanup] Confirm domain fixes; consider retiring Approach B
...
**Status:** B9 + B10 [x] confirmed in a 2-human engine test; broader mode coverage
...
- **B10 (icon placement):** each player's profile icon sits in its OWN domain box on the
  client (the reported bug) - [x] confirmed. Re-check on Crystal Capture + Joust.
- **B9 (counts):** each domain box's number tracks the host - [x] confirmed. Re-check on CC + Joust.
