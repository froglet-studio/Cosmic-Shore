...
## Status legend
[red] open * [yellow] partially mitigated * [green] fixed (verify only) * [white] deferred

---

### B1 - [green] `PlayerScoreCard` empty DataPanels background never re-hides
...
### B2 - [green] Joust "jousts left" differs between reveal and scoreboard
...
### B3 - [green] `scoreboardRowStagger` is dead config
...
### B4 - [green] Two crystal-reward amounts can double-award / drift
...
### B5 - [white] Winner-delta recomputed independently per surface
...
### B6 - [green] Score-card secondary stat never renders (field unwired in prefab)
...
### B7 - [green] End-game vessel podium ranked golf modes backwards
...
### B8 - [green] Client domain boxes built from a stale turn-start snapshot
...
### B9 - [green] Client's OWN metric count frozen on its own screen (domain layout)
...
**[x] Verified in engine** (2-human test - client domain counts track the host).
...
### B10 - [green] Client's OWN profile icon grouped into the wrong domain box
...
**[x] Verified in engine** (2-human test - the client's own profile icon now sits in its
own domain box). Broader mode coverage (CrystalCapture / Joust) continuing.

### B11 - [green] Client-local menu domain writes desynced the live mirrors (B10-relapse class)
...
### B12 - [green] Stale pre-party RoundStats shadowed the live entry (own ready-text frozen Jade)
...
**[x] Verified in engine** (2-human test - client's own ready-text now shows its picked
...
### B13 - [green] Scoreboard Play Again dead in Joust + Crystal Capture (null onClick target)
...
**[x] Verified in engine** (Joust: Play Again reloads the scene and a fresh match
plays; owner-tested). Wiring requirement recorded in `JOUST.md` Sec 9 /
`CRYSTAL_CAPTURE.md` Sec 9 so the next scene edit doesn't reintroduce it.
...
### B14 - [green] Host-only scoreboard nav gating no-oped (fields unwired) + no anti-spam hide
...
**[x] Verified in engine** (owner-tested). Clients see neither nav button and
...
### B15 - [green] Game end dead on the SECOND game after a menu return (stale RoundStats subscribers)
...
**[x] Verified in engine (2026-06-12)** - the reported repro (party menu-return ->
...
the second race must still end cleanly (the poison path); (c) repeat 2-3x per
