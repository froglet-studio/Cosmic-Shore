...
4. Repeat the full cycle 2-3x per `../PartySystem/TESTS.md` S9.
...
Finish each mode and verify against `ARCHITECTURE.md` Sec 5:
...
- **Crystal Capture / Cellular Duel** (points v): highest score on top.
- Winner's card shows the `+N` crystal reward.

### T7 - Score formatting per mode
Verify each card's formatted score matches Sec 5: HexRace/Joust winners
