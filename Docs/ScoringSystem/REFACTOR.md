...
## Status legend
[red] open * [yellow] partially done * [green] done (verify only) * [white] deferred
...
call-site (see `ARCHITECTURE.md` Sec 8 fork map):
...
### R1 - [red] [discuss-first] Remove `IsMultiplayerMode` forking -> unified path
Route all scoring/lobby/cinematic behavior through the always-networked host
model; drive per-site behavior off concrete signals (Sec 8) and delete the flag.
...
### R2 - [green] Deduplicate the score-text animation (DRY) - incl. R2b (entrance)
...
(~80 lines x3). **Done** (commit `3f708a5b`):
...
### R3 - [green] Single source of truth for the winner crystal reward
...
### R4 - [green] Centralize the loser-score sentinel encode/decode
...
### R5 - [green] Unify domain -> color resolution
...
### R6 - [yellow] Remove the legacy per-player HUD layout
...
### R7 - [white] Replace `PlayerScoreEntry`-as-avatar-chip with a dedicated chip
...
### R8 - [green] Remove `[FLOW-*]` `Debug.Log` spam from `MiniGameHUD`
...
### R9 - [green] Scoreboard banner uses authoritative `WinnerDomain`
...
### R10 - [yellow] [in progress] One server-authoritative ranked results list
...
- [green] **A1** (`8820f8c8`) - `ScoreResult` + `ScoreResultBuilder` + `GameDataSO.Results`/`SetResults`
  (derives `WinnerName`/`WinnerDomain` from `Results[0]`).
- [green] **A1.5** (`d3305e62`) - added `ScoreResult.ScoreText` (formatted primary) + shared `FormatTime`.
- [green] **A2/A3/A4** (`05b7c71e`, `ff69baf9`, `7478d0d9`) - HexRace / Joust / CrystalCapture each
...
- [green] **B** (consumers) - **done via the `ScoringRuleSO` strategy** (commits `af07a171`,
...
- [yellow] **C** (R1) - partially advanced: `IsLocalUser` -> `IsMultiplayerOwner` (commit `10e541fc`, no
...
- [red] **D** (next session) - **server-ORDERED results sync**: the rows are still re-SORTED on every
...
### R11 - [white] [deferred * needs device + UGS testing] Fully retire `GolfScoreSentinels`
...
### R12 - [white] [deferred * low payoff] Fold per-mode end-game stat providers behind the rule
