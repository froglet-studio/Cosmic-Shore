...
- A creature **parent-scale** lerp (`WormFauna.GlideScales` — and `Fauna.GrowToScale`,
  ...
  scale is mover-contract. A per-prism grow stamp cannot express a parent transform —
...
| `HoldColliderAtFullSize` | Boost rings / joust rings / Squirrel tube | Per-frame coroutine compensating `BoxCollider.size` against animated transform scale | ✅ DELETED 2026-08-25 (Prompt 7) — transform final at stamp; BoostRingBuilder applies all kinds at lay |
...
| Shatter overlay (disengage) | `Disengage` | ONE batched pure-entity spawn per frame — since §4.8.1 as ORDINARY explosion debris (`PrismShieldShatter` → `SpawnExplosionDebrisBatch`, grouped per shield mesh); retired by a flat time sweep | ✅ shipped 2026-08-15, re-based onto the explosion pipeline 2026-08-24 (was: a per-prism child GameObject rebuilding a shatter mesh every frame) |
...
| Flora leaf growth (growPeriod loop → new health prisms) | scheduling CPU / visual stamp | ✅ visual (C7) | `Controller/Environment/FloraAndFauna/Flora.cs:297-305` (`AddHealthBlock`) + `:570` (`GrowCoroutine`); `PhyllotacticFlora.cs:432-440` | Visuals ride the engine: each new leaf stamps `leafSize` as its clock-material target via `EnvironmentPrismPool.Get` → `AddHealthBlock` → `Initialize` → `BeginGrowthAnimation` → `StampClockGrowth` (C7 closes with C6; C13b 2026-08-25 folded the HealthPrism Instantiates). Grow-period scheduling stays CPU. Volume is final at start (flora mass counts in Cell.LiveVolume the moment the leaf is laid — PhaseThresholds re-baselined ✅ 2026-08-02). |
...
| Grow (reverse suction, StartGrow 1→0) — LIVE 2026-08-09 (Sparrow ReverseSuction) | stamp (STRICT) | ✅ visual; pooled path required | `Utility/Effects/PrismImplosion.cs` (`StartGrow`); dispatched `PrismFactory.SpawnGrow` | Same SuctionGraph stamp with `_SuctionDirection = −1`. **D4 2026-08-25 kept this surface** — Sparrow ReverseSuction (`FullAutoBlockShootActionExecutor`) is a live producer; batched implosion has no completion callback. |
...
| Wither/devour prism-side routing (cross-reference) | stamp (spindle fade) / per-frame (devour) | ✅ spindle / ❌ devour | `Controller/Environment/FloraAndFauna/LifeForm.cs` (`WitherToSkeleton`); `LightFauna.cs` (`WitherCoroutine`); `Spindle.cs` (`StampDeathFade`) | ✅ SHIPPED 2026-08-25 (C11 / Prompt 13). Prism side: inherited from explosion/implosion. Spindle fade: `_DeathStartTime`/`_DeathDuration`/`_DeathDirection` stamped once; `PrismDeathClock` on SpindleGraph / AnimatedSpindleGraph; ordered wither is `ForceWither(i * interval)` offsets. Devour/graze suction still C6 remainder. |
...
| Environment trail lay (PrismTrailBuilder LayOne/LaySync/LayGradual/LayBatched/LayBudgetedAsync) | per-frame CPU | ❌ remaining grow-in / arena; ✅ pooling (C13b) | `Controller/Environment/Spawning/PrismTrailBuilder.cs` (`LayOne` → `EnvironmentPrismPool.Get`; `CloneBatchAsync` → `GetBatchAsync`; `ConfigureLaid`: ChangeTeam → pose → TargetScale → Initialize → PrismKinds.Apply → WatchForReveal; LaySync / LayGradual / LayBatched / LayBudgetedAsync unchanged cadence) | ✅ SHIPPED 2026-08-25 (C13b): dedicated unbounded prefab-keyed `EnvironmentPrismPool` — snap Blue materials, then `ChangeTeam` clock-lerps Blue→domain. Remaining: grow-in stamps / arena-ready poll / gradual+budgeted cadence. Do not route through `GenericPoolManager` (overflow Destroy) or wire `OnReturnToPool` (would vacuum Wanderway). |
...
| Flora growth (grow tick + paced instantiation drain) | scheduling CPU / visual stamp | ✅ visual (C7) | `Controller/Environment/FloraAndFauna/Flora.cs:297-305` (`AddHealthBlock`) + `:570` (`GrowCoroutine`); `PhyllotacticFlora.cs:432-440` | Flora-side visuals are done by construction (C7): `EnvironmentPrismPool.Get` → `AddHealthBlock` → `Initialize` → `BeginGrowthAnimation` → `StampClockGrowth` (C13b folded Phyllotactic / Branching / Assembled HealthPrism Instantiates). Decision tick and drain pacing stay CPU; collider/index/volume final at stamp (`TryReserve` already claims the site up-front). Named, not folded: `Boid.cs` body Instantiates, `SpawnableBase` non-prism `leafPrefab`, `SpawnableCord`. |
...
| Batched lay + bloom-in (microscene first population, cell environment build) | per-frame CPU | ❌ remaining stamps | `Controller/Toys/Microscene.cs:84-102` | C13b 2026-08-25: `PrismTrailBuilder` now pool-Gets. Remaining: a lay is N stamps with staggered start times (t₀ᵢ = now + i·Δ) issued in one or few frames — the veil/load-gate fast-grow special case disappears (stamp t₀ in the past = already settled). |
| Cell swap — retiring-world suction (GPU stamp + rider-scale wait) | one-shot stamp | ✅ | `Controller/Environment/Cell.cs` (`StampRetiredWorldSuction` / `SwapCellConfigRoutine` / `RetireWorldIntoSuctionRoot`) | ✅ SHIPPED 2026-08-25 (C9 / Prompt 4). Per-prism `StampSuctionToward` at retire time (GPU converges on cell centre). Root `localScale` wait kept for non-prism riders only — instanced prism entities ignore parent scale. Drain cadence unchanged (row below). |
| Cell swap — hidden drain (500/frame) + pooled returns | one-shot | ✅ | `Controller/Environment/Cell.cs` (`ReleaseRetiredWorld`) | Keep the slicing. C13b 2026-08-25: issued environment/flora prisms `EnvironmentPrismPool.TryRelease` (inactive stack, never overflow-Destroy); remainder still `Destroy`. Vessel trail still `ReturnToPool` *before* this drain (`OnReturnToPool` discriminator in `RetireWorldIntoSuctionRoot` unchanged — Wanderway stock is not gathered). Rebuild is pool Get + Blue snap + ChangeTeam stamp. |
...
| Worm colony segment-scale glide (GlideScales) | per-frame CPU | ✅ | `Controller/Environment/FloraAndFauna/WormFauna.cs` (`GlideScales`) | ✅ SHIPPED 2026-08-25 (C6 remainder, **(b)**). Scale twin of `FollowChain` — live taper that moves on growth/split/death, not a one-shot stamp. `Update` already `SyncBodyPrismsToIndex` after the glide; no extra notify. |
...
| Spindle phase-variant materials + `_DeathAnimation` fade (adjacent flora path) | one-shot stamp (fade) / shared materials (phase) | ✅ fade / ✅ phase | `Controller/Environment/FloraAndFauna/Spindle.cs` (`StampDeathFade`; phase buckets unchanged); `_Graphics/Materials/Graphs/SpindleGraph.shadergraph` + `AnimatedSpindleGraph.shadergraph` | ✅ SHIPPED 2026-08-25 (C11). Phase still quantized shared materials (SRP Batcher). Fade stamps Hybrid `_Death*` once; `PrismDeathClock` off `_PrismClock`. MPB still unbatches DURING the fade (~1s); batcher recovers at settle via `SetPropertyBlock(null)`. |
...
| SkimFxRunner ship→prism stretch beam | per-frame CPU | ✅ | `Controller/ImpactEffects/EffectsSO/Helpers/SkimFxRunner.cs:35-93` | **Out of clock-law scope (Prompt 7 / C12, 2026-08-25).** Verified: writes only the beam particle transform (`:83`/`:85`), never a prism. The UniTask loop is vessel FX. Optional follow-up (not clock): one loop per skim contact + Instantiate/Destroy per contact instead of pooling. |
| Spindle evaporate fade (flora/fauna limb rods — prism-adjacent) | one-shot stamp | ✅ | `Controller/Environment/FloraAndFauna/Spindle.cs` (`StampDeathFade` / `StampEvaporate` / `StampCondense`) | ✅ SHIPPED 2026-08-25 (C11). Stamp `_DeathStartTime` (+ duration + direction) once; `PrismDeathClock` computes `_DeathAnimation`; `PrismTimerManager` settle. Same recipe as prism color; not on the prism entity prototype. |
...
| GAP-DETAIL: ShapeDrawingManager captured-trail shrink bypasses the render bridge (+ pool-detach and event-driv… | per-frame CPU | ✅ deletion | `Controller/Environment/MiniGameObjects/ShapeDrawingManager.cs` (DELETED, C15 2026-08-25 — `ShrinkPrismsIntoShape` was per-frame `transform.position` + `localScale` Lerp, no `SyncRenderTransform` / `NotifyPositionChanged`) | ✅ resolved by deletion (C15). Same ruling as the inventory row: unreachable; migrating would ship an untested clock path. |
...
1. **Cell swap suction is invisible on the instanced path** — `Cell.RequestCellSwap`'s
   1.1 s retiring-world suction scaled a root transform but never synced child prisms'
   companion entities (zero `PrismRenderService`/`NotifyPositionChanged` references in
   `Cell.cs`), so entity-rendered prisms stood at full size then vanished at the drain.
   **Closed 2026-08-25 (C9 / Prompt 4):** prisms GPU-converge on the cell centre via
   `Prism.StampSuctionToward` (`PrismSuctionConverge` on both live graphs). The root
   `localScale` wait is **kept for non-prism riders** (membrane / nucleus / cytoplasm /
   lifeform spindles) — one write/frame on one transform, not the C9 inventory item.
   (C8 shipped the Wanderway recycle on grow-clock re-stamps + `HideForTransport` —
   `Microscene.AnimateScaleAsync` was deleted; do not re-migrate the conveyor.)
...
6. **`PrismImplosion` wall-clock watchdog** — ✅ DONE 2026-08-25 (C12 / Prompt 7):
   per-instance `Update()` retired; armed from `OnEnable` via `PrismTimerManager`,
   re-armed after stamp (which cancels prior schedules), cancelled on complete/disable.
   Deliberately does **not** gate on `IsActive` (covers pool re-activation where
   `StartImplosion` never runs).
...
8. **Live producer (2026-08-09)**: `PrismType.Grow` / `PrismImplosion.StartGrow` —
   the Sparrow turret's `ReverseSuction` visual
   (`FullAutoBlockShootActionExecutor.cs:476`, dispatched via `PrismFactory.SpawnGrow`
   at `:198` → `:448`). The pooled `PrismImplosion` path is therefore a gameplay
   consumer for reverse suction as well as fauna implosion — D4 cannot assume Grow is
   unreachable.
9. **Spindles** (flora/fauna limb rods — prism-adjacent, same law family): ✅ SHIPPED
   2026-08-25 (C11). Per-frame MPB `_DeathAnimation` fade migrated to `PrismDeathClock`
   on SpindleGraph / AnimatedSpindleGraph. Ordered wither is per-spindle StartTime
   offsets. SRP Batcher still excluded DURING the fade; recovered after settle.
10. **✅ FIXED 2026-08-02 — the shield engage-morph ate the grow stamp of every
    ...
    SFX at lay time.
11. **`ShapeDrawingManager` shrink-to-outline** — ✅ CLOSED 2026-08-25 (C15 /
    Prompt 15) **by deletion**. Flagged here with **no §5 row**, which is why
    every sweep missed it. `ShrinkPrismsIntoShape` snapped `localScale` then
    per-frame `Lerp`d `transform.position` and `transform.localScale` — no
    `SyncRenderTransform` / `NotifyPositionChanged`. GUID
...
- **Gameplay finality at stamp**: collider enabled at authored size on `Initialize`;
  ...
  verbatim. **PhaseThresholds re-baselined ✅ 2026-08-02** against volume-final-at-spawn
  (six freestyle configs re-authored from measured baselines — `Docs/ECOSYSTEM.md` §18,
  `PRISM_CLOCK_WIRING_CHECKLIST.md` Phase 6). New cells still need the measuring tool;
  the migration-wide re-baseline is done.
- **Three whole compensation subsystems existed ONLY to patch CPU-clocked grow-in**:
  `HoldColliderAtFullSize` ✅ DELETED 2026-08-25 (Prompt 7 — BoostRingBuilder applies all
  kinds at lay; transform final at stamp), the arena-ready gate's force-settle sweep ✅
  rewritten 2026-08-25 (waits on `AnalyticGrowSettleTime` / `IsSettledForReveal`; no
  `CompleteGrowthImmediately` snaps), and the `CreateBlockCoroutine` waitTime window —
  **kept** until trail/env lays claim-before-spawn (`TryReserve` is growth-only today).
  Creation-budget spin is scheduling, not animation.
...
(or an instant swap where no transition is wanted — spawn-paint of a *vessel-pooled*
reuse is a recolor of existing mass). Environment lays are the opposite case:
`PrismTrailBuilder.LayOne` used to raw-`Instantiate` a prefab whose MeshRenderer
already wore the **final domain material**, so `ChangeTeam(Blue→Jade)` stamped a
Jade→Jade no-op and the spawn repaint never played. C13b (2026-08-25) pulls from
`EnvironmentPrismPool`, **snaps Blue** (`ResetToNeutralForReuse` +
`BindMaterialsImmediate`), then `ChangeTeam` clock-lerps Blue→domain. Do **not**
restore "final domain material from frame 0" on this path — that is the bug the
row exists to kill. The pool is unbounded and never wires `OnReturnToPool`
(a `maxSize` overflow Destroy is an ecology-law breach; the trail discriminator
would vacuum Wanderway).
...
#### 4.6.2 Fauna suction playtest record (D4 gate)

The grid explosion rig produces **zero implosions** (every AOE death is `Damage → Explode`). The only implosion producer is `Prism.Consume` — fauna feeding and
predation. Judge suction here, not on the benchmark lattice.

| date | scene / cell | observer | result | harness `debris` row (`N exp / N imp`) | notes |
|---|---|---|---|---|---|
| **2026-08-24** | — | agent | **not run** | — | Editor not open for play mode; see Prompt 11 session log in `PRISM_CLOCK_WIRING_CHECKLIST.md` |
| **2026-08-25** | `PrismGridExplosionTest` leftover lattice after 47³ blast — **Consume-on-grid, not a live fauna cell** | agent (Editor 6000.3.17f1) | **instrumented pass; visual unresolved** | blast leftover **0/0 imp** (negative control). Run A: **0/48** then retired **0/0**. Run B: **0/24** then **0/0**. Run D close-cam: **0/7** then polled **0/0** (`clock=4341.2740`) | Same producer as fauna (`Prism.Consume` + a moving `Transform`). Eater `Prompt11SuctionEater` circled r=35 while HUD stayed at 48 imp (`suction_playtest.json`: centroid `[-13.13, 88.13, 30.63]` moving each sample — that is the moving-target proof). Capture `Assets/BenchmarkResults/PrismExplosion/captures/suction_close2.png` shows 7 live imp as large bright-blue planes; **shrink-to-sink toward the eater is not visually resolved at that FOV**. Criterion 1 visual / criterion 2 “nothing pops” **not claimed**. Criterion 3 (`imp`→0 when Consume stops) **passed on this grid**. Live Lattice fauna cell is the next row. |
| **2026-08-25** | Menu_Main Lattice boot world, parked Squirrel `(-190, 310, 140)`, TadPoleFauna / Boid | agent (same Editor session, Netcode `LoadScene(Menu_Main)`) | **HUD pass; visual unresolved** | watch-start **0/4** (`prompt11_watch_start.txt` t=9052.460). PlayerLoop peak **15 imp / 4 exp** (`fauna_watch.json`; trust peaks, not the sample array — later frames overwrite it). Cheap polls: **0/4** t=9507, **0/6** t=9657 | Feeding still in progress at last poll — live-cell criterion 3 (`imp`→0 when feeding stops) **not shown**. Capture `Assets/BenchmarkResults/PrismExplosion/captures/fauna_suction_live.png`: cyan-blue rectangular implosion debris near the hull; lime greens are fauna, **not** suction. Criterion 1 “converges on moving eater” **not claimed from a still**. Corridor active (`corridor=True`). |

**Pass criteria (record verbatim observations when run):**

1. Mass **converges on the moving eater** — no pop-in at a fixed world point; predation
   (`mouth` transform, no feed brake) should look worse than grazing, not better.
2. **Nothing pops** — continuity of existence; suction is shrink-to-sink, not `Destroy`.
3. When feeding stops, the DiagnosticsHUD / harness **`imp` count returns to 0** (no stuck
   live implosion records). Grid rig: `debris` row shows `N exp / 0 imp` after a blast-only run.

**Fastest scene:** any cell with fauna (Menu_Main after Restless, Rampage, Wildlife
Liberation, etc.). Watch grazing herbivores on trail mass; optionally shoot fauna to
exercise predation suction.
...
- **`PrismType.Grow` has a live producer** (2026-08-09): the Sparrow turret's
  ...
  completion-callback machinery (fire-and-forget is the live contract), so the
  pooled `PrismImplosion.StartGrow` is a **gameplay consumer** — D4 kept the
  pooled class for it (death spawn retired 2026-08-25).
- **A death visual wears the palette of the TIER the prism was wearing, not just its
  ...
  `PrismFactory.TryGetTeamColors` (death) and `ConfigureForTeam` (Grow) resolve their pair from
  ...
  `dangerDetonationMultiplier`), applied on the batched death path (read off that
  prefab; there is no pooled death route after D4); it scales the debris
...
#### 4.6.1 Retiring the pooled explosion/implosion *death* path — DONE 2026-08-25 (Grow stays)

Death pooling is retired. `PrismFactory.SpawnExplosion` / `SpawnImplosion` are
batch-only; a declined request **warns once and returns null** — it does not
`Get()` a pooled GameObject. **That is not a license to delete `PrismImplosion`:**
`PrismType.Grow` / `StartGrow` has a live gameplay producer (Sparrow ReverseSuction,
2026-08-09) and the batched implosion carrier has no completion-callback machinery,
so Grow stays on the pooled class. `TriggerExplosion` / `StartImplosion` have
zero factory callers after this change; keep the sibling APIs on the class
(Grow still needs `StartGrow` + `OnGrowCompleted`).

**Config decision (do not extract a new SO).** Authored effect config stays on
the pool prefabs. Reasons: they already author mesh / material / layer / clamp /
duration / danger multiplier; `PrismDebris` already reads off
`explosionPool.Prefab` / `implosionPool.Prefab`; Grow still needs the implosion
prefab + pool + class. A new SO would duplicate authored data and require
scene/prefab YAML rewiring. Explosion pool is never `Get()`d and is not
prewarmed. Implosion pool prewarm dropped 64 → 12 (Grow-sized).

**Why death pooling was never a working visual fallback.** The comment that
called it a "fallback for a disabled render service" was wrong:

- With `PrismRenderService.Enabled` false, a pooled explosion renders **nothing**
  (`TriggerExplosion` disables its renderer unconditionally). A pooled implosion
  draws a **static, un-animated block**. Strict clock mode has no CPU animation
  tier by design (§4.4) — "loud and frozen" IS the contract.
- `PrismRenderService.SetRuntimeOverride` has **no caller anywhere in Assets**,
  and the shipped `Resources/PrismRenderConfig.asset` has `useInstancedRendering: 1`.
- No death consumer needs a GameObject back. `Prism.Explode` / `Prism.Implode`
  discard the channel's `PrismReturnEventData`. Grow *does* need the pooled
  instance (completion callback).

**The three dependencies, after D4.**

1. **Prefabs remain CONFIG.** `PrismDebris.Configure` / `ConfigureImplosion`
   still read mesh, material, layer, clamp band and duration off the pool
   prefabs. Do not delete the serialized `explosionPool` / `implosionPool`
   references on `PrismFactory`.
2. **Zombie audit** still walks `EnabledInstances`. Explosion walk is empty in
   gameplay (factory never `Get()`s). Implosion walk covers Grow pool zombies.
   Batched death records ARE `PrismDebris`'s live set.
3. **`GameLoadSampler`** counts death explosions as `PrismDebris.LiveDebrisCount`
   and implosions as `LiveImplosionDebrisCount` + `PrismImplosion.EnabledInstances`
   (batched death suction + pooled Grow).

**The gate landed.** Prompt 11 items (1) and (2) are recorded in §4.6.2
(explosion markers + FPS table; Consume-on-grid `imp`→0; live Lattice fauna
HUD 0/4 → peak 15 imp). Death retirement followed; Grow kept.
...
**Deliberate exclusions, named rather than hidden.** `SuctionGraph` is LIVE — batched
implosion debris draws `ImplodingPrismMaterial` on it (`PrismDebris.ConfigureImplosion`
reads `sharedMaterial` off `PrismImplosion.prefab`, that material's only reference) — but
it renders a prism DURING consumption (a sub-second implode of mass being removed), never
standing mass that can occlude. Named in `PrismOcclusionWiringValidator.KnownCorridorExcludedGraphs`
and in `PrismOcclusionDiagnostics.WiredPrismShaderNames`'s comment, so the exclusion cannot
look like an omission. Do not add it to `WiredPrismShaderNames` without wiring
`PrismOcclusionFade` — `IsCorridorCapable` would then fail every suction material at
runtime. Four legacy prism prefabs on pre-corridor shaders — `GreenDartBlock`,
...
are listed by name in `KnownLegacyPrismPrefabs`. Those four are DEAD; SuctionGraph is not.
If a legacy prefab is revived as live gameplay mass, **rebase it onto a wired prism graph;
do not grow the exclusion list.**
...
- **A dither kernel that keeps coverage honest.** Whatever pattern the screen door uses, the
  ...
  **Choose the look in the Lab, not by editing `#define`s.** FrogletTools > Ecology >
  ...
  compiles all seven kernels into every prism shader and allocates registers for the
  ...
  - **5 — WORLD-SPACE SHATTER3D (carried — REJECTED ON LOOK 2026-08-10, the day it
    ...
    stays right if the glancing-plane failure is solved. Kernel 6 (SHARD3D) is that
    successor: Euclidean distance-to-owner fill, level sets are spheres. Coverage
    proven offline 2026-08-25; look on real mass at speed is unearned. Do not
    re-ship SHATTER3D as-is. Dials: `PRISM_OCCLUSION_SHATTER3D_CELL` / `..._WALL`
    (ratio), live in the Lab, which shows the same warning.

  - **6 — WORLD-SPACE SHARD3D (Lab candidate, 2026-08-25 — coverage proven offline,
    look unearned).** The successor to SHATTER3D's glancing-plane failure: same
    world frame, octave ladder, 2×2×2 owner search, and morph; the fill is
    Euclidean distance-to-owner (3D Worley F1). Level sets of distance-from-a-point
    are SPHERES — closed surfaces — so they cannot lie flat against a plane over a
    whole plate. A polyhedral gauge could still plate-flash if a facet were
    parallel to the viewed face; Euclidean is the geometrically safe metric.
    Wall is meaningless (radial fill, no band). CDF fitted by
    `Tools/Shaders/verify_prism_shard3d.py` against a clang build of the shipped
    HLSL: **LO=0.155 HI=0.915, compiled |coverage − alpha| = 0.00783** (n=8000,
    python fit 0.00740, target < 0.01). Glancing-plane construction: SHATTER3D
    threshold range < 1e-4 on 415/417 constructed crack planes (median 8.34e-07);
    SHARD3D F1 range > 1e-6 on 417/417 (min 0.0178, median 0.171). **Lab Measure
    and the preview pass `positionWS = (pixel, 0)` — a z=0 slice that cannot see a
    glancing-plane flash. Judge on real mass at speed.** Shipped kernel stays
    SHATTER. Do not Bake as CURRENT until the look is earned. Dial:
    `PRISM_OCCLUSION_SHATTER3D_CELL` (shared with kernel 5).
...
### 4.7.3 The fourth citizen of §4.7 — the SHOCKWAVE FRONT (shipped 2026-09-24)

The cradle's sibling, built with the `/prism-morph` skill the cradle's history produced, and the
first member of that family chosen from the skill's own candidate list rather than from a bug. Like
the cradle it moves **vertices**; unlike it, it belongs to no vessel — and, after four playtests,
to exactly one object in the game.

A **detonating warhead** throws its own wavefront outward through the mass around it: a thin
spherical **shell** of rippled prisms, born at the shell's own half-thickness and travelling out to
the exact radius that blast reaches, **once**, as the blast expands. The shell is not decoration —
**it is the blast's own volume, drawn on the mass rather than on the HUD**.

**And it is honest precisely BECAUSE that blast touches no prism mass.** The 10-point warhead
shockwave is the one blast in the skyburst's detonation whose whole payload is aimed at LIVING things
— it debuffs pilots and jousts creatures, and it authors `affectsPrisms: 0` — so until now it had no
expression in the arena at all beyond a translucent sphere flashing for 0.15 s. The prisms ripple as
the shockwave crosses them and are **still standing afterwards**, which is exactly what happened to
them. A blast that *destroyed* the mass it rippled would be saying the same thing twice, and the
ripple would be the less legible of the two.

**It is half of a PAIR, and the pair is the point — now separated in TIME as well as in channel.**
On the way IN, the round publishes its armed fuze volume as a **LIT SPHERE** (`Docs/LIT.md`,
`Projectile.PublishFuzeLit`): mass standing where this warhead *will* go off, in the shooter's domain
colour. That says **WHERE**, statically, in colour. **WHEN** it goes off, the blast's own front says
**HOW FAR**, kinetically, in vertices. A promise and its payoff, drawn on the same mass. Two channels
of the surface description, one weapon, and neither duplicating the other — which is also the
constraint the design was cut against: the lit sphere already existed and already answered the static
question, so a second static read would have been wallpaper on top of information.

**It belongs to ONE object, and the three playtests are why.** It shipped on every vessel, bound
where the corridor and the speed tunnel bind, and the verdict was *"in this state this is an awesome
effect, but it will be overused as a wake on every vessel."* That is the finding to carry, because
nothing measurable would have produced it: the per-vessel grant was cheap, correct, replicated and
structurally un-omittable, and it was still wrong. **An effect this strong is an EVENT, and an event
that happens continuously behind everyone is a texture.** It then spent one branch on two carriers —
the Sparrow's skyburst missile and the Scarab's ball — and the ball was pulled for the same reason
one step down: **a ball is in play for a WHOLE MATCH**, so its ripple was continuous too. Then the
**missile IN FLIGHT** went the same way, one step further down again, and on a playtest that
*approved* it — *"this was surprisingly performant when interacting with many prisms. I could see the
pulses as a **wake** for the travelling heavy prism."* The word is the verdict: a front trailing a
travelling object is a wake, a wake is a texture, and the round is in the air for seconds. What
survives is the **detonation** — 0.15 s, once, at the moment the weapon actually does something.
*Three carriers were each individually good and each wrong for the same reason, which is the whole
lesson: the question is never "does this look good on the thing", it is "is the thing an EVENT".*

There is a second, quantitative half to the same call: the high-poly residency budget
(`maxResidentPrisms`) is **shared and split evenly across every live front**, so a per-carrier grant
does not merely add fronts, it *divides the one that mattered*. Four hulls is 24 dense prisms each;
eight is twelve. Past a certain headcount every front is the authored 24-triangle prism again and the
effect is gone from all of them simultaneously — the failure mode of a shared budget is not a slow
frame, it is the feature silently switching itself off everywhere at once. **Adding a second carrier
is therefore a design call, not a wiring one**, and `PrismWakeTests.NothingButTheHeavyWarheadIsGrantedAFront`
is what stops a reasonable-looking diff from re-adding either grant.

**The discriminator is the WARHEAD, and it needed no authored field.** The Sparrow's base and heavy
rockets are **ONE prefab and ONE pool**, told apart by a per-shot `ProjectilePayload`
(`ArmWarhead`) — so the previous cut's `Projectile.leavesWake` bool could not have told them apart at
all, and would have granted the front to the cheap wing rocket as well. What can:
`Projectile.WarheadBlastRadiusMultiplier` is already `Payload.ArmWarhead ? authored : 0`, and is
authored **zero on every non-skyburst prefab in the fleet** — so *"the heavy skyburst and nothing
else"* falls out of the weapon rather than out of a flag somebody has to remember to set. It is the
same no-op argument `PublishFuzeLit` already makes for itself, which is the tell that the two halves
belong together. **The discriminator did not have to move with the carrier**, which is the cheapest
part of the whole change: the warhead BLAST only ever exists on a heavy shot, because the one site
that spawns it (`ProjectileDetonatorSO`) is *already* fenced by that multiplier — so granting the
front there inherits the gate with nothing new to author and nothing new to keep in step. *General
rule: when two variants of one thing share a prefab and a pool, a prefab field is not a discriminator
— find the gameplay quantity that already differs, and then prefer to hang new behaviour off the site
that already asks it.*

**A carrier is a CAPABILITY, never a type** (`IPrismWakeCarrier`), and it answers with a **REACH**
and a **PROGRESS**. `PrismWakeSource` knows nothing about explosions: it asks whichever component on
its own object answers that interface. The interface used to ask for a VELOCITY, and the speed window
that came with that is what made the first cut invisible (below). A blast's criterion was never speed
— it is *is there a blast, and how big is it* — so there is **no speed window at all**, and
deliberately **no fallback**: with nothing answering, the component does nothing. The cylinder cut
measured a transform delta when the interface was absent; for a front the equivalent fallback would
have to invent the one number that must not be invented, a radius the blast does not actually have.
Both are asked **every frame** rather than cached — the progress obviously moves, and the reach is
live because a carrier is entitled to resize (the previous carrier's warhead radius grew **20×** in
flight as MASS scaled the round).

**THE FRONT HAS NO CLOCK OF ITS OWN, and that is what the carrier move bought.** A round in flight
had no wavefront, so the source integrated one at an authored `pulsesPerSecond` — a front leaving the
round every 0.33 s, forever, all the way in. A detonating blast **already has** a travelling
spherical front: its trigger volume expands from nothing to its full radius over its own authored
duration, and `currentRadius = colliderRadius × MaxScale × ease` is the number its damage pass uses.
So the position is **read** (`AOEExplosion.TryGetShockwave` hands over that same eased fraction) and
the pulse rate is **retired outright** — field, accessor, `PrismWake.PulseRate` and all. *A rate
authored beside a clock the carrier already owns is a second answer to one question, and the two
drift: the ripple would arrive at a prism before or after the blast that is supposed to be causing
it.* What the source still owns is the mapping into the shell's legal travel band
(`FrontRadiusAt`, which floors the front at σ) — and that division of labour is load-bearing rather
than tidy: **a carrier reports a progress, never a radius**, so it cannot hand over a front position
that breaks the no-fold proof. An expanding blast honestly starts at radius **0**, and a front there
would straddle its own centre.

**The FIRST FRAME is silent, and the whole invisible-swap contract now rests on it.**
`AOEExplosion.ExplodeAsync` awaits its delay before its first expansion frame, so the blast exists
for one frame with its progress still exactly 0 — and `FrontEnvelope(0)` is exactly zero, value and
slope. That is the frame `PrismWake.Flush` hands every prism in the blast's volume the high-poly
mesh, and the map provably moves nothing on it. On the travelling carrier this held *statistically*
(the shell is thin, so a prism swapped in at reach-range was almost always somewhere the front was
not); here it holds **structurally**, which is strictly better and is why the interface's contract
says a carrier must answer true at progress 0 *before* its front starts moving. A carrier that only
began answering once it was moving would hand those prisms the dense mesh with their vertices already
displaced, and they would pop.

**Two consequences of the swap, stated rather than buried.** The **engage ease is authored 0** — the
envelope's C1 zero at birth already *is* the engage, and on a 0.15 s event a second one is pure
attenuation (at the old 0.15 s engage the front never reached half depth). The release ease is kept,
because a blast cancelled mid-sweep by a turn end freezes with the envelope at full value and that
must fade rather than blink. And a documented **imprecision is retired by construction**: while the
carrier was the round, the front was a sphere about a MOVING centre, so a pulse launched a moment
earlier was re-centred on where the round was *now* — an error no closed-form map can fix without
per-pulse history. A detonation does not move. *The honest cost in the other direction is TEMPO:*
the sweep is **0.15 s**, about nine frames at 60 FPS, once per heavy rocket — a hard whump rather
than a rhythm, and much cheaper than the old continuous draw (the residency set is 128 prisms around
a stationary point for nine frames instead of churning along a flight path for seconds). If it reads
as too fast, the dial is the warhead prefab's own `ExplosionDuration`, and moving it is a
**gameplay** change — it sets how fast the debuff volume reaches a victim, and how much time a victim
has to leave it — so it wants a playtest rather than a quiet edit.

**The two non-spherical blast shapes refuse in CODE.** `AOEExplosion.TryGetShockwave` reads
`MaxScale` through the authored collider radius, which is a radius only for a sphere: on
`AOEConicExplosion` it is the capsule's base diameter across the gape axis, and on
`AOECylindricalExplosion` the plate's diameter with its length on a separate dial. Both override it
to return false, rather than being trusted never to be granted a source — *a rule enforced by which
prefab somebody dropped a component onto is not enforced by the code.*

**Spherical, not cylindrical, and the frame follows the FORCE.** The cylinder framed this about the
path a fast vessel had just flown, because a wake is about a path. A warhead's force is not about a
path at all: it is radial about a point, it has a RADIUS the gameplay already authors, and its front
travels **outward** rather than streaming backward. So the frame is a sphere about the round — the
cradle's frame (§4.7.2), not the old wake's — and every cylinder-specific piece went with it:
`_PrismWakeAxis` is deleted (a sphere has no axis), the Jacobian's SHEAR term is gone, and the
residency pass is a plain sphere query with no second filter.

**The map is one line, and the amplitude is ABSOLUTE rather than a strain.** With `U` the round's
centre, `q = p − U`, `r = |q|`, `r̂ = q/r`, `c` the front's current radius and `σ` its half-thickness:

```
s    = (r − c) / σ                                   where the vertex sits across the shell, −1..1
P(s) = (1 − s²)² · sin(2πQs)     for |s| < 1, and EXACTLY 0 outside
f(r) = r + σ · A · w · P(s)
p'   = U + f(r) · r̂
```

Read it as two factors and it is a shockwave front. `sin(2πQs)` is the **pulse**: `Q` whole cycles
across the shell and nothing outside it — **the short bandwidth is the whole design.** The cylinder
carried a TRAIN of crests filling its entire support; this carries one wavelet, so what the eye
follows is an edge arriving and passing rather than a standing corrugation. `(1 − s²)²` is the
**window** that makes "and nothing outside it" true to first derivative as well as value: it has a
DOUBLE root at both faces, so `P` and `P′` both vanish there on the window's own account, whatever
`Q` is — which matters because the shell's two faces sweep through mass at speed and a kink at either
would read as an invisible wall passing. Taking `Q` **whole** then buys the second derivative too
(the sine vanishes at the faces as well), and buys the honest reason: a whole number of cycles is a
complete wavelet rather than one cut off mid-swing.

**Why the amplitude is absolute.** The cylinder scaled `r` by `(1 + E)`, a dimensionless strain,
which made displacement grow with distance from the path — correct for a 30-unit reach and wrong for
a 95-unit one, where the outermost mass would move **40 units**. Here the displacement is `σ·A·w·P`,
**bounded by `σ·A` everywhere**: a front of a given thickness carries a ripple of a given depth,
which is what a front is. The strain map's one real virtue is kept for free — the blast's centre is
still a fixed point, because the shell is born at `c = σ` and so never reaches it.

**NO FOLD is proven, and ONE condition does both halves.** The map folds if the radial stretch
`a = f′(r) = 1 + A·w·P′(s)` or the tangential stretch `b = f(r)/r` goes non-positive.
`max|P′| = 2πQ` **exactly**, attained at `s = 0` — so `a > 0 ⟺ A < 1/(2πQ)`: one dimensionless
number with no radius, no reach and no thickness in it, so retuning any of those cannot invalidate
it, and `PrismWakeConfigSO.FoldingAmplitude` **derives** the clamp from `Q` rather than hard-coding
one (raising the bandwidth makes the crest steeper, so it must carry less amplitude — that is the
bound saying so). `b > 0` then follows with **no second bound**: `a > 0` makes `f` strictly
increasing, so on the support `f(r) > f(c − σ) = c − σ ≥ 0`, because the publisher births the front
at `c = σ` and the shell can never straddle the centre. That birth radius is therefore not a taste
decision — it is half the proof, and `PrismWakeTests.FrontRadius_IsBornAtTheShellAndDiesAtTheReach`
asserts it. The shipped asset sits at 90% of its own bound.

**The normal is the analytic inverse-transpose, and it is the CRADLE's shape.** A purely radial
`f(r)` has differential `diag(a, b, b)` in `(r̂, θ̂, φ̂)`, so
`n′ = r̂·(n_r/a) + (n − r̂·n_r)/b` — two terms, **no shear**. The old wake's shear existed only
because that wave travelled along an axis while displacing along a radius, and here the travel IS the
radius. At `P ≡ 0` it is `n` bit for bit, which is what makes the shell's faces seamless. The radial
term `a` carries the ENTIRE derivative of the wavelet, so it is exactly what a normal that merely
looks plausible would omit — hence it is the file's `#ifndef` dial (`PRISM_WAKE_RADIAL_GAIN`) and the
harness's negative control.

**The pulse is INTEGRATED on the CPU, and both ends of its life reach zero.** A front is born at `σ`
and dies at the reach (`PrismWakeConfigSO.FrontRadiusAt`); the source advances one normalised pulse
clock at the config's rate (3/s) and derives both the front's radius and its own strength from it.
Integrating rather than evaluating a function of position is what keeps the shell travelling smoothly
...
the first cut's `0.25 / 24 u` pair and false by **19 units** the first time the shell was thickened,
which would have swapped prisms to the dense mesh while their own vertices were already displaced —
the one thing §4.2's invisible-swap contract forbids. Adding σ makes the contract structural and gives
the margin its job back. (The general rule, the prism occlusion corridor's own, one level down: *a
clearance written as an absolute distance, against a volume whose extent is a fraction of something
...
**Splice ORDER, and why the front is not last.** The chain ends
...
position; a front applied AFTER the drape would ripple the very vertices the drape had just closed
...
`Tools/Shaders/prism_vertex_chain.py` identifies a vertex morph **structurally** — a Custom Function
node with exactly the four correctly-directed Vector3 slots `Position`/`Normal`/`OutPosition`/
`OutNormal` — and walks past any chain of them with a cycle guard. *A name list needs one edit per
wirer per morph forever, which is a defect amplifier rather than a fix.*

**Proven by `Tools/Shaders/verify_prism_wake.py`** — clang++ over the SHIPPED file, executed, ten
properties on randomized inputs under a real non-uniform model matrix: identity with no live slot;
identity outside the shell on **both faces separately** (6,000 vertices inside the inner face and
6,000 beyond the outer one, counted separately so a support that collapsed on one side cannot pass on
the other's strength — and the inner half is what makes the blast's centre a fixed point); **radial
purity** (direction from the blast held to 1-dot `2.4e−7`, and the displacement measured at most
**0.890** of `σ·A·w`, which is `max|P|` for `Q = 1` and confirms the bound is real rather than
generous); **BANDWIDTH** — exactly `4Q−1` sign changes of the displacement along a radial sweep, all
at the radii `s = j/(2Q)`, over 600 sweeps at `Q = 1..3`, i.e. one wavelet and not a train; **no
fold** over 2,160,720 samples spanning the config's whole authored amplitude range at `Q = 1..6`
(tightest `df/dr` **0.124**, displacement never above `σ·A·w`); affine in the strength; the normal by
**CONVERGENCE RATE** — `5.1e−3 → 1.3e−3 → 3.4e−4 → 8.3e−5` as the patch halves; no seam at either
face; and slot authority (the greater `w·window` wins and the loser contributes **bit-exactly**
nothing, so two spherical fields about two different centres never sum into something no analytic
normal describes). The negative control rebuilds the file with the radial stretch term neutered and
the error **plateaus flat at 0.084** across all four patch sizes. The harness also **fails loud if
`_PrismWakeAxis` returns**, because every property it measures is a statement about a spherical map.

**One test had to be fixed before it measured the shader at all, and the lesson is the skill's own.**
The bandwidth sweep recovers the displacement by subtracting a large radius from a large radius, and
near a zero of the wavelet that is exactly where the signal is — so its first cut reported 658 of 720
sweeps with the wrong cycle count while the map was correct. It now carries a **precision gate** of 8
ULPs at the sample radius, a statement about float arithmetic rather than about the shader, and the
round sits at the origin to remove one more cancellation. *When a numeric test fails, ask whether the
...
**First playtest: the shipped tuning was not subtle, it was OFF, and the reason generalises.** The
report was *"too subtle to notice on myself or the AI vessels"* in Skim Race. Measured: the Squirrel
authors `DefaultThrottleScaler 60` and `MaxBoostMultiplier 5`, so it cruises around **54 u/s** and
tops out at **300** — against an authored engage speed of **150**. At cruise the window returned 0,
`PrismWakeSource` called `PrismWake.Clear`, and the bank published **nothing at all**. The window was
written to be ABSOLUTE and fleet-wide (the speed tunnel's argument, deliberately not normalised
against a hull's own top speed) and then authored against numbers nobody had measured, so on the hull
the mode actually flies it was a window that never opened. *An absolute window is a claim about the
fleet's real speeds, so it has to be authored from measurements of them — and a window that never
opens is indistinguishable on screen from an effect that is too weak, which is why one report covered
both.* The speed window is **gone** rather than re-authored: a warhead's criterion was never a speed.

**Second playtest: right effect, wrong owner — and nothing measurable would have said so.** The
verdict was *"in this state this is an awesome effect, but it will be overused as a wake on every
vessel"*, and the per-vessel grant it rejected was cheap, correct, replicated, un-omittable by wiring
and inside budget. **The general rule is about FREQUENCY, not cost: an effect strong enough to be an
event stops being one the moment it is continuous, and "who has this" is a design decision that no
amount of correctness in the binding can make for you.**

**Third playtest: the two carriers were still one too many, and the shape was wrong for both.** The
verdict named the ball and the missile's implementation separately, and the redesign answers both
with one move — the ball is out (continuous), and what is left is not a wake at all but the **front**
of the blast the round is about to deliver, so the frame became the force's own. The look is not a
weakening of the cylinder: it is a different claim about the same weapon, made to compose with the
lit fuze sphere that was already telling the other half of it.

**`PrismWake.ReportState` exists because of the first report.** Every way this can fail reads the
same on screen — a carrier answering no reach, a config switched off, a live amplitude too small to
see, a spatial index that returned no candidates — and no amount of staring at the arena separates
them. Once a second on `CSLogChannel.PrismRuntime` (FrogletTools ▸ Toolbox ▸ Logging, off by default,
one float compare per frame when it is) it names the live slot count, each slot's strength / front /
reach / σ / Q, and how many prisms carry the dense mesh. It reports the IDLE state too, **with the
reason**, because a system that goes quiet when it has nothing to say cannot be told apart from one
that is not running.
...
exactly like an effect that is too weak*, and this family has now paid for that sentence twice.

**Then the OVERTUNE, to the structural ceiling.** Amplitude is bounded by the no-fold proof
(`A < 1/(2πQ)`, the asset clamping to 90% of it) and σ is bounded by the travel it has to leave the
front (`halfThicknessFraction ≤ 0.45`, since a front born at σ and dying at the reach has `reach − σ`
of travel), so peak displacement at `Q = 1` has a hard ceiling of
`reach × frac × 0.9/(2π) × max|P|`. Both dials went to it: amplitude **0.143**, half-thickness
**0.45** of the reach → a **42.8 u** shell carrying a **5.45 u** peak ripple at a skyburst's
resting-Mass reach of **95.2 u**, **1.98×** the first cut's 2.75 u and as far as the proof allows
without folding a prism inside out. Raising Q cannot buy more (a shorter wavelet has a steeper crest,
so the bound falls with it) and `wavesInFront` is clamped at 1 anyway; a fatter σ cannot either (the
front would be born where it dies). **The thick shell is the bigger half of the win and it is not
about amplitude:** at 0.45 of the reach almost every prism inside the blast volume is in the shell at
once, so what the player reads is a whole neighbourhood of mass moving together rather than a thin
band passing through it.

**The residency budget was re-spent rather than raised, twice.** The GPU pays in TRIANGLES, and prism
count against subdivision is a trade inside that: the overtune took **128** resident prisms at
subdivision **10** = **153,600** triangles, *fewer* than the first cut's 96 × 12 = 165,888, because
what could not be seen is prisms MOVING and residency decides how many of them move smoothly. The
sixth playtest's *"very subtle"* spent it the same way again — **160 × 9 = 155,520**, a quarter more
prisms in motion for **1.25%** more triangles, on the reasoning that one subdivision of smoothness is
invisible at arena range and a quarter more mass in motion is not. `MaxResidentPrisms` carries its own
ceiling (160) and `Subdivision` its own floor (8) in the edit-mode suite, so the triangle bound alone
cannot be satisfied by a thousand prisms at subdivision 2, which is the authored prism with none of
the smoothness the family exists for. Both re-spends are now AT those bounds, so the next one has to
raise the triangle budget and say so.
...
subdivision 9, **160** resident prisms (155,520 triangles), 24 u margin, **0 s in** (the envelope's
...
`ExplosionDuration` of **0.15 s** (349 u/s) — a length that **cannot be spent**, see §4.7.3b.
Recipe: `.claude/skills/prism-morph`.
...
authored `ExplosionDuration` of **0.15 s** — about nine frames at 60 FPS — inside the loudest nine
frames in the game: the warhead's own expanding translucent sphere, the sibling spherical blast
destroying mass with debris, and the 72-prism cairn blooming over twelve frames. This project's own
measured answer to *how long is a beat the player reads as one event* is the crystal capture's
**0.44 s** (`Docs/ECOSYSTEM.md §31`), and 0.15 s is a third of it. The only dial is that prefab's
`ExplosionDuration`, which is a GAMEPLAY number. It was **deliberately not changed in the same commit
as the ranking**, for the reason the Serpent's scope already records: *when two fixes ship in one
commit and either could be the one that worked, the report cannot tell them apart* — and the ranking
is both the one with a proof and the cheaper to undo. §4.7.3b is what happened when it was reached
for.

### 4.7.3b Sixth playtest: *"very fast and very subtle"* — the length cannot be spent, so the SHAPE was

The ranking fix worked (*"i see it now"*) and the same report asked for the obvious follow-up: slow
the blast down. **It cannot be slowed, and the reason is a shipped test with a play-tested
justification behind it.** `SparrowMissileFuzeTests.TheWarheadExpandsFastEnoughToCatchOrdinaryFlight`
requires the blast to CONTAIN a target that was receding at 120 u/s when the fuze tripped, before it
stops expanding — *reach is not capture*, since the sphere grows as `R·sin(t/D · π/2)` and a trigger
enter is an overlap rather than a surface crossing. Solving that against the shipped geometry (trip at
76.2 u, blast 95.25 u) gives a **ceiling of 0.1658 s**: the authored 0.15 s is 90% of the way to it,
and **0.5 s was the ORIGINAL value and was rejected for exactly this** — it bought only ~40 u/s of
catchable escape speed, below every vessel's cruise, so a rocket could detonate beside a pilot and
touch nobody while the radius ordering passed. So the honest options were to buy margin by widening
the warhead multiplier (a bigger blast — a weapon buff) or narrowing the fuze (a rocket that flies
closer before going off — a nerf), and **neither is a tuning change to make inside a visual-effect
commit.** The blast is untouched.

**What was left was WHEN the front is strong, and that turned out to be most of the problem.** The
envelope was `4·S(u)·S(1−u)`, a single hump that touches full strength for an instant — measured over
the real nine-frame sweep at the blast's own eased progress, it is at ≥0.9 for exactly **one frame**
and ≥0.5 for **three**. The amplitude and the shell thickness are both already ON their structural
ceilings (the no-fold bound `A < 1/(2πQ)` at 90%, and `IsSane`'s 0.45), so there was nothing left to
make the ripple BIGGER; the only quantity nobody had spent was how much of the sweep it spends at the
size it already is. The envelope is now a **PLATEAU** — `S(u/a)·S((1−u)/a)` with
`a = EnvelopeRiseFraction = 0.3`, two clamped smoothstep ramps that each reach their own C1 zero
inside the span — which holds ≥0.9 for **three** frames and ≥0.5 for **five**, takes the fraction of a
front's life above 90% strength from **21.4% to 51.7%** and its mean strength from 0.514 to 0.700
(**1.36×**), with the **peak unchanged at exactly 1**. That last clause is what keeps the whole
no-fold proof untouched, since the bound is stated against a strength of 1: *a plateau is free, an
amplitude is not.*

Three things about that shape are load-bearing rather than taste. **Both ends are still EXACTLY zero
in value and slope**, and each for its own reason — at the reach it is continuity of existence, at
BIRTH it is the entire invisible-swap guarantee (the carrier reports progress 0 for one frame before
its sweep begins, and that is the frame the residency pass hands every prism in the volume the dense
mesh). **The rise fraction is bounded at both ends and neither bound is aesthetic**: below ~0.25 the
ramp is steep enough that the slope at birth stops reading as zero (`S(h/a) = 3h²/a²` against the
suite's 0.05 ratio gate, which `a = 0.25` meets by 4%) and a front that appears with a kink is a front
whose swap can be SEEN, while at 0.5 the shape degenerates back into a crest with no plateau at all.
And **the plateau is ASSERTED, not left to the shape** — `FrontEnvelope_IsZeroWithZeroSlopeAtBothEnds`
now gates ≥40% of a front's life above 90% strength, and its monotonicity check went from strict to
non-strict on purpose: *strict was what the single-hump shape happened to satisfy, not what the
property is* — what must never happen is a DIP.

**The general rule: when an effect's LENGTH is owned by a gameplay guarantee, the shape is the only
budget left** — and ask which of the three quantities (how big, how long, how much of the time) is
actually unspent before reaching for the one a weapon depends on.
...
`_ShieldMorphDirection`, `_ShieldMorphOffset`. (Since §4.8.1 the DISENGAGE no longer
stamps them at all — the shatter is ordinary explosion debris — so in practice only the
engage bloom drives this morph.) `PrismShieldMorph_float` lives in
...
- **The shatter is batched pure-entity debris, not a child GameObject** — and since
  §4.8.1 it is not this morph at all: the shards are ORDINARY explosion debris on the
  §4.6 carrier's explosion family. The prism itself is back on its box the instant the
  shield drops, so the shards are necessarily a separate visual; `PrismShieldShatter`
  queues a frame's disengages per shield mesh and spawns them through
  `SpawnExplosionDebrisBatch` with the debris pipeline's own material, the shield's team
  palette riding per-entity colour overrides. This morph's shatter direction survives in
  the properties for compatibility but nothing stamps it any more — only the engage
  bloom drives it.
...
### 4.8.1 The shield shatter IS the prism explosion (final form 2026-08-24)

**A dropped shield sheds its faces as ordinary prism-explosion debris.** `Disengage`
spawns ONE explosion-debris entity through the exact path a dying prism's pieces take —
`PrismShieldShatter` groups a frame's disengages per shield mesh and calls
`SpawnExplosionDebrisBatch` with the debris pipeline's own material — so the shards get
the same clock stamps, the same whole-body flight along the breaking impulse
(`PrismExplosionClock`), the same per-face rotate-away (`RotateFacesAlongAxis`), the same
...
- **flat per-face TANGENTS** (`dP/dU` of the UV frame, `normalize(v1−v0)`) — the
  subgraph's other rotation axis. This was the one attribute the shield meshes lacked, and
  its absence is exactly why the original "faces don't rotate away" report existed: a
  missing tangent hands the subgraph a zero axis and the rotation silently degenerates.
  The edit-mode suite (`BothGenerators_BakeTheDebrisAttributeSet`) now gates all three.
...
1. *An HLSL re-expression* of drift + tumble inside `PrismShieldMorph_float`, justified at
   the time by "RotateFacesAlongAxis is not on BlockGraph and needs a tangent the shield
   meshes do not carry." Both facts were true; the conclusion was wrong — the answer to a
   missing attribute is to AUTHOR the attribute, and the answer to the wrong graph is to
   put the shards on the right one.
2. *A duration/pressure retune* of that bespoke effect toward the base's numbers —
   convergence by imitation, which drifts, instead of by identity, which cannot.
3. *A mirrored back-face mesh bake* to fix the back-face fresnel the open shards exposed —
   machine-verified per-vertex and still wrong on screen: under `Cull Off` both coincident
   copies rasterize, and reversed-winding coplanar triangles z-fight (per-vertex proofs
   are structurally blind to rasterization phenomena). Reverted.
4. *A BlockGraph erosion splice + a FresnelPower4 two-sided edit* — both deleted with the
   architecture that needed them. The fresnel's authored back-face branch
   (`x = N·V > 0 ? N·V : (N·V+1)·0.2`) is the BASE debris' own back-face look, and the
   base looks right; it was never the defect.

The governing rule, an instance of "favor emergent systems" applied to VFX: **when the
base effect already looks right, port the mesh into the pipeline, never the pipeline into
the mesh.** Every bespoke re-expression is a second implementation of a look that must
never drift from the first.

Net shader delta of the final form: **zero** — BlockGraph, ExplodingBlockGraph,
`PrismClockAnimation.hlsl` and `FresnelPower4.shadersubgraph` are byte-identical to
bleeding-edge. The change is two mesh generators, the debris hand-off
(`PrismShieldShatter` → `SpawnExplosionDebrisBatch`, with `PrismDebris.TryGetExplosionConfig`
...
> they read (C7 is done by construction, C8 shipped, C9 shipped, C11 shipped,
> and C6 parent-scale closed as mover-contract (b); C6 remainder is
> devour/graze suction + boid husk shrink). When a row here changes status, that
...
| # | Item | Status |
|---|---|---|
| B1 | Grow-in → clock (all ~12 feeder paths ride the one engine); gameplay-final-at-start (volume/spatial stamps, clock predicates, `ExecuteOnScaleComplete` → start) | ✅ LIVE (strict, the only path) 2026-08-01 — `PrismScaleManager` DELETED (D2, 2026-08-02). Graph wiring ✅. **Prompt 7 (2026-08-25) closed the three B1 pendings:** `HoldColliderAtFullSize` DELETED (BoostRingBuilder applies all kinds at lay; transform final at stamp); arena-gate settle no longer force-snaps (`Prism.AnalyticGrowSettleTime` / `PrismScaleAnimator.AnalyticSettleTime` exposed; `SettleGrowWatch` waits on clock predicates); `CreateBlockCoroutine` 0.6 s window **kept** — `TryReserve` is growth/assembler-only, so trail/`PrismFactory` still need the disable window. PhaseThresholds re-baseline was already ✅ 2026-08-02 |
...
| C11 | Spindle `_DeathAnimation` fade (prism-adjacent) → clock inputs on spindle material | ✅ SHIPPED 2026-08-25 — `PrismDeathClock_float` on SpindleGraph + AnimatedSpindleGraph (`Tools/Shaders/wire_prism_spindle_death_clock.py`); stamp `_DeathStartTime`/`_DeathDuration`/`_DeathDirection` once; `PrismTimerManager` settle; `SetPropertyBlock(null)` recovers SRP Batcher after the fade. Ordered wither is `ForceWither(i * interval)` offsets (`LightFauna.WitherCoroutine` / `LifeForm.WitherToSkeleton`); `LeaveSkeleton` still before stamps; heart at `count × interval`. **SRP honesty:** MPB still unbatches DURING the ~1s fade (unique staggered StartTimes cannot share quantized fade materials without collapsing the ecology-LOCKED order). Do not fade HealthPrisms. Playtest outstanding → Prompt 11. |
| C12 | `PrismImplosion` watchdog → scheduler; orphan cleanup; `SkimFxRunner` stretch beam review; `CloakSeedWall` dead code removal | ✅ DONE 2026-08-25 (Prompt 7). Watchdog → `PrismTimerManager` from `OnEnable` (re-arm after stamp). Orphans: `TrailBlockBufferManager` + `TrailViewer` already deleted earlier. `SkimFxRunner`: vessel FX only (beam particle), not prism animation — recorded so it stops being re-litigated; pooling optional/out of scope. `CloakSeedWallAction.cs` (legacy ShipAction, guid `e4f432f2…`) DELETED; SO + executor + asset **kept** (Serpent.prefab; only live `IsTransparent` producer for checklist Phase 3) |
...
## 6. Handoff — wiring complete (record, not a live gate)

> Step-by-step checklist: `Docs/PRISM_CLOCK_WIRING_CHECKLIST.md` (phases 1–4 ✅,
> Phase 6 / D2 ✅ programmatically). **Do not tell a fresh session to "wire the
> graphs per §4.4" — that work finished 2026-08-02.**

**STRICT MODE has been the only animation path since 2026-08-01.** All four graph
phases are ✅ wired programmatically in-branch and playtest-confirmed (checklist
Phases 1–4; tracker A1 / B1–B3). D2's programmatic deletion pass landed
2026-08-02 (`PrismScaleManager`, `MaterialStateManager`, animation passes in
`PrismEffectsManager`, `AdaptiveAnimationManager`, `TrailViewer`). PhaseThresholds
re-baseline ✅ 2026-08-02 — six freestyle configs re-authored from measured baselines
(`Docs/ECOSYSTEM.md` §18; checklist Phase 6).

**What remains is verification and playtest debt — not wiring:**
...
| **D4** — pooled death spawn | ✅ retired 2026-08-25 (batch-only; declined request dropped). **Grow stays pooled** (Sparrow ReverseSuction). Prefabs remain CONFIG. |
| **C13b** — environment-lay pooling | ✅ shipped 2026-08-25 (unbounded `EnvironmentPrismPool`; snap Blue then ChangeTeam clock-lerps). Playtest outstanding → Prompt 11 item (9): Blue→domain spawn repaint + Wanderway conservation across a cell swap. |
| **C15** — ShapeDrawingManager shrink-to-outline | ✅ resolved by deletion 2026-08-25 (Prompt 15). Unreachable after `MinigameFreestyle.unity`; exclusive dependents gone; SOAP events kept on live prefabs (inert). No clock path to playtest. |

If graphs revert, prisms snap and the console logs one `[PrismClock]` error per unwired
material — run **FrogletTools > Ecology > Prism Animation > Validate Clock Wiring**
and the Phase 5 protocol; do not fall back to CPU animation.
...
- Every new prism visual state MUST arrive as: pool material + stamp + clock shader +
  ...
  per-frame loop.
- **Edit-mode suites** (CI via `bleeding-edge-guard.yml` edit-mode test run):
  `PrismClockWiringTests` (iterates `PrismClockWiringValidator.Specs` — grow, color,
  explosion, suction, flight, shield-morph, jiggle, plus erosion / back-face /
  destruction sight, live-graph `PrismSuctionConverge`, and their load-bearing
  edges; corridor is delegated to `PrismOcclusionWiringValidator.CheckGraphWiring`
  so Validate Clock Wiring is not silently partial; a graph revert that drops a
  Hybrid-Per-Instance property fails naming the property and the graph),
  `PrismCellSwapSuctionTests` (prototype suction cluster, graph props, Cell stamp
  sites, HLSL Duration-0 identity),
  `PrismShieldMorphTests`, `PrismOcclusionCoverageTests` (also names SuctionGraph as
  a live corridor exclusion), `PrismSuperShieldJiggleTests`,
  `PrismDeathVisualTierTests`,
  `PrismSpindleDeathClockTests` (SpindleGraph / AnimatedSpindleGraph carry
  `PrismDeathClock` + Hybrid `_Death*`; live prism graphs do **not**;
  `Spindle.cs` has no per-frame fade; ordered wither is `ForceWither(i * interval)`),
  `EnvironmentPrismPoolTests` (C13b: unbounded pool, no pool-return-delegate wire,
  LayOne/CloneBatch do not raw-Instantiate, Cell TryRelease + trail discriminator
  kept, flora HealthPrism Instantiates folded, Boid / SpawnableBase / SpawnableCord
  still Instantiate, `ResetToNeutralForReuse` does not raise `OnTeamChanged`).
- **Python `--check` gates** (CI via `bleeding-edge-guard.yml` + `unity-ci.yml`,
  no Unity licence): `Tools/Shaders/wire_prism_backface_fade.py`,
  ...
  properties including `_ShieldMorph*` and the live `_Suction*`/`_Location`
  cluster; it does not re-draw CF nodes or edges) plus opaque+clip, corridor
  topology, and spindle death fade. Ten python gates. Do **not** dump
  `PrismDeathClock` onto BlockGraph Specs.
- **FrogletTools validators** (editor-only; the **Specs** they print are CI-gated
  by `PrismClockWiringTests`): **Validate Clock Wiring** (ANDs the corridor graph
  census), **Validate Occlusion Corridor**, plus the occlusion dither lab for
  kernel tuning.
- The three CPU animation managers (`PrismScaleManager`, `MaterialStateManager`,
  `AdaptiveAnimationManager`) were **deleted in D2 (2026-08-02)**. New registrations
  into their per-frame passes are impossible — any revival is a regression.
