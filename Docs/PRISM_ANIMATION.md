...
**The residency budget was re-spent rather than raised.** The GPU pays in TRIANGLES, and prism count
against subdivision is a trade inside that: **128** resident prisms at subdivision **10** is
**153,600** triangles — *fewer* than the first cut's 96 × 12 = 165,888 — because what could not be
seen is prisms MOVING, and residency decides how many of them move smoothly. `MaxResidentPrisms` now
carries its own ceiling (160) and `Subdivision` its own floor (8) in the edit-mode suite, so the
triangle bound alone cannot be satisfied by a thousand prisms at subdivision 2, which is the authored
prism with none of the smoothness the family exists for.
