...
## 11. The status lamp: tint only, and a dropped icon set (2026-08-27)

**The lamp conveys state by TINT alone** — lime online, grey offline, on whatever sprite the
scene authored. `OfflineMenuWirer` deliberately does not touch its sprite or its rect, so a
wiring pass can never rewrite a designer's layout.

### 11.1 What was built and dropped

A purpose-built pair was designed and rendered: **online filled, offline hollow**, inside a
shared ring border, so shape carried the state alongside colour and the lamp stayed readable
small, in peripheral vision, or for a player who cannot separate lime from grey. Depth came from
**alpha rather than a second hue** (opaque ring, 0.60 fill), since a single tint is applied at
runtime. The sprite swapped at the **midpoint** of the colour crossfade so filled↔hollow read as
one motion rather than two.

It was **removed before shipping** (explicit call, 2026-08-27). The sprites are generated by an
editor tool, so landing them needed another editor round-trip — run the tool, save, commit — and
the tint-only lamp was judged good enough to not be worth the trip. Nothing is half-wired: the
generator, the `onlineSprite`/`offlineSprite` fields and the midpoint swap are all gone, so the
component promises exactly what it does.

**If a shape cue is wanted later**, that is the gap: colour alone is the weaker signal, and the
design above is the one to restore (it is in this branch's history).

### 11.2 What was kept — the renderer

The icon renderer was rebuilt during that work and **stays**, because the check/cross benefit
from it. The first pass took one centre sample per pixel plus a fixed 1.5px linear feather on the
distance field — a coarse approximation of coverage that shows as ragged curves. It now evaluates
a **hard** 0/1 shape function over a 4×4 grid per pixel at 256px, so all smoothing is real
coverage and an edge resolves correctly at any curvature.
...
⚠ The check/cross **committed on this branch were produced by the OLD renderer** and are not
regenerated automatically: `EnsureIcon` never overwrites, so authored art always wins. To bring
them up to the new sampling, run **FrogletTools ▸ Interface ▸ Wire Offline Menu Surfaces
(Regenerate Icons)** and commit the result.
