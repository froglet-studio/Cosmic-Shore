...
Statuses: [red] open * [yellow] investigating * [green] fixed (commit) * [white] deferred.

| ID | Title | Confidence | Status |
|----|-------|-----------|--------|
| B2 | `ObjectDisposedException` (semaphore) on Play-Mode abort / fast invite-accept | ~95% | [red] |
| B3 | TC4 bounce leaves 2 vessels + dead controls | Fixed-by-construction | [green] |
| B5 | TC2/TC4 second joiner fails to join | Uncertain (diagnose first) | [red] |
| B7 | Client pair-init runs before remote identity replicates (`InitializePair Player=` empty, vessel-type `Random`) | Verified mostly benign | [white] |
| B9 | Host-return: one client's vessel stuck in autopilot drift + party domains not reset to menu (Jade) | Root-caused & fixed | [green] |
| B10 | Host leaves/disconnects mid-party -> client stuck (no bounce-to-solo + "Host disconnected") | Fixed & verified | [green] |

---

## B2 - `ObjectDisposedException`: "The semaphore has been disposed" [red]
...
## B3 - TC4: bounce leaves two vessels + broken control toggle [green] (fixed-by-construction)
...
**Status. [green] Fixed-by-construction.** The same architectural refactor that
...
**Status.** [green] Fixed via architectural refactor - **clean-leave path
...
## B5 - TC2/TC4: the second joiner fails to join [red]
...
## B7 - Client pair-init runs before remote identity replicates [white]
...
## B8 - Host-side phantom-rejoin loop after client leaves party (stale `joined_party` presence property) [green] (fixed + MPPM-verified 2026-06-02)
...
- **b) `LobbyPropertyWriter` retry exhausts on B1 stale-index churn.**
  ...
  of range" and retries 3x with 2 s backoff. The cycle logs *include*
...
**Status.** [green] Both fixes landed - needs MPPM re-verify (the
...
## B9 - Host-return: one client's vessel stuck in autopilot drift + party domains not reset to menu (Jade) [green]
...
**Status.** [green] Fixed - domain-pick + ready-feed flow verified in a
2-human MPPM test (2026-06-11); full 4-player host-return sweep still to
be re-run.

---

## B10 - Host leaves/disconnects mid-party -> client stuck (no bounce-to-solo) [green] (fixed; MPPM-verify)
...
**Status. [green] Fixed - verified in engine (2026-06-19).** The core bounce-to-solo,
...
Method: see `../README.md` Sec "How we work bugs". Party-side priority
