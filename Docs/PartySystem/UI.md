...
| `FriendInfoSlot` | A single slot in `ArcadeLobbyList` - one of three states: local player, occupied (member avatar + name; plus a **host-only kick X** on remote-member slots), or empty ("+" add button). On `FriendsInfo.prefab`. |
...
| `OnlineInfoEntry` | A row in the Online section with a small **Invite** button (shown only when the player is invitable) and a **X** that cancels a pending outgoing invite or (host only) kicks an in-party member. Tints yellow + pulses while an invite is pending; Invite/cancel/kick share an anti-spam cooldown. Status label: ONLINE / IN PARTY N/M / PARTY FULL / IN A MATCH / IN YOUR PARTY N/M. On `OnlineFriendsInfo Variant.prefab` (a variant of `RequestsInfo`). |
...
The service/SOAP-level happy path is in `ARCHITECTURE.md` Sec "SOAP event flow -
...
button when the player can be invited, plus a X to cancel a pending outgoing invite
or (host only) kick an in-party member. The party panel's `FriendInfoSlot` carries
the same host-only kick X per occupied member slot.
