...
| **R16** | **Fix the open multiplayer join failures.** The party-join path carries **two open red bugs, both second-player join failures** — Party B5 (the second joiner fails to join) and Presence B4 (second invite not delivered). Plus Party B2 (`ObjectDisposedException`) open and B11's fix reverted. A cohort that compounds through invites cannot compound through a broken join. | Session + editor | DoD #2, H10 | [`MULTIPLAYER_JOIN_BUGS_PROMPT.md`](prompts/MULTIPLAYER_JOIN_BUGS_PROMPT.md) |
