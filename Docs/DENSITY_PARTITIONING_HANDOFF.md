...
1. **Phase 1 (density grid) - `c058663`.** `BlockDensityGrid` is sized to the
   ...
   has the correct domain ordering (Sec 2.3.1 in the audit). Geometric benchmark
...
4. **Flora behavior - `52e4f4a`.** Both `AssembledFlora` and `BranchingFlora`
   ...
   (d) plant within `0.6 x MembraneRadius` instead of huddling at 75-200m
...
The audit Sec 7.4 notes this: only in-game observation can confirm
...
This is blocked on Sec 3.1 - fauna have to actually spawn before we can
observe consumption.

### 3.3 Audit doc lag

`Docs/DENSITY_PARTITIONING_AUDIT.md Sec 7.3` lists Phase 2 as landed (correct)
but doesn't yet cover the ecology-side fixes (`52e4f4a`, `809572d`) or
the HUD work. Add a Sec 8 covering those, or fold the HUD work into a new
`Docs/ECOLOGY_HUD.md` and link it from Sec 7.
...
| File | Why |
|---|---|
| `Assets/_SO_Assets/Cell Configs/Blob Cell/Blob Cell Config.asset` | Phase bands widened (Quiet 100/50, Settled 300/180, Restless 500/300, Frozen 700/420, Rabid 900/540) so oscillation is visible. **Suspect**: Quiet=100 too high for Menu_Main - see Sec 3.1. |
...
4. **Expected to be broken (Sec 3.1):** the spawn-cycle ring never advances, no
   fauna spawn.
...
1. **Diagnose Sec 3.1.** Lower `Blob Cell Config.QuietEnter` to 30, re-test
...
3. **Document Sec 3.3.** Audit doc Sec 8 covering ecology + HUD.
...
- **Three fundamentals doing most of the work:** Mass (per-domain prism counts),
  ...
  density grid). Audit Sec 7.3 and CLAUDE.md "Design Philosophy" cover this.
