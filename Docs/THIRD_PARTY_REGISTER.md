...
| **NiceVibrations** ⚠️ | Lofelt / More Mountains | **4.1.1** (Lofelt SDK 1.3.3) | `NiceVibrations` | 47 MB / 395 | ⚠️ **not the product EULA** — see below | **Ships**, incl. the **Demo** (30 `.cs`, not editor-only) + `Resources/`. **Nothing first-party references it any more** (12 Sep): no code (row 4), and no assets — measured by sweeping all **411** guids owned under `Demo/` and `HapticSamples/` against every shipped scene, prefab and asset (rows 4b/5). The only holders left are inside `_Prefabs/MIgration_Prefabs (DELETE LATER)/`, which ships nothing and is an open decision (`LAUNCH_BLOCKER_INDEX.md` §B3). The folder is now deletable on its own evidence |
