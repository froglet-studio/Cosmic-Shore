...
| 5.4b | Six ElementalFloat fields on five MonoBehaviour actions under `VesselActions/` (pre-`R_`, live on Falcon, Shrike and Urchin) still read `.Value` through the bound path. Behaviour-neutral to convert; needs the editor | **OPEN — own branch** |
...
| 5.11 | **A shared `ShipActionSO` mutates its own serialized field at runtime.** `GrowSkimmerActionSO.ApplyMaxSizeDebuff` writes `maxSize.Value = original * multiplier`, awaits, then writes it back — on a SHARED asset, so in multiplayer two Rhinos debuffed at overlapping times race on one number and the second restore writes the FIRST one's already-multiplied value back as "original". This is the exact last-initializer-wins hazard `ARCHITECTURE.md §2(a)` and the vessel contract both name as their cautionary tale, and it predates this branch — found by D1 while proving the `Enabled: 0` flip on that same field is a no-op. The fix is executor-side state (a per-vessel multiplier the executor applies at use time), not a guard on the SO | **OPEN — pre-existing, found by the element pass** |
