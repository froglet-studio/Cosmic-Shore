...
| 5.7 | ~~**Nine** SO-hosted `ElementalFloat` fields are only ever read as `.Value` and should be plain `float`s~~ — **SHIPPED 2026-09-20.** All nine are now `float` (`depthScale` 1, `boostMultiplier` 3, `projectileTime` 3, `speedValue` 375, `maxSize` 120, `shrinkRate` 6, `boostMultiplier` 1.25, `maxSize` 4, `shrinkRate` 1), each scalar equal to the block's authored `Value`. Two corrections to this row as written: **four of the nine exposed the whole `ElementalFloat` as a public property**, so it was an API change and not a field rename (`FullAutoActionSO.SpeedValue` turned out to have zero consumers and is deleted); and **it did not need the editor** — the hazard is the serialized DATA, because changing the type turns the YAML from a mapping into a scalar and Unity falls back to the C# initializer for a key it does not find (rule 4-i), which on `GrowSkimmerActionSO.maxSize` would have been a 40× blade-length change with nothing to report it. The migration rewrote every block in the same commit, asserting the old `Value` against the new scalar before writing. | **SHIPPED** |
