...
### 3.1 Mounting a jet on a BONE

A jet that belongs on an engine has to follow that engine when the model animates, which means
parenting to a bone rather than to the vessel root. For the skinned family that is the normal
case, and `VesselJet.mountBone` is how it is authored: put the bone's NAME on the jet instance and
it re-parents itself at Awake, keeping its authored local position as an offset FROM the bone — so
`(0,0,0)` pins it exactly.

**By name, not by reference, and that is not a shortcut.** A bone inside a nested model prefab can
only be referenced from a vessel prefab by the model file's own sub-asset id. Those ids are
generated by the importer, are not written into the `.meta` when `internalIDToNameTable` is empty
(as it is for every vessel model here), and do not survive a re-export — so an FX mount authored
that way detaches silently the next time the art is updated. Name resolution is the same choice
`VesselAnimation.ResolvePart` already makes for animated parts, for the same reason
(`Docs/VESSEL_CONSTRUCTION.md` §5). An unresolvable name is a loud error naming the vessel and the
bone, and the jet stays where the prefab put it.
...
**These two took four and five passes, and what the passes cost is the useful part:**

- The **Sparrow** took five passes, and what each one cost is the useful part. Two jets at the
  measured rear lobes `|x| ∈ [1.59, 1.64]`; one centred plume at the hull's rearmost point `z =
  −4.65`; one centred plume pulled forward to `(0, 0.40, −0.09)`, which had the right depth and
  sat on the centreline — i.e. inside the fuselage fan (69 hull vertices within 0.5 of it); two
  jets on the plug nozzles at `(±3.03, 0.40, −0.96)`; and finally six, bone-mounted on `b_Tail1..3
  .L/.R`.

  **The skin weights settle what every earlier pass was guessing at.** A bone's cluster names
  exactly the vertices it drives, so ranking all 65 bones by the z-centroid of the geometry they
  skin describes the whole ship with no interpretation: `b_WingPiece*` and `b_SlideChip*` at the
  back (down to `z −4.66`), `a_Body` through the middle, and the six `b_Tail*` bones alone at the
  far end, `z +4.58 … +7.57`. The structures the fourth pass called nacelles are skinned by
  **`b_ShipGun1.L/.R`** — they are the ship's GUNS, which is why the prefab's hand-authored
  `LeftGun`/`RightGun` muzzle points sit on them at `(±3.2, 0.4, +1.3)`, just past the barrel end
  at `z +1.19`.

  **Which way the ship faces was worth pinning down three independent ways**, because the artist's
  naming and the geometry disagree: Unity's own Z-up→Y-up conversion `(−x, z, −y)`; the gun bone
  landing on the authored muzzle on all three axes; and the SkyBurst `Gun Point` at `z +9.9`, just
  ahead of the hull's `+z` extremity at `+7.57`. The gameplay camera's `followOffset.z = −50`
  agrees. So **`+z` is forward**, and the six bones the artist named `b_Tail*` drive the blades at
  the ship's NOSE. Recorded as measured, not resolved — the naming may be a hold-over from the
  model's orientation before the ship's forward was chosen.
