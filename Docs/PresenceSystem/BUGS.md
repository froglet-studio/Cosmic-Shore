...
|----|-------|-----------|--------|
| B1 | `ArgumentOutOfRangeException` (LobbyPatcher) spam at game start | High (cause) | [green] (needs Editor retest) |
| B4 | TC1 second invite not delivered + party members vanish from 3rd player's panel | High, needs retest | [red] |
| B6 | TC3 NRE (`WrappedLobbyService`) + empty online/request lists | Medium | [red] |
...
## B1 - `ArgumentOutOfRangeException` in `LobbyPatcher.ApplyPatchesToLobby` at game start [yellow] (console noise silenced; underlying SDK defect persists)
...
## B4 - TC1: second invite not delivered + party members vanish from 3rd player's online panel [red]
...
## B6 - TC3: `NullReferenceException` (`WrappedLobbyService.GetLobbyAsync`) + empty online/request lists [yellow] (refresh-path noise silenced in MPPM Session 1; TC3 empty-lists symptom untested since fix)
...
Method: see `../README.md` Sec "How we work bugs". This is the fragile,
