...
Living spec for the Maelstrom (Maelstrom meta-mode) AAA polish pass. Canonical mechanics still live
...
> stay **Maelstrom** (`MaelstromController`, `MaelstromSceneView`, `MaelstromDataSO`,
> `GameModes.Maelstrom = 36`). One word changed surfaces; the code identity did not.
...
## 3. Data model — `MaelstromDataSO` (Phase 1)

Per-round history survives the per-scene reset (the UI-only scene has no live `gameData.Players`):

- `MaelstromPlayerSnapshot` — `Name, Domain, AvatarId, IsAI, Rank, ScoreText, Secondary`.
- `MaelstromRoundRecord` — `RoundNumber, ModeDisplayName, Intensity, List<Players>, List<DomainOrder>`;
  `WinningDomain => DomainOrder[0]`.
- `List<MaelstromRoundRecord> History` (`[NonSerialized]`, cleared by `ResetRuntime`).
- Captured in `RecordResults(results, snapshots, modeName, intensity)` at game-end;
  `MaelstromController.BuildPlayerSnapshots` resolves avatar/AI from the still-live `gameData.Players`.
...
`MaelstromController`:
- `AdvanceToNextGame()` → `LoadMaelstromScene()` (hub or summary, decided at the load below).
- `BeginNextRound()` → draw + load the next random game (called by the hub ready-up).
- `RestartMaelstrom()` (Play Again) → loads a **fresh lobby** (reset keeps the intensity ceiling via
  `RestartFromSummary`).
...
> deterministic `MaelstromDataSO.IsShuffleComplete` (folded identically on every peer), **not** the
...
> results screen. `HandleMiniGameEnd` still sets `Complete` (best-effort signal + `OnMaelstromCompleted`),
...
`MaelstromSceneView` drives three layouts by phase: **active** (lobby + hub), **summary**, **rank**.
The active/summary scrolls hold **Maelstrom Data Cards**, each nesting its round's **Player Data Cards**.

### Two prefabs (+ one for rank rows)
- **Maelstrom Data Card** = `MaelstromRoundCard` — round header + its own player-card container.
  ...
  (`MaelstromPlayerCard`), `playerCardContainer`. `Setup(record,…)` for a completed round;
  `SetupPreview(roundNumber, roster,…)` for the round-0 lobby (roster, no round score/winner).
- **Player Data Card** = `MaelstromPlayerCard` — fields: `avatarImage`, `nameText`, `roundScoreText`,
...
- `MaelstromDomainScoreView` — `placeText, domainNameText, pointsText, colorTargets[], youBadge`.
  Used for the **summary rank rows**.

### `MaelstromSceneView` serialized fields
| Group | Fields |
|---|---|
...
| Active — scroll | `roundCardPrefab` (Maelstrom Data Card) + `historyContent`, `historyScrollRect` |
...
| Rank | `rankRoot`, `rankRowPrefab` (`MaelstromDomainScoreView`), `rankContainer`, `playAgainButton`, `mainMenuButton`, `onClickToMainMenu` |
...
- `MaelstromStandingsFormatter.FormatConnecting()` — lightweight splash text (leading domain +
...
- `MaelstromConnectingInfo` — in-scene connecting-panel reveal (`modeNameText, intensityText,
...
4. **Connecting reveal:** add `MaelstromConnectingInfo` to each domain-game HUD's connecting panel and
...
### What changed (code)
- **Player Data Card → `MaelstromPlayerCard`** (new component): shows **Round Score** (that round's
...
- **Maelstrom Data Card (`MaelstromRoundCard`)** nests `MaelstromPlayerCard`s and takes a domain→total
...
**`MaelstromSceneView`** (on `GameCanvas - MaelstromSceneView`):

| Field | Hierarchy object |
|---|---|
| `gameData` / `tournamentData` | GameData SO / `MaelstromData.asset` |
...
**`MaelstromRoundCard`** (on the MaelstromRoundScoreCardBG prefab):
...
**`MaelstromPlayerCard`** (on the MaelStromPlayerScoreCardContainer prefab):
...
- **Domain colours via `DomainColorPaletteSO`** (`Assets/_SO_Assets/DomainColorPalette.asset`). Every card
  ...
  - `MaelstromPlayerCard` → `domainBackground` + `palette` (player row tints to the player's domain).
  - `MaelstromRoundCard` → `cardBackground` + `palette` (card BG tints to the **winning** domain); the
    WINNING DOMAIN **name** is rich-text coloured from the palette.
  - `MaelstromDomainScoreView` → `domainBackground` + `palette`; `Setup(domain, points, place, isLocal)`.
  - `MaelstromSceneView` → `palette`; LEADING DOMAIN name rich-text coloured; winner banner via
  ...
  > `MaelstromSceneView` resolves the colour from `gameData.ThemeManagerData` and passes it down
...
- **`MaelstromSummaryPlayerCard`** (MaelstromSummaryScoreCardContainer) — `avatarImage`, `nameText`,
...
- **`MaelstromSceneView` summary fields** (replaces winner-banner/rank-row fields):
...
- `MaelstromDomainScoreView` is now **unused** (the summary uses cards + a rank text); kept for reference.
...
- **`MaelstromConnectingPanel`** (new, on a prefab under the MiniGameHUD — same for all scenes): enables
  ...
  `MaelstromConnectingInfo` reveal (mode / intensity / leading domain), holds for `dwellSeconds` (2s),
  then hides and hands off to the pre-game cinematic. **No-ops outside a tournament**
  (`gameData.IsMaelstromMode`), so other modes skip it.
...
`MaelstromConnectingInfo` with `gameData`/`tournamentData`/`palette` + its mode/intensity/lead texts).
Assign the panel to `MiniGameHUD.connectingPanel` (on `GameCanvas-SkimRace.prefab`, shared by the three
...
Replaces the v2.3 `MaelstromConnectingPanel` + `MaelstromConnectingInfo` (both deleted) with **one**
component, and makes the panel generic (every scene), not tournament-gated:

- **`ConnectingPanelController`** (one script on the ConnectingPanel object — *not* "Maelstrom…"):
...
- **Final standings listed by per-round placement, not cumulative total.** `MaelstromRoundCard.BuildPlayers`
