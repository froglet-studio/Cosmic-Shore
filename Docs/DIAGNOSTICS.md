...
### The store — why files, why there

`BugLedger/issues/<id>.bug.json`, one small JSON file per issue, at the **project root**:

- outside `Assets/` → Unity never imports it, no `.meta` churn, invisible to the asset pipeline;
- **committable** → pushing an issue shares it with the team like any doc edit; keeping it local
  is equally valid;
...
issue per editor session** so committed files stay quiet.
...
- At the issue's own `cleanSessionsRequired` (stamped at creation, so the policy is deterministic
  per issue) it is **resolved**: removed from `issues/`, stamped (`state: resolved`,
  `resolvedUtc`, `resolution`) into **`BugLedger/resolved/`** (pruned to a cap; toggleable — git
  history keeps everything regardless), and one console line says so.
...
- Signature normalization is best-effort: bare GUID-ish tokens without `0x` survive partially, so
  ...
  identity anyway.
- Two machines auto-filing the same signature before either pushes is the one add/add conflict
  possible; either side can be kept (durable fields agree, counters are advisory).
...
3. **Bugs tab ▸ Test Capture** → one `E-…` issue appears (and only one, however many times you
   click). **Mark Fixed** → VALIDATING. Two play runs ≥ the minimum length → the issue resolves
   itself, archived under `BugLedger/resolved/`.
