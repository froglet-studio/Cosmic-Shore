...
> ASCII fallback of the core loop is inline in Sec 3 for quick reading.
...
> the food web, Sec 6-Sec 7) or **pause/throttle the spawner** while idling (not creating
...
2. The flora **phase-gated self-limit**: planting stopped at a low phase and
   ...
   Sec 0) until something eats its mass back below the Frenzy exit threshold, at
   ...
   flora-vs-fauna events that no longer differ. See Sec 1, Sec 5.
...
> *only* prism sinks. There is no decay/aging row - mass is conserved (Sec 0). And
...
> gates were a growth-side cheat, retired in Sec 0).
...
1. **Flora freeze at Frenzy (hard ceiling, NOT a self-limit).** `count^ -> phase^ ->
   ...
   staggered self-limit gates were a growth-side cheat, retired Sec 0). Frenzy is just
   ...
   the down-force on flora is the predator-prey loop, not flora throttling itself. [x]
2. **Predator-prey (negative, the heartbeat).** `count^ -> aggression^ -> fauna hunt
   harder/closer -> consume more -> countv -> aggressionv -> ...` This is the
   oscillation that should make the cell feel alive. Per the latest decision this
   loop runs **through aggression (behavior)**, not through spawn rate. [x]
3. **Population control (the Lotka-Volterra coupling).** Feeds convert to births
   ...
   to its seed floor (bootstrap + extinction recovery), never drives growth. [x]
...
|---|---|---|---|---|---|
| 1 | Prism count (state) | trail + flora - fauna/combat | tracked via Add/RemoveBlock, per-domain | same | [x] the spine, leave alone |
| 2 | Cell phase | prism count + hysteresis | Calm->Frenzy (3 phases) | same | [x] 2 thresholds tunable per biome |
| 3 | Flora **planting** | `Phase < Frenzy` | steady rate until Frenzy | prism-count driven | [x] steady-until-frenzy (cheat removed) |
| 4 | Flora **growth** | `Phase < Frenzy` | steady rate until Frenzy | prism-count driven | [x] `AssembledFlora`/`BranchingFlora`; same gate as planting |
| 5 | Fauna **aggression** | Phase -> L0/L1/L2 | seek crystal->opposing->densest | prism-count driven | [x] works; extension seam for a 4th tier / per-subtype |
| 6 | Fauna **spawn timing** | fixed-period seed timer | seeds at `BaseFaunaSpawnTime` | same | [x] timer-only, fixed period |
| 7 | Fauna **spawn count** | deficit below seed floor | tops species up to `PopulationSize` | same | [x] seeder (reproduction drives growth, Sec 6.1) |
| 8 | Fauna **domain** | `host.ControllingDomain` | controlling color | same | [x] (was the "no Jade fauna" bug) |
| 9 | Spawn-cycle HUD ring | `CurrentFaunaSpawnPeriod` (base period) | fixed sweep | same | [x] no aggression scaling |
| 10 | Fauna **population bound** | starvation + reproduction + `MaxLivePopulation` | prey-linked rise and crash | same | [x] Sec 6 + Sec 6.1 (full Lotka-Volterra) |
| 11 | Fauna **consume -> -prisms** | aggression behavior -> impact | reduces opposing prisms | same | [x] this is the prey side of loop #2 |
...
- **Spawner = SEEDER** *(supersedes the original "fixed N per tick" decision -
  ...
  floor, **reproduction** is the population driver (see Sec 6) - the spawner never
  races the food web.
...
- **Keep 3 aggression tiers** - and they are now the **same thing as the phases**.
  ...
  when the predator/herbivore split deepens (Sec 7).
- **The phase ladder is the aggression ladder.** Because flora are no longer
  staggered on their own rungs (steady until Frenzy, Sec 0), a cell needs only **two
...
- **Capacity is a PERFORMANCE budget, not a "fill it up" dial.** `FrenzyEnter` is
  the steady-state prism count (the cell pins there, Sec 6/Sec 12), and prism count is the
  ...
  (was 3000/5400, which sat the menu at ~5 fps - see Sec 12). `FrenzyEnter` also sets
...
- **Flora grow steadily until Frenzy.** The old "stop planting at a low phase, stop
  ...
  **regrowth pulse**) are **both retired** - they were growth-side cheats faking the
  breathing the food web is meant to produce (Sec 0). `Cell.FloraGrowingEnabled` /
...
> **food web**, never decay or a growth/decay oscillator. See Sec 0 and Sec 10.
...
| **B. Lifespan (natural cull)** | Each fauna despawns after `T` seconds | Self-bounding (pop ~ rate x T), simple, predictable | Decoupled from prey - not yet "emergent", just a timer |
...
- **Production pauses** when `OpposingVolume(controllingColor) < FaunaFoodFloor x 16`
...
**taming dial**, Sec 6.2 - *and* a performance ceiling, Sec 12).
...
> down-force is the food web (Sec 0). "Taming" just means sizing the predator so the
...
- **Spawn gating (by diet):** `RandomLifeSpawner` seeds a herbivore species when
  `OpposingVolume >= FaunaFoodFloor x 16` (prism prey, in volume) and a predator species when
...
- **Seed floor = `PopulationSize`** (Blob tadpole 25, Skim Race tadpole 12). The
  ...
  population is reproduction-driven (`FeedsPerOffspring` etc., Sec 6.1) and bounded by
...
> the dead `Cell.fauna2` field (removed) and never spawn (see the Sec 7 note). The
...
**TAME, not devour** (see Sec 6.2): a small 3-tier presence - tadpole forager (seed
...
the shark eats both herbivores. Caps are also **performance-bounded** (Sec 12: each
fauna's per-tick `OverlapSphere` is a top frame cost). Levers: per-species
`MaxLivePopulation` (the taming dial, Sec 6.2) + reproduction knobs (Sec 6.1),
...
1. [x] Map + agree the redesign and the Sec 6 bound.
2. [x] Spawn rewrite in `RandomLifeSpawner`: timer-only, fixed period, fixed
...
3. [x] Sec 6 bound = option C: opposing-mass prey signal (now `OpposingVolume`) + `FaunaFoodFloor`
   production gate + `starvationSeconds` despawn. Config on `SpawnProfileSO` /
   `FaunaConfigurationSO` / `Fauna`.
4. [wip] Validate in Menu_Main: Jade fauna appear when Jade controls; populations
   appear, hunt, and thin out as prey runs low; ring sweeps at the fixed period.
   Tune the Sec 6 knobs.
5. [x]/[wip] Predator/herbivore diet split - **code landed** (`FaunaDiet` + `Fauna.diet`
   ...
   `SpawnProfileSO`, then tune in-editor (Sec 7.1).
6. [x] Retire the regrowth pulse AND the flora phase-gated self-limit - **done**
...
| Headless perf+ecology tuner (no Unity) | `Tools/ecosim/ecosim.py` (+ `calibration.csv`, `README.md`) - see Sec 12 |
...
1. **~~Prism mortality / decay~~ - REJECTED (see Sec 0).** The original plan was
...
3. **Fauna reproduction - [x] LANDED (see Sec 6.1).** Well-fed fauna reproduce
...
are all **retired** (Sec 0/Sec 5/Sec 6.1). What deliberately remains is the **seeder** - the
...
decay (rejected, Sec 0) - mass is conserved, and a cell only comes back down when an
...
1. **Prism count (per-frame ceiling).** Each prism is a full GameObject (renderer +
   collider + MonoBehaviours). Because flora grow steadily to `FrenzyEnter` (Sec 0) and
   the cell pins there (fauna rarely out-graze flora, Sec 6), **`FrenzyEnter` ~ the
...
2. **Fauna `OverlapSphere` queries (fixed-rate).** Every fauna runs a
   ...
   colliders in its radius. Cost ~ `Sigma_species (count/period)*prisms*(radius/cellR)^3`.
...
cut hard - first for perf, then further for **taming** (Sec 6.2) when the first cut
...
double duty here: the Sec 6.2 taming dial *and* the per-frame `OverlapSphere` budget.
...
- **Decouple detection from consume radius**, or shrink `detectionRadius` toward
  ...
  brittlestar `detectionRadius 70->45` cuts its overlap ~3.7x with little behavior
