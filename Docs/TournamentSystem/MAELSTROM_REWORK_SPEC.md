...
> difficulty draw pool (`3 modes x intensity` "experiences").
...
| 0 Scene rename | [x] |
| 1 Data model | [x] |
| 2 Flow / state machine | [x] |
| 3 Networked ready-up | [x] |
| 4 Scene view + card components | [x] |
| 5 Connecting reveal + splash trim | [x] |
| 6 Polish (auto-scroll, pulse) | [x] |
| **7 UI authoring + wiring** | [white] (user) |
...
2. **Lobby network:** add a GameObject with **`NetworkObject` + `TournamentLobbyNetwork`** (autoStart 30,
   allReady 5).
3. **Scene view:** build `activeRoot` / `summaryRoot` / `rankRoot` and wire every field in Sec 6 on the
...
## v2 - UI integration (Shombith) [x] (2026-06-19)

Driven by the final Maelstrom intro-panel UI. All [x] done; everything above is unchanged - this section
...
- **Separate animated countdown text** (`countdownText`) - "Game will start in {N}s", DOTween punch each
  tick. The button face is now optional (`readyButtonLabel` shows START / READY [x] only when wired).
...
- **First card full size, the rest x0.9** (`PopulateSummaryCards` sets localScale before the pop-in).
...
- **NEXT / Play Again / Main Menu started a new game instead of advancing/ending.** All three summary
  ...
  **inspector `onClick -> OnHostStartPressed`** on top of the code listener `Awake()` adds - the exact
  double-fire Sec 6 warns about. On NEXT the first invocation ran `ShowSummaryPanel()` (which clears
...
- **Final standings listed by per-round placement, not cumulative total.** `TournamentRoundCard.BuildPlayers`
  ...
  by cumulative Total Score descending (stable tiebreak by domain enum), per Sec 6 "reorder by the overall
