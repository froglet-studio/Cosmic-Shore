...
> **(!) Historical (point-in-time session log).** Some specifics below are superseded: the scene was later
...
> and recommended next tasks. Canonical mechanics live in `Docs/TournamentSystem/ARCHITECTURE.md`
> (Sec 4 between-game overlay, readable dwell, owner tag) - this doc summarizes and points there for depth.
>
> **Branch:** `claude/focused-thompson-jvbt3x` (built on top of in-flight scene edits
> `e982fdc3 Update Tournament.unity`, `7064d503 Update Bootstrap.unity`).
>
> **Reminder - naming:** *Maelstrom* is the **player-facing display name only**. Code, scene, data,
> enum, and this folder family stay **Tournament** (`GameModes.Tournament = 36`). Internal identifiers
...
graph TD
    subgraph Persistent["Persistent (DontDestroyOnLoad, survive Single loads)"]
        TC["TournamentController<br/>(pure C# DI singleton, static Instance)"]
        SL["SceneLoader<br/>(MonoBehaviour)"]
        BSB["BootStatusBroadcaster<br/>(on Bootstrap splash canvas)"]
        TD["TournamentDataSO<br/>(SOAP data asset)"]
        GD["GameDataSO<br/>(SOAP data asset)"]
        PL["Player(s)<br/>(NetworkObject, CreatePlayerObject=true)"]
    end
    subgraph SceneLocal["Tournament scene (lobby AND summary)"]
        TSV["TournamentSceneView<br/>(MonoBehaviour)"]
    end
    FMT["TournamentStandingsFormatter<br/>(pure static - DRY text)"]
...
- **`TournamentController`** (`_Scripts/Controller/Arcade/Tournament/`) - the persistent brain. Draws the
...
- **`TournamentSceneView`** (`_Scripts/Controller/Arcade/Tournament/`) - the Tournament scene's view; in
  **Summary** phase renders the final standings (with `(You)`) and drives the host-only buttons.
- **`TournamentStandingsFormatter`** (`_Scripts/Utility/DataContainers/Tournament/`) - single, pure
  source for both the running and final standings strings (DRY, unit-tested).
- **`TournamentDataSO`** (`_Scripts/Utility/DataContainers/Tournament/`) - config + runtime state
...
sequenceDiagram
    ...
    participant TC as TournamentController
    participant SL as SceneLoader
    participant TSV as TournamentSceneView
    participant FMT as Formatter

    Host->>TC: AdvanceToNextGame() (Continue, last game)
    Note over TC: IsShuffleComplete -> LoadTournamentScene()
    TC->>SL: InvokeGameLaunch() (Tournament scene)
    Note over SL: dwell = 0 (IsShuffleComplete => not "between games")<br/>ResetRuntimeData() clears LocalPlayer here
    SL->>SL: LoadScene(Tournament, Single)
...
```mermaid
graph LR
    Card["ArcadeGameTournament.asset<br/>DisplayName = 'Maelstrom'"] --> MN["TournamentDataSO.ModeName"]
    MN --> Grid["Arcade grid card title"]
    MN --> Title["TournamentSceneView title<br/>(MAELSTROM / MAELSTROM RESULTS)"]
    MN --> Splash["FormatRunning header<br/>(MAELSTROM - first to N)"]
    Scene["Tournament.unity authored title<br/>m_text = MAELSTROM"] -.overwritten at runtime by.-> Title
```

To rename again for players, change **only** `ArcadeGameTournament.asset.DisplayName`. The authored
...
- **`TournamentSceneView`** - added `_summaryActionTaken` guard; `OnPlayAgainPressed` / `OnMainMenuPressed`
...
- **`TournamentController.LoadRandomGame`** - stamps `TournamentDataSO.NextGameName` / `NextGameIntensity`
  right before `InvokeGameLaunch`.
- **`TournamentDataSO`** - added the two `[NonSerialized]` runtime fields.
- **`TournamentStandingsFormatter.FormatRunning`** - emits `Up next: <name> * Intensity N` when set.
- Host-only (clients never draw); omitted when unset -> shows from game 2 onward.

### 3 - `c4a0d98f` Rename Shuffle -> Maelstrom (display only)
- **`ArcadeGameTournament.asset`** `DisplayName: Maelstrom`; **`Tournament.unity`** title `m_text: MAELSTROM`.
- Refreshed comments/tooltips + docs (`CLAUDE.md`, `Docs/ShuffleSystem`, `Docs/TournamentSystem`).
...
- **`TournamentDataSO.BetweenGameSummaryDwellSeconds`** - config, default **2s** (persisted in
  `TournamentData.asset`).
- **`TournamentController.IsBetweenGamesStandingsShown`** (mirrors the splash condition `IsActive &&
  !IsShuffleComplete && GamesPlayed > 0`) + **`MinLoadSplashDwellSeconds`** (that value when between games,
  else 0).
- **`SceneLoader.LaunchGame`** reads `TournamentController.Instance.MinLoadSplashDwellSeconds` and passes it
...
- Scoped precisely: first game, the final-summary load, menu returns, and all non-tournament launches stay
  at the normal short wait. Routed via `TournamentController.Instance` (always alive - `AppManager` injects
  it) rather than a hard `[Inject] TournamentDataSO` so an un-wired asset still degrades gracefully.

### 5 - `ce85bed7` `(You)` owner tag
- **`TournamentStandingsFormatter`** - `FormatFinal` / `FormatRunning` gained `Domains localDomain =
...
- **`TournamentSceneView`** (summary) + **`BootStatusBroadcaster`** (splash) pass the local player's domain.
- **`TournamentStandingsFormatterTests`** - verifies single-tag behavior + the `Domains.Blue` no-op.

### 6 - `16333f94` Fix: local domain on the summary scene
- **Bug:** `gameData.LocalPlayer` is **null** on the Tournament summary scene - `SceneLoader.LoadSceneAsync`
...
- **Fix:** `TournamentSceneView.GetLocalDomain()` prefers `gameData.LocalPlayer`, then falls back to
...
## 5. (!) Inspector wiring the features depend on (verify in a new session)
...
|---|---|---|---|
| Between-game standings text, "Up next", splash `(You)` | `BootStatusBroadcaster.tournamentData` <- `TournamentData.asset` | Bootstrap splash canvas | Clean splash (no standings) - **dwell still happens** (it reads the controller, not the wire), so you'd get a ~2s blank-ish hold. |
| Readable dwell (~2s) | *(none - reads `TournamentController.Instance`)* | - | Always active. |
| Summary `(You)` tag + final standings | `TournamentSceneView.gameData`, `.tournamentData`, `.resultsText` | Tournament scene | No results text / no tag. |
| Summary buttons | `TournamentSceneView.playAgainButton`, `.mainMenuButton`, `.onClickToMainMenu` | Tournament scene | Host can't advance/exit from summary. |
| Placement wallet credit + card badge (pre-existing, P3) | `Scoreboard.tournamentData` <- `TournamentData.asset` | each domain-game `Scoreboard` (`GameCanvas-HexRace.prefab` + Joust/Crystal Capture) | Flat winner reward, no badge. |

> The user was editing scenes this session (`Tournament.unity`, `Bootstrap.unity`). **First action next
> session: confirm `BootStatusBroadcaster.tournamentData` and the `TournamentSceneView.*` references are
> wired**, then run the Sec 7 checklist.

---

## 6. Design invariants honored (don't relitigate)

- **Display-only rename.** Maelstrom is presentation; Tournament is identity. One source: the card's
  `DisplayName` -> `TournamentDataSO.ModeName`.
...
- **Pure formatter.** `TournamentStandingsFormatter` takes a `Domains` enum, never touches Unity objects,
...
6. **Edit-mode tests.** `TournamentStandingsFormatterTests` + `TournamentDataSOTests` green.
...
- **Confirm the Sec 5 wires** (esp. `BootStatusBroadcaster.tournamentData`) - the most probable reason a
...
- **Unit-test the dwell decision.** `TournamentController.MinLoadSplashDwellSeconds` /
  `IsBetweenGamesStandingsShown` are pure given a populated `TournamentDataSO` - add a small fixture
  (active + GamesPlayed>0 => value; first game / complete => 0).

**Deferred / pre-existing (not this session)** - see `Docs/TournamentSystem/ARCHITECTURE.md Sec 9` and
...
|---|---|
| `_Scripts/Controller/Arcade/Tournament/TournamentController.cs` | `IsBetweenGamesStandingsShown`, `MinLoadSplashDwellSeconds`; `LoadRandomGame` stamps `NextGame*` |
| `_Scripts/Controller/Arcade/Tournament/TournamentSceneView.cs` | `_summaryActionTaken`, `DisableSummaryButtons`, idempotent button handlers, `RenderSummary`, `GetLocalDomain()` |
...
| `_Scripts/Utility/DataContainers/Tournament/TournamentDataSO.cs` | `NextGameName`, `NextGameIntensity`, `BetweenGameSummaryDwellSeconds` |
| `_Scripts/Utility/DataContainers/Tournament/TournamentStandingsFormatter.cs` | `FormatRunning/FormatFinal(data, localDomain)`, `YouTag` |
| `_Scripts/Tests/EditMode/TournamentStandingsFormatterTests.cs` | new test fixture (`(You)` single-tag + Blue no-op) |
| `_SO_Assets/Games/ArcadeGameTournament.asset` | `DisplayName: Maelstrom` |
| `_SO_Assets/Tournament/TournamentData.asset` | `BetweenGameSummaryDwellSeconds: 2` |
| `_Scenes/Multiplayer Scenes/Tournament.unity` | authored title `m_text: MAELSTROM` |
| `Docs/TournamentSystem/ARCHITECTURE.md` | Sec 4 owner tag + readable dwell notes |
| `Docs/ShuffleSystem/ARCHITECTURE.md`, `CLAUDE.md` | rename references -> Maelstrom |

**Canonical references:** `Docs/TournamentSystem/ARCHITECTURE.md` (mechanics, Sec 4/Sec 7/Sec 9),
`Docs/ShuffleSystem/ARCHITECTURE.md` (Maelstrom = Tournament pointer + deferred deltas).
