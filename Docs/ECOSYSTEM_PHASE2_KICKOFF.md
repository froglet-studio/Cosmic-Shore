...
1. **`Docs/ECOSYSTEM.md`** - the system map and your bible: the prism-count->phase
   spine, the three feedback loops, the Sec 6 prey-linked-starvation decision, Sec 7
   predator/herbivore extensibility, Sec 9 key files, **Sec 10 roadmap (Phase 2)**.
...
- **Diet split landed (code):** `FaunaDiet` (Herbivore default | Predator) on the
  ...
  see ECOSYSTEM.md Sec 7.1 (predator prefab + config + wire into a `SpawnProfileSO`).
- **Density:** Blob (menu) thresholds `RestlessEnter 600 / FrenzyEnter 1000`
  (perf-bounded - prism count is the dominant frame cost; see ECOSYSTEM.md Sec 12).
- **All scaffolding cheats retired:** regrowth pulse, flora self-limit, and the
  ...
  only seeds back up to the floor - ECOSYSTEM.md Sec 6.1). Prism decay was rejected
  (Sec 0), not added.

### (!) Locked invariant - mass is conserved (no passive decay). Read ECOSYSTEM.md Sec 0.
...
### Phase 2 build order (ECOSYSTEM.md Sec 10 - each step ships alone, composes with the fundamentals, and retires a cheat)
1. **~~Prism mortality / decay~~ - REJECTED (see Sec 0 above).** A timed culler is
...
2. **Predator / herbivore split - code DONE, authoring/tuning remains.** The diet
   ...
   config in a `SpawnProfileSO`, then tune to a breathing equilibrium (Sec 7.1). This
   is the active down-force that lets accumulations come down through the food web.
3. **Fauna reproduction - [x] LANDED (ECOSYSTEM.md Sec 6.1).** Well-fed fauna breed;
...
already implemented** (see "Current state" + ECOSYSTEM.md Sec 7). Two tracks remain,
...
- **Activate + tune the food web (no code, then tuning).** Author a predator
  ...
  `SpawnProfileSO` (Sec 7.1). Then validate in Menu_Main and tune `PopulationSize`,
...
"fix" an accumulation** (Sec 0). If a cell stays frozen, strengthen the food web.
