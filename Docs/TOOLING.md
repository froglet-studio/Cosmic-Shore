...
| Interface | **Codex** | The in-game encyclopedia, authored in one place across all three kingdoms — **Ethirions** (every crystal), **Ecology** (every lifeform) and **Toys** (every freestyle station, grouped by the fundamental it changes: Pilot / World / Creation). The layout is a drill-down: a glowing kingdom tab strip on top, the lit kingdom as a grid of illustrated cards under it, and a clicked card opens that entry's page with its own DETAILS / EDIT tabs. *Scan & Merge* re-reads the project, *Bake* renders each entry's hero image and each distinct variant's icon, and the EDIT tab changes any entry by hand. Keeper (idempotent — the codex needs re-scanning every time a crystal, species or toy is added; **do not retire it**). Its load-bearing property is that **Scan & Merge is always safe to run**: a field-ownership contract makes harvested facts the tool's and all prose the writer's, and an entry whose source asset vanished is reported as an orphan and never auto-deleted. WRITER: records the codex asset and every PNG on the ledger; ships via Pending Tool Changes. See `Docs/CODEX.md`. |
...
| Codex harvester — toys (freestyle stations) | `Assets/_Scripts/Editor/Codex/ToyCodexHarvester.cs` |
| Codex image baker (photographs a prefab) | `Assets/_Scripts/Editor/Codex/CodexImageBaker.cs` |
| Codex toy portrait (draws a toy, which has no prefab) | `Assets/_Scripts/Editor/Codex/ToyPortraitBuilder.cs` |
