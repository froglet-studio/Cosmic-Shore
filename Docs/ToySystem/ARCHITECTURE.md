...
- *Volume is the spine / collider budget* — belt mass is bounded at
  ...
  prisms now use a shield AABB proxy BoxCollider that IS collider-LOD-cullable (no MeshCollider;
  exact surface via the analytic narrowphase gate) — the palette caps (`MaxShielded = 3`,
  `MaxSuperShielded = 1` per scene, low scheme weights) are retained as a gameplay/visual bound,
  no longer a collider-budget necessity. Danger prisms
