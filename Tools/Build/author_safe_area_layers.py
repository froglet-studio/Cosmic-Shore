...
# ONE in-game canvas. There were TWO - CORE/GameCanvas and a hard-copy GameCanvas-SkimRace - and
# this script authored both, because the fork was what 12 of the domain scenes actually instanced.
# The fork was RETIRED on 2026-09-08 (F1, Docs/GAMECANVAS.md §9): the fork prefab is deleted and all
# 15 scenes run CORE. The absorb kept fileIDs, so every target this script names resolved unchanged
# across it - only the list shrank. If a second canvas is ever added, add it here rather than
