#!/usr/bin/env python3
"""A VESSEL MAY NOT MOVE AN OPPOSING VESSEL.

Being shoved, spun or re-aimed by somebody else's weapon is the one kind of hit a pilot cannot
answer with flying, and it was reported as simply not fun to receive. What a weapon MAY take from
another pilot is their ELEMENTAL CRYSTALS - stolen on a contact verb, ejected as free-for-all
crystals on a ranged one (`Docs/ELEMENTAL_ECONOMY.md`) - which is a loss they can chase down and
win back.

Four effects broke that law until Sep 2026 and were removed: the Sparrow's guns and rocket, the
Urchin's spikes and the Rhino's sword all spun their victim (the sword laterally shoved it too).
`VesselTransformer.SpinShip` went with them, since those were its only callers.
...
------------------------
  * `Vessel Prism Effects/` - a vessel deflecting off MASS it flew into is the FLIGHT MODEL, not
    ...
    deflecting you is you hitting their wall, which a pilot answers by flying around it.
...
# Motion / attitude APIs on VesselTransformer. Written as the member name so a renamed method
# does not silently fall out of the list - a rename that drops a row here fails the self-test.
FORBIDDEN = (
    "ModifyVelocity",
...
def run(verbose=True):
        ...
        for name in sorted(os.listdir(d)):
            ...
            for lineno, member in scan(text):
                problems.append(
                    ...
                    f"AN OPPOSING VESSEL. Take their elemental crystals instead "
                    f"(Docs/ELEMENTAL_ECONOMY.md)."
                )
    ...
    if verbose:
        ...
        print(f"vessel-on-vessel motion check: {tag} "
